...
- To go back to an image from before the split on the same volume, move the legacy entries back first: `docker run --rm -v unsloth-studio:/h alpine sh -c 'cd /h && for e in .unsloth-studio-legacy/* .unsloth-studio-legacy/.[!.]*; do [ -e "$e" ] || [ -L "$e" ] || continue; rm -rf "${e##*/}"; mv "$e" .; done'`. Images from after the split need nothing.
- `docker rm` still discards anything written into the image's copy: an in-container `unsloth-studio-update`, the `cloudflared` binary `unsloth-jupyter-tunnel` downloads, and Studio's caches under `/opt/unsloth-studio/cache` (download resume state, dataset caches). Models stay in the Hugging Face cache mount.
