...
| `JUPYTER_PORT` | JupyterLab port inside the container, read by the `latest` service launcher. Default `8888`. Studio's port (`UNSLOTH_STUDIO_PORT`, default `8000`) is refused. |
| `UNSLOTH_STUDIO_PORT` | Studio port inside the container, read by the `latest` service launcher, for `--network host` when 8000 is taken. Default `8000`. |
