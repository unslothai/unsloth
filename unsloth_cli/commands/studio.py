...
def studio_default(
    ...
    if not silent:
        ...
        typer.echo(f"Starting Unsloth Studio on http://{_url_host(share_host)}:{port}")
...
def run(
    ...
    base_host = _network_share_host_for_bind(run_mod, host)
    if _is_wildcard_bind(base_host):
        base_host = _loopback_bind_host_for(host)
