...
def _load_substrings():
    ...
    result = {}
    for var_name in ["TRANSFORMERS_5_MODEL_SUBSTRINGS", "TRANSFORMERS_550_MODEL_SUBSTRINGS"]:
...
def test_lfm2_base_not_matched():
    ...
    g = _load_substrings()
    all_subs = g["TRANSFORMERS_5_MODEL_SUBSTRINGS"] + g["TRANSFORMERS_550_MODEL_SUBSTRINGS"]
...
def test_gemma3_not_matched_by_gemma4_substring():
    ...
    models = ["google/gemma-3-27b-it", "unsloth/gemma-3-4b-it", "google/gemma-3n-e4b-it"]
    for model in models:
        assert not any(sub in model for sub in subs_550), f"{model} falsely matched by 550"
