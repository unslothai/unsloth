...
class GoogleCalendarClient:
    ...
    def __init__(self, config: dict):
        self.credentials_file = Path(config.get("credentials_file", "config/google_credentials.json"))
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                if not self.credentials_file.exists():
                    ...
                    print("    Please follow the setup instructions to configure Google Calendar")
                    self.service = None
                    return
    ...
    async def create_appointment(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._create_appointment_sync,
            date, time, title, description, duration_minutes
        )
    ...
    async def update_appointment(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._update_appointment_sync,
            appointment_id, date, time, title, description
        )
