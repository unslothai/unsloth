...
class MacPatcher:
    ...
    def patch_device_type(self) -> PatchResult:
        ...
        if not self.config.patch_device_type:
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "disabled in config"
            )
        ...
        try:
            ...
            return self._create_patch_result(
                name, PatchStatus.SUCCESS, "MPS device type configured"
            )
    ...
    def patch_torch_cuda(self) -> PatchResult:
        ...
        try:
            ...
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "torch not installed"
            )

        if torch.cuda.is_available():
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "CUDA is available"
            )
        ...
        try:
            ...
            self._original_modules[name] = {
                "get_device_properties": getattr(
                    torch.cuda, "get_device_properties", None
                ),
                "get_device_capability": getattr(
                    torch.cuda, "get_device_capability", None
                ),
            ...
            class AppleSiliconProps:
                ...
                def __init__(self):
                    ...
                    self.total_memory = int(
                        hw_info.get("usable_memory_gb", 16) * 1024**3
                    )
                    self.major = 0
                    self.minor = 0
            ...
            return self._create_patch_result(
                name,
                PatchStatus.SUCCESS,
                f"Mocked for {hw_info.get('chip_name', 'Apple Silicon')}",
            )
    ...
    def patch_triton(self) -> PatchResult:
        ...
        if "triton" in sys.modules:
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "already imported"
            )
        ...
        try:
            ...
            return self._create_patch_result(
                name, PatchStatus.SUCCESS, "Triton mocked successfully"
            )
        ...
        try:
            ...
            class FakeBnB(ModuleType):
                ...
                def __init__(self, name: str):
                    ...
                    self.__path__ = []
                    self.__spec__ = ModuleSpec(
                        name=str(name), loader=BnBMockLoader(), origin="mocked"
                    )
            ...
            class BnBMockFinder(MetaPathFinder):
                def find_spec(self, fullname: str, path, target=None):
                    if fullname == "bitsandbytes" or fullname.startswith(
                        "bitsandbytes."
                    ):
            ...
            return self._create_patch_result(
                name, PatchStatus.SUCCESS, "bitsandbytes mocked"
            )
    ...
    def patch_fused_losses(self) -> PatchResult:
        ...
        try:
            ...
            if original_forward:
                ...
                return self._create_patch_result(
                    name, PatchStatus.SUCCESS, "MPS forward method added"
                )

            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "no forward method found"
            )

        except ImportError:
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "module not yet imported"
            )
        except Exception as e:
            return self._create_patch_result(name, PatchStatus.FAILED, str(e), e)
        ...
        try:
            ...
            try:
                ...
                if hasattr(peft.utils.other, "is_bnb_available"):
                    other_patches["is_bnb_available"] = (
                        peft.utils.other.is_bnb_available
                    )
                    peft.utils.other.is_bnb_available = lambda: False
                if hasattr(peft.utils.other, "is_bnb_4bit_available"):
                    other_patches["is_bnb_4bit_available"] = (
                        peft.utils.other.is_bnb_4bit_available
                    )
                    peft.utils.other.is_bnb_4bit_available = lambda: False
                if hasattr(peft.utils.other, "is_bnb_8bit_available"):
                    other_patches["is_bnb_8bit_available"] = (
                        peft.utils.other.is_bnb_8bit_available
                    )
            ...
            return self._create_patch_result(
                name, PatchStatus.SUCCESS, "bnb detection disabled"
            )

        except ImportError:
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "PEFT not installed"
            )
        except Exception as e:
            return self._create_patch_result(name, PatchStatus.FAILED, str(e), e)
    ...
    def patch_compilers(self) -> PatchResult:
        ...
        if patched:
            return self._create_patch_result(
                name, PatchStatus.SUCCESS, f"patched: {', '.join(patched)}"
            )
        else:
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "no CUDA functions found"
            )
    ...
    def patch_patching_utils(self) -> PatchResult:
        ...
        name = "patching_utils"
        return self._create_patch_result(
            name, PatchStatus.SKIPPED, "deferred to late patch"
        )
    ...
    def _create_unsloth_zoo_mock_module(self, fullname: str, loader=None):
        ...
        if fullname == "unsloth_zoo":
            ...
            mod.encode_conversations_with_harmony = lambda *a, **k: (
                a[0] if a else None
            )
            ...
            class Version:
                ...
                def __repr__(self):
                    return f"Version('{self._parsed}')"

                def __eq__(self, other):
                    return self._parsed == Version(other)._parsed
            ...
            def _get_dtype(config=None, **kwargs):
                import torch
            ...
            def dtype_from_config(cfg, key="torch_dtype"):
                if hasattr(cfg, "get"):
                    return cfg.get(key, "float32")
                return getattr(cfg, key, "float32")

            mod.dtype_from_config = dtype_from_config
            mod.add_dtype_kwargs = lambda *a, **k: {}
            ...
            mod.cross_entropy_loss = ModuleType(
    ...
    def patch_unsloth_zoo(self) -> PatchResult:
        ...
        if name in sys.modules:
            return self._create_patch_result(
                name, PatchStatus.SKIPPED, "already imported - too late"
            )
        ...
        try:
                ...
                def find_spec(self, fullname, path, target=None):
                    if fullname == "unsloth_zoo" or fullname.startswith("unsloth_zoo."):
                        if fullname in sys.modules:
                            print(
                                f"[MLX MOCK] find_spec: {fullname} - already in sys.modules, skipping"
                            )
                            return None
            ...
            if "unsloth_zoo" not in sys.modules:
                sys.modules["unsloth_zoo"] = self._create_unsloth_zoo_mock_module(
                    "unsloth_zoo", loader
                )
            ...
            patched = [
                ...
                "flex_attention",
                "fused_losses",
                "fused_losses.cross_entropy_loss",
            ]

            for submodule in patched:
                full_name = (
                    f"unsloth_zoo.{submodule}"
                    if "." not in submodule or not submodule.startswith("unsloth_zoo")
                    else submodule
                )
                if full_name not in sys.modules:
                    sys.modules[full_name] = self._create_unsloth_zoo_mock_module(
                        full_name, loader
                    )
                ...
                attr_name = (
                    full_name.split(".")[-1]
                    if "." in full_name
                    else full_name.replace("unsloth_zoo.", "")
                )
            ...
            print(
                f"[MLX MOCK] vision_utils type: {type(vu)}, has process_vision_info: {hasattr(vu, 'process_vision_info') if vu else 'N/A'}"
            )

            return self._create_patch_result(
                name, PatchStatus.SUCCESS, f"mocked: {', '.join(patched)}"
            )
    ...
    def patch_patching_utils_late(self) -> PatchResult:
        ...
        try:
            ...
            return self._create_patch_result(
                name, PatchStatus.SUCCESS, "Bnb_Linear4bit patched"
            )
        except Exception as e:
    ...
    def apply(self) -> List[PatchResult]:
        ...
        if self._applied:
            self._log(
                logging.INFO, "Patches already applied, returning existing results"
            )
        ...
        self._log(
            logging.INFO,
            f"Detected: {hw_info.get('chip_name', 'Unknown Apple Silicon')}",
        )
        ...
        self._log(
            logging.INFO, f"Patches applied: {successful}/{len(results)} successful"
        )
...
def patch_for_mac(
    verbose: bool = False, strict: bool = False, enable_logging: bool = True
) -> Dict[str, str]:
    """
    ...
    """
    config = PatchConfig(
        verbose=verbose, strict_mode=strict, enable_logging=enable_logging
    )
    ...
    return {
        r.name: f"{r.status.name}: {r.message}" if r.message else r.status.name
        for r in results
    }
...
is_patched = (
    lambda: _default_patcher.is_applied if "_default_patcher" in globals() else False
)
get_patch_status = (
    lambda: _default_patcher.patch_results if "_default_patcher" in globals() else {}
)
