...
def patch_Gemma3Processor():
    ...
    def __call__(
        ...
        **kwargs: Unpack[Gemma3ProcessorKwargs],
        ...
        mm_token_type_ids = [
            [1 if y == image_token_id else 0 for y in x] for x in input_ids
        ]
    ...
    old_keys = inspect.signature(
        transformers.models.gemma3.processing_gemma3.Gemma3Processor.__call__
    ).parameters
    ...
    def forward(
        ...
        **lm_kwargs,
        ...
        if position_ids is None:
            ...
            )  # Gemma3 positions are 1-indexed
...
def patch_Gemma3ForConditionalGeneration_causal_mask():
    ...
    pass
    old_keys = inspect.signature(
        transformers.models.gemma3.modeling_gemma3.Gemma3ForConditionalGeneration._update_causal_mask
    ).parameters
...
def patch_Gemma3TextScaledWordEmbedding():
    ...
    if old_keys != new_keys:
        ...
        forward = torch.compile(
            forward, fullgraph=True, dynamic=True, options=torch_compile_options
        )
        transformers.models.gemma3.modeling_gemma3.Gemma3TextScaledWordEmbedding.forward = (
            forward
        )
...
def patch_Gemma3RMSNorm():
    ...
    pass
    old_keys = inspect.signature(
        transformers.models.gemma3.modeling_gemma3.Gemma3RMSNorm.forward
    ).parameters
    new_keys = inspect.signature(forward).parameters
    if old_keys != new_keys:
...
def patch_Gemma3MLP():
    ...
    if old_keys != new_keys:
        ...
        forward = torch.compile(
            forward, fullgraph=False, dynamic=True, options=torch_compile_options
        )
