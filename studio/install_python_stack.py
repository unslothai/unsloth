...
def _path_inside_venv(path: str) -> bool:
    ...
    _rocm_sdk_core/bin/), and the bnb fix below prepends the venv Scripts dir to
    PATH. That venv-internal hipInfo is NOT a HIP SDK and must not be mistaken
    for one (see _amd_smi_allowed)."""
    try:
...
def _amd_smi_allowed() -> bool:
    ...
    # hipinfo-on-PATH is the proxy for that. But ignore the venv-internal
    # hipInfo.exe shipped by the AMD torch wheel (which the bnb fix prepends to
    # PATH) -- it is not a HIP SDK and does NOT stop amd-smi from popping the
    # DiskPart UAC.
