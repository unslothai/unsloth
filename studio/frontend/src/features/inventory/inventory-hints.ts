...
export type InventoryHintReconciliation<T> = {
  rows: T[];
...
function optimisticRow<T extends InventoryHintRow>(hint: InventoryHint): T {
  return {
    repo_id: hint.repoId,
...
function mergeInventoryHint<T extends InventoryHintRow>(
  rows: T[],
  hint: InventoryHint,
): T[] {
  const lower = repoKey(hint.repoId);
  const seed = optimisticRow<T>(hint);
  const idx = rows.findIndex((row) => rowRepoId(row).toLowerCase() === lower);
  if (idx === -1) {
    return [...rows, seed];
  }
  const merged = {
