...
export function DatasetSourceToggle({
  ...
  const t = useT();
  const isS3Selected = datasetSource === "s3";

  if (isMultimodalModel) {
    return null;
  }

  const selectBrowseSource = () => {
    if (isS3Selected) {
      restoreBrowseDatasetSource();
    }
  };

  const selectS3DatasetSource = () => {
    if (!isS3Selected) {
      selectS3Source();
    }
  };
  ...
  return (
    ...
    >
      <span
        aria-hidden="true"
        className="hub-tab-toggle-pill pointer-events-none absolute inset-y-0 left-0 w-1/2 rounded-full transition-transform duration-200 ease-out"
        style={{ transform: `translateX(${isS3Selected ? 100 : 0}%)` }}
      />
      <button
        type="button"
        aria-pressed={!isS3Selected}
        onClick={selectBrowseSource}
        className={cn(
          buttonClassName,
      ...
      >
        {t("studio.wizard.sourceBrowse")}
      </button>
      <button
        type="button"
        aria-pressed={isS3Selected}
        onClick={selectS3DatasetSource}
        className={cn(
          buttonClassName,
          PICKER_FOCUS_VISIBLE_CLASS,
          isS3Selected
