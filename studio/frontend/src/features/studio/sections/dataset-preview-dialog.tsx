    ...
    const dataCols: ColumnDef<Record<string, unknown>>[] = columns.map((colName) => ({
      ...
      header: () => (
        <div className="flex flex-col gap-2">
          <span className="font-heading text-[0.8125rem] font-semibold tracking-tight text-foreground">
            {colName}
          </span>
          {mappingEnabled && (
            <HeaderRolePicker
      ...
      cell: ({ getValue }: { getValue: () => unknown }) => {
        ...
        if (!text) {
          return (
            <span className="text-muted-foreground/40 italic text-[0.8125rem]">
              --
            </span>
          );
        }
        ...
        return (
          <p
            className="text-[0.8125rem] leading-relaxed line-clamp-6"
            title={full}
          >
            {text}
          </p>
      ...
      dataCols.unshift({
        ...
        header: () => (
          <div className="flex flex-col gap-2">
            ...
            {mappingEnabled && (
              <Badge variant="outline" className="h-6 w-fit text-[0.625rem] px-2 py-0 border-dashed text-muted-foreground">
                System
              </Badge>
            )}
          </div>
        ...
        cell: () => (
          <p
            className="text-[0.8125rem] leading-relaxed line-clamp-6 text-muted-foreground italic"
        ...
        <div className="flex flex-col min-h-0 flex-1 overflow-auto px-6 pb-6">
          ...
          {!loading && !error && data && (
            <>
                    ...
                    <span className="flex items-center gap-1.5 flex-wrap">
                      {columns.map((col) => (
                        <Badge
                          key={col}
                          variant="outline"
                          className="text-[0.6875rem] font-mono h-5"
                        >
                          {col}
                        </Badge>
                      ))}
              ...
              {/* Footer */}
              <div className="mt-3">
                <p className="text-[0.6875rem] text-muted-foreground/60 text-center tabular-nums">
                  Showing {rows.length}
                  {data.total_rows != null &&
                ...
                {mode === "preview" && mappingEnabled && (
                  <p className="mt-2 text-[0.6875rem] text-muted-foreground/70 text-center">
...
function MetaRow({
  ...
  return (
    <div className="flex items-baseline gap-3 text-sm">
      ...
      <span className="text-foreground text-[0.8125rem] min-w-0">{value}</span>
    </div>
  );
}
