...
// One backend progress reading reconciled against the job's current figures. Split out of poll-loop
// so it can be exercised on its own: poll-loop reaches the store, the toaster and the API client,
// and this is the part that decides what a card shows and when a download counts as finished.
...
// A number off the wire (a byte count or a fraction), or 0 when it is not a usable one. The payload
// is cast from raw JSON with no runtime validation (apiGetProgress -> ProgressLike), and a NaN or
// an Infinity survives every comparison below to reach the store as a width of "NaN%" on the
// progress bar. 0 reads as "not measured", which is the conservative answer here: it holds the
// card's last figure rather than publishing a garbage one, and never manufactures a completion.
...
export function resolveProgressUpdate(
  ...
  // Hold the last reading through a poll that found nothing, rather than publishing the "I could
  // not measure this" answer as a measurement. The backend recomputes both counters from the shared
  // per-repo blobs/ dir every poll, and when it cannot resolve a variant's expected files it
  // returns an all-zero reading against the caller's catalog-hinted total; the failure behind that
  // is negatively cached, so every poll for the whole TTL says the same thing and a finished card
  // reads "0 B of 33 GB" until the app restarts. Deliberately NOT a high-water mark, which is what
  // the fraction below uses. Bytes have legitimate reasons to fall inside one generation and the
  // fraction does not: an XET run that falls back to HTTP re-claims with the same generation and a
  // freshly computed completed_baseline_bytes, and resuming in XET mode purges the partial
  // outright. A floor would pin the card near full for the whole retry and starve the
  // rolling-window rate of the movement it needs to publish a speed or an ETA. Only a zero is
  // ignored, so every real change -- up or down -- still lands on the next poll.
  const reportedDownloaded = finiteReading(progressResp.downloaded_bytes);
  // Whether this poll MEASURED the transfer counter or merely held the last one. A held figure is
  // fine to draw a bar with, but it is priced against the PREVIOUS total, so a caller subtracting
  // it from the current expectedBytes gets a remainder for a plan that no longer exists. The XET
  // fallback reclaim is exactly that pairing: the retry's first reading is a legitimate 0 against a
  // shrunken total, and the 3 GB held behind it wipe out a 0.5 GB remainder.
  ...
  // ...and the completion flag is only honoured on a poll that measured the counter it is a
  // statement about. Holding the bytes over made a new pairing reachable that the plain reading
  // never was: LAST poll's completed_bytes beside THIS poll's complete_on_disk, a completion
  // neither reading showed on its own. The backend does hold the invariant that would rule that out
  // (complete_on_disk is only set once completed_bytes clears the total), but a false "complete"
  // retires the card and drops the download, and that is not an outcome to leave resting on a
  // cross-tier promise no code here checks. Both halves of hasObservedExpectedBytes now provably
