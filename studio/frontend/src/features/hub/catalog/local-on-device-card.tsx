...
export function LocalOnDeviceCard({
  ...
  const canDelete = source === "hf_cache" && !!repoId;
  ...
  const sortedVariants = useMemo(
    () =>
      variants
        ...
        : null,
    [variants, currentVariantState.defaultVariant, gpuGb, systemRamGb],
  );
  ...
  const canUpdate =
    ...
    !!repoId &&
    !updateJobActive &&
    (downloadToCurrentCache || updateAvailable);
  ...
  // cached copy stays available until the new revision verifies.
    ...
    void downloadManager
      ...
      .then((outcome) => {
        ...
        if (outcome === "conflict") {
          ...
          // the quant is loaded for inference. This card shows no job state, so without a
          // toast the refusal would only ever appear in the Downloads panel.
          const failed =
            useDownloadManagerStore.getState().jobs[
              jobKeyOf("model", repoId, updateTargetVariant)
            ];
          toast.error(
            ...
            {
              description:
                failed?.error || "Check the connection, then try again.",
            },
          );
        }
  ...
  return (
    <div className="flex w-full flex-col gap-2">
            ...
            <div className="ml-auto flex items-center gap-0.5">
              {canUpdate && (
                <CardUpdateButton
                  ...
                  emphasized={true}
                  actionLabel={
                    downloadToCurrentCache ? "Download here" : "Update"
                  }
                  tooltip={
                    downloadToCurrentCache
                      ? "Download to the cache location selected in Settings"
      ...
      <DeleteConfirmDialog
        ...
        description={
          <>
            ...
            its downloaded files from this cache location. You can re-download
            it later.
            <DeleteImpactSummary
      ...
      <UpdateConfirmDialog
        ...
        description={
          downloadToCurrentCache
            ? "Download this model to the cache location selected in Settings. The copy in this older location will remain. Progress shows in the Downloads panel."
            : "Re-download the latest version of this model from Hugging Face. Progress shows in the Downloads panel."
        }
      />
