...
// Curated list of sloth emoji stickers offered as profile pictures. The full `public/Sloth emojis`
// folder has ~38 PNGs, but many are non-square or carry heavy whitespace on one or more edges,
// which crops badly inside the round avatar frame. This list is the subset that is (a) effectively
// square (aspect ratio within ~10% of 1:1) and (b) low-whitespace on every edge, so each one fills
// the avatar circle cleanly. Exact duplicates are de-duped. Paths are relative to the public
// folder; resolve with `publicAssetUrl(...)` before using as an <img> src so spaces and subpath
// deploys are handled.
