...
// response can land last and resurrect the entry the newer one replaced.
...
export async function putModelOverride(
  ...
  // Keyed by the folded identity: the backend resolves a legacy casing and the normalized one
 ...
 *  localStorage write already happened, so a failed sync must not fail the save. */
