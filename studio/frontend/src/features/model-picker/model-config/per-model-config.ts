...
/** The OLDEST version that still understands every field present, so a record an older
 *  client can safely rewrite stays in its reach. Only a TRUE disableVision needs v4: false is
 *  what a pre-vision client reconstructs anyway, and stamping every record v4 would put the
 *  whole store out of reach. The tuning group and the reasoning pair follow the same rule. */
