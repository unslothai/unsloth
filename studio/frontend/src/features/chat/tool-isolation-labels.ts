...
export const TOOL_ISOLATION_LIMITATION_TEXT: Readonly<Record<string, string>> =
  {
    shared_container_proc:
      "Uses the container's existing /proc, exposing additional process information. The outer container provides part of the isolation boundary.",
    ...
    network_unrestricted: "Limited can reach any network host.",
...
export function limitedBackendLabel(
  limitedBackend: string | null,
): string | null {
...
export function networkAllowlistSummary(hosts: readonly string[]): string {
  ...
  if (
    hosts.some(
      (host) => host.endsWith("pypi.org") || host.endsWith("pythonhosted.org"),
    )
  ) {
    families.push("PyPI");
  }
  if (
    hosts.some(
      (host) => host.endsWith("huggingface.co") || host.endsWith("hf.co"),
    )
  ) {
  ...
  const named =
    families.length > 0 ? `${families.join(", ")} (${count})` : count;
