 ...
 *  mode and revocation epoch under the same UI session. The epoch prevents a mode round
 *  trip from reviving old consent. The session id rotates on every authentication change
...
export function queuedIsolationDecisionIsCurrent(
  ...
  return (
    live.toolIsolationDecisionEpoch === snapshot.toolIsolationDecisionEpoch &&
