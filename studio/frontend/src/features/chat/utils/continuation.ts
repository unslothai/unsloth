 ...
 *  drives compaction harder. `paused` is refused, pinned by a test: a pause is the backend
 *  rationing one KV cache and it resumes in place, so a client continuation would ask for a
 *  SECOND slot. */
