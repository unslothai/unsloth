...
export function shapeToolResult(input: ShapeToolResultInput): unknown {
  ...
  // Same rule as the file envelope above: only for a tool that emits the image envelope. Elsewhere the backend
  // keeps a well-formed __IMAGES__ line as content the model reads, so slicing it would hide text and fetch a
  // sandbox file that was never written.
  const imgIdx = IMAGE_SENTINEL_TOOLS.has(toolName)
    ? rawResult.lastIndexOf(SANDBOX_IMAGES_MARKER)
    : -1;
...
// Where a Sources entry comes from lives next to the shaper only as a re-export: the replay folds stored frames
// into parts under node --test, where importing this module with it would drag in React. Both readers must agree
// on an id shape down to the byte, or one entry lands twice.
export {
