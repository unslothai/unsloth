 ...
 *
 * @returns whether the adapter should swallow the event instead of drawing a tool card. An
 * ungated call is hidden on both halves: the research card is the reply, and a tool pill would
 * just precede it saying the same thing.
 */
export function readDeepResearchToolEvent(
  ...
  if (event.type === "tool_start") {
    // The local loop paints a provisional tool_start with empty arguments before the real
    // one, so only a start carrying a question is remembered, and only the first: a second
    // call in the same turn is the model repeating itself.
  ...
  if (
    ...
    // The run starts on the question the model passed; if that could not be read, on the
    // user's own message, which is what research did before the model had a say.
    handoff.question =
      callId === handoff.pendingCallId ? handoff.pendingQuestion : "";
  }
  // Close whatever this pair opened: a hidden start gets a hidden end, and a card the user
  // was asked to approve gets closed by the event that ends it.
