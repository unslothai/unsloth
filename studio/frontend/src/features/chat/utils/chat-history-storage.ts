...
export async function getStoredChatThreadReadResult(
  ...
  if (isPlatformChatPersistenceEnabled()) {
    return {
      thread: (await getPlatformThreadForChat(threadId)) ?? undefined,
      cacheable: true,
    };
  }
...
async function clearPlatformStoredChats(): Promise<ClearStoredChatsResult> {
...
export function clearStoredChats(
  ...
  if (isPlatformChatPersistenceEnabled()) {
    return clearPlatformStoredChats();
  }
