...
export function ChatSettingsPanel({
  ...
  // Direct-file / custom-folder GGUFs load without a variant label but still report a GGUF
  // context, so detect them via the context and the checkpoint suffix too. Otherwise Max
  // Tokens would fall back to params.maxSeqLength instead of the loaded GGUF context.
  const isGguf =
    isLoadedGguf ||
  ...
  // The loaded model's own kind, not the pending control: the notice explains a fallback that
  // already happened, so a staged edit must not re-label it and point at the wrong file.
