...
export function useTransferStats(
  ...
  useEffect(() => {
    ...
    if (typeof document !== "undefined" && document.hidden) {
      // Callers poll on an interval, and a hidden tab's is clamped to about
      // once a minute, so these gaps time the poller and the estimator would
      // read them as the burst cadence. Publishing nothing also stops a stale
      // reading outliving the transfer: this effect is keyed on ``bytes``, so a
      // counter that stops moving never runs it again to correct itself.
