          ...
          let loadSelectedGpuIds =
            pendingLoadConfig?.selectedGpuIds !== undefined
              ? reconcilePersistedGpuIds(
                  ...
                  // A per-model config carries its saved index space; an absent
                  // stamp is a pre-stamp physical pick (matches apply-per-model).
                  pendingLoadConfig.selectedGpuIdsIndexKind ?? "physical",
                )
              : reconcilePersistedGpuIds(
