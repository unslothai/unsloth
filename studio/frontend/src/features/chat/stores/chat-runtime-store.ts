...
/** The context the last load or status published for the model on screen. ggufContextLength
 *  covers only GGUF, so a safetensors model has nothing else to clamp the replay against. */
...
/** A model that took over while the request was in flight. Its defaults lose to its own entry
 *  but outrank the global set, which belongs to whichever model was used last. */
...
function noteModelDefaultsBeforeHydration(
  ...
  // The model resident at startup is the one the saved global set describes, so its defaults
...
function getHydratedSettingsState(
  ...
  for (const key of SCALAR_SETTING_KEYS) {
    ...
    // Only a local model or an openai_codex provider can run deep research, so a stored true
    // must not arm the pill for another external checkpoint.
    if (
  ...
  // ggufContextLength describes whatever is resident, which an external pick leaves loaded, so
  // it is not this checkpoint's context to clamp against.
...
export const useChatRuntimeStore = create<ChatRuntimeStore>((set, get) => ({
    ...
    settingsHydrationPromise = (async () => {
      ...
      try {
        ...
        if (applied) {
          ...
          // Only an authoritative read proves a mirrored field is unset on the server: a GET that fell
          // back to legacy storage knows nothing, and backfilling would push stale values.
          ...
          // The previous session's tab-close writes, for rows that did not exist when it sent them. A
          // replay of one that did land is refused by its own seq.
  ...
  setCheckpoint: (modelId, ggufVariant, options) =>
    set((state) => {
      ...
      // Remember what the outgoing model was running with. Not for a restore: it is one a
      // background load put there, and its load defaults are not settings the user chose.
      const outgoing =
  ...
  // Re-apply the incoming thread's own usage rather than blanking the bar: a background run
  // never wrote the visible value, and a mounted runtime skips the history loader.
