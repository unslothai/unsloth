      ...
      try {
          ...
          try {
            ...
            for await (const chunk of stream) {
              ...
              if (toolEvent !== undefined) {
                ...
                // ``renderFullContent()`` preserves any Codex per-tab text
                // accumulated in earlier ``_toolEvent`` frames so the tabs do
                // not briefly appear and then vanish when the regular content
                // path arrives (the round 8 render-order fix lives in there).
        ...
        yield {
          content: [
            ...
            // the regular content path would have erased it. The
            // orderAssistantContent step keeps the rest of the message
            // (tools / images) in the right slot relative to the text.
            ...orderAssistantContent(parseAssistantContent(renderFullContent())),
            ...sourceParts,
