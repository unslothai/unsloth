        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          {onRunList && (
            <button
              ...
              className="flex h-7 items-center gap-1 rounded-lg px-2 text-xs font-medium text-primary hover:bg-primary/10 transition-colors"
              title="Run list"
            >
              Run
            </button>
            ...
            <>
              ...
              {filteredLists.length > 0 ? (
                filteredLists.map((entry) => (
                  <PromptListCard key={entry.id} entry={entry} onRunList={onRunList} />
                ))
              ) : (
                !showNewList && (
                  <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground gap-2">
                    {searchQuery.trim() ? (
                      ...
                      <>
                        ...
                        <p className="text-xs opacity-80">
                          A prompt list saves a sequence of prompts for quick reuse
                        </p>
                      </>
                    )}
                  </div>
