...
// vLLM does not advertise reasoning per model on OpenAI-compat, so the user
// flags the connection instead. Ollama is not here: its native /api/tags names
// a "thinking" capability per model, which is a truer answer than a checkbox on
// a connection serving both thinking and non-thinking models.
