...
/**
 * Owns the dialog interaction lifecycle independently of React. Native picker
 * results and project creation can both outlive a render, so the controller
 * gives the component and executable tests the same stale-result behavior.
 */
export function createNewProjectDialogController<TSource>(
  ...
  async function clickExistingFolder(): Promise<void> {
    ...
    try {
      ...
      if (!(selected && mounted && pickerGuard.isCurrent(pickerClaim))) {
        return;
      }
      patch({
        folder: selected,
  ...
  return {
    ...
    openChanged(open) {
      if (open) {
        dependencies.onOpenChange(true);
      } else {
        close();
      }
    },
