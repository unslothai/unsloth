      ...
      ({ dream: updatedDream, proposal: updatedProposal }) => {
        ...
        if (updatedProposal.acceptedEntry) {
          setMemoryEntries((current) => [
            ...
            ...current.filter(
              (entry) => entry.path !== updatedProposal.acceptedEntry?.path,
            ),
          ]);
        }
        if (updatedProposal.deletedEntry?.deleted) {
          setMemoryEntries((current) =>
            current.filter(
              (entry) => entry.path !== updatedProposal.deletedEntry?.path,
            ),
          );
        }
      },
                ...
                <div className="space-y-1.5">
                  {memoryEntries.slice(0, 20).map((entry) => (
                    ...
                    >
                      ...
                      {entry.content ? (
                        <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap break-words text-ui-11 leading-5 text-muted-foreground">
                          {entry.content}
                        </pre>
                      ) : null}
                    </details>
              ...
              {workspace.capabilities.dreaming ? (
                <div className="mt-3 rounded-xl border border-border/60 bg-background/45 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-ui-11 text-muted-foreground">
                        Analyze selected transcripts asynchronously. Nothing
                        enters memory until you accept a proposal.
                      </p>
                    </div>
                    <Button
                      type="button"
                      ...
                      disabled={
                        Boolean(busy) || selectedDreamTranscripts.size === 0
                      }
                    >
                      {isBusy("dream-start") ? (
                  ...
                  {memoryTranscripts.length ? (
                    <div className="mt-2 grid gap-1 sm:grid-cols-2">
                      {memoryTranscripts.map((transcript) => (
                        <label
                          key={transcript.id}
                          className="flex min-w-0 items-center gap-2 rounded-lg bg-muted/35 px-2.5 py-2 text-ui-11"
                        >
                          <input
                            type="checkbox"
                            checked={selectedDreamTranscripts.has(
                              transcript.id,
                            )}
                            onChange={(event) =>
                    ...
                    <p className="mt-2 text-ui-11 text-muted-foreground">
                  ...
                  <Textarea
                    ...
                    onChange={(event) =>
                      setDreamInstructions(event.target.value)
                    }
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="min-w-0 flex-1 truncate text-ui-11 text-muted-foreground">
                              {dreamProposals(dream).length} proposal
                              {dreamProposals(dream).length === 1 ? "" : "s"}
                            </span>
                            ...
                            dream.status === "queued" ? (
                              <Button
                                ...
                                onClick={() =>
                                  void mutateBackgroundTask(dream, "cancel")
                                }
                                disabled={Boolean(busy)}
                              >
                                <Square />
                          ...
                          {dreamProposals(dream).map((proposal) => (
                            <div
                              key={proposal.id}
                              className="mt-2 rounded-lg border border-border/50 bg-background/45 p-2"
                            >
                              <div className="flex items-center gap-2">
                                <span className="min-w-0 flex-1 truncate font-mono text-ui-11">
                                  {proposal.path}
                                </span>
                                <Badge variant="outline">
                                  {proposal.prevalence.transcripts}/
                                  {proposal.prevalence.selected}
                                </Badge>
                              ...
                              <details className="mt-2 rounded-lg bg-muted/35 px-2 py-1.5">
                                <summary className="cursor-pointer text-ui-11 font-medium text-foreground">
                                  {proposal.operation === "delete"
                                    ? "Review proposed deletion and evidence"
                                    : "Review proposed memory and evidence"}
                                </summary>
                                {proposal.operation === "delete" ? (
                                  <p className="mt-2 text-ui-11 leading-5 text-muted-foreground">
                                    This deletes the selected memory entry after
                                    you accept it.
                                  </p>
                                ) : (
                                  <pre className="mt-2 max-h-32 overflow-auto whitespace-pre-wrap break-words text-ui-11 leading-5 text-muted-foreground">
                                    {proposal.content}
                                  </pre>
                                )}
              ...
              {gitStatus ? (
                <>
                  ...
                  {!workspace.capabilities.gitMutations ? (
                    <p className="mt-3 rounded-xl border border-border/60 bg-muted/25 px-3 py-2 text-ui-11 text-muted-foreground">
                  ...
                  >
                    ...
                    <div className="mt-2 max-h-44 overflow-auto rounded-lg bg-muted/30 p-2">
                      ...
                      {visibleCommitFiles.length ? null : (
