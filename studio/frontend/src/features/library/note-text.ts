...
/** Saves go back as UTF-8, so only a UTF-8 file comes back unchanged. The preview words these. */
...
export function decodeNote(bytes: Uint8Array, truncated = false): DecodedNote {
  ...
  let raw: string;
  let readOnlyReason: NoteReadOnlyReason | null = encoding === "utf-8" ? null : UTF16_READ_ONLY;
  try {
