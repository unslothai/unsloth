      ...
      >
        {values.map((v, i) => (
          <Badge
            key={`m-${v}-${i}`}
            variant="secondary"
            className="corner-squircle h-4 shrink-0 px-1.5 text-[0.625rem]"
          >
            {v}
          </Badge>
        ))}
      ...
      <div className="flex flex-wrap gap-1">
        {values.slice(0, visibleCount).map((v, i) => (
          <Badge
            key={`${v}-${i}`}
            variant="secondary"
            className="corner-squircle h-4 px-1.5 text-[0.625rem]"
          >
            {v}
          </Badge>
        ))}
        {overflow > 0 && (
          <Badge variant="outline" className="corner-squircle h-4 px-1.5 text-[0.625rem]">
            +{overflow}
          </Badge>
        )}
      </div>
