...
export async function fetchCurrentAccount(): Promise<CurrentAccount> {
  ...
  if (!response.ok) throw new Error(await detail(response, "Failed to load account"));
  return response.json();
}

export async function fetchAccounts(): Promise<ManagedAccount[]> {
  const response = await authFetch("/api/auth/users");
  if (!response.ok) throw new Error(await detail(response, "Failed to load accounts"));
...
export async function createAccount(
  ...
  if (!response.ok) throw new Error(await detail(response, "Failed to create account"));
  return response.json();
}
...
export async function deleteAccount(username: string): Promise<void> {
  ...
  if (!response.ok) throw new Error(await detail(response, "Failed to delete account"));
}
