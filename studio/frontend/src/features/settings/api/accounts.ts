...
async function accountsRequest(path = "", init?: RequestInit): Promise<Response> {
  const response = await authFetch(`/api/accounts${path}`, init);
  if (!response.ok) {
    const payload = await response.json().catch(() => null) as { detail?: unknown } | null;
    throw new Error(typeof payload?.detail === "string" ? payload.detail : "Account request failed.");
  }
  return response;
}
const accountPath = (username: string) => `/${encodeURIComponent(normalizeAccountUsername(username))}`;
...
export async function createAccount(username: string): Promise<AccountSetupCode> {
  const response = await accountsRequest("", {
    method: "POST",
...
export async function regenerateSetupCode(username: string): Promise<AccountSetupCode> {
  const response = await accountsRequest(`${accountPath(username)}/setup-code`, { method: "POST" });
  return response.json();
}
export async function setAccountActive(username: string, active: boolean): Promise<void> {
  await accountsRequest(`${accountPath(username)}/${active ? "reactivate" : "deactivate"}`, { method: "POST" });
  await refreshAccountPolicy();
}
