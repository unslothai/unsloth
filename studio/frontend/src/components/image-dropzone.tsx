...
/** Click-or-drop image picker that hands back a `data:` URL.
 *
 * Shared by the Images page (the img2img / reference source) and the Video page (MiniMax-H3's
 * first and last keyframes), so both send the same payload shape to the same backend decoder
 * rather than each growing its own upload path. */
export function ImageDropzone({
  ...
  /** Alt text on the preview and the accessible name of its remove button. */
  label?: string;
  /** Empty-state caption. */
  prompt?: string;
  className?: string;
  ...
  if (value) {
    return (
      <div className={cn("relative overflow-hidden rounded-[10px] border border-border", className)}>
        <img src={value} alt={label} className="max-h-44 w-full object-contain bg-muted/30" />
        <Tooltip>
