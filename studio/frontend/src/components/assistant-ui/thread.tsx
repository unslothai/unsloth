...
function stopPromptQueueRun(
  ...
  for (const run of runs) {
    ...
    const shouldCancelActiveRun =
      cancelActiveRun && Boolean(activeItem?.dispatched);
    deletePromptQueueRun(run);
...
function stopAllPromptQueueRuns(cancelActiveRun = true) {
...
if (typeof window !== "undefined") {
  window.addEventListener(PROMPT_QUEUE_STOP_EVENT, (event) => {
    const { threadIds, cancelActiveRun = true } =
      (event as CustomEvent<PromptQueueStopOptions>).detail ?? {};
    if (threadIds && threadIds.length > 0) {
      stopPromptQueueRun(threadIds, cancelActiveRun);
      return;
    }
    stopAllPromptQueueRuns(cancelActiveRun);
  });
