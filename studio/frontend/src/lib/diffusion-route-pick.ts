...
export function diffusionRoutePick(
  ...
  if (quant)
    return {
      repoId: model,
      opts: { ...target, kind: "gguf", filename: quant },
    };
  ...
  if (spec)
    return {
      repoId: model,
      opts: { ...target, kind: spec.kind, filename: spec.filename },
    };
  ...
  if (lower.endsWith(".gguf")) {
    return { repoId: dir, opts: { ...target, kind: "gguf", filename } };
  }
  if (lower.endsWith(".safetensors")) {
    return { repoId: dir, opts: { ...target, kind: "single_file", filename } };
  }
  // A repo id: the curated/pipeline case, loaded as before.
  return { repoId: model, opts: { ...target, kind: "pipeline" } };
}
