  ...
  settings: {
    ...
    general: {
      ...
      remoteAccess: {
        ...
        description:
          "Используйте Unsloth и его API с других устройств через Cloudflare.",
        ...
        qrDescription:
          "Отсканируйте, чтобы открыть удалённый URL в браузере телефона.",
        ...
        streamingBuffered:
          "Временные туннели буферизуют потоковые ответы до завершения.",
        passwordLabel: "Удалённый пароль",
        passwordDescription:
          "Удалённые браузеры входят как unsloth. Настольное приложение продолжает входить автоматически.",
        customHostnameTitle: "Пользовательское имя хоста",
        customHostnameDescription:
          "Используйте своё имя хоста, чтобы адрес не менялся.",
        ...
        setupConfirmDescription:
          "Проверьте имя хоста ниже. Затем Cloudflare попросит авторизовать этот туннель: выберите домен, которому принадлежит это имя хоста, и нажмите Authorize.",
        setupPreparing:
          "Открываем Cloudflare, чтобы вы могли авторизовать этот туннель…",
        setupAuthorize:
          "Ожидание авторизации в Cloudflare. Выберите домен, которому принадлежит {hostname}, и нажмите Authorize.",
        ...
        removeDescription:
          "Это удаляет только то, что сохранил Unsloth. Туннель и его DNS-запись останутся в Cloudflare, пока вы не удалите их там.",
        ...
        removeLocally:
          "Откройте настройки на компьютере с Unsloth, чтобы удалить этот подключённый туннель.",
        dnsConflict:
          "{hostname} уже указывает на другой ресурс в Cloudflare. Удалите эту DNS-запись или настройте другое имя хоста.",
        requestFailed: "Запрос удалённого доступа не выполнен.",
        runtimeFailed:
          "Удалённый доступ неожиданно остановился. Повторите попытку или проверьте журналы сервера.",
        startAction: "Запустить",
        stopAction: "Остановить",
        autoStartLabel: "Запускать автоматически",
        autoStartTemporary:
          "Создавать новый временный URL при запуске Unsloth. Отключение сейчас не выключит настройку.",
        autoStartCustom:
          "Подключать это пользовательское имя при запуске Unsloth. Отключение сейчас не выключит настройку.",
        autoStartCustomMissing:
          "Ничего не будет подключено, пока вы не настроите пользовательское имя хоста ниже.",
        blockServerStarting:
          "Удалённый доступ станет доступен после завершения запуска сервера.",
        blockCustomNotConfigured:
          "Настройте ниже пользовательское имя хоста, прежде чем запускать удалённый доступ.",
        blockPasswordDesktop:
          "Задайте ниже пароль для удалённого доступа, прежде чем запускать его.",
        blockPasswordWeb:
          "Попросите администратора хоста задать пароль удалённого доступа.",
        blockExplicitlyDisabled:
          "Удалённый доступ был отключён при запуске этого сервера.",
        blockLaunchManaged:
          "Этот туннель управляется командой запуска сервера.",
        blockColabManaged: "Этот туннель управляется сеансом Colab.",
        blockColab: "Настройки удалённого доступа управляются Colab.",
        dnsConflictUnknown:
          "Это имя хоста уже указывает на другой ресурс в Cloudflare. Удалите эту DNS-запись или настройте другое имя хоста.",
        errorInvalidHostname:
          "Это не полное имя хоста. Введите имя вида unsloth.example.com.",
        errorWrongDomain:
          "Домен, который вы авторизовали в Cloudflare, не покрывает {hostname}. Настройте заново и выберите домен, которому принадлежит это имя хоста.",
        errorHostnameUnreachable:
          "Туннель подключился, но {hostname} не открылся. Удалите этот пользовательский туннель, настройте его заново и выберите домен, которому принадлежит это имя хоста.",
        errorCertificateExists:
          "В {path} уже есть сертификат Cloudflare, и Unsloth не станет его перезаписывать. Переместите или удалите этот файл и настройте заново.",
        errorBusy:
          "Другая операция удалённого доступа ещё выполняется. Дождитесь её завершения и повторите.",
        errorIdentityExists:
          "Пользовательский туннель уже настроен. Удалите его, прежде чем настраивать другое имя хоста.",
        errorUnavailable:
          "Не удалось связаться с Cloudflare. Проверьте интернет-соединение на этом компьютере и повторите.",
        errorCancelled:
          "Настройка отменена. Настройте заново, когда будете готовы.",
        errorLoginTimeout:
          "Cloudflare не был авторизован вовремя. Настройте заново и нажмите Authorize, когда откроется страница Cloudflare.",
        errorLoginFailed:
          "Cloudflare не завершил авторизацию. Настройте заново.",
        errorSetupFailed:
          "Не удалось завершить настройку пользовательского туннеля. Настройте заново или проверьте журналы сервера.",
        errorTeardownFailed:
          "Не удалось удалить сохранённый пользовательский туннель. Повторите попытку или проверьте журналы сервера.",
        errorTeardownManual: "Пользовательский туннель удалён из Unsloth.",
        errorUnknown:
          "Изменение пользовательского туннеля не завершилось. Повторите попытку или проверьте журналы сервера.",
      },
      notifications: {
