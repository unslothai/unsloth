...
export const ru = {
  ...
  settings: {
    ...
    resources: {
      ...
      llamaBackend: {
        exactConcurrency: {
          label: "Точная конкурентность",
          description: "Снижает пропускную способность, когда чаты идут одновременно, и применяется при следующей загрузке модели.",
          hint: "Просить llama-server декодировать так, чтобы результат не зависел от других чатов, разделяющих KV-кэш: чат выдаёт одни и те же токены и в одиночку, и рядом с тремя соседями. «Авто» просит и продолжает без этого, если сервер отказал; «Вкл» просит и вместо этого прерывает загрузку; «Выкл» не просит никогда.",
          auto: "Авто",
          off: "Выкл",
          on: "Вкл",
          envLocked: "Задано переменной окружения UNSLOTH_LLAMA_EXACT_CONCURRENCY, поэтому этот выбор не используется.",
          reloadRequired: "Перезагрузите модель, чтобы применить новую настройку.",
        },
