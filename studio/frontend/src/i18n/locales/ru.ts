  ...
  settings: {
    ...
    apiKeys: {
      ...
      decisionApi: {
        ...
        description: "Задавайте вопросы «да/нет», с выбором и с оценкой о любом тексте и получайте откалиброванные вероятности от локальной модели Laya. Совместимо с API Jev, поэтому SDK TypeSafe работает без изменений.",
        enable: "Обрабатывать запросы",
        enableDescription: "Отвечает на POST /v1/systemone. При включении модель скачивается; в память она загружается при первом запросе.",
        lockedByEnv: "Задано переменной {name}.",
        model: "Модель",
        modelDescription: "Многоязычная поддерживает более 100 языков. Английская крупнее и работает только с английским.",
        ...
        tryIt: "Попробовать",
        caveat: "Базовые модели универсальны. Проверьте ответы на своих данных, прежде чем полагаться на них.",
      },
      usageNoModel:
