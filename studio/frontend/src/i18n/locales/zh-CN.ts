  ...
  settings: {
    ...
    voice: {
      ...
      recents: {
        ...
        manage: "管理",
        pageDescription:
          "每次语音输入都会保存。你可以查看、复制或删除，也可以打开使用过该语音输入的聊天。",
        ...
        deleteLinkedDescription:
          "要删除这条已保存的语音输入吗？你也可以一并删除使用过它的聊天。此操作无法撤销。",
    ...
    general: {
      ...
      modelAutoSwitch: {
        ...
        autoDownloadDescription:
          "获取 API 请求中指定但尚未下载的 GGUF。开启后，任何持有 API 密钥的人都会消耗你的磁盘空间和带宽。",
        keepKv: "空闲卸载后保留聊天上下文",
        keepKvDescription:
      ...
      permissions: {
        ...
        bypassLabel: "工具权限",
        bypassDescription:
          "设置 Unsloth 执行聊天中的工具调用（终端、python、网页、MCP）前的审批方式。“完全访问”会关闭审批和代码沙箱。",
      },
    },
    profile: {
      title: "个人资料",
      ...
      stats: {
        ...
        activityDescription: "过去 {weeks} 周共 {total} 个 token",
        mode: {
          daily: "按天",
          weekly: "按周",
          cumulative: "累计",
        },
        cellTooltip: "{date} · {tokens} 个 token，{messages} 条消息",
        weekTooltip: "{date} 当周 · {tokens} 个 token",
        less: "较少",
        more: "较多",
        insightsTitle: "活跃度分析",
        totalChats: "聊天总数",
        ...
        modelSummary: "{tokens} · {messages} 条消息",
    ...
    agents: {
      ...
      modelNote:
        "Codex 需要通过 llama-server 提供服务的 GGUF 模型。其他智能体也可以使用基于 transformers 的模型；去掉 --model 即可使用 Unsloth Studio 中已加载的模型。",
      subagent: {
        title: "将本地模型用作子智能体",
        description:
