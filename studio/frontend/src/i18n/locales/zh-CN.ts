  ...
  settings: {
    ...
    general: {
      ...
      downloads: {
        ...
        transportHint:
          "HTTPS 就是普通 TLS：任何网络、代理和 VPN 都允许，取消或断开的传输会从磁盘上已有的字节续传，内存占用也很平稳。Xet 获取去重后的数据块，因此与已有仓库共享数据时可以快得多，但它需要 hf_xet、占用更多内存，取消时会丢弃正在传输的文件。Auto 会针对本机判断：它会权衡内存以及 Xet 在这里是否曾经卡住，并在必要时退回 HTTPS。新安装使用 HTTPS。",
        https: "HTTPS",
        xet: "Xet",
        auto: "Auto",
