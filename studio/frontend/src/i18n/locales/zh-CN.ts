  ...
  settings: {
    ...
    apiKeys: {
      ...
      decisionApi: {
        title: "决策 API",
        experimental: "实验性",
        description: "针对任意文本提出是/否、选择和评分问题，并从本地 Laya 模型获得校准后的概率。它兼容 Jev API，因此 TypeSafe 的 SDK 无需修改即可使用。",
        enable: "处理请求",
        enableDescription: "响应 POST /v1/systemone。开启后会下载模型，并在首次请求时加载到内存。",
        lockedByEnv: "由 {name} 设置。",
        model: "模型",
        modelDescription: "多语言版支持 100 多种语言。英语版更大，仅支持英语。",
        ...
        tryIt: "试一试",
        caveat: "基础模型是通用模型。在依赖它之前，请先用你自己的数据核对答案。",
      },
      usageNoModel:
