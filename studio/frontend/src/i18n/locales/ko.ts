  ...
  settings: {
    ...
    resources: {
      ...
      modelMemory: {
        ...
        noRamReserveHint: "가중치의 전체 사본을 시스템 RAM에 예약하지 않습니다. llama.cpp의 메모리 매핑 로딩을 유지하고 --no-mmap과 --mlock을 제거합니다. 매핑 자체는 남아 있으므로 로더가 건드린 페이지는 운영체제가 회수할 때까지 이 프로세스 사용량으로 표시됩니다.",
        ...
        mlockUngoverned: "불러온 모델은 모델 메모리가 제어하지 않는 실행기를 사용하므로 이 설정이 현재 실행에는 영향을 주지 않습니다. 설정은 저장된 상태로 유지되며 다음에 불러오는 llama.cpp 모델에 적용됩니다.",
