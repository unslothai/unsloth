  ...
  settings: {
    ...
    apiKeys: {
      ...
      decisionApi: {
        title: "判定 API",
        experimental: "実験的",
        description: "任意のテキストについて、はい/いいえ・選択・スコアの質問を行い、ローカルの Laya モデルから較正済みの確率を受け取れます。Jev API 互換なので、TypeSafe の SDK をそのまま使えます。",
        enable: "リクエストに応答",
        enableDescription: "POST /v1/systemone に応答します。オンにするとモデルをダウンロードし、最初のリクエストでメモリに読み込みます。",
        lockedByEnv: "{name} で設定されています。",
        model: "モデル",
        modelDescription: "多言語は 100 以上の言語に対応。英語はサイズが大きく、英語専用です。",
        ...
        tryIt: "試してみる",
        caveat: "ベースモデルは汎用です。頼る前に、自分のデータで回答を確認してください。",
      },
      usageNoModel:
