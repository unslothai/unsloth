...
export function preflightStaleMessage(
  ...
  // Not an install problem: an unmounted roaming or network profile. Telling
  // this user to update points them at a command that needs the same folder.
  if (reason === WORKING_DIRECTORY_UNAVAILABLE) {
    return "Unsloth cannot reach your user folder, so it has nowhere to run from. This usually means a network or roaming profile is not available yet. Reconnect and try again.";
  }
