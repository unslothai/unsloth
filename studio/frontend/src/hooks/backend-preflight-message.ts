...
/// which reach here through llama_runtime_reason on the capability payload.
...
export function preflightStaleMessage(
  ...
  // Nothing is out of date here: the install is current and some of its files
  // are gone. Saying "too old" sends people to look for an update that reports
  // they are already up to date, and the usual cause is worth naming since a
  // reinstall that lands in the same quarantine needs an exclusion, not a retry.
