...
export const BUDGET = {
  ...
  // Find in page is eager rather than behind `lazy`: the shell prevents the chord's default before
  // the chunk would exist, and idle warming never runs on Safari, which keeps requestIdleCallback
  // behind a flag. It fits the ceiling above with 5.8 KB spare, so it raises nothing.
