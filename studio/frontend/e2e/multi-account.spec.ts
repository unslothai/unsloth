...
async function logOut(page: Page) {
  await page.evaluate(async () => {
    ...
    for (const key of ["unsloth_auth_token", "unsloth_auth_refresh_token", "unsloth_auth_must_change_password", "unsloth_auth_session_mark"]) {
      localStorage.removeItem(key);
    }
  });
}

test("owner creates an account, setup is private, and browser switching clears account data", async ({ page, context, request }) => {
  test.skip(!ownerPassword, "Requires a disposable Studio server and owner password.");
  const username = `e2e_${Date.now()}`;
  const managedPassword = `Managed-${Date.now()}-password`;
  const ownerLogin = await request.post(`${baseURL}/api/auth/login`, { data: { username: "unsloth", password: ownerPassword } });
  ...
  try {
    ...
    if (initialMode === "single") await expect(page.locator("#username")).toHaveCount(0);
    await login(page, "unsloth", ownerPassword!);
    await expect(page).toHaveURL(/\/chat/);
    expect(await page.evaluate(() => localStorage.getItem("unsloth_e2e_private"))).toBe("owner-private");

    // Use the documented admin API to avoid depending on a platform-specific Settings shortcut.
    const response = await request.post(`${baseURL}/api/accounts`, { headers: ownerHeaders, data: { username } });
    ...
    await secondTab.evaluate(() => { (window as Window & { oldAccountDocument?: boolean }).oldAccountDocument = true; });

    await login(page, username.toUpperCase(), setup_code);
    await expect(page).toHaveURL(/\/change-password/);
    expect(await page.evaluate(() => localStorage.getItem("unsloth_e2e_private"))).toBeNull();
    expect(await page.evaluate(() => localStorage.getItem("chat-draft:e2e"))).toBeNull();
    expect(await page.evaluate(() => localStorage.getItem("theme"))).toBe("dark");
    expect(await page.evaluate(() => localStorage.getItem("unsloth_chat_permission_mode"))).not.toBe("full");
    await expect.poll(() => secondTab.evaluate(() => (window as Window & { oldAccountDocument?: boolean }).oldAccountDocument)).toBeUndefined();
    ...
    await expect(page).toHaveURL(/\/chat/);
    const managedToken = await page.evaluate(() => localStorage.getItem("unsloth_auth_token"));
    const denied = await request.get(`${baseURL}/api/accounts`, { headers: { Authorization: `Bearer ${managedToken}` } });
    expect(denied.status()).toBe(403);
    // A persisted Accounts selection cannot mount or expose the owner-only panel.
    await page.evaluate(() => localStorage.setItem("unsloth_settings_active_tab", "accounts"));
    await page.reload();
    await expect(page.getByTestId("settings-tab-accounts")).toHaveCount(0);
    await page.evaluate(() => localStorage.setItem("unsloth_e2e_private", "managed-private"));
    await logOut(page);
    await login(page, "unsloth", ownerPassword!);
    await expect(page).toHaveURL(/\/chat/);
    expect(await page.evaluate(() => localStorage.getItem("unsloth_e2e_private"))).toBeNull();
    ...
    if (created) {
      const response = await request.delete(`${baseURL}/api/accounts/${encodeURIComponent(username)}`, { headers: ownerHeaders });
      expect(response.ok()).toBeTruthy();
    }
  }
});
