...
import {
  ACCOUNT_CHROME_KEYS, ACCOUNT_DATABASES, BROWSER_ACCOUNT_KEY,
  installAccountTransitionListener, resetFullAccessForMultiUser, transitionBrowserAccount,
} from "../src/lib/account-transition.ts";

function browserWith(values: Record<string, string> = {}, databaseResult = "success") {
  ...
  const storage = {
    ...
    removeItem: (key: string) => { removed.push(key); data.delete(key); },
  } as Storage;
  const browser = {
    ...
    indexedDB: { deleteDatabase: (name: string) => {
      ...
      queueMicrotask(() => {
        const callback = databaseResult === "blocked" ? request.onblocked : databaseResult === "error" ? request.onerror : request.onsuccess;
        callback?.call(request, {} as IDBVersionChangeEvent);
      });
      return request;
    } },
    location: { replace: (route: string) => { replaced.push(route); }, reload: () => { reloads++; } },
    addEventListener: (_type: string, listener: (event: Partial<StorageEvent>) => void) => { listeners.push(listener); },
  } as unknown as Window;
  return { browser, data, removed, deleted, replaced, listeners, reloads: () => reloads };
}

for (const marker of [undefined, "unsloth"]) {
  test(`single-user login never purges, marker ${marker ?? "absent"}`, async () => {
    const b = browserWith({ unsloth_chat_permission_mode: "full", "chat-draft:1": "keep", ...(marker ? { [BROWSER_ACCOUNT_KEY]: marker } : {}) });
    let committed = 0;
    assert.equal(await transitionBrowserAccount(" UNSLOTH ", "/chat", () => { committed++; }, b.browser), false);
...
test("switch removes every content prefix and preserves only listed chrome and unrelated keys", async () => {
  const chrome = Object.fromEntries([...ACCOUNT_CHROME_KEYS].map((key) => [key, "chrome"]));
  const content = ["unsloth_auth_token", "unsloth_hf_token", "unsloth_new_feature", "unsloth_chat_permission_mode", "unsloth-profile", "chat-draft:1", "chat-draft-pastes:2"];
  const b = browserWith({ ...chrome, ...Object.fromEntries(content.map((key) => [key, "private"])), "unrelated": "keep", "unsloth_web_update_dismissed:pip:1": "keep", [BROWSER_ACCOUNT_KEY]: "unsloth" });
  const changed = await transitionBrowserAccount("Alice", "/change-password", () => {
  ...
  for (const key of ACCOUNT_CHROME_KEYS) assert.equal(b.data.get(key), "chrome");
...
test("same managed account keeps content and avoids IndexedDB work", async () => {
  const b = browserWith({ [BROWSER_ACCOUNT_KEY]: "alice", "chat-draft:1": "alice" });
...
test("returning to the owner clears the previous managed account", async () => {
  const b = browserWith({ [BROWSER_ACCOUNT_KEY]: "alice", "unsloth-private": "alice" });
  assert.equal(await transitionBrowserAccount("unsloth", "/chat", () => {}, b.browser), true);
...
for (const failure of ["blocked", "error"]) {
  test(`IndexedDB ${failure} prevents new session publication and navigation`, async () => {
    ...
    let committed = false;
    await assert.rejects(transitionBrowserAccount("alice", "/chat", () => { committed = true; }, b.browser));
...
test("cross-tab switches reload once, ignoring initial owner markers, removals and unrelated storage", () => {
  ...
  send({ key: BROWSER_ACCOUNT_KEY, oldValue: "unsloth", newValue: "alice", storageArea: {} as Storage });
  assert.equal(b.reloads(), 0);
  send({ key: BROWSER_ACCOUNT_KEY, oldValue: "unsloth", newValue: "alice", storageArea: b.browser.localStorage });
