...
const { uploadAttachmentFile, withSandboxAttachmentPaths } =
...
test("stored attachments across the history name their sandbox copy once each", () => {
  ...
  assert.deepEqual(rendered[2].attachments[0].content, [
    {
      type: "text",
      text: "[deck?.pptx is saved at .unsloth_attachments/x/deck_.pptx in the python tool's working directory]",
    },
