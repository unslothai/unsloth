...
const BACKEND_SOURCES = [
  ...
  "../../backend/core/inference/srt_runtime.py",
...
test("SRT names the runtime without implying platform qualification", () => {
  assert.match(TOOL_ISOLATION_LIMITATION_TEXT.srt_platform_qualification_incomplete, /preview.*incomplete.*CUDA/);
  assert.equal(backendLabel("srt", "windows", "srt-0.0.75-strict-v1"), "Sandbox Runtime (SRT)");
  for (const code of ["srt_windows_system_dns_unfenced", "srt_windows_shared_account_grants", "srt_macos_system_dns_unfenced", "srt_private_unix_ipc_unqualified", "srt_runtime_unavailable", "srt_platform_unqualified"]) {
    assert.ok(TOOL_ISOLATION_LIMITATION_TEXT[code]?.length > 20);
  }
});
