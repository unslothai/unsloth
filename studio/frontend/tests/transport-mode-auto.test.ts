...
test("http is the fallback transport preference", () => {
  // Only a fallback: the install's own setting arrives from /api/settings/download-transport, which
  // seeds HTTPS for a new install and Auto for one that predates that default. This constant is
  // what downloads use before that answer lands, so it has to be the safe transport, not Xet.
