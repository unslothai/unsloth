...
// Asserted against the source like the other shaper tests: importing the adapter drags in the whole
// app. The backend keeps a well-formed `__IMAGES__` line from a tool that does not emit the envelope
// as content the model reads, so the card must not slice it off and fetch a sandbox file that was
// never written. The shaper owns the slice now; the adapter only calls it.
test("the adapter slices the image envelope only for the tools that emit it", () => {
  const source = readSrc("features/chat/utils/tool-result-shape.ts");
  const gate = source.indexOf('IMAGE_SENTINEL_TOOLS.has(toolName)');
  const slice = source.indexOf("rawResult.lastIndexOf(SANDBOX_IMAGES_MARKER)");
  ...
  assert.equal(
    source.indexOf("rawResult.lastIndexOf(SANDBOX_IMAGES_MARKER)", slice + 1),
    -1,
