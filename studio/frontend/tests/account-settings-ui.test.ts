...
const tabUrl = new URL("../src/features/settings/tabs/accounts-tab.tsx", import.meta.url);
...
function content(node: unknown): string {
  ...
  if (node && typeof node === "object" && "props" in node) return content((node as StubElement).props.children);
  return "";
}
function translate(key: string, values: Record<string, string> = {}) {
  const message = key.split(".").reduce<unknown>((value, part) => (value as Record<string, unknown>)[part], en) as string;
  return message.replace(/\{([^}]+)\}/g, (_match, name: string) => values[name] ?? "");
}
function tab(owner = true) {
  ...
  const calls: string[] = [];
  const accounts = [
    { account_id: "owner", username: "unsloth", role: "owner", is_active: true },
    { account_id: "alice-id", username: "alice", role: "user", is_active: true },
  ];
  const setup = { username: "alice", setup_code: "one-time-secret", expires_at: "2026-09-06T13:00:00Z" };
  const api = loadWithStubs<{ AccountsTab: () => StubElement | null }>(tabUrl, {
    ...
    "@/components/ui/alert-dialog": Object.fromEntries(["AlertDialog", "AlertDialogAction", "AlertDialogCancel", "AlertDialogContent", "AlertDialogDescription", "AlertDialogFooter", "AlertDialogHeader", "AlertDialogTitle"].map((name) => [name, name])),
    "@/lib/copy-to-clipboard": { copyToClipboard: async (value: string) => { calls.push(`copy:${value}`); return true; } },
    "@/i18n": { useT: () => translate },
    "../api/accounts": {
      fetchAccounts: async () => { calls.push("list"); return accounts; },
      createAccount: async (username: string) => { calls.push(`create:${username}`); return setup; },
      regenerateSetupCode: async (username: string) => { calls.push(`regenerate:${username}`); return { ...setup, setup_code: "regenerated-secret" }; },
      setAccountActive: async (username: string, active: boolean) => { calls.push(`active:${username}:${active}`); accounts[1].is_active = active; },
      deleteAccount: async (username: string) => { calls.push(`delete:${username}`); accounts.splice(1); },
    },
  });
  const render = () => {
    cursor = 0;
  ...
  return { render, calls, initialize: async () => {
    render();
...
const click = async (tree: unknown, label: string) => {
  const button = nodes(tree).find((node) => node.type === "Button" && content(node) === label);
...
test("owner lists accounts without administrative actions on the owner row", async () => {
  ...
  const ownerRow = nodes(tree).find((node) => node.props["data-testid"] === "account-unsloth");
  assert.ok(ownerRow);
  assert.equal(nodes(ownerRow).filter((node) => node.type === "Button").length, 0);
...
test("create shows a copyable expiring setup code once and regeneration replaces it", async () => {
  ...
  const input = nodes(tree).find((node) => node.props.id === "new-account-username");
  (input?.props.onChange as (event: unknown) => void)({ target: { value: "alice" } });
  tree = ui.render();
...
test("Accounts is registered, searchable, and filtered from managed navigation and deferred panels", () => {
  assert.deepEqual(SETTINGS_SEARCH_INDEX.accounts, ["settings.accounts.title", "settings.accounts.create"]);
  const dialog = readFileSync(new URL("../src/features/settings/settings-dialog.tsx", import.meta.url), "utf8");
  const store = readFileSync(new URL("../src/features/settings/stores/settings-dialog-store.ts", import.meta.url), "utf8");
