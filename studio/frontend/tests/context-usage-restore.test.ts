...
test("refresh restores a hydrated snapshot but honors explicit invalidation", async () => {
  ...
  const state = {
    ...
    loadedContextLength: 32_000,
    modelLoading: false,
    models: [],
    runningByThreadId: {},
    setThreadContextUsage: (_id: string, value: unknown) =>
      restored.push(value),
    setContextUsage: (value: unknown) => restored.push(value),
  };
  const refresh = loadWithStubs<{
    setActiveBranchReader: (reader: () => unknown[]) => void;
    ...
    ),
    {
      ...
      "./message-order": {},
      "./context-usage-restore": { restorableContextUsage },
    },
  );
  refresh.setActiveBranchReader(() => [
    {
      ...
      role: "assistant",
      content: [],
      metadata: { custom: saved },
    },
  ]);
  await refresh.refreshContextUsage();
  assert.deepEqual(restored, [saved.contextUsage, saved.contextUsage]);
  assert.equal(mediaChecks, 0);
  restored.length = 0;
  await refresh.refreshContextUsage({ invalidate: true });
  assert.deepEqual(restored, [null]);
  assert.equal(mediaChecks, 1);
});
