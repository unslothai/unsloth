...
function permissionUi(loginMode: string, permissionMode: string) {
  ...
  const state = { permissionMode, setPermissionMode: (value: string) => { changes.push(value); } };
  const component = loadWithStubs<{
    PermissionModeMenuItems: (props: { onRequestFullAccess: () => void }) => StubElement;
    FullAccessConfirmDialog: (props: { open: boolean; onOpenChange: () => void }) => StubElement | null;
  }>(new URL("../src/features/chat/permission-mode-select.tsx", import.meta.url), {
    "react/jsx-runtime": stubJsxRuntime(),
    "react": { useEffect: (effect: () => void) => effect(), useState: () => [false, () => {}] },
    "lucide-react": { ChevronDown: "ChevronDown", CircleAlert: "CircleAlert", Hand: "Hand", ShieldCheck: "ShieldCheck" },
    ...
    "@/lib/utils": { cn: () => "" },
    "@hugeicons/react": {},
    "./stores/chat-runtime-store": { useChatRuntimeStore: (selector: (state: unknown) => unknown) => selector(state) },
  });
...
for (const mode of ["single", "multi"]) {
  test(`${mode} permission menu enforces installation full-access policy`, () => {
    ...
    const menu = ui.component.PermissionModeMenuItems({ onRequestFullAccess() {} });
    ...
    const dialog = ui.component.FullAccessConfirmDialog({ open: true, onOpenChange() {} });
...
test("multi-user policy leaves non-full preferences untouched", () => {
  for (const mode of ["ask", "auto", "off"]) {
    ...
    ui.component.PermissionModeMenuItems({ onRequestFullAccess() { assert.fail("Full access must be unavailable"); } });
