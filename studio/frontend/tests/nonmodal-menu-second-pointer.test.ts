...
const { installDismissingClickGuard } = await import("../src/lib/menu-dismiss.ts");
...
const down = (pointerId: number, pointerType: string, target: unknown): void => {
  fakeDocument.dispatchEvent({ type: "pointerdown", pointerId, pointerType, button: 0, target });
};
...
test("a second pointer's cancel does not retire it either", () => {
  withOpenMenu(() => {
    down(11, "touch", OUTSIDE);
    down(22, "mouse", INSIDE_MENU);
    fakeDocument.dispatchEvent({ type: "pointercancel", pointerId: 22, pointerType: "mouse" });
    fakeWindow.advance(900);
    assert.equal(
      clickReachedTheControl(OUTSIDE),
      false,
...
test("the armed pointer's own cancel still disarms", () => {
  withOpenMenu(() => {
    down(11, "touch", OUTSIDE);
    fakeDocument.dispatchEvent({ type: "pointercancel", pointerId: 11, pointerType: "touch" });
    assert.equal(
      clickReachedTheControl(OUTSIDE),
      true,
