...
test("the near-limit help keeps the context-length guidance", () => {
  const component = readSrc("features/chat/components/context-usage-bar.tsx");
  assert.match(component, /Generation will stop at 100%/);
  assert.match(
    component,
    /Context Length<\/span> in\s+the chat Settings panel to keep going/,
  );
});
...
test("the limit advice follows the backend and the unclamped ratio", () => {
  ...
  assert.equal(
    deriveContextUsageBar({ ...at, isMlx: true, contextEnforced: true })
      ?.advice,
    "mlx-past-limit",
  );
  assert.equal(
  ...
  assert.equal(
    deriveContextUsageBar({ used: 4096, total: 32768 })?.advice,
    "none",
  );
  // no window and no count: nothing to advise against
  assert.equal(
    deriveContextUsageBar({ used: 40000, total: null })?.advice,
    "none",
  );
});
