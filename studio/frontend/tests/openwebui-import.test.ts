...
function recordOf(
  ...
  return chatRecord({ ...chat, history: historyOf(messages, currentId) });
}
...
test("an empty chat imports as nothing rather than as a blank thread", () => {
  assert.equal(openWebUIRecordToConversation(recordOf([], null), "x"), null);
