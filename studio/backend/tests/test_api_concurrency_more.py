...
"""Additional coverage for studio/backend/utils/api_concurrency.py.

The base file ``test_api_concurrency.py`` covers parsers, the basic reject
path, and the streaming wait path. This file adds the gaps that surfaced
during review of PR #5482:

  - the /v1 mirrors of /api/inference generation routes (the inference
    router is mounted at both prefixes, so both must be gated)
  - app-exception, http.disconnect, more_body-omitted, and lifespan
    behavior of InferenceConcurrencyMiddleware
  - reject-burst and multiple-waiter invariants of the underlying limiter
  - middleware-ordering interactions with MaxBodyMiddleware and
    SecurityHeadersMiddleware as registered in studio/backend/main.py
"""
...
@pytest.mark.parametrize(
    ...
    [
        "/v1/generate/stream",
        "/v1/audio/generate",
        # Existing entries — keep a few as regression anchors.
        "/v1/chat/completions",
        "/v1/completions",
        "/v1/messages",
        "/v1/responses",
...
@pytest.mark.parametrize(
    "path",
    [
        # Management / status — must not be gated.
        "/api/inference/load",
        "/api/inference/unload",
        "/api/inference/validate",
        "/api/inference/status",
        "/api/inference/cancel",
        # Embeddings are intentionally excluded — they are typically much
...
async def test_429_carries_security_headers(main_module):
    """Regression: 429s must be wrapped by SecurityHeadersMiddleware so
    Content-Security-Policy and X-Content-Type-Options are present.

    add_middleware inserts at user_middleware[0]; the LATER-added middleware
    wraps OUTER. To make Security OUTER of the gate (so 429 passes through
    Security on the way out), add the gate FIRST and Security AFTER."""
...
async def test_413_does_not_hold_concurrency_slot(main_module):
    """Regression: an oversized POST must be 413'd by MaxBodyMiddleware
    BEFORE InferenceConcurrencyMiddleware acquires a slot. A slow uploader
    must not block other inference traffic while their body is rejected.

    We drive raw ASGI with a chunked slow receive() and observe the
    underlying limiter._active counter via a watcher task.
    """
    inner_calls = {"count": 0}
...
async def test_excluded_load_path_not_gated_by_full_stack(main_module):
    """With the gate held by a slow chat request, /api/inference/load must
    still respond — it is intentionally excluded from the gate."""
    holding = asyncio.Event()
