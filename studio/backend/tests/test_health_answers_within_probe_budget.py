...
def test_the_budget_stays_under_the_desktop_probe_timeout():
    ...
    # Connect, routing and JSON all come out of the same 2s. A budget that only
    # just fits leaves the launch one slow scheduling hiccup from the dead end.
    assert probe_timeout - budget >= 0.4, (
...
def test_health_still_waits_when_detection_finishes_inside_the_budget():
    ...
    assert result["chat_only"] is False, (
        "health published chat_only=True on a host whose detection completed "
