...
"""The account contract every isolation change builds on: one account resolves EXACTLY as it did before accounts existed, and a second account can never reach the first one's storage or generations."""
...
def test_reusing_a_resident_model_leaves_the_loader_in_control(monkeypatch):
    """An already-loaded fast path re-asserts CHAT as a claim on the GPU, not a load, so it must not rewrite the account that loaded what is resident: doing so hid the loader from its own model."""
    from auth import policy
    from core.inference import gpu_arbiter as arb
