...
def _write_marker(install_dir: Path, **overrides) -> Path:
    ...
    # The installer always writes `tag` and `release_tag` from the same release (a normalized base
    # vs the full release tag), so keep the pair consistent when a test overrides only `tag`.
...
def test_read_install_marker_finds_windows_cmake_layout(tmp_path):
    # Windows cmake puts the .exe under build/bin/Release/, so the marker is four levels above the
    # binary.
    install_dir = tmp_path / "llama.cpp"
...
def test_read_install_marker_carries_published_repo_dynamically(tmp_path, repo):
    # The freshness check queries whichever release repo the marker records: new installs record the
    # fork, legacy CPU/macOS markers still say ggml-org, and both must get the right "latest" tag.
    install_dir = tmp_path / "llama.cpp"
...
def test_check_prebuilt_freshness_not_behind_on_mix_latest(monkeypatch, tmp_path):
    # Installed the mix latest: marker base tag b9596, full release_tag with sha, GitHub latest is
    # that same full tag.
    install_dir = tmp_path / "llama.cpp"
...
def test_fetch_latest_release_tag_uses_publish_time(monkeypatch):
    # Resolves newest by published_at (like the installer), skips drafts/prereleases, and does NOT
    # just take GitHub's first / `/releases/latest` item.
...
def test_reset_caches_default_keeps_disk_cache(tmp_path):
    # The no-arg form is in-memory only (its existing test-only contract); it must not delete the on-disk cache.
    cache_file = _seed_disk_cache(tmp_path, "b9596-mix-aaa")
    fr.reset_caches()
    assert cache_file.exists()
...
def test_drop_disk_lets_banner_fail_open_after_same_base_mix_swap(monkeypatch, tmp_path):
    # P2 #2: the disk cache holds a still-fresh same-base mix (b9596-mix-aaa) from before an update
    # to a different same-base mix (b9596-mix-bbb).
    ...
    bin_path = _fake_binary(install_dir, layout = "root")
    # GitHub unreachable for the rest of the test (the offline post-install refresh, and the later
    # status check).
    monkeypatch.setattr(fr, "_fetch_latest_release_tag", lambda repo, timeout = 5.0: None)
...
def test_in_memory_only_reset_replays_stale_same_base_mix(monkeypatch, tmp_path):
    # Contrast/guard for the case above: an in-memory-only reset leaves the stale same-base mix on
    # disk, so an offline check replays it and falsely reads behind/stale.
    _seed_disk_cache(tmp_path, "b9596-mix-aaa")
    install_dir = tmp_path / "llama.cpp"
    _write_marker(
        install_dir,
...
def test_update_size_unsloth_prebuilt_exact_match(monkeypatch):
    # The unsloth fork's own bundle (app-<tag>-<platform>): the want= exact match on
    # app-<latest>-<suffix> wins.
    marker = {
        "asset": "app-b9190-linux-x64-cuda13-newer.tar.gz",
        "published_repo": "unslothai/llama.cpp",
    }
...
def test_update_size_macos_fork_asset_suffix_fallback(monkeypatch):
    # macOS bundles use the upstream-style llama-<tag>-bin-macos-*, matched via the endswith
    # fallback in the publish repo.
    marker = {
...
def test_update_size_upstream_ubuntu_uses_binary_repo(monkeypatch):
    # #6338 P2: the ggml-org ubuntu-* prebuilt lives in binary_repo, not the fork publish repo.
    marker = {
...
def test_update_size_no_matching_asset_fails_open(monkeypatch):
    # A ROCm version drift (installed 6.4 vs latest 7.2) leaves no suffix match; the helper fails
    # open to None rather than guessing a wrong artifact.
    marker = {
