...
def test_the_cached_model_reveal_still_goes_through_the_moved_helper(tmp_path, monkeypatch):
    ...
    opened = []
    monkeypatch.setattr(
        models, "_resolve_cached_model_path", lambda repo_id, variant, cache_path: link
    )
    monkeypatch.setattr(
        path_utils,
        "reveal_in_file_manager",
    ...
    result = asyncio.new_event_loop().run_until_complete(
        models.reveal_cached_model(
            repo_id = "unsloth/Llama-3.2-1B", variant = None, cache_path = None, current_subject = "unsloth"
        )
    )
