...
def _keyframe_png(width = 64, height = 32) -> str:
    """A base64 PNG, the transport the keyframe fields use (same as the image routes)."""
    import base64
    import io
    ...
    buffer = io.BytesIO()
    Image.new("RGB", (width, height), (32, 96, 160)).save(buffer, format = "PNG")
    return base64.b64encode(buffer.getvalue()).decode()


def test_generate_forwards_keyframes_to_the_backend(client, monkeypatch):
    # The route's whole job for keyframes is to hand the base64 payloads through under the names
    # the backend decodes; the fake family accepts them so only the wiring is under test.
    backend = video_module.get_video_backend()
    backend.loaded = True
    seen: dict = {}

    def _capture(family, image, last_image):
        seen.update(image = image, last_image = last_image)
        return None, None

    monkeypatch.setattr(backend, "_decode_keyframes", _capture)
    first, last = _keyframe_png(), _keyframe_png(96, 48)
    _generate_and_wait(client, {"prompt": "p", "image": first, "last_image": last})
    assert seen == {"image": first, "last_image": last}


def test_generate_without_keyframes_passes_none(client, monkeypatch):
    backend = video_module.get_video_backend()
    backend.loaded = True
    seen: dict = {}
    ...
    monkeypatch.setattr(backend, "_decode_keyframes", _capture)
    _generate_and_wait(client, {"prompt": "p"})
    assert seen == {"image": None, "last_image": None}


def test_generate_rejects_a_keyframe_the_family_cannot_use(client):
    # The fake backend's state is a bare object with no family, so the keyframe guard refuses:
    # a synchronous 400 with the reason, not an accepted job that fails minutes later.
    backend = video_module.get_video_backend()
    backend.loaded = True
    resp = client.post(
        "/api/inference/video/generate", json = {"prompt": "p", "image": _keyframe_png()}
    )
    assert resp.status_code == 400
    assert "reference frame" in resp.json()["detail"]
    # And nothing was started.
    assert client.get("/api/inference/video/generate-progress").json()["active"] is False


def test_generate_rejects_an_undecodable_keyframe(client, monkeypatch):
    backend = video_module.get_video_backend()
    backend.loaded = True
    # Swap the bare sentinel state for one that reports a keyframe-capable family, so the request
    # gets past the capability guard and fails on the payload itself.
    import types

    backend._state = types.SimpleNamespace(family = _h3_family_stub())
    resp = client.post(
        "/api/inference/video/generate", json = {"prompt": "p", "image": "@@ not base64 @@"}
    )
    assert resp.status_code == 400
    assert "first reference frame" in resp.json()["detail"]


def _h3_family_stub():
    """The real MiniMax-H3 family entry, so the route sees a keyframe-capable model."""
    from core.inference.video_families import detect_video_family

    return detect_video_family("MiniMaxAI/MiniMax-H3")
