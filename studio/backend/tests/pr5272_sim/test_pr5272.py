...
def test_migration_legacy_subject_invisible_to_real_user(env):
    ...
    assert (
        db.get_chat_thread("migrated", subject = db._LEGACY_UNSCOPED_SUBJECT) is not None
    )
...
def test_chunking_boundary_999_threads(env):
    ...
    for i in range(n):
        db.upsert_chat_message(
            _msg(f"m{i}", thread_id = f"t{i}", created_at = i), subject = SUB
        )
...
def test_chunking_boundary_at_exactly_900_and_901(env):
    ...
    for n in (900, 901):
        for i in range(n):
            ...
            db.upsert_chat_message(
                _msg(f"xm{n}-{i}", thread_id = f"x{n}-{i}", created_at = i), subject = SUB
            )
...
def test_cross_thread_message_hijack_via_upsert_rejected(env):
    ...
    db.upsert_chat_message(
        _msg("shared", thread_id = "t_owner", content = "legit"), subject = SUB
    )
    with pytest.raises(db.ChatMessageThreadMismatch):
        db.upsert_chat_message(
            _msg("shared", thread_id = "t_attacker", content = "hijack"), subject = SUB
        )
...
def test_concurrent_settings_writers_no_lost_update(env):
    ...
    _, db, _ = env
    db.upsert_chat_settings({"autoTitle": False, "reasoningEffort": "low"}, subject = SUB)
...
def test_two_subjects_isolated_via_http(http_env):
    ...
    assert all(
        t["id"] != "alice-private" for t in threads
...
def test_http_upsert_message_rejects_cross_thread_reparent(http_env):
    ...
    client.post("/api/chat/threads", json = _thread("b"))
    # Create m1 under a
    r = client.put("/api/chat/threads/a/messages/m1", json = _msg("m1", thread_id = "a"))
    assert r.status_code == 200
    # Attempt to PUT m1 under b — should 409 because b's body says threadId=b
    # but m1 already exists under a
    r = client.put("/api/chat/threads/b/messages/m1", json = _msg("m1", thread_id = "b"))
