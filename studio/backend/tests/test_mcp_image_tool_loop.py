...
def image_request(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        mcp_client, "mcp_image_recipient_location", lambda identity: "https://example.test/mcp"
    )
...
def test_configured_tools_share_only_after_exact_explicit_approval(image_request, tool_index):
    ...
    assert tools[tool_index]["function"]["parameters"]["properties"]["threshold"] == {
        "type": "number"
    }
    ...
    assert not tool_approvals.resolve_mcp_image_disclosure(
        approval.approval_id, "allow", current_subject = "other", session_id = "session"
    )
    assert tool_approvals.resolve_mcp_image_disclosure(
        approval.approval_id, "allow", current_subject = "subject", session_id = "session"
    )
    ...
    assert (
        image_loop.wait_call_decision(approval, approval.slot, approval.approval_id, f.cancel)
        == "allow"
    )
...
@pytest.mark.parametrize(
    "change", ["conversation", "current_message", "private_payload", "feature", "no_channel"]
)
def test_request_validation_rejects_invalid_private_selection(image_request, change):
    ...
    if change == "conversation":
        ...
        studio_db.upsert_chat_message(
            {"id": "new", "threadId": "thread", "role": "user", "createdAt": 3, "content": []}
        )
...
def test_unrelated_vision_image_is_preserved(image_request):
    f = image_request
    f.payload.messages[0]["content"] = [
        {"type": "image_url", "image_url": {"url": "https://example.test/ordinary.png"}}
    ]
    run, _ = prepare(f)
...
def test_provider_loop_cannot_bypass_image_consent(image_request, monkeypatch, mode):
    ...
    async def collect():
        ...
        async for line in studio_tool_loop.stream_with_studio_tools(
            transport,
            run = studio_tool_loop.ToolLoopRun(
                messages = f.payload.messages, session_id = "session", thread_id = "thread"
            ),
            policy = studio_tool_loop.ToolLoopPolicy(
                tools = tools,
                max_calls = 2,
                timeout = 30,
                rag_scope = None,
                permission_mode = "auto" if mode == "bypass" else mode,
                bypass_permissions = mode == "bypass",
                confirm_calls = False,
            ),
            cancel_event = f.cancel,
            mcp_image_run = run,
        ):
            events.append(line)
            if line.startswith("data: "):
                ...
                if event.get("image_disclosure"):
                    ...
                    assert not tool_approvals.resolve_tool_decision(
                        event["approval_id"], "allow", "session"
                    )
                    assert tool_approvals.resolve_mcp_image_disclosure(
                        event["approval_id"],
                        "allow",
...
def test_safetensors_close_at_image_card_revokes_without_execution(image_request):
    ...
    call = (
        "<tool_call>"
        + json.dumps({"name": "mcp__inspect__inspect", "arguments": arguments})
        + "</tool_call>"
    )
    executed = []
    generator = run_safetensors_tool_loop(
        single_turn = lambda messages: iter([call]),
        messages = f.payload.messages,
        tools = tools,
        execute_tool = lambda *a, **k: executed.append(a),
        cancel_event = f.cancel,
        session_id = "session",
        thread_id = "thread",
    ...
    assert run.closed
    assert not tool_approvals.resolve_mcp_image_disclosure(
        approval_id, "allow", current_subject = "subject", session_id = "session"
    )
...
def test_failed_recipient_metadata_leaves_no_approval(image_request, monkeypatch):
    ...
    assert not any(
        slot["binding"].generation_id == "generation"
        for slot in tool_approvals._mcp_image_pending.values()
    )
    run.close()
...
def test_enabled_feature_without_selection_still_hides_payload_field(image_request):
    ...
    assert run is None
    assert (
        tools[0]["function"]["parameters"]["properties"]["picture_blob"]["title"]
