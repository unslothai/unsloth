...
def test_legacy_download_progress_metadata_is_explicitly_anonymous(monkeypatch):
    dataset_tokens = []
    model_tokens = []
    ...
    monkeypatch.setitem(
        ...
        "huggingface_hub",
        SimpleNamespace(
            dataset_info = lambda *_args, **kwargs: dataset_tokens.append(kwargs.get("token"))
            or SimpleNamespace(siblings = []),
            model_info = lambda *_args, **kwargs: model_tokens.append(kwargs.get("token"))
            or SimpleNamespace(siblings = []),
        ),
    )
    datasets_route._dataset_size_cache.clear()
    models_route._repo_size_cache.clear()

    assert datasets_route._get_dataset_size_cached("Org/Data") == 0
    assert models_route._get_repo_size_cached("Org/Model") == 0
    assert dataset_tokens == [False]
    assert model_tokens == [False]
