...
def test_diffusion_cache_root_follows_a_live_switch(settings_store, tmp_path):
    ...
    # set_hf_cache_home does not update, so the download wrote to the new root while progress
    # counted the old one and a load could split across both.
...
def test_diffusion_loader_calls_pin_the_cache_dir():
    # Every from_pretrained / from_single_file must carry cache_dir, else diffusers resolves it
    # through the stale constant.
