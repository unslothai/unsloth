...
class TestMaxBodyMiddleware:
    ...
    def test_stt_routes_use_audio_specific_caps(self, main_module):
        ...
        # The OpenAI transcriptions route is multipart, so it gets headroom over the raw cap.
    ...
    def test_chunked_upload_over_cap_rejected(self, main_module):
        # Regression: a declared-Content-Length-only check could be bypassed by chunked encoding.
        app = _make_protected_app(1024, main_module)
        c = TestClient(app)
    ...
    def test_diffusion_dataset_upload_in_body_passthrough(self, main_module):
        # The diffusion dataset upload route sits under /api/train, so it must be in the REAL passthrough
        # allowlist or MaxBodyMiddleware 413s near-limit batches. EXACT path, so JSON sub-routes stay capped.
        from utils.upload_limits import (
    ...
    def test_diffusion_dataset_json_subroutes_keep_default_cap(self, main_module):
        # A prefix match would let a large caption/import body bypass the default JSON cap.
    ...
    def test_diffusion_dataset_trailing_slash_gets_upload_cap(self, main_module):
        # The trailing-slash variant reaches the middleware BEFORE the router's 307, so it must resolve
        # to the same passthrough + upload cap.
        from utils.upload_limits import (
            default_request_body_limit_bytes,
            upload_request_limit_bytes,
        )
    ...
    def test_v1_surface_is_body_protected(self, main_module):
        # /images/generations is mounted at both /api/inference and /v1, and every /v1 POST must be
        # body-capped or an unbounded prompt buffers outside the Unsloth request limit.
        for path in (
...
class TestSecurityHeadersMiddleware:
    ...
    def test_docs_csp_never_widens_script_src(self, main_module):
        # The docs branch may relax style/font/worker only: a third party in script-src here would reach
        # the tokens localStorage holds for the whole origin.
    ...
    def test_docs_pages_load_no_third_party_script(self, main_module):
        # FastAPI's built-in docs point at cdn.jsdelivr.net; they are re-registered on the same paths
        # against assets/docs_ui, and the built-ins must stay off or they would win the path.
    ...
    def test_docs_inline_script_runs_off_the_response_nonce(self, main_module):
        # Swagger's init is inline, so the nonce must be spliced into the header to match the tag; a
        # mismatch renders blank, which is what CDN-era /docs did.
        c = TestClient(main_module.app)
    ...
    def test_docs_urls_follow_the_root_path(self, main_module):
        # Behind a path-stripping proxy the browser sees a prefix the server never does, so every URL
        # the pages emit has to carry it.
    ...
    def test_swagger_nonce_survives_a_reflowed_upstream_template(self, main_module):
        # fastapi is unpinned, so the tag is matched by what follows it: a version that reflows the page
        # must still get the nonce, not a 500.
        reflowed = (
    ...
    def test_img_and_media_allow_https_sources(self, main_module):
        # Model-card READMEs and citation favicons pull images/media from many https origins, so
        # img-src/media-src allow any https source; this mirrors the desktop CSP in tauri.conf.json.
        ...
        for name in ("img-src", "media-src"):
            ...
            # Tokenise and compare with `==` so CodeQL's URL-substring rule does not read directive-string
            # `in` membership as URL sanitisation.
    ...
    def test_response_start_with_tuple_headers_is_hardened(self, main_module):
        # An ASGI server may emit tuple-valued raw headers; the middleware must coerce to a list.
        import asyncio
    ...
    def test_is_pure_asgi_not_basehttp_middleware(self, main_module):
        # Regression: as a BaseHTTPMiddleware this wrapped the SSE stream in its own anyio task group,
        # breaking disconnect detection (GPU stuck at 100%). Must stay pure ASGI.
    ...
    def test_forwards_receive_channel_unchanged(self, main_module):
        # Must forward the ASGI receive channel untouched so client disconnects reach the handler.
        seen = {}
    ...
    def test_streaming_response_survives_client_disconnect(self, main_module):
        # A StreamingResponse that polls is_disconnected() must unwind cleanly: no cancel scope error,
        # the generator's finally runs, and security headers are still applied.
...
def health_app(tmp_path, monkeypatch):
    ...
    # (chat_only, chat_only_reason, chat_only_detail): health_check reads all three, so a two-tuple
    # here raised IndexError once main added the detail field.
...
class TestHealthAuthGate:
    # Launcher / frontend bootstrap fields are unauth so the Tauri watchdog can re-adopt a sibling
    # backend and the SPA can detect chat-only mode before any token exists.
    LAUNCHER_BITS = (
        "service",
        "studio_root_id",
        "chat_only",
