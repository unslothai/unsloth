...
def _patch(
    ...
    *,
    ...
    # Pinned rather than ambient: a real broken-torch host would leak in, and the Apple branch reads it to tell apart.
    ...
    # Pin the OS too: video_capability() asks platform.system(), so these would flip on a macOS runner.
...
@pytest.mark.parametrize(
    ...
    [
        # Both states detect_hardware() can publish for a broken torch on Apple Silicon: MLX needs no
        # torch, so a healthy stack reports MLX with no chat-only reason while a broken one falls back to
        # CPU. Pairing MLX with mlx_unavailable would let the check be narrowed to either.
...
def test_apple_silicon_with_broken_torch_is_not_told_to_install_it(
    ...
    # A wheel with unresolved native libs raises from torch's own __init__, so _has_torch() reads False
    # exactly as for an absent one, and without the explicit check the host is told to install the
...
def test_apple_silicon_without_a_metal_device_is_not_supported(monkeypatch):
    # Apple Silicon alone does not imply MPS: a torch built without it would fail at load instead of at the gate.
...
def test_mlx_device_is_apple_even_without_the_apple_probe(monkeypatch):
    # MLX only exists on Apple, so an is_apple_silicon() that cannot answer must not reclassify the host.
...
# The matrix above stubs _torch_mps_available, so these drive the real helper against a fake torch.
...
def test_mps_probe_reads_availability_not_whether_torch_was_built_with_it(monkeypatch):
    ...
    # is_built() is true on any Metal-capable build, including one that reaches no device: it would over-promise.
...
def test_mps_probe_reports_no_metal_when_torch_is_absent(monkeypatch):
    # Only the answer is asserted, not that the import was skipped: an import of the None raises into the same except.
...
def test_video_capability_separates_apple_silicon_from_intel_macs():
    ...
    # Strip comments before asserting: this named is_apple_silicon() and kept passing on the word left in a comment.
    code = "\n".join(
...
def test_intel_mac_is_macos_unsupported_not_a_missing_gpu(monkeypatch):
    # An Intel Mac detects as plain CPU with is_apple_silicon() False, so the reason came out as
    # pytorch_not_installed or no_accelerator, neither of which enables video with no Metal device.
...
def test_a_broken_probe_still_beats_the_macos_branch(monkeypatch):
    # detection_failed is checked first: a Mac whose probe fell over is told that, not that video is coming soon.
    _patch(
        monkeypatch,
        torch = True,
        device = hw.DeviceType.CPU,
