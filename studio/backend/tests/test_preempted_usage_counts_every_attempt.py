...
class TestOnePause:
    ...
    def test_without_timings_the_chunks_decoded_are_the_estimate(self, monkeypatch):
        """A build that does not send per-chunk timings still must not report zero for the aborted
        attempt: one chunk is about one token, which is what the rest of the preemption path
        already assumes.
        """
        signal = preemption.PreemptSignal()
        upstream = _Upstream(
