...
def test_a_graphed_dit_captures_one_tuned_graph_per_branch_and_replays_both(monkeypatch):
    """Two captures, the FP4 tactic tuned before either, every replay bit-identical to the un-graphed forward of its own branch, and the second graph costing about what the first did."""
    torch = _cuda_or_skip()
    import torch.nn as nn
