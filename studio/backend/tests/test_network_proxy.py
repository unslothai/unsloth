...
def test_the_connect_budget_bounds_the_whole_answer_list(monkeypatch, upstream):
    ...
    monkeypatch.setattr(AllowlistProxy, "_connect_address", slow_connect)
    instance = AllowlistProxy(
        NetworkAllowlist.from_entries(["upstream.test"]),
...
def test_close_during_dns_prevents_late_outbound_connect(monkeypatch):
    ...
    try:
        ...
        workers = tuple(instance._workers)
        release.set()
        for thread in workers:
            thread.join(1)
    assert errors == ["proxy closed during DNS resolution"]
    assert attempted == []
...
def test_close_interrupts_registered_connecting_socket(monkeypatch):
    ...
    assert entered.wait(1)
    instance.close()
    assert interrupted.is_set() and connecting.closed
    assert not instance._workers
    assert not instance._tunnels
    assert errors
...
def test_close_is_bounded_when_a_worker_will_not_stop(monkeypatch, upstream):
    ...
    try:
        ...
        release.set()
        for worker in tuple(instance._workers):
            worker.join(1)
        with pytest.raises(RuntimeError, match = "cleanup left"):
            instance.close()
