...
def test_list_images_caption_precedence(client, ds_root):
    ...
    # a.png has a sidecar (an explicit edit beats the metadata row), b.png is metadata-only, c.png
    # has none.
...
def test_list_images_tolerates_invalid_utf8_sidecar(client, ds_root):
    # Sidecars are stored as raw bytes, so one can hold non-UTF-8 text and read_text raises
...
def test_import_example_streams_instead_of_preparing_the_whole_split(client, ds_root, monkeypatch):
    # The cap keeps 10-100 rows while the curated repos run to tens of thousands, all of which a
    # prepared load downloads first.
    calls = _install_fake_load_dataset(monkeypatch, n_rows = 3)
    r = client.post("/api/train/diffusion/dataset/import-example", json = {"id": "tuxemon"})
    assert r.status_code == 200, r.text
...
def test_import_example_keeps_the_dataset_when_rmdir_fails(client, ds_root, monkeypatch):
    # Promotion folds the folder's existing entries into the staging dir.
    import os
...
def test_import_example_keeps_the_dataset_when_the_rename_fails(client, ds_root, monkeypatch):
    # Same guarantee one step later: rmdir succeeded, so the folder's entries live only in the
    # staging dir when os.replace raises.
    import os
...
def test_upload_rejects_same_stem_different_extension(client, ds_root):
    # sample.png and sample.jpg share the stem "sample", so both map to one sidecar and the second
    # must 400.
...
def test_upload_rejects_exact_duplicate_name_within_one_batch(client, ds_root):
    # Two parts with the SAME name in ONE batch are distinct files the staged commit would silently
    # collapse, so the batch is rejected whole.
    r = _upload(
        client,
        "styleset",
    ...
    assert r.status_code == 400
    assert "more than once" in r.json()["detail"]
    # A STEM case variant pair stays exempt: one file / an overwrite on case-insensitive
    # filesystems, separate sidecars on Linux.
...
def test_upload_rejects_case_only_duplicate_on_a_case_insensitive_filesystem(
    ...
    # 'Cat.png' and 'cat.png' are ONE destination on NTFS / default APFS, so the staged commit
    # replaced the first with the second while reporting both uploaded: a training image silently
    # dropped.
...
def test_import_promotion_leaves_no_partial_dataset_on_failure(ds_root, monkeypatch):
    # The staging dir is promoted in one atomic rename.
    import os
...
def test_upload_rechecks_training_state_before_commit(ds_root, monkeypatch):
    # A start reserving AFTER the upload's entry guard but BEFORE the commit must not mutate the
    # dataset: the pre-commit recheck 409s.
    ...
    def fake_active():
        # Inactive at the entry guard, active by the pre-commit recheck: the run started while the
        # upload was streaming.
        calls["n"] += 1
        return calls["n"] >= 2
...
def test_resolve_dataset_folder_rejects_symlink(ds_root, tmp_path):
    ...
    # caption / read reach external files.
...
def test_delete_image_with_glob_chars_only_removes_own_thumbs(client, ds_root):
    # Deleting a filename with glob metacharacters must remove only its own thumbnails, not a
    # spuriously matched sibling's.
...
def test_import_preserves_unrelated_files_when_folder_not_empty(client, ds_root, monkeypatch):
    # A folder holding unrelated NON-image files still has image_count 0, so the import runs: those
    # files fold into the staging dir and are promoted with it, keeping one atomic rename.
...
def test_import_replaces_a_pre_existing_file_the_import_also_writes(client, ds_root, monkeypatch):
    # Same name on both sides (a stray caption sidecar, so image_count is still 0): the imported
    # file wins, as the old per-file move did.
    _install_fake_load_dataset(monkeypatch, n_rows = 2)
    folder = ds_root / "my-tux"
    folder.mkdir(parents = True)
...
def test_upload_rejects_windows_reserved_dataset_names(client, ds_root, bad):
    # Reserved in every directory on Windows (NUL.txt is NUL), so mkdir dies there.
...
def test_upload_rejects_a_trailing_period_dataset_name(client, ds_root):
    # Win32 strips a trailing period, so 'photos.' opens the existing 'photos' dataset and an upload
    # would modify the wrong one.
...
def test_upload_refuses_while_an_import_holds_the_same_folder(client, ds_root):
    # The training interlock counts mutations and only imports took the per-folder lock, so an
    # upload could merge into a materializing import.
