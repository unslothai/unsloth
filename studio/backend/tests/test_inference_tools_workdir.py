...
def test_tool_workdir_exposes_wiki_shortcuts(tmp_path: Path, monkeypatch):
    ...
    assert (
        workdir.joinpath("sources").resolve()
        == (vault_root / "wiki" / "sources").resolve()
    )
    assert (
        workdir.joinpath("entities").resolve()
        == (vault_root / "wiki" / "entities").resolve()
    )
    assert (
        workdir.joinpath("concepts").resolve()
        == (vault_root / "wiki" / "concepts").resolve()
    )
    assert (
        workdir.joinpath("analysis").resolve()
        == (vault_root / "wiki" / "analysis").resolve()
    )
