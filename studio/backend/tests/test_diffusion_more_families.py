...
def test_flux_dev_and_krea_do_not_inherit_the_schnell_nvfp4_checkpoint():
    # The NVFP4 artifact is baked from schnell weights and gated on schnell only. As a family-wide row dev and Krea-dev inherited it: planning budgeted a prequant-only load, the checkpoint downloaded, _validate_checkpoint refused it on base_model_id, and the dense fallback was already disabled.
    # The fp8 / int8 variant rows for those two bases must be untouched.
