...
# Runs in a child: memory_reserved(i) reads allocator stats, empty before a context exists, so it is safe.
...
# Per-PID nvidia-smi is ground truth, since a primary context shows as this process holding memory.
...
def test_the_probe_would_catch_a_regression():
    # Guards the two tests above: prove mem_get_info does pin a context here, so green means the poll avoided it.
...
def test_gpu_summary_apu_prefers_wddm_counters_over_process_local_hip(monkeypatch):
    # hipMemGetInfo is process-local on Windows WDDM, so an APU takes its total from HIP and used from counters.
    gib = 1 << 30
    torch_stub, _props = _summary_torch(
        properties_total = 8 * gib,
        driver_free = 98 * gib,
...
def test_context_free_rocm_smi_translates_hip_ordinals(monkeypatch):
    ...
    gib = 1 << 30
    monkeypatch.setattr(hw, "IS_ROCM", True)
    # The visibility vars are read from the real environment, so an exporting runner changes the outcome.
    for name in (
        "GPU_DEVICE_ORDINAL",
    ...
    # amd-smi GPU 1 is HIP 0 and owns the 96 GiB pool, so passing ids through publishes GPU 0's 1 GiB free.
...
def test_nvidia_uuid_mask_keeps_public_indices_relative(monkeypatch):
    ...
    # The mask asks for the b-card first, so ordinal 0 carries its 2048 MiB whatever order nvidia-smi emitted.
...
def test_context_free_free_excludes_the_reserved_framebuffer(monkeypatch):
    # nvidia-smi reports reserved apart from used and torch's total excludes it, so driver-total math frees it twice.
    gib = 1 << 30
...
def test_context_free_free_is_zero_on_a_full_card(monkeypatch):
    # The reservation is the whole margin: a fully allocated card must not advertise the reserved 0.5 GiB.
    gib = 1 << 30
...
def test_context_free_declines_when_cuda_order_is_not_pci_bus_id(monkeypatch):
    # CUDA enumerates FASTEST_FIRST while nvidia-smi reports PCI order, and equal-sized cards pass the total check.
...
def test_context_free_keeps_smi_on_a_single_gpu_under_fastest_first(monkeypatch):
    # One card makes every ordering the identity, so the gate must not cost this host a permanent primary context.
    gib = 1 << 30
    monkeypatch.setenv("CUDA_DEVICE_ORDER", "FASTEST_FIRST")
    _nvidia_smi_reading(monkeypatch, used_gb = 2.0, driver_total_gb = 24.0)
...
def test_context_free_declines_when_the_gpu_count_is_not_smi_confirmed(monkeypatch):
    # `nvidia-smi -L` failing falls back to torch, which counts VISIBLE devices, so a one-device mask
    # reads as single-GPU and the exemption would read the mask in nvidia-smi's index space. The
    # count is cached, so one transient timeout would disable the gate for the process.
    gib = 1 << 30
    monkeypatch.setenv("CUDA_DEVICE_ORDER", "FASTEST_FIRST")
    _nvidia_smi_reading(monkeypatch, used_gb = 2.0, driver_total_gb = 24.0)
    monkeypatch.setattr(hw, "get_physical_gpu_count", lambda: 1)
...
def test_physical_gpu_count_records_whether_the_smi_answered(monkeypatch):
    ...
    # The flag is what the order gate reads, so it must track the SOURCE of the count, not merely that one was produced.
...
def test_free_in_torch_scope_never_exceeds_the_total(monkeypatch):
    # Both SMI parsers accept signed values, so a transient negative used counter advertises phantom free.
    gib = 1 << 30
...
def test_context_free_uuid_mask_skips_the_cuda_order_gate(monkeypatch):
    ...
    # A UUID mask names devices absolutely and CUDA keeps the listed order, so declining spends the ~612 MiB context.
    gib = 1 << 30
    monkeypatch.setenv("CUDA_DEVICE_ORDER", "FASTEST_FIRST")
    monkeypatch.setattr(hw, "IS_ROCM", False)
    monkeypatch.setattr(hw, "get_physical_gpu_count", lambda: 2)
...
def test_parent_visible_spec_ignores_a_rocr_mask_on_windows(monkeypatch):
    # Windows HIP has no ROCr layer, so a stray ROCR var masks nothing and is not an ordinal mapping.
    monkeypatch.setattr(hw, "IS_ROCM", True)
    monkeypatch.setattr(hw, "get_physical_gpu_count", lambda: 2)
    monkeypatch.delenv("HIP_VISIBLE_DEVICES", raising = False)
    monkeypatch.delenv("CUDA_VISIBLE_DEVICES", raising = False)
...
def test_context_free_rocm_windows_declines_gpu_device_ordinal(monkeypatch):
    # GPU_DEVICE_ORDINAL is a ROCclr variable _get_parent_visible_gpu_spec never reads, so ordinal 0 publishes GPU 0.
    monkeypatch.setattr(hw, "IS_ROCM", True)
    monkeypatch.setattr(hw.platform, "system", lambda: "Windows")
    monkeypatch.setenv("GPU_DEVICE_ORDINAL", "1")
    monkeypatch.setattr(
...
def test_context_free_nvidia_smi_declines_whole_gpu_metrics_for_mig(monkeypatch):
    ...
    # torch reports the 10 GiB MIG allocation while --query-gpu reports its 80 GiB parent: capping frees the instance.
...
def test_context_free_never_spawns_amd_smi_on_windows_without_a_hip_sdk(monkeypatch):
    ...
    # Windows reads per-adapter counters, so declining amd-smi still answers, one instance per visible device.
    monkeypatch.setattr(
        hw,
...
def test_amd_smi_is_refused_on_a_real_windows_host_without_a_hip_sdk():
    # The test above fakes the platform; this one runs the same predicate against the real OS, PATH and environment.
    from utils.hardware import amd
...
def test_context_free_rocm_smi_declines_whole_gpu_metrics_for_a_partition(monkeypatch):
    ...
    # amd-smi is only the first probe, and leaving the Linux sysfs branch to the real host made this
    # vacuous exactly where it matters: where sysfs is reachable the inventory was built from the
    # host's own card rather than the partition, so the totals matched.
...
def test_inventory_totals_match_the_occupancy_path(monkeypatch, totals):
    # Parity: switching to the inventory helper must move no displayed number; only used_gb differs and is discarded.
...
# props.total_memory and the hipMemGetInfo total agree everywhere except a unified-memory APU,
# where props reports the dedicated carve-out. This module already treats the larger GTT figure
# as authoritative, so the inventory must not budget a 128 GiB Strix Halo as ~8 GiB.
...
def test_rocm_apu_sysfs_overlay_still_declines_against_the_gtt_total(monkeypatch):
    # With the carve-out the two totals agree and the overlay adopts sysfs, shrinking the APU; the GTT total declines.
    mod = _apu_mod()
    monkeypatch.setattr(hw, "IS_ROCM", True)
    monkeypatch.setattr(hw, "_torch_get_device_module", lambda: (mod, "cuda"))
...
# The classifier answers unified or not-unified, never "cannot tell", and knows an APU by the
# driver's integrated flag or a hardcoded gfx1150/1151/1152 set, so every other iGPU reads as
# not-unified and its carve-out would be published as the whole device. clr only fills that flag
# in from ROCm 6.1.2.
...
def test_rocm_unclassified_apu_keeps_the_driver_total_without_the_flag(monkeypatch):
    # A wheel omitting the field reads as 0 through getattr, like a runtime that never set it: arch is all.
...
def test_clear_gpu_cache_skips_synchronize_with_nothing_reserved(monkeypatch):
    ...
    hw.clear_gpu_cache()
    # Only the synchronize is skipped: empty_cache/ipc_collect need no context and are no-ops without an allocator.
...
def test_clear_gpu_cache_still_drains_a_live_allocator(monkeypatch):
    ...
    hw.clear_gpu_cache()
    # Reserved on a NON-current device still counts: summing the visible set keeps a multi-GPU process seen.
...
def test_clear_gpu_cache_still_propagates_a_sticky_cuda_fault(monkeypatch):
    # The diffusion and video unload paths wrap this in try/finally precisely because a sticky fault has to surface.
    calls: list[str] = []
...
def test_rocm_linux_sysfs_first_matches_the_old_torch_then_overlay_result(monkeypatch):
    # Parity: the overlay overwrote torch's used AND total, so every old field is reproduced without the context.
...
def test_rocm_linux_falls_back_to_torch_when_sysfs_covers_only_some(monkeypatch):
    # One uncovered device must send the whole set back to torch rather than ship a device with no number at all.
...
def test_nvidia_torch_fallback_keeps_using_occupancy(monkeypatch):
    # No sysfs shortcut off ROCm: an NVIDIA host without nvidia-smi genuinely needs torch for live usage.
    monkeypatch.setattr(hw, "IS_ROCM", False)
    monkeypatch.setattr(hw, "get_device", lambda: hw.DeviceType.CUDA)
    monkeypatch.setattr(hw, "_smi_query", lambda *a, **k: None)
    monkeypatch.setattr(
...
def test_rocm_relative_index_skips_the_sysfs_shortcut(monkeypatch):
    # A UUID/MIG mask makes index relative, so it is not a host ordinal sysfs can be keyed on.
    _rocm_linux(monkeypatch, {0: (12.0, 64.0), 1: (3.0, 64.0)})
...
def test_context_free_windows_rocm_declines_when_counters_outnumber_devices(monkeypatch):
    # A hidden adapter makes the counter set larger than the visible set, so capacity ranking can hand
    # a card really holding 86 GiB a reading of 10, overstating free by 76 GiB. The System tab
    # tolerates that, but free_gb feeds training-method selection, where overstating free OOMs.
    gib = 1 << 30
    monkeypatch.setattr(hw.platform, "system", lambda: "Windows")
    monkeypatch.setattr(hw, "IS_ROCM", True)
...
def test_gpu_summary_apu_counters_need_positively_identified_uma(monkeypatch):
    # _rocm_props_total_is_carve_out answers True for an UNCERTAIN device too, because a too-small
    # total hides models. That justifies the driver total, not adding Shared Usage: shared bytes are
    # not part of a discrete card's props.total_memory.
    gib = 1 << 30
    torch_stub, _props = _summary_torch(
        properties_total = 8 * gib,
        driver_free = 98 * gib,
