...
"""A managed account's API keys are addressed by its immutable id, so a namesake recreated under a freed username cannot reach them."""
...
def test_a_managed_key_with_no_id_does_not_validate(auth_db):
    """Only the owner's key rows may carry no account id."""
    alice = _alice()
