...
def test_zimage_base_downloads_bf16_while_turbo_downloads_fp32():
    ...
    # undistilled base ships bf16, so charging it the factor doubles the free-disk demand.
...
def test_klein_base_9b_is_sized_like_the_9b_not_the_4b():
    # klein-base-9B is the undistilled 9B (18.2 GB transformer + Qwen3-8B encoder) and is what
    # upstream points fine-tuning at, so without klein-9B's override every size decision under-reserves.
    default = family_bf16_components_gb(_fam("flux.2-klein"))
    nine_b = family_bf16_components_gb(
        _fam("flux.2-klein"), base_repo = "black-forest-labs/FLUX.2-klein-9B"
    )
...
def test_candidate_disk_gate_spares_an_already_cached_prequant(monkeypatch):
    ...
    # Uncached on the same low disk still trips the gate: this excuses a cached artifact, not every
    # prequant.
...
def test_disk_gate_sizes_fp32_families_by_their_real_download(monkeypatch):
    ...
    # Z-Image publishes fp32 shards (23,479 vs 11,730 MiB), so gating on resident filled the disk.
    import core.inference.diffusion_auto_policy as ap
...
def test_disk_gate_reserves_the_checkpoint_not_the_dense_shards_for_a_hosted_prequant(monkeypatch):
    # The gate branch the dense-sizing tests never reach: with a prequant source the loader downloads
    # the small quantised checkpoint, so space between the steady and dense sizes must pass with the
    # shortcut and be refused without it.
    _patch_selector(
        monkeypatch,
        scheme = "int8",
...
def test_quant_candidate_fits_resident_where_gguf_plan_offloads():
    # The ordering fix on a 32 GiB consumer card: the user picked a large BF16 GGUF so the file-size
    # plan forces offload, but the dense-quant candidate is far smaller and re-planning keeps it in.
...
def test_the_refusal_only_offers_auto_where_auto_exists():
    ...
    # text_encoder_quant has no "auto" mode: both request models restrict it to fp8 / fp8_dynamic /
    # int8 / nvfp4, so a user who followed "Choose Auto" here got a 422 from request validation.
