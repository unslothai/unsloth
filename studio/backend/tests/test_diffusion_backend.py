...
def test_detect_family_finds_video_full_repo_families():
    ...
    assert base.default_width == 1536
    assert base.default_height == 1024
    ...
    assert fam.default_width == 1536
    assert fam.default_height == 1024
...
def test_supported_optimization_options_payload_shape():
    ...
    assert compile_options["gguf_dequant_compile"]["default_enabled"] is True
    assert compile_options["gguf_balanced_dequant_compile"]["default_enabled"] is True
    assert (
        compile_options["gguf_balanced_dequant_compile"]["deprecated_alias_of"]
...
def test_balanced_gguf_keeps_cuda_cache_and_dequant_compile(monkeypatch):
    _install_fake_diffusers(monkeypatch)
...
def test_none_gguf_uses_dequant_compile_without_cuda_cache(monkeypatch):
    _install_fake_diffusers(monkeypatch)
    ...
    def _fake_cache(root, max_bytes):
        cache_calls.append((root, max_bytes))
        return {
            "modules": 3,
            "budget_bytes": 1024,
            "candidate_bytes": 512,
    ...
    fake_gguf_text_encoder.configure_lazy_gguf_cuda_cache = _fake_cache
    fake_gguf_text_encoder.install_compiled_lazy_gguf_linear_dequant = _fake_install
    monkeypatch.setitem(
        sys.modules,
        "core.inference.gguf_text_encoder",
    ...
    assert counts["diffusion_compiled_dequant_modules"] == 7
    assert "diffusion_cuda_cache_modules" not in counts
    assert "diffusion_compiled_dequant_skipped_for_denoiser_compile" not in counts
...
def test_generate_video_with_metadata_uses_ltx23_family_defaults(
    ...
    if expects_two_stage and family == "ltx2-3-base":
        ...
        monkeypatch.setattr(
            d.DiffusionBackend,
            "_generate_ltx2_base_two_stage_video",
            _fake_base_two_stage,
        )
    if expects_two_stage and family == "ltx2-3-distilled":
    ...
    assert video == ["frame0", "frame1"]
    observed_kwargs = (
        backend.two_stage_kwargs["base_call_kwargs"]
        if expects_two_stage
        else pipe.last_kwargs
    )
    assert observed_kwargs["negative_prompt"] == "ltx default negative"
    assert observed_kwargs["num_inference_steps"] == steps
    assert observed_kwargs["guidance_scale"] == guidance
    assert observed_kwargs["width"] == width
    assert observed_kwargs["height"] == height
    assert observed_kwargs["num_frames"] == 121
    assert observed_kwargs["frame_rate"] == 24.0
    ...
    assert observed_kwargs.get("use_cross_timestep") == (
        True if expects_base_guidance else None
    )
    assert observed_kwargs.get("sigmas") == expected_sigmas
    ...
    assert meta["sampling_profile"] == expected_profile
    if expects_two_stage:
        assert meta["stage_1_width"] == 768
