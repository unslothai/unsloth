...
def _clear_bash_cache():
    # Bound before the test so a monkeypatched _windows_bash, a lambda with no cache_clear, still tears down.
...
def test_untrusted_path_bash_is_rejected(monkeypatch, tmp_path):
    # A bash in a user-writable dir (Scoop, a per-user Git, a project checkout) would run every
    # sandboxed command, defeating the PATH/PATHEXT hardening in _build_safe_env. cmd is worse to
    # write for but stays trusted.
...
def test_trusted_path_bash_behind_an_untrusted_shim_is_found(monkeypatch, tmp_path):
    # shutil.which stops at the first hit, so a user shim early on PATH decided for a host with a real bash.
...
def test_wsl_launcher_is_rejected(monkeypatch, wsl_path):
    # WSL's bash runs in another filesystem, so the sandbox workdir would not apply; cmd stays inside it.
...
def test_windowsapps_under_program_files_is_still_rejected(monkeypatch, tmp_path):
    # A store package installs under Program Files, so the trust check alone would accept the WSL shim there.
...
def test_blocklist_lexes_bash_syntax_when_the_shell_is_bash(monkeypatch):
    # The scan is keyed to the shell, not the OS: the non-posix lexer never splits on `;`, so the loop ran.
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(tools, "_windows_bash", lambda: r"C:\Program Files\Git\bin\bash.exe")
...
def test_blocklist_sees_through_quoting_when_the_shell_is_bash(monkeypatch, command):
    # The non-posix lexer keeps the quote marks and _token_basename strips only meta-chars, so `'rm'` never matched.
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(tools, "_windows_bash", lambda: r"C:\Program Files\Git\bin\bash.exe")
...
def test_multiline_script_runs_every_line_on_windows():
    # The regression itself: under cmd /c only the first line ran, dropping the loop body and redirect.
...
def test_paths_note_names_the_real_platform():
    # A note citing only /mnt/data and /tmp/outputs reads as Linux, so models decline real Windows programs.
...
def test_shell_note_names_the_shell_that_will_run(monkeypatch):
    # Telling a model it has bash on a host that fell back to cmd reintroduces the half-execution.
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(tools, "_windows_bash", lambda: r"C:\Program Files\Git\bin\bash.exe")
...
def test_notes_never_recommend_a_blocked_program(monkeypatch, bash):
    # powershell/pwsh are blocked, so the prompt earns a hard block; `cmd /c start` is not, so naming it hurts.
    monkeypatch.setattr(sys, "platform", "win32")
...
def test_terminal_tool_description_is_the_two_notes_appended():
    # _TERMINAL_SHELL_NOTE is "" off Windows, so `note in description` proves nothing: compare the whole string.
...
def test_windows_shell_note_is_never_empty(monkeypatch, bash):
    # The half the test above cannot reach on a Linux runner: whichever shell wins, the description names it.
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(tools, "_windows_bash", lambda: bash)
...
def test_python_tool_description_omits_the_shell():
    # The shell note on the python description would point a model at subprocess as a blocklist bypass.
...
def test_notes_say_where_commands_run(monkeypatch):
    # Without this, models decline to launch a window they believe the user cannot see.
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(tools, "_windows_bash", lambda: r"C:\Program Files\Git\bin\bash.exe")
...
def _windows_blocklist(monkeypatch):
    # _BLOCKED_COMMANDS resolves the Windows names at import, so patching sys.platform is too late.
    monkeypatch.setattr(
        tools,
        "_BLOCKED_COMMANDS",
...
def test_cmd_shellout_is_screened_through_mangled_switches(command, _windows_blocklist):
    # Git Bash turns a lone /c into a path, so a model writes //c, which skipped the nested scan and reached powershell.
...
def test_detached_windows_stay_launchable(command, _windows_blocklist):
    # `start` is the only route to a desktop window, so screening must not blanket-block cmd.
    assert not tools._find_blocked_commands(command)


# Whether Git Bash resolves picks the lexer, and the cmd lexer keeps the marks: run both.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "cmd //v:on //c powershell -Command ls",
        # MSYS rewrites a POSIX path before cmd sees it, so skipping every /-word as a switch stepped over the program.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "npm start",
        "./start.sh",
        # `start` and `cmd` as data, not as programs: a shell name the shell never runs is text.
...
@pytest.mark.parametrize(
    "command",
    [
        # The documented `start "title" prog` form, visible only to the cmd lexer: the posix one strips the marks.
...
@pytest.mark.parametrize(
    "command",
    [
        # An assignment prefixes a command that still runs, and a shell handed a shell still runs it.
        'FOO=1 bash -c "rm -rf x"',
        'env FOO=1 bash -c "rm -rf x"',
        'FOO=1 start "" powershell',
        'cmd //c start "" cmd /c powershell',
        # A wrapper option's value sits between the wrapper and `start`, so the token before it says nothing.
...
def test_a_prefixed_shell_is_still_screened(monkeypatch, bash, command, _windows_blocklist):
    # An assignment is not cmd syntax, so under the cmd lexer `FOO=1` is itself the program name.
...
def test_a_program_path_is_not_read_as_a_cmd_switch(monkeypatch, _windows_blocklist):
    # Matched loosely, the pattern would find the `/b` of /bin/bash and never reach the shell name behind it.
