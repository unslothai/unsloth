...
def test_windows_setup_uses_selected_interpreter_without_tool_replay(monkeypatch):
    ...
    monkeypatch.setattr(srt_setup.subprocess, "run", lambda command, **kwargs: (commands.append(command) or SimpleNamespace(returncode=0)))
...
def test_setup_does_not_spawn_on_linux_or_macos(monkeypatch):
    monkeypatch.setattr(srt_setup.subprocess, "run", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("unexpected setup process")))
