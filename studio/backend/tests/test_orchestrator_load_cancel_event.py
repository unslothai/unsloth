...
def test_safetensors_load_event_interrupts_worker_wait(monkeypatch):
    ...
    monkeypatch.setattr(
        orchestrator,
        "_spawn_subprocess",
        lambda _config: spawned.__setitem__("value", True),
    )
    monkeypatch.setattr(
        orchestrator,
        "_read_resp",
        lambda timeout = 1: time.sleep(timeout) or None,
    )
    monkeypatch.setattr(
        orchestrator,
    ...
    try:
        ...
        loaded = orchestrator.load_model(
            SimpleNamespace(identifier = "local.safetensors"),
            load_cancel_event = cancel_event,
        )
    finally:
        timer.cancel()
