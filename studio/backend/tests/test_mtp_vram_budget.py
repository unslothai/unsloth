...
class TestExtraArgsMtpDetection:
    ...
    def test_load_model_ranks_subsets_by_active_pin_fraction(self):
        ...
        # fraction resolved once per load: the user's VRAM budget when they set
        # one, else _CTX_FIT_VRAM_FRACTION.
        compact = "".join(inspect.getsource(LlamaCppBackend.load_model).split())
        assert "_gpu_usable(g,pin_fraction)" in compact
        assert "_gpu_usable(g,_vram_frac-_flat_mtp_reserve)" in compact
        # Resolved once, so ranking and fitting cannot price against different
        # budgets if a save lands mid-load.
