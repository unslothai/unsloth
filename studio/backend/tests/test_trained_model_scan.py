...
def test_lora_identifier_resolves_remote_adapter_base(tmp_path: Path):
    # Remote adapter: the identifier helper fetches adapter_config.json so the gate can scan the base.
    ...
    assert get_base_model_from_lora("someone/my-remote-lora") is None
...
def test_lora_identifier_persistent_transient_returns_none():
    # Two transient errors yield None, logged at WARNING. Assert on the logger, so the backend can change.
    from utils.models import model_config as _mc
    with (
