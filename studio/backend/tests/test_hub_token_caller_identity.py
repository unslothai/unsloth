...
def test_legacy_models_cached_listing_is_not_hub_inventory_gated(monkeypatch):
    """``/api/models/cached-models`` stays on the subject-scoped legacy path; only ``/api/hub/*`` hides host inventory from sk-unsloth keys."""
    called = {"n": 0}
    ...
    monkeypatch.setattr(models_routes, "cached_model_rows", _rows)
    app = FastAPI()
    app.include_router(models_routes.router, prefix = "/api/models")
    app.dependency_overrides[get_current_subject] = lambda: "alice"
    app.dependency_overrides[authenticated_with_sk_unsloth_key] = lambda: True
    client = TestClient(app, raise_server_exceptions = False)

    response = client.get(
        "/api/models/cached-models",
        headers = {"Authorization": "Bearer sk-unsloth-deadbeefdeadbeef"},
    )
    assert response.status_code == 200
    assert called["n"] == 1
