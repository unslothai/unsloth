...
class TestGgufVariantFileResolution:
    ...
    def test_remote_listing_skips_big_endian_quant_sibling(
    ...
    def test_download_online_reuses_complete_cached_snapshot(
        self, monkeypatch, hf_cache
    ):
    ...
    def test_download_reuses_older_snapshot_when_offline_env_is_true(
        self, monkeypatch, hf_cache
    ):
    ...
    def test_download_companion_resolves_from_case_variant_snapshot_offline(
        ...
        requested_repo = "unsloth/gemma-4-e2b-it-gguf"
        snap = _build_cache(
            hf_cache, canonical_repo, {"mmproj-F16.gguf": 4}, snapshot_sha = "a" * 40
        )
        # A partial lower-case dir exists so casing resolution keeps the requested spelling.
        _build_cache(
            hf_cache, requested_repo, {"config.json": 1}, snapshot_sha = "b" * 40
        )
        ...
        def fail_download(*_args, **_kwargs):
            raise AssertionError(
                "should resolve the companion from cache, not download"
            )
    ...
    def test_download_refetches_split_gguf_when_shards_span_snapshots(
        self, monkeypatch, hf_cache
    ):
...
class TestCachedColocatedSplitMain:
    def test_prefers_older_complete_snapshot_over_newer_partial(self, hf_cache):
        ...
        old = _build_cache(
            hf_cache,
            "unsloth/split-GGUF",
            {shard1: 100, shard2: 100},
            snapshot_sha = "a" * 40,
        )
        new = _build_cache(
            hf_cache, "unsloth/split-GGUF", {shard1: 100}, snapshot_sha = "b" * 40
        )
        os.utime(old, (1000, 1000))
        os.utime(new, (2000, 2000))
    ...
    def test_returns_none_when_shards_span_snapshots(self, hf_cache):
        ...
        shard2 = "m-00002-of-00002.gguf"
        a = _build_cache(
            hf_cache, "unsloth/split-GGUF", {shard1: 100}, snapshot_sha = "a" * 40
        )
        b = _build_cache(
            hf_cache, "unsloth/split-GGUF", {shard2: 100}, snapshot_sha = "b" * 40
        )
        os.utime(a, (1000, 1000))
        os.utime(b, (2000, 2000))

        assert (
            _cached_colocated_split_main("unsloth/split-GGUF", shard1, [shard2], {})
            is None
        )
...
class TestResolveRepoIdCasing:
    def test_maps_to_canonical_casing(self, monkeypatch):
        monkeypatch.setattr(
            ...
            lambda repo: "unsloth/Gemma-4-GGUF"
            if repo.lower() == "unsloth/gemma-4-gguf"
            else repo,
        )
    ...
    def test_companion_only_newer_snapshot_does_not_shadow_real_variants(
        self, hf_cache
    ):
...
class TestDetectGgufModelRemoteOffline:
    ...
    def test_remote_big_endian_only_repo_is_not_detected(
        self, clean_offline_env, monkeypatch
    ):
        siblings = [
