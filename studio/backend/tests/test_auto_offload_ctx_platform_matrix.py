...
# The one this suite needs is the LAST, the fallback re-check's, always below
# SITE A. The count is deliberately not pinned: the fitted-context loop above
# awards residency through this statement only while it takes the first subset
# that fits, and keeping the best one instead is a refactor of that loop.
