...
class TestAnInheritedContextEnvironment:
    """Studio removes inherited llama-server settings before preflight and spawn."""

    AUTO_LAYERS = {"gpu_memory_mode": "manual", "gpu_layers": -1}

    @pytest.mark.parametrize("value", ["0", "8192", "", "  ", "abc", "-1"])
    def test_every_inherited_value_is_scrubbed(self, tmp_path, monkeypatch, value):
