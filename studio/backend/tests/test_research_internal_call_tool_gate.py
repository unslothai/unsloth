...
def test_forced_tool_policy_would_reach_the_tool_loop_without_the_opt_out(monkeypatch):
    # Guards the test below, which would otherwise pass if the route stopped forcing tools on.
...
def test_the_opt_out_changes_nothing_a_default_install_does(monkeypatch, policy):
    # Without --enable-tools the hop was already tool-free, so nothing sent may differ; `tools_withheld` stays local.
    ...
    # Both are fresh per request, so comparing them by identity would fail for any pair of requests.
    drop = {"cancel_event", "perf_callback", "tools_withheld"}
    # Dropping perf_callback would pass even if the opt-out stopped supplying it, costing that path its tok/s.
    ...
    # `tools_withheld` reaches the compaction gate, never the prompt: a default install re-admits
    # search_conversation through checkpoint repair, so an epoch reset is safe; under the opt-out that
    # repair is closed forever and a reset would strand the epoch. Never both False.
...
def test_json_mode_research_calls_send_llama_server_an_unchanged_body():
    # JSON-mode phases take the llama-server passthrough, so pin that body: no tool_choice, no Unsloth extensions.
