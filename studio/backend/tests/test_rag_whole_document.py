...
def test_build_rag_autoinject_skips_project_companion_over_budget(
    rag_conn, monkeypatch
):
    _add_doc(
        rag_conn, store.thread_scope("t1"), "td", "thread.txt", "h1", ["thread body"]
    )
...
def test_build_rag_autoinject_thread_whole_doc_ignores_project_size(
    rag_conn, monkeypatch
):
...
def test_build_rag_autoinject_args_carry_user_query(rag_conn):
    ...
    _add_doc(rag_conn, scope, "d1", "doc.pdf", "h1", ["small body"])
    result = inf_tools.build_rag_autoinject(
        _convo("what is in here"), {"thread_id": "t1"}
    )
