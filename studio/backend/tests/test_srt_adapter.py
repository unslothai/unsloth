...
def mount_table(monkeypatch):
    monkeypatch.setattr(Path, "read_text", lambda *args, **kwargs: "1 0 0:1 / / rw - ext4 /dev/root rw\n")
...
def test_nested_runtime_mount_is_masked(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(Path, 'read_text', lambda *args, **kwargs:
                        f'1 0 0:1 / {nested} rw - ext4 /dev/foreign rw\n')
...
def test_extra_trust_alias_mounts_only_canonical_target(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(srt_adapter, 'validate_roots', lambda roots, workdir: [])
    monkeypatch.setattr(srt_adapter.os.path, 'realpath', lambda path:
                        str(target) if str(path) == str(alias) else original_realpath(path))
    request = srt_adapter.request_for([str(runtime / 'python')], str(tmp_path), {}, 5,
                                      additional_read_roots=[str(alias)])
...
def test_sysconfig_data_does_not_admit_entire_system_prefix(monkeypatch):
    monkeypatch.setattr(srt_adapter.sysconfig, 'get_paths', lambda: {'data': '/', 'stdlib': '/usr/lib/python'})
