...
class TestTrainingRawSupport(unittest.TestCase):
    ...
    def test_mlx_max_grad_norm_is_honored_without_changing_the_default(self):
        # The worker hardcoded 0.0 and dropped the request, so explicit values pass and unset stays 0.0.
        ...
        # Unset must survive to the resolver, so "no opinion" stays distinguishable from an explicit 0.
    ...
    def test_mlx_worker_asks_the_trainer_to_report_the_gradient_norm(self):
        # What refills Unsloth's Gradient Norm chart on Apple Silicon; the rationale is at the opt-in site.
        source = (_BACKEND_ROOT / "core" / "training" / "worker.py").read_text(encoding = "utf-8")
        ...
        # Feature-detected like the other newer fields, so an older unsloth_zoo keeps working.
    ...
    def test_start_training_leaves_unset_max_grad_norm_for_worker_default(self):
        # None lets the worker apply the trainer default; 0.0 would erase the "no opinion" distinction.
        backend = TrainingBackend()
    ...
    def test_mlx_worker_falls_back_init_seeds_to_random_seed(self):
        ...
        # `dict.get(key, default)` only fills on absent keys, so an explicit None would reach get_peft_model.
    ...
    def test_mlx_worker_preserves_null_max_grad_value_for_trainer_default(self):
        ...
        # None must survive to the MLX trainer for its own default, and other values must coerce without rebinding None.
    ...
    def test_training_backend_normalizes_explicit_none_seed_and_dtypes(self):
        # `random_seed=None` and `cast_norm_output_to_input_dtype=None` must not leak past
        # `TrainingBackend.start_training`: set_seed(None) raises, PEFT init goes nondeterministic, and
        # the norm-output cast flips. The MLX clip knobs are the exception.
    ...
    def test_mlx_worker_feature_detects_optional_mlx_config_fields(self):
        # These fields ship in the paired unsloth-zoo update, so the worker gates them until that floor is in place.
        source = (_BACKEND_ROOT / "core" / "training" / "worker.py").read_text(encoding = "utf-8")
        ...
        # The unconditional kwargs must include no gated field, and paren tracking is needed: find(")") stops inside.
