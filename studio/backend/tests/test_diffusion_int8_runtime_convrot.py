...
def _filter(family):
    return tq.make_filter_fn(512, exclude_name_tokens = tq.exclude_tokens_for_scheme(tq.TQ_INT8, family) + ("lora_",))
...
def test_int8_artifact_names_the_rotated_build_first_and_keeps_the_plain_one():
    ...
    assert names[:2] == ("Qwen-Image-2.1-INT8-ConvRot.safetensors", "Qwen-Image-2.1-INT8.safetensors")
    assert candidate_filenames_of(resolve_prequant_source(fam, "fp8"))[0] == "Qwen-Image-2.1-FP8.safetensors"
...
def test_convrot_does_not_touch_the_exclusion_set():
    ...
    assert tq.exclude_tokens_for_scheme(tq.TQ_INT8, "qwen-image-2.1") == tq._INT8_EXCLUDE_NAME_TOKENS + ("txt_in",)
...
def test_runtime_convrot_is_inert_elsewhere():
    ...
    assert tq.apply_runtime_convrot(model, tq.TQ_FP8, "qwen-image-2.1", _filter("qwen-image-2.1")) == ()
...
def _stub_torchao(monkeypatch, seen):
    ...
    def quantize_(module, config, filter_fn = None):
        seen.append(sorted(n for n, m in module.named_modules() if is_rotated_linear(m) and filter_fn(m, n)))
...
def test_quantize_transformer_rotates_before_quantize(monkeypatch, family, expect_rotated):
    ...
    monkeypatch.setattr(tq, "select_transformer_quant_scheme", lambda target, mode, family = None: tq.TQ_INT8)
...
def test_quantize_transformer_leaves_fp8_unrotated(monkeypatch):
    seen = []
    _stub_torchao(monkeypatch, seen)
    monkeypatch.setattr(tq, "select_transformer_quant_scheme", lambda target, mode, family = None: tq.TQ_FP8)
    pipe = types.SimpleNamespace(transformer = _Tiny())
...
def test_unreachable_hub_still_loads_the_plain_artifact_already_cached(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda repo_id, filename, cache_dir = None, **k: str(tmp_path / filename) if (tmp_path / filename).is_file() else None,
    )
    asked = []
    ...
    source = pq.resolve_prequant_source(detect_family("Qwen/Qwen-Image-2.1", override = "qwen-image-2.1"), "int8")
