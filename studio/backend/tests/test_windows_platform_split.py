...
@pytest.mark.parametrize("platform,backend", [("linux", "sandbox_linux"), ("darwin", "sandbox_macos")])
def test_native_capability_never_imports_srt(monkeypatch, platform, backend):
    from types import SimpleNamespace
    monkeypatch.setattr(sys, "platform", platform)
    monkeypatch.setitem(sys.modules, "core.inference." + backend, SimpleNamespace(
        BACKEND_NAME=backend, PROFILE_ID="native", LIMITATIONS=(),
    ))
    monkeypatch.setitem(sys.modules, "core.inference.sandbox_probe", SimpleNamespace(probe=lambda *a, **k: (True, "passed")))
...
def test_unavailable_mode_matrix(monkeypatch, tmp_path, kind, mode, reason):
    monkeypatch.setattr(os_sandbox, "capability_snapshot", lambda **k: os_sandbox.SandboxCapability("srt", False, reason))
    plan = os_sandbox.ToolLaunchPlan((sys.executable, "-c", "print(17)"), str(tmp_path), {}, requested_mode=mode, execution_kind=kind)
    starts = []
...
@pytest.mark.parametrize("error", [OSError("startup"), RuntimeError("uncertain"), os_sandbox.SandboxBuildError("unsafe")])
def test_auto_preparation_failure_does_not_replay(monkeypatch, tmp_path, error):
    def fail(plan): raise error
...
def test_cancel_after_capability_wait_refuses(monkeypatch, tmp_path):
    ...
    with pytest.raises(os_sandbox.SandboxBuildError, match="cancelled"):
        os_sandbox.prepare_tool_launch(os_sandbox.ToolLaunchPlan((sys.executable,), str(tmp_path), {}, cancel_event=cancel))
