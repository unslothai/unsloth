...
def durable_run(request):
    ...
    studio_db.upsert_chat_message(
        {"id": "user-1", "threadId": "thread-1", "role": "user", "content": [{"type": "text", "text": "Hello"}], "createdAt": 2}
    )
    run, _created = runs_db.create_run(
        ...
        request_payload = {"model": model, "messages": [{"role": "user", "content": "Hello"}], "stream": True, "cancel_id": "run-1", "thread_id": "thread-1", "generation_run_id": "run-1"},
    )
...
def _create_payload(content = "Hello"):
    return run_routes.CreateChatGenerationRun(runId = "run-1", threadId = "thread-1", userMessageId = "user-1", assistantMessageId = "assistant-1", requestPayload = {"model": "local", "messages": [{"role": "user", "content": content}]})


def _route_request(supervisor):
    return SimpleNamespace(app = SimpleNamespace(state = SimpleNamespace(chat_generation_supervisor = supervisor)))
...
def test_only_durable_mlx_normalizes_stop_at_token_cap(
    ...
    assert inference._safetensors_finish_reason(
        stats, payload, is_mlx = is_mlx, durable_run = durable_run
    ) == expected
...
async def _subscriber_sequences(after = 0):
    response = await run_routes.chat_generation_events(
        "run-1", SimpleNamespace(is_disconnected = AsyncMock(return_value = True)), after = after, last_event_id = None, current_subject = "alice"
    )
    raw = ""
...
def _route_engine(monkeypatch, model, first, release):
    ...
    def generate(*, stats_holder = None, **_kwargs):
        first.set(); yield "A"; assert release.wait(5); yield "AB"
        if stats_holder is None: yield {"type": "metadata", "finish_reason": "stop"}
        else: stats_holder["stats"] = {"usage": {"completion_tokens": 2}}
    llama = SimpleNamespace(is_loaded = gguf, model_identifier = model, base_url = "http://llama.test", effective_parallel_slots = 1, supports_tools = False, is_vision = False, _is_audio = False, context_length = None, generate_chat_completion = generate)
    mlx = SimpleNamespace(active_model_name = model, models = {model: {"is_mlx": True, "chat_template_info": {"template": "chatml"}}}, generate_chat_response = generate, reset_generation_state = lambda *_a, **_k: None)
    ...
    monkeypatch.setattr(inference, "_detect_safetensors_features", lambda *_a, **_k: {"supports_tools": False})
...
async def test_subscribers_detach_then_replay_the_same_engine_run(durable_run, monkeypatch):
    ...
    task = asyncio.create_task(ChatGenerationSupervisor(SimpleNamespace(state = SimpleNamespace()))._produce("run-1"))
    ...
    deltas = [event["payload"].get("choices", [{}])[0].get("delta", {}).get("content") for event in runs_db.list_events("run-1") if event["type"] == "chunk"]
