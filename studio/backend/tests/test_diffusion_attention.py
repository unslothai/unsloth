...
def test_explicit_backend_dropped_off_nvidia_cuda(monkeypatch):
    # Explicit cuDNN/flash/sage on ROCm / MPS / CPU passes diffusers' set-time check then crashes at
    # first generation, so selection drops to native.
    monkeypatch.setattr(att, "_is_cuda_nvidia", lambda target: False)
    monkeypatch.setattr(att, "_cuda_capability", lambda: (10, 0))
...
def _no_real_installs(monkeypatch):
    ...
    # So is the resolved xFormers wheel: left set, one test's stub decides the next one's answer,
    # and unset the resolver shells out to probe the real torch.
...
def test_sage_install_carries_the_dispatcher_version_floor(monkeypatch):
    # PyPI's newest sageattention wheel is 1.0.6 but diffusers refuses anything below 2.1.1, so an
    # unpinned install "succeeds" then is rejected.
    monkeypatch.setenv("UNSLOTH_DIFFUSION_ATTENTION_INSTALL", "auto")
    import importlib.util
...
def test_a_query_string_token_is_stripped_too():
    # Tokens live in the query as often as in the userinfo, and the free-text form has to find URLs
    # inside a sentence, which is where pip puts them.
...
def test_failed_install_not_retried_in_same_process(monkeypatch):
    # The loader pre-installs the kernel OUTSIDE its locks and re-resolves under _generate_lock; a
    # failed pre-install must NOT re-run pip in the lock (a second 600s install would block
    # unload/cancel).
    monkeypatch.setenv("UNSLOTH_DIFFUSION_ATTENTION_INSTALL", "auto")
    import importlib.util
    import subprocess as sp
...
def test_kernels_install_skipped_on_old_hub(monkeypatch):
    # Current `kernels` wheels require huggingface_hub >= 1.10, and with an older hub `import
    # kernels` raises at module scope while diffusers imports it whenever installed, so one
    # auto-install would break every later pipeline import.
    monkeypatch.setenv("UNSLOTH_DIFFUSION_ATTENTION_INSTALL", "auto")
    import importlib.util
...
def test_kernels_gate_only_applies_to_kernels_package(monkeypatch):
    # sage / xformers / flash-attn wheels do not import huggingface_hub at module scope, so the hub
...
def test_kernels_hub_compatible_reads_hub_version(monkeypatch):
    ...
    # The gate is (major, minor) >= 1.10, the floor the kernels wheels declare; a major-only check
    # would pass every 1.x. Measured with kernels 0.16.0.
...
def test_deterministic_refusal_is_memoised(monkeypatch):
    ...
    def _probe(**kwargs):
        ...
        return {
            "platform_tag": "win_amd64",
            "python_tag": "cp313",
            # A CUDA family with no xFormers wheel on any index: the refusal has to be a real one,
            # and torch 2.11 itself now resolves through the stable-ABI rows.
...
def test_math_only_is_read_off_a_probe_not_the_flags(monkeypatch):
    ...
    # both answer True while every dispatch raises "No available kernel", so it degrades to math and
    # materialises the whole N x N score matrix.
...
def test_any_subquadratic_kernel_means_not_math_only(kernels, monkeypatch):
    ...
    # the score matrix is never materialised.
...
def test_an_unanswerable_probe_is_not_a_math_only_verdict(monkeypatch):
    # "Only math" is a claim about the hardware: a probe that could not run has made no such claim
    # and must not refuse or warn on a guess.
...
def test_the_probe_runs_once_per_device_and_dtype(monkeypatch):
    # A kernel cannot appear or vanish under a running interpreter and this sits on the load path,
    # so the probe is memoised; cuda:0 and cuda:1 are the same device TYPE.
    seen: list = []
    _stub_probe(monkeypatch, ("math",), record = seen)
...
def test_apply_native_reports_a_math_only_device(monkeypatch):
    ...
    # diffusers default and torch's dispatch decides: with no fused kernel that decision is math,
    # and nothing says so until the OOM.
    monkeypatch.setattr(att, "_active_attention_backend", lambda: "native")
    _stub_probe(monkeypatch, ("math",))
    logged: list = []
...
def test_apply_reports_a_math_only_device_for_a_unet_pipeline(monkeypatch):
    ...
    # set -- but its attention has the same quadratic blow-up, so the diagnosis is reported too.
    _stub_probe(monkeypatch, ("math",))
    logged: list = []
    logger = types.SimpleNamespace(
        warning = lambda fmt, *a: logged.append(fmt % a),
