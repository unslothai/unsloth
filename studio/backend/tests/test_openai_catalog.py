...
def test_catalog_lists_loaded_and_available(monkeypatch):
    ...
    async def _fake_catalog():
        return [
            _Info("/data/models/Qwen3-Q4.gguf", "Qwen3-Q4"),
            _Info("/data/models/Llama-8B-Q8.gguf", "Llama-8B-Q8"),
            # HF-cache GGUF: model_format is unset for these, so a files-based check must still list it.
    ...
    # Format and quant labels come from one on-disk scan; drive both off the flag.
    monkeypatch.setattr(
        resolver,
...
def test_a_resident_non_gguf_model_is_marked_loaded_and_stays_quantless(monkeypatch):
    # llama-only residency lists the serving model as unloaded, and the ungated hf_variant read
    # stamps a stale quant on this row.
    class _Orchestrator:
...
def test_catalog_lock_is_per_loop():
    # A module-level asyncio.Lock ties its waiters to the loop that first awaited it, so a second
    # event loop awaiting it can hang: the catalog lock must be per-loop, and the old shared
    # _CATALOG_LOCK must be gone so it cannot be reintroduced.
    async def _get():
...
def test_empty_and_errored_scans_are_cached(monkeypatch):
    # Cache validity is keyed on the timestamp, not list contents, so an empty or errored scan is
    # still cached for the TTL instead of rescanning on every /v1/models poll.
...
def test_catalog_ttl_starts_after_scan_completes(monkeypatch):
    # The cache timestamp must be taken AFTER the scan: a scan outliving the TTL would leave the
    # cache born-expired, so the next caller rescans instead of reusing it.
    import routes.models as models_mod
    ...
    async def _run():
        ...
        second = await inf._cached_local_catalog()
        return first, second
...
def test_retrieve_loaded_model_skips_catalog_scan(monkeypatch):
    # Retrieving a loaded id must resolve from the loaded set alone, never paying for the scan.
    monkeypatch.setattr(inf, "get_llama_cpp_backend", lambda: _FakeLlama())
    monkeypatch.setattr(inf, "get_inference_backend", lambda: _FakeUnsloth())
...
def test_cached_local_catalog_offloads_and_caches(monkeypatch):
    # The filesystem scan must run off the event loop and be cached, so a burst of /v1/models calls
    # does not re-scan or block.
...
def test_monitor_reports_nothing_once_a_non_gguf_model_is_unloaded(monkeypatch):
    # An auto-switch load records the requested repo id on the orchestrator, and an unload clears
    # active_model_name without clearing that alias, so reading it ungated reported a ready model
    # with nothing loaded.
    class _Orchestrator:
        active_model_name = None
...
def test_a_non_gguf_model_reports_one_id_across_every_v1_surface(monkeypatch):
    # /v1/models, GET /v1/models/{id} and the chat-completions body must name an auto-switched model
    # the same way, or a client cannot round-trip the id.
    class _Orchestrator:
        active_model_name = "/srv/lmstudio/mlx-community/Qwen3-8B-4bit"
...
def test_a_standalone_gguf_does_not_advertise_a_quant_that_stops_resolving(monkeypatch):
    # llama.cpp reads hf_variant off the filename, but the resolver stores standalone files with no
    # quants, so a pinned "<stem>:<quant>" would 404 once it is not resident.
    ...
    # A cold index cannot prove the reference either, and publishing on no proof hands out the pin
    # that later fails to resolve.
    monkeypatch.setattr(resolver, "_scan", (0.0, {}))
    # Stub the walk: a real multi-root scan inside the cold-wait budget times out into a 503.
...
def test_a_loaded_alias_advertises_the_quant_that_is_actually_loaded(monkeypatch):
    # Marking the alias loaded while publishing the preferred on-disk quant said alias:Q4 was loaded
    # while Q8 was serving, and pinning that 404s with switching off.
    monkeypatch.setattr(inf, "get_inference_backend", lambda: _FakeUnsloth())
    llama = _FakeLlama()
...
def test_a_nested_model_directory_is_not_the_resident_one(monkeypatch):
    # Two indexed models can nest, and a plain prefix test made loading B mark A resident, so a
    # request for A was answered with B's weights. The innermost indexed model owns the file.
    ...
    # With nothing indexed there is no nesting to tell apart, so the directory-to-file match holds.
...
def test_a_transformers_model_does_not_mark_a_gguf_alias_loaded(monkeypatch):
    # A Transformers model live from a directory that also holds GGUF exports is not a GGUF, and
    # marking the alias loaded had the examples pin a quant nothing can serve with switching off.
...
def test_an_alias_for_the_resident_weights_is_not_listed_as_unloaded(monkeypatch):
    # A GGUF loaded by absolute path keys the resident entry by basename, so an id-only dedup would
    # emit the alias again marked not loaded.
    monkeypatch.setattr(inf, "get_llama_cpp_backend", lambda: _FakeLlama())
    monkeypatch.setattr(inf, "get_inference_backend", lambda: _FakeUnsloth())
