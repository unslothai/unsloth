...
_UNAVAILABLE = SandboxCapability(
    ...
    remediation = "Install the pinned Studio SRT runtime, then check again.",
)
...
def test_unavailable_capability_prints_the_full_notice(capsys):
    ...
    assert "Detected: srt backend, native_linux environment." in out
    ...
    assert "pinned Studio SRT runtime" in out
