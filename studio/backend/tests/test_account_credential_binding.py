...
"""An API key binds the account it was validated for; a deactivated account keeps the host closed until deleted."""
...
def test_the_owner_login_id_needs_no_account_lookup(auth_db, monkeypatch):
    """The owner's username is reserved, so its id is constant; a managed name must be resolved because it can be reused."""
    from routes import auth as auth_routes
