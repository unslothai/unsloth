...
def test_evict_chat_unloads_a_still_loading_chat_backend(monkeypatch):
    # A chat model still starting up is is_active but not yet is_loaded, and eviction must still
    # unload it or the load keeps allocating VRAM after the handover.
...
def test_release_if_predicate_sees_a_reregistered_same_owner_load(calls):
    # The race release_if closes: a slow unload's predicate reports a load now in flight, so
    # ownership stays with DIFFUSION.
    arb.acquire_for(arb.DIFFUSION)
...
def test_register_runs_under_ownership_and_returns_result(calls):
    # A register callback runs after ownership transfers and its return value is forwarded; the
    # route registers the in-flight load with it.
    seen_owner: list = []
...
def test_register_failure_leaves_ownership_in_place(calls):
    # A failing register (e.g. begin_load reporting a load in progress) propagates but must not drop
    # ownership: the prior handoff stands.
    arb.acquire_for(arb.CHAT)
...
def test_competing_acquire_blocks_until_register_completes(monkeypatch):
    ...
    # load is in-flight; holding the lock across register stops eviction racing it.
    import threading
    import time
...
def test_evict_chat_cancels_a_chat_load_that_has_not_spawned_yet(monkeypatch):
    # An HF chat load has no llama-server process until its GGUF finished downloading, so gating
    # only on is_active let the evictor find nothing to cancel and the chat load spawn onto the same
    # device.
    import core.inference as core_inference
    import routes.inference as routes_inference
...
def test_evict_chat_cancels_an_in_flight_safetensors_load(monkeypatch):
    ...
    # in-flight safetensors load is visible ONLY in loading_models; gating on active_model_name let
    # that worker allocate alongside the new pipeline.
    import core.inference as core_inference
    import routes.inference as routes_inference
...
def test_evict_chat_cancels_every_pending_load_over_a_live_snapshot(monkeypatch):
    # cancel_load discards the marker it cancels, so iterate a snapshot (mutating the live set
