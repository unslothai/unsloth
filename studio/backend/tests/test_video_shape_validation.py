...
def test_generate_rejects_256x256_with_422_naming_the_presets(client, backend):
    ...
    progress = client.get("/api/inference/video/generate-progress").json()
    # Named, not subscripted: the one way this has gone wrong is the account gate firing
    # off a leaked routes.video._generation_account, and a bare KeyError points at neither.
