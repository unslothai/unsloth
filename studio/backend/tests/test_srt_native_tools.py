...
pytestmark = [pytest.mark.allow_network, pytest.mark.skipif(
...
def test_nonzero_has_no_false_execution_attestation(native_session):
    from core.inference import tools
    records = []
    result = tools._python_exec("raise ValueError('EXPECTED_PAYLOAD_FAILURE')", None, 30, "native",
...
"""
    result = tools._python_exec(code, None, 30, 'native',
                               launch_record_callback=records.append)
...
"""
    result = tools._python_exec(code, None, 30, 'native', network_policy='allowlist',
                                launch_record_callback=records.append)
