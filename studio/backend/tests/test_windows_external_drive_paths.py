...
def test_readable_dir_within_times_out(monkeypatch):
    # A probe that outlives the timeout is not-readable, so a hung mapped drive is skipped, not waited on.
    import time
...
def test_windows_drive_roots_skips_hung_drive(monkeypatch):
    # A disconnected mapped drive stays set in the bitmask and stalls isdir: skip without stalling.
    import time
...
def test_windows_drive_roots_probes_hung_drives_in_parallel(monkeypatch):
    # Several disconnected mapped drives must add ~one timeout total, not one per drive.
    import time
    ...
    assert roots == [Path("C:\\")]
    # 3 stalled drives probed in parallel finish within ~1 timeout, well under a serial probe's ~3x.
    assert elapsed < 3 * timeout


def test_browse_allowlist_includes_windows_drive_roots(monkeypatch, tmp_path):
    # End-to-end wiring: windows_drive_roots() output flows into the browse allowlist built by routes/models.py.
    tree = ast.parse((_BACKEND_ROOT / "routes" / "models.py").read_text(encoding = "utf-8"))
    function_names = {
...
def test_build_browse_allowlist_reuses_passed_roots(monkeypatch, tmp_path):
    # Double-probe fix: a browse request probes the roots once and passes them in, so the allowlist must not rescan.
    tree = ast.parse((_BACKEND_ROOT / "routes" / "models.py").read_text(encoding = "utf-8"))
    functions = [
        node
        for node in tree.body
...
def test_is_path_inside_allowlist_real_descendants_and_siblings(tmp_path):
    # Component-wise containment: a sibling sharing only a prefix ("models_root_evil") is not a descendant.
...
def test_is_path_inside_allowlist_posix_root_does_not_authorize_descendants(monkeypatch):
    # Regression for the POSIX "/" unlock: a bare root may match itself but authorizes no descendants.
    ns = _extract_routes_function("_is_path_inside_allowlist")
...
def test_is_path_inside_allowlist_windows_drive_root_descendants():
    # Exercise the Windows drive-root branch on a POSIX host by backing os.path with ntpath and an identity realpath.
    import ntpath
