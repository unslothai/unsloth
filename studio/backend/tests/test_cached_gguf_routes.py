...
def test_compat_local_inventory_requests_share_scan(monkeypatch, tmp_path):
    # Total and stable on hostile input is the contract; the exact string is platform-dependent
    # (POSIX realpath() raises ValueError on an embedded NUL, Windows falls back to abspath).
    for hostile in ("\0", "\ud800"):
...
def test_an_audio_only_projector_in_the_cache_is_not_vision(monkeypatch, tmp_path):
    ...
    # A projector serving both modalities declares both (Qwen2.5-Omni), so the row cannot be read
    # off the audio claim alone.
...
def test_is_hidden_model_hides_validation_probe_everywhere():
    ...
    # A Windows-style snapshot path must match too, even on a POSIX interpreter (the filename check
    ...
    # The exact-filename needle must not hide a real repo that merely references stories260K in its name.
...
def test_is_hidden_model_keeps_stale_default_embedder_hidden(monkeypatch):
    ...
    # Repo IDs still use exact matching, and similar local basenames must have a real separator
    # after the static default name.
...
def test_list_cached_models_prefers_complete_over_larger_partial(monkeypatch, tmp_path):
    ...
    # COMPLETE one, or the picker hides a usable model.
    complete = _repo(
        "Org/Dup",
        [_file("model.safetensors", 10_000)],
...
def test_native_context_read_gives_up_when_the_cache_walk_drags(monkeypatch, tmp_path):
    ...
    # A signature drift here raises inside the broad except and returns instantly, which would pass
    # every other assertion without walking anything.
...
def test_arch_to_task_hides_unsupported_diffusion_from_chat():
    ...
    # task, so both pickers skip them.
    ...
    # A video arch that does not resolve from the bare arch alone ("wan" covers TI2V-5B and the A14B
    # MoE) stays unsupported.
    ...
    # With a repo/file name hint the loadable TI2V-5B resolves to Video while the A14B MoE stays
    # unsupported, matching the loader.
    assert (
...
def test_arch_to_task_tags_the_h3_gguf_bundle_as_video():
    ...
    # arch read alone left the repo without a task -- dropped from the Video picker and offered to
    # chat.
...
def test_arch_to_task_resolves_z_image_gguf_tagged_lumina2():
    # Z-Image's DiT is a Lumina2 derivative, so both Z-Image GGUF repos declare general.architecture
    # = "lumina2"; reading the arch alone tagged the line unsupported and hid it.
    for repo, fname in (
...
def test_arch_to_task_agrees_with_the_loader_on_ambiguous_archs():
    # The picker and the loader must not disagree: whatever _arch_to_task advertises as loadable,
    # validate_load_request must accept, else the Images list hides a working model or offers a 400.
...
def test_delete_cached_refuses_loaded_native_companion_repo(monkeypatch):
    ...
    # generation, and the repo_id does not match, so the guard needs loaded_repo_ids().
    from fastapi import HTTPException
    from hub.services.models import deletion
    import core.inference.diffusion_engine_router as der
    import core.inference.video as video_mod
...
def test_delete_cached_rechecks_the_load_guard_after_reserving_the_scope(monkeypatch):
    ...
    # publishes its claim too late; video and image loads bypass the registry, so the second read
    # is what refuses it.
    from fastapi import HTTPException
    from hub.services.models import deletion
    import core.inference.diffusion_engine_router as der
...
def test_cached_repo_partial_scopes_probe_to_snapshot_dir(monkeypatch):
    ...
    # .incomplete copy in one cache root would flag a complete copy in another.
...
def test_repo_has_pipeline_index_requires_root_model_index(tmp_path):
    ...
    # clear the single_file flag; the helper scopes by snapshot path.
...
def test_pipeline_scans_read_the_snapshot_the_loader_will_open(tmp_path):
    ...
    # GGUF load leaves) must be judged on the snapshot from_pretrained resolves, the newest by
    # mtime.
    import os
...
def test_list_cached_models_flags_single_file_diffusion_repos(monkeypatch, tmp_path):
    # A diffusion-tagged repo with NO top-level model_index.json is a single-file checkpoint
    # (single_file=True); a full pipeline or chat repo carries no flag.
    single = _repo(
...
def test_pipeline_class_guard_fires_before_any_download():
    # The newer families used to die with a bare AttributeError deep in the load, after the
    # checkpoint was fetched. 0.35.2 is the last release before ZImagePipeline existed.
    import pytest
    ...
    # diffusers` alone fixes this on a 3.9 host.
    assert "3.10" not in msg

    # Krea 2 is the other side: its class only exists from 0.39.0, which needs 3.10, so the Python
    # floor belongs in THAT message.
    sys.modules["diffusers"] = stub
    try:
        with pytest.raises(ValueError) as excinfo:
...
def test_pipeline_class_guard_passes_every_shipped_family():
    # Split out of the guard test above so it can skip on its own: the backend CI image installs the
    # CPU-only dep set with no diffusers, where the native sd.cpp engine still serves GGUF picks.
    import pytest
...
def test_pipeline_class_guard_is_silent_when_diffusers_is_absent(monkeypatch):
    # Not this check's business: with nothing installed there is no version to judge.
    import builtins
...
def test_pipeline_class_guard_is_silent_when_a_lazy_submodule_cannot_import(monkeypatch):
    # Same contract for a PRESENT but partially usable install: diffusers' lazy top level makes the
    # attribute probe import the submodule, which raises RuntimeError. hasattr absorbs only
    # AttributeError, so that escaped as a bare 500.
    import types
...
def test_cached_pipeline_needs_a_detectable_image_family(monkeypatch):
    ...
    # class this backend cannot assemble cleared the trust gate, then failed validate_load_request.
    monkeypatch.setattr(
...
def test_cached_picker_hides_a_family_this_diffusers_cannot_build(monkeypatch):
    # The newer families exist only from diffusers 0.39, uninstallable on Python 3.9, so advertising
    # one there is a pick that can only fail.
    import types
...
def test_gguf_picker_hides_a_family_no_engine_here_can_build(monkeypatch):
    ...
    # picker actually offers -- still showed as text-to-image on a diffusers too old to build them.
...
def test_gguf_picker_keeps_a_family_the_native_engine_serves(monkeypatch):
    # The opposite mistake: on a CPU/MPS or force-native host sd.cpp loads the GGUF and never
    # instantiates a diffusers class, so hiding the row would withhold a working model.
...
def test_the_loader_demands_the_diffusers_class_only_when_diffusers_loads_it(monkeypatch):
    # Same predicate on the load path: refuse a too-old diffusers before the download, and never
    # when sd.cpp serves the GGUF.
    import pytest
    ...
    # ValueError, not RuntimeError: /images/load maps RuntimeError to 409 and /images/download-plan
    # catches only (ValueError, FileNotFoundError), so the message escaped as a 500.
...
def test_the_video_picker_hides_a_family_this_diffusers_cannot_build(monkeypatch):
    # Same gap on the video branches: LTX-2's pipeline class is 0.39-only too, and video has no
    # native engine to fall back to, so the load asserts it unconditionally.
...
def test_every_shipped_video_family_resolves_on_this_diffusers():
    # Drift guard: the video picker hides a family whose pipeline class the installed diffusers
    # lacks, so a stale class name here would hide a working model.
...
def test_the_gguf_picker_and_the_image_loader_agree_on_an_old_diffusers(monkeypatch):
    ...
    # applied to the GGUF path on both host kinds.
    from core.inference.diffusion import DiffusionBackend
    from core.inference.sd_cpp_engine import ENGINE_DIFFUSERS, ENGINE_SD_CPP
...
def test_a_moved_image_pipeline_the_picker_shows_is_one_the_loader_accepts(tmp_path):
    ...
    # both sides: the index adds models whose name said nothing, it never overrules a name that said
    # no.
...
def test_only_the_first_shard_of_a_split_gguf_is_read_to_classify_a_repo(tmp_path, monkeypatch):
    ...
    # Trailing shards first, an order the filesystem is free to hand back: without the skip they are
    # opened and read as unclassifiable before shard 1 is reached.
    monkeypatch.setattr(
        catalog_classification,
        "_iter_gguf_paths",
...
def test_a_slow_walk_cannot_hand_back_the_encoder_this_function_exists_to_avoid(
    ...
    # On _classification, where _gguf_folder_task resolves the name: patching the routes alias
    # leaves the real walk running, and the test then passes on flux winning the sort, not on the
    # budget.
...
def test_a_folder_trimmed_exactly_back_to_the_cap_never_answers_speech(tmp_path):
    ...
    # 2 * cap + 1 trips the trim on its final candidate and 3 * cap + 2 is the next that does,
    # confirming a recurring boundary rather than one bad number.
...
def test_a_sibling_whose_header_will_not_read_keeps_speech_off_the_folder(tmp_path):
    ...
    # The control: with the sibling readable and runnable the folder still files as chat, so the fix
    # withholds speech on an unread sibling rather than always.
...
def test_speech_gguf_classification_preserves_decoder_provenance(tmp_path):
    ...
    # Orpheus ships as a llama GGUF: its family hint makes it speech-only, and the codec field is
    # what lets Audio route it instead of mistaking it for CSM.
...
def test_a_buildable_denoiser_outranks_an_arch_the_backend_cannot_assemble(tmp_path):
    ...
    # With nothing buildable in it the folder still reports the unsupported tag, rather than falling
    # through to a text one and offering a denoiser in chat.
...
def _listing_families():
    ...
    z_image = detect_family("unsloth/Z-Image-Turbo")
...
def test_the_listing_probe_hides_a_class_the_installed_diffusers_predates(monkeypatch):
    ...
    assert family_pipeline_available(wan) is False
    installed[0] = "0.33.0"
...
# Moved into catalog_classification, or no longer imported from utils.gguf_archs.
