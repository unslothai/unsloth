...
def test_config_source_extensions_exist():
    ...
    # Check newly aligned extensions from text attachments (Dan Han review)
    for ext in (".ipynb", ".csproj", ".jsonc", ".csv", ".svg", ".text", ".log", ".mdx"):
        assert ext in config.ALL_UPLOAD_EXTS, f"{ext} missing from ALL_UPLOAD_EXTS"
...
def test_folder_sync_scan_finds_source_code_and_ignores_dirs(tmp_path):
    ...
    (src_dir / ".env.local").write_text("SECRET=1", encoding = "utf-8")
    (src_dir / "prod.env").write_text("SECRET=1", encoding = "utf-8")
    (src_dir / "package-lock.json").write_text("{}", encoding = "utf-8")
    (src_dir / "Cargo.lock").write_text("[[package]]", encoding = "utf-8")
    ...
    assert "build/output.js" in found_keys
    ...
    assert "src/.env" not in found_keys
    assert "src/.env.local" not in found_keys
    assert "src/prod.env" not in found_keys
    assert "src/package-lock.json" not in found_keys
    assert "src/Cargo.lock" not in found_keys
