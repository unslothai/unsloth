...
def isolated_root(tmp_path, monkeypatch):
    ...
    # The lock lives in the studio home, and this file redirects everything else to tmp_path, so redirect it.
...
def test_write_pid_file_records_the_start_time(tmp_path):
    # Pins the record to this process, so a reused PID is not mistaken for it.
    run._write_pid_file(8901)
...
def test_the_legacy_pointer_moves_to_a_live_sibling(tmp_path):
    # Only one server owns studio.pid, and deleting it would leave an older CLI unable to stop the sibling.
    run._write_pid_file(8901)
    (tmp_path / "studio-8902-8600.pid").write_text("8600", encoding = "utf-8")
...
def test_windows_liveness_does_not_call_every_pid_alive(monkeypatch):
    # os.kill(pid, 0) raises for every pid on Windows, so without tasklist a stale record blocks its port.
    import subprocess
...
def test_windows_liveness_keeps_the_record_when_tasklist_fails(monkeypatch):
    # Unconfirmed must mean keep, as the CLI's _pid_alive does: pruning a live record strands that server.
    import subprocess
...
def test_graceful_shutdown_drops_the_record_last(monkeypatch):
    # Cleanup can take seconds while the server is alive, so dropping the record first hides it from `stop`.
    order = []
...
def test_own_studio_on_port_is_found_without_psutil(tmp_path, monkeypatch):
    # psutil is optional, and a listener scan finds nothing without it, so detection must come from our own records.
    monkeypatch.setitem(sys.modules, "psutil", None)
...
def test_an_unverifiable_record_still_blocks_a_duplicate(tmp_path, monkeypatch):
    # Cannot tell: refusing with a clear message beats a silent second instance.
    monkeypatch.setattr(run, "_pid_is_studio_backend", lambda pid, created_times = (): True)
    (tmp_path / "studio-8901-8550.pid").write_text("8550", encoding = "utf-8")
...
def test_a_stale_record_does_not_veto_a_live_server_sharing_the_pid(monkeypatch):
    # The OS reuses a crashed server's PID on another port, so keeping the first timestamp rejects the live one.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
    monkeypatch.setattr(run, "_process_create_time", lambda pid: 999.0)
...
def test_a_bare_run_py_command_line_is_not_rejected(monkeypatch):
    # `cd studio/backend && python run.py --port 8901` has no "studio" in argv, so argv-guessing called it foreign.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
...
def test_an_untimed_legacy_record_is_trusted(monkeypatch):
    # `python run.py --port 8901` has no telltale argv, so only a start time can disprove a real server.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
    monkeypatch.setattr(run, "_process_create_time", lambda pid: 999.0)
...
def test_the_untimed_legacy_record_does_not_cancel_a_timed_one(monkeypatch):
    # Mirrors _pid_is_studio_server: an untimed record carries no information and must not overrule a start time.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
    monkeypatch.setattr(run, "_process_create_time", lambda pid: 999.0)
...
def test_a_legacy_server_on_the_port_is_recognised(tmp_path, monkeypatch):
    # Pre-upgrade servers wrote only studio.pid, so falling back past one strands it and then overwrites its record.
...
def test_a_stale_per_port_record_does_not_mask_a_legacy_server(tmp_path, monkeypatch):
    # A stale per-port record for a reused PID must not read as "port known" and send us past the live server.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
    monkeypatch.setattr(run, "_process_create_time", lambda pid: 999.0)
    monkeypatch.setattr(run, "_get_pid_on_port", lambda p: (8550, "python"))
    (tmp_path / "studio-8901-8550.pid").write_text("8550\n111.5\n127.0.0.1", encoding = "utf-8")
...
def test_a_current_server_elsewhere_does_not_block_a_foreign_port(tmp_path, monkeypatch):
    # Without psutil the legacy check cannot see the listener, so it must not claim our 8901 holds 8888.
    monkeypatch.setattr(run, "_get_pid_on_port", lambda p: None)
...
def test_our_studio_on_another_bind_address_does_not_abort(tmp_path):
    # Our server holds ::1:8889, so binding 127.0.0.1:8889 is no conflict and the launch takes the next port.
...
def test_a_hostname_resolves_the_same_way_the_bind_does(tmp_path):
    # `localhost` and the address _is_port_free binds must agree, or a recorded server is missed.
...
def test_a_hostname_records_every_address_it_resolves_to(tmp_path):
    # `localhost` binds 127.0.0.1 AND ::1, so recording only the first lets a later launch on the other literal miss us.
...
def test_our_own_server_on_the_requested_port_aborts_rather_than_falling_back(
    ...
    # The reported bug: 8888 is ours, so falling back to 8889 leaves 8888 serving with nothing recording it.
...
def test_a_caller_that_reads_the_port_back_keeps_the_plain_fallback(tmp_path, monkeypatch):
    # api-only callers follow us to the new port, so aborting only turns a working launch into a reported crash.
    monkeypatch.setattr(run, "_is_port_free", lambda host, p: p != 8888)
    (tmp_path / "studio-8888-8550.pid").write_text("8550\n\n127.0.0.1", encoding = "utf-8")
...
def test_the_recorded_address_is_every_address_the_bind_resolves_to(tmp_path):
    # The only test that runs the writer with a real host: recording `host` verbatim, or dropping the
    # line, passes every other test here and silently stops matching a differently spelled launch.
...
def test_bind_addresses_keeps_every_family_a_hostname_resolves_to(monkeypatch):
    # Independent oracle: the sibling test derives its expectation from this function, so a drop would pass.
    import socket
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
...
def test_the_legacy_file_is_written_even_when_the_per_port_record_fails(tmp_path, monkeypatch):
    # A studio root that cannot take a new entry left the server recorded nowhere; studio.pid overwrites, so try it.
...
def test_a_record_whose_pid_is_not_ascii_digits_is_discarded(tmp_path):
    # A superscript two passes isdigit() but int() rejects it, so a ValueError escaped _read_pid_record.
...
def test_the_legacy_file_is_not_taken_from_a_live_server(tmp_path):
    # A pre-upgrade server is recorded in studio.pid alone, so a second launch overwriting it strands it.
    (tmp_path / "studio.pid").write_text("8550", encoding = "utf-8")
...
def test_the_legacy_file_is_taken_over_from_a_dead_server(tmp_path, monkeypatch):
    # A stale record must not keep the pointer forever, or an older CLI could never stop anything again.
    monkeypatch.setattr(run, "_pid_alive", lambda pid: False)
    (tmp_path / "studio.pid").write_text("8550", encoding = "utf-8")
...
def test_a_live_sibling_is_found_so_the_shared_cache_survives(tmp_path):
    # The compiled cache is install-tree relative, so a second backend must not wipe it from under the first.
    (tmp_path / "studio-8888-8550.pid").write_text("8550\n\n127.0.0.1", encoding = "utf-8")
...
def test_a_sibling_that_is_still_binding_is_found(tmp_path):
    # Lifespan startup runs, and would clear the cache, long before uvicorn reports a port to record.
...
def test_a_legacy_only_sibling_is_found(tmp_path):
    # A pre-upgrade server is recorded here and nowhere else, and so is one whose best-effort per-port write failed.
    (tmp_path / "studio.pid").write_text("8550", encoding = "utf-8")
...
def test_a_startup_marker_is_invisible_to_the_pid_file_glob(tmp_path):
    # Everything reading PID_FILE_GLOB expects a port in the name, and a process that has not bound yet has none.
    (tmp_path / "studio-starting-8550.marker").write_text("8550\n", encoding = "utf-8")
...
def test_the_marker_outlives_the_early_record_drop(tmp_path):
    # _graceful_shutdown drops the records before the thread is joined, so the marker waits for the thread to end.
    run.write_startup_marker()
    marker = tmp_path / f"studio-starting-{os.getpid()}.marker"
...
def test_a_bare_legacy_record_cannot_resurrect_a_reused_pid(tmp_path, monkeypatch):
    # An untimed record is trusted, but a timestamped one for that PID written after it proves the server is gone.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
    monkeypatch.setattr(run, "_process_create_time", lambda pid: 999.0)
...
def test_an_older_timed_record_does_not_veto_a_newer_legacy_one(tmp_path, monkeypatch):
    # The other order is a pre-upgrade backend starting on a PID a crashed current-version backend
    # left a record for; the older timestamp describes the process that is gone, so letting it veto
    # would clear the compiled cache under a serving backend.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
    monkeypatch.setattr(run, "_process_create_time", lambda pid: 999.0)
    legacy = tmp_path / "studio.pid"
    timed = tmp_path / "studio-8888-8550.pid"
...
def test_a_timed_record_for_another_pid_leaves_the_legacy_one_alone(tmp_path, monkeypatch):
    # Corroboration is per PID: a dead record for a different server says nothing about the pre-upgrade one.
    monkeypatch.setattr(run, "_pid_is_studio_backend", _REAL_IS_STUDIO_BACKEND)
    monkeypatch.setattr(run, "_process_create_time", lambda pid: 999.0)
...
def test_the_cache_lock_is_exclusive(tmp_path):
    # Two backends racing is the point, so the second must be told the section is taken rather than let into it.
...
def test_the_probe_and_the_clear_happen_inside_one_lock(tmp_path, monkeypatch):
    # The race: our probe finds nobody, a sibling publishes and compiles, and our rmtree deletes it.
    events = []
...
def test_a_lock_that_cannot_be_taken_at_all_still_clears(tmp_path, monkeypatch):
    # An unwritable temp dir or a flock-less filesystem must not stop the cache being cleared ever again.
    blocked = tmp_path / "not-a-directory"
    blocked.write_text("", encoding = "utf-8")
...
def test_the_startup_marker_is_published_under_the_cache_lock(tmp_path, monkeypatch):
    # Publication must be inside the same section as a sibling's probe and clear, or it is clear-and-lost.
    marker = tmp_path / f"studio-starting-{os.getpid()}.marker"
    events = []
...
def test_a_zombie_backend_is_not_a_live_sibling(tmp_path, monkeypatch):
    # A backend that exited under a non-reaping parent stays a zombie: it answers kill(0) and keeps
    # its start time, so liveness alone reads it as serving and its record would pin the cache.
...
def test_every_record_call_can_be_repeated(tmp_path):
    # Startup and shutdown both run more than once in a long-lived embedded host, so each must be re-callable.
    run.write_startup_marker()
    marker = tmp_path / f"studio-starting-{os.getpid()}.marker"
...
def test_reading_the_siblings_changes_nothing_on_disk(tmp_path):
    # The probe runs on every startup and is a read: pruning would let one install edit another's records.
    (tmp_path / "studio-8888-4242.pid").write_text("4242\n111.5\n127.0.0.1", encoding = "utf-8")
...
def test_a_busy_lock_publishes_then_waits_for_the_clear_to_finish(tmp_path, monkeypatch):
    # Publishing unlocked keeps this backend visible to sibling probes, but returning straight away
    # would let the caller compile into a cache the holder is still deleting, so the marker goes first.
    marker = tmp_path / f"studio-starting-{os.getpid()}.marker"
    timeouts = []
...
def test_a_clear_that_never_finishes_still_lets_the_backend_start(tmp_path, monkeypatch):
    # Refusing to start is worse than starting on a lossy cache, so the wait is bounded and startup goes on.
    @contextlib.contextmanager
    def _lock(timeout = None):
        yield cache_cleanup.LOCK_BUSY
...
def test_a_filesystem_that_cannot_lock_is_not_read_as_contention(tmp_path, monkeypatch):
    # ENOSYS is the lock being unsupported, not a sibling holding it, and busy is read as proof of a sibling.
    def unsupported(fd):
...
def test_a_startup_that_raises_takes_its_marker_back(tmp_path, monkeypatch):
    # colab.py keeps the process alive around run_server, so no exit hook runs and a marker answers as live.
    run.write_startup_marker()
    assert list(tmp_path.glob(run.STARTUP_MARKER_GLOB)) != []
...
def test_an_interrupt_leaves_the_marker_to_a_live_server_thread(tmp_path, monkeypatch):
    # The KeyboardInterrupt path asks uvicorn to stop and re-raises without joining, so the marker must stay.
    run.write_startup_marker()
    assert list(tmp_path.glob(run.STARTUP_MARKER_GLOB)) != []
    ...
    # Once that thread is gone the same failure does take the marker back, since nothing else is going to.
    with pytest.raises(KeyboardInterrupt):
        run.run_server()
...
def test_a_server_thread_that_dies_after_readiness_drops_its_records(tmp_path, monkeypatch):
    # An embedded host outlives the uvicorn thread, so records left behind keep validating against its PID.
    run.write_startup_marker()
...
def test_two_cold_starts_keep_rather_than_delete_each_others_modules(tmp_path, monkeypatch):
    # The documented limitation of scoping this back: both keep a cache neither cleaned, the safe direction.
    events = []
    monkeypatch.setattr(
        cache_cleanup, "clear_unsloth_compiled_cache", lambda *a, **k: events.append("clear")
    )
...
def test_a_transient_unlink_failure_is_retried_on_the_spot(tmp_path, monkeypatch):
    # A holder that lets go a moment later must not cost the marker its removal; an embedded host may not exit.
    run.write_startup_marker()
    marker = tmp_path / f"studio-starting-{os.getpid()}.marker"
...
def test_a_marker_that_will_not_delete_stays_retryable(tmp_path, monkeypatch):
    # Once the retries run out, dropping the path loses the only reference and the marker pins the cache.
    run.write_startup_marker()
    marker = tmp_path / f"studio-starting-{os.getpid()}.marker"
    assert run._OWN_STARTUP_MARKERS == [marker]
