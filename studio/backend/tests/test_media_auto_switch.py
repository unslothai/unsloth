...
def test_setup_keeps_the_gate_and_lock_after_the_caller_gives_up(
    ...
    async def _drive():
        # One warm pass first: reaching _start_load cold measured ~2s and counts against the switch
        # budget on purpose. The budget is shrunk to 0.3s because a large one must be waited out, and
        # a cold path then spends it in the scan, so the 503 comes from the scan and not the block
        # under test (that is how this failed on 3.13 while 3.10 passed the same commit).
...
def test_a_cpu_switch_does_not_hold_the_gates_it_cannot_evict_behind(
    ...
    # let an unrelated teardown time the switch out and blocked new chat requests.
    import core.inference.llama_keepwarm as chat
...
def test_an_incomplete_local_pipeline_is_refused_before_the_resident_model_goes(
    ...
    # A hand-copied or interrupted pipeline still carries model_index.json, so treating the directory
    # as complete evicted the resident pipeline before from_pretrained found the missing weights.
...
def test_a_weightless_component_is_still_accepted(catalog, enabled, tmp_path, backend, loads):
    # Schedulers and processors ship no weights and declare *_config.json, so a weight file must not
    # be required; a tokenizer ships none either but is useless without its vocabulary.
    pipeline = tmp_path / "z-image"
...
def test_a_hosted_modular_component_is_checked_against_the_cache(
    ...
    # load_components pulls every repository the modular index names, so a hosted component not on
    # disk would be downloaded after the resident pipeline had gone.
    import core.inference.diffusion as diffusion_module
...
def test_a_media_request_parked_on_a_gate_is_not_read_as_running_chat_work(monkeypatch):
    # Every media route is counted on chat's in-flight counter as well as its own. With the media
    # gates taken first, a request arriving in between passed the open chat gate, incremented chat's
    # _inflight, then blocked on the media gate: discounted on the media side but read as running
    # chat work, so an idle switch answered 409. Driven with the real gates and middleware.
    import core.inference.llama_keepwarm as chat
