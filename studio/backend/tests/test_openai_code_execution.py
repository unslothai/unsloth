...
def test_non_stream_expired_container_retry_returns_one_json_completion(monkeypatch):
    calls: list[dict] = []

    def handler(request: httpx.Request) -> httpx.Response:
        body = json.loads(request.content.decode("utf-8"))
        calls.append(body)
        shell_tool = next(tool for tool in body["tools"] if tool.get("type") == "shell")
        if shell_tool["environment"]["type"] == "container_reference":
            return httpx.Response(
                400,
                json = {
                    "error": {
    ...
    lines = _drive(run_non_stream())
    assert len(calls) == 2
    assert [
        next(tool for tool in body["tools"] if tool.get("type") == "shell")["environment"][
            "type"
        ]
        for body in calls
    ] == ["container_reference", "container_auto"]
    assert len(lines) == 1
    completion = json.loads(lines[0])
    assert completion["id"] == "resp_retry"
