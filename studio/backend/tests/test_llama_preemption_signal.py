...
class TestTheUnsafeWindow:
    ...
    def test_a_request_made_before_the_window_is_hidden_too(self):
        """It had not been acted on yet, so honouring it now would be the very mid-execution abort
...
class TestCancelAndPauseTogether:
    """The stream plumbing takes one event; these two have to share it without becoming each other."""
...
class TestTheCheckpoint:
    ...
    def test_nothing_generated_is_not(self):
        """`continue_final_message` refuses an empty assistant turn, so such an attempt is
        re-issued whole rather than continued.
        """
    ...
    def test_reasoning_alone_is_not_a_resume_point(self):
        """Reasoning is not replayed as assistant content, so it cannot be the prefix a
