...
# The reload-capability checks ask whether idle unload is CONFIGURED, not what the effective TTL
# is, so a stubbed TTL is paired with the configured reader wherever one is set.
...
def test_an_expired_positive_hit_refreshes_before_switching(monkeypatch):
    ...
    # #8389 made a hub-style id a CONCRETE reference, so a name the refresh just proved is not here
    # is refused rather than answered by whatever is resident. The refusal is asserted alongside the
    # original subject (one rescan, then a re-resolve that does not rescan).
    with pytest.raises(HTTPException) as excinfo:
        _run_hook("unsloth/B-GGUF")
...
def test_openai_compat_routes_bound_to_handlers_with_auth():
    # Inserting a helper between a @router.post decorator and its handler silently rebinds the route
    # to the helper and drops its auth dependency (this happened to /messages/count_tokens), which
    # the source-inspection tests above miss because they call the handler directly.
    expected = {
...
def test_local_gguf_entry_rejects_standalone_companions(tmp_path, monkeypatch):
    # _scan_models_dir's standalone-.gguf pass emits an entry for a bare mmproj projector, which is
    # not a servable model, so the resolver must reject it or a switch could load it over the real
    # weights.
    from types import SimpleNamespace
...
def test_resolver_failsafe_on_internal_error(monkeypatch):
    # Resolution is best-effort: any internal failure must fall through to None so the request still
    # serves the loaded model instead of 500-ing.
    def boom():
...
def test_idle_loop_unloads_after_ttl_and_stashes_for_reload(monkeypatch):
    # The headline behavior: with nothing in flight and the TTL elapsed, the loop frees the GGUF
    ...
    async def _drive():
        ...
        # Wall clock with a generous deadline: a fixed 0.2s sleep is enough locally and not on a loaded
        # runner, where the loop's first poll can miss the window entirely.
...
def test_settings_report_idle_unload_active_when_env_backed(monkeypatch):
    # With UNSLOTH_MODEL_IDLE_TTL driving idle-unload while the toggle is off, the settings response
    # must report idle_unload_active so the UI shows the feature as active via env.
    import routes.settings as settings_route
...
def test_responses_system_only_rejected_before_switch(monkeypatch):
    # Instructions-only input normalises to a lone system message, which passes the empty-input
    # check; it must 400 before the switch so an invalid request cannot evict the resident model.
    from fastapi import HTTPException
    from models.inference import ResponsesRequest
...
def test_sibling_revision_resolves_to_its_own_weights(tmp_path):
    # /v1/models advertises only the snapshot dir name, so a durable pin holds one revision hash: a
    # newer snapshot must not strand it, and the old revision must resolve to ITS OWN directory.
...
def test_already_serving_by_path_records_advertised_alias(monkeypatch):
    # A model loaded by local path and requested via an advertised alias resolving to the same path
    # is already serving, but /v1/models would report the path basename unless the alias is recorded
    # as the advertised id on the already-serving return.
...
def test_already_serving_requested_by_path_records_advertised_alias(monkeypatch):
    # The resident short circuit matches on model_identifier, the load path for a manual local load,
    # so answering from it skips the alias recording and a loose .gguf whose scanner alias is not
    # its filename would be advertised as the filename instead.
...
def test_streaming_responses_uses_advertised_id_helper():
    # Streamed /v1/responses envelopes must derive the model id from _llama_public_model_id, not the
    # raw model_identifier: after an auto-switch to a cached HF GGUF the identifier is the snapshot
    # path while the repo id lives in _openai_advertised_id.
    import inspect
...
def test_concurrent_same_target_requests_load_once(monkeypatch):
    # Two concurrent requests for the same unloaded model must load once, not each 409 the other.
    from core.inference import llama_keepwarm as kw
...
def test_idle_alias_reload_preserves_override_via_advertised_id(monkeypatch):
    # The idle stash carries (load_path, quant, advertised_id), and an alias reload must look the
    # override up by the advertised repo id so the user's saved launch flags survive.
    from core.inference import llama_keepwarm as kw
...
def test_load_route_holds_lifecycle_gate(monkeypatch):
    # Lock the manual /load gate against silent revert: the route must wrap the load in
    # inference_lifecycle_gate so idle-unload cannot fire mid-load.
    import inspect
...
def test_model_replacements_recheck_sidecar_swap_before_either_backend_is_unloaded():
    # Both replacement directions drain, then recheck whether a sidecar install reserved the gate
    # meanwhile; that recheck is the last thing that can reject the load, so the destructive cancel
    # must follow it.
    import inspect
...
def test_switch_waiter_deregisters_before_swap_gate_release():
    # A waiter left registered after the swap gate is released would let a swap on another event
    # loop count the finished request as still queued and unload the model it is about to use.
    import inspect
...
def test_swap_waits_until_concurrent_request_finishes_resolving(monkeypatch):
    # The real middleware counts a concurrent same-model request as in-flight before it resolves and
    # registers a target waiter, so treat it as active until its target is known.
    from core.inference import llama_keepwarm as kw
...
def test_env_idle_standalone_reloads_freed_model_with_auto_switch_off(monkeypatch):
    # C3: a standalone UNSLOTH_MODEL_IDLE_TTL (auto-switch OFF) freed the model on idle, so the next
    # request must restore exactly what was freed even though the resolver never runs.
    from core.inference import llama_keepwarm as kw
...
def test_stash_reload_skipped_while_unsloth_model_active(monkeypatch):
    # An Unsloth model loaded after an idle-unload leaves the GGUF slot empty but is the live model,
    # so an unknown /v1 name must NOT resurrect the stale GGUF stash and tear it down.
    from types import SimpleNamespace
    from core.inference import llama_keepwarm as kw
...
def test_local_servable_model_reads_files_not_model_format(tmp_path):
    # HF-cache GGUF snapshots leave model_format unset, so /v1/models must decide the format from
    # the on-disk files; a checkpoint dir needs a root config.json to prove it is not a bare adapter.
    from types import SimpleNamespace
...
def test_retrieve_model_resolves_raw_path_to_advertised_id(monkeypatch):
    # A client caching the legacy absolute .gguf path must still retrieve a loaded auto-switch
    # model: its /v1/models entry is keyed by the advertised repo id, so the raw-path fallback must
    # map the raw id to that advertised id or a loaded model 404s.
    from types import SimpleNamespace
...
def test_chat_streaming_n_gt_1_rejected_before_switch(monkeypatch):
    # Only the non-streaming GGUF path returns multiple choices, so stream=true + n>1 is invalid on
    # every local serving path and both fields are known pre-switch: 400 before the switch.
    from fastapi import HTTPException
...
def test_keepwarm_does_not_stamp_activity_on_401(monkeypatch):
    # The keep-warm middleware runs before auth, so a 401 must decrement the in-flight count without
    # stamping activity, or unauthenticated probes would keep the model warm.
...
def test_messages_does_not_503_before_reload_hook_when_idle_on(monkeypatch):
    # #3: /v1/messages 503'd before the reload hook when auto-switch was off, so a standalone idle
    # TTL could never restore the freed model; the early 503 now defers to any automatic-load trigger.
    backend = _FakeBackend(None)
    rec = _LoadRecorder(backend)
    _wire(monkeypatch, enabled = False, resolves_to = None, backend = backend, recorder = rec)
    _stash(monkeypatch)
...
def test_raw_body_without_model_reloads_freed_model(monkeypatch):
    # #6: a raw body omitting `model` passed None, which skipped the idle-stash reload and 503'd. A
    # non-empty sentinel now lets the reload run while still resolving as unknown.
    backend = _FakeBackend(None)
    rec = _LoadRecorder(backend)
    _wire(monkeypatch, enabled = False, resolves_to = None, backend = backend, recorder = rec)
    _stash(monkeypatch)
...
def test_preview_scope_disables_auto_switch(monkeypatch):
    # #7: the public preview route delegates to the chat handler, so a caller-supplied model must
...
def test_omitted_model_does_not_resolve_to_a_named_gguf(monkeypatch):
    # A raw-body request that omits `model` must never run the resolver, so a downloaded GGUF
    # literally named "default" cannot be switched to.
    backend = _FakeBackend("org/A-GGUF")
    rec = _LoadRecorder(backend)
    _wire(
        monkeypatch,
...
def test_anthropic_mixed_tools_rejected_before_switch(monkeypatch):
    # Combining an Anthropic server tool with a custom client tool is unsupported and must 400
    # before the switch, so the request cannot evict the loaded model only to be rejected.
    from fastapi import HTTPException
...
def test_responses_forcing_tool_choice_without_name_rejected_before_switch(monkeypatch):
    # A forcing-function tool_choice with no name must 400 before the switch, so the streaming path
    # cannot forward a bad choice and an invalid request cannot evict the model.
    from fastapi import HTTPException
    from models.inference import ResponsesRequest
...
def test_chat_confirm_without_stream_rejected_before_switch(monkeypatch):
    # confirm_tool_calls=true + stream=false + local tools is an invalid shape and must 400 before
    # the switch hook.
    from fastapi import HTTPException
...
def test_chat_audio_input_guards_target_before_switch(monkeypatch):
    # A chat request carrying audio_base64 must guard the target before the switch -- audio rides
    # the same companion mmproj as vision -- so a text-only target cannot evict the working audio
    # model. The handler flags require_vision and asks for the projector ALONE: an audio model's
    # projector carries no vision tower, so requiring one would refuse the very models that serve it.
...
def test_completions_rejects_object_prompt_before_switch(monkeypatch):
    # An object prompt like {"prompt": {}} is a deterministic client error, so it must 400 before the
    # switch rather than loading the named GGUF only to be rejected after eviction.
    from fastapi import HTTPException
...
def test_chat_confirm_without_stream_mcp_rejected_before_switch(monkeypatch):
    # mcp_enabled opens the local tool loop on its own, so confirm+no-stream+mcp is the same invalid
    # shape as confirm+no-stream+tools; the old guard only checked explicit tool fields.
...
def test_an_audio_only_target_still_switches_for_an_audio_request(monkeypatch, tmp_path):
    # End to end through the real probe: a Voxtral-style snapshot whose projector declares audio and
    # no vision tower must still be swapped in for an audio request.
    import struct
...
def test_an_audio_request_is_not_refused_for_want_of_a_vision_tower(tmp_path):
    # ultravox / Voxtral / Qwen3-ASR serve audio through a projector with no vision tower, so gating
    # their swap on image capability would 400 a request they can serve.
    import struct
...
def test_responses_and_anthropic_wire_require_vision_from_images():
    # The modality guard must fire on /v1/responses and /v1/messages too, so an image request cannot
    # evict a vision model for a text-only target.
    import inspect
...
def test_count_tokens_forwards_vision_guard_to_switch(monkeypatch):
    # An image /v1/messages/count_tokens naming a text-only GGUF must carry the same require_vision
    # guard as /messages.
    class _Reached(Exception):
        pass
...
def test_chat_count_tokens_refuses_a_generation_that_starts_mid_count(monkeypatch):
    ...
    # starting one inside the last call before the second guard must make the count abandon.
    _switched, counted = _count_tokens_backend(monkeypatch, count = 1234)
    started: list = []
    real = inference_route._llama_status_checkpoint_id
...
def test_chat_count_tokens_ignores_an_mcp_server_the_request_did_not_enable(tmp_path, monkeypatch):
    ...
    # tool_choice "none" would not be a control: mcp_enabled is treated as an explicit ask.
    _switched, counted = _count_tokens_backend(monkeypatch, count = 1234, supports_tools = True)
    _enabled_mcp_server(tmp_path, monkeypatch)
...
def test_audio_generate_is_reload_only(monkeypatch):
    # /audio/generate must not switch to a client-named GGUF: audio-input capability is not a cheap
    # pre-load probe (the mmproj signal cannot tell an audio projector from a vision one), so
    # resolving the client model could evict the working audio model. Only the idle-stash restore runs.
    from models.inference import ChatCompletionRequest
...
def test_note_model_unloaded_clears_reload_stash(monkeypatch):
    # A deliberate unload must drop the idle reload stash so the next /v1 request cannot resurrect
    # the just-unloaded model.
    import core.inference.llama_keepwarm as kw
...
def test_auto_switch_serializes_across_event_loops(monkeypatch):
    # The per-loop asyncio lock cannot serialize two swaps on different event loops in one process;
    # the process-wide gate must, so the two slow loads never overlap on the single model slot.
    import threading
...
def test_acquire_swap_gate_is_cancellation_safe():
    # A waiter cancelled while waiting for the gate (client disconnect mid-swap) must not leak it.
    # The to_thread(acquire) approach would: its worker keeps acquiring after cancel, so the gate
    # is taken but never released.
    async def main():
...
def test_no_model_loaded_detail_appends_hint_only_when_off(monkeypatch):
    # The "no model loaded" errors point at the opt-in auto-switch toggle so a request naming a
    # listed-but-unloaded model is self-explanatory -- but only when it is off.
...
def test_gguf_only_paths_keep_the_generic_error_for_the_resident_non_gguf_model(monkeypatch):
    ...
    resident = "unsloth/Qwen3.5-4B-GGUF"
...
def test_a_carried_ctx_flag_cannot_outrank_a_freshly_saved_context(monkeypatch):
    # The settings page has no control for pass-through flags, so a save carries the stored ones
    # over while writing the edited field. Sent together they reach llama-server with the extras
    # appended last, and its parser takes the final -c, so a stale flag would pin the old context.
    _mock_override_store(monkeypatch)
...
def test_a_standalone_gguf_save_keeps_its_filename_label_launch_flags(override_store):
    # An early build keyed a loose .gguf by the quant label from its filename, and the bare path the
    # picker writes today is read before that key, so writing the bare entry without the legacy
    # flags puts them out of reach of a page that cannot show or restore them.
...
def test_a_fill_never_creates_a_snapshot_path_key_over_a_repo_id_entry(override_store):
    # A fill only adds, so it cannot retire the other spelling the way a save does. Creating the
    # snapshot path key would leave two entries for one quant, and the loader reads the load path
    # before the advertised repo id, so a pre-upgrade browser copy would shadow the newer config.
    settings.set_model_override("unsloth/B-GGUF:Q4_K_M", max_seq_length = 32768)
...
def test_a_tag_that_names_no_quant_resolves_to_the_repo(monkeypatch):
    # A downloaded but unloaded GGUF asked for as org/model:latest missed the resolver, so the
    # switch could not load it. A real quant that is not on disk must still miss, or a swap would
    # serve the wrong weights under the right name.
...
def test_any_finished_download_drops_the_resolver_cache(monkeypatch):
    # Only the API auto-download watcher invalidated, so a GGUF fetched in the Hub UI stayed absent
    # to the cache-only request path. Every worker exits through here.
    import logging
...
def test_invalidating_keeps_the_entries_it_already_had(monkeypatch):
    # The request path reads this cache without scanning, so emptying it leaves no evidence until
    # the rebuild lands; only a completed download invalidates, and that only adds.
...
def test_a_bare_local_id_takes_the_quant_a_plain_load_would(monkeypatch, tmp_path):
    # list_local_gguf_variants orders by descending size, so resolving a bare id to the head could
    # evict a working model and then OOM on an F16 next to a fitting Q4.
...
def test_a_finished_dataset_is_not_recorded_as_a_local_model(monkeypatch):
    # finalize_worker_exit is shared with dataset downloads, and noting one as a local model would
    # refuse a bare /v1 request naming that id and kick off a multi-directory scan for nothing.
    import logging
    import time
...
def test_two_local_paths_differing_only_in_case_are_not_the_same_model(monkeypatch):
    # _loaded_satisfies lowercased the request and every backend identifier, so on a case-sensitive
    # filesystem /srv/models/foo.gguf read as satisfied by a resident /srv/models/Foo.gguf.
    import os
...
def test_two_spellings_of_one_cached_quant_do_not_delete_each_others_save(monkeypatch):
    ...
    def _set_then_sync(model_id, *args, **kwargs):
        ...
        if ident not in wrote:
            wrote.add(ident)
            try:
                # Hold each save just after it stored its own key, so neither looks for aliases until both are
                # there; broken instead when the two are serialized.
...
def test_an_invalidated_index_rebuilds_on_a_host_that_just_booted(monkeypatch):
    # monotonic() counts from boot, so on a host whose uptime is under the TTL an invalidated stamp
    # used to read as recent. Patch the module's reference, not the real time module: a stray warm
    # thread shares that one, and a frozen global clock hangs its duty-cycle arithmetic.
...
# The resident short circuit is the one path that answers without consulting the index, so it
# must never say yes where the pre-existing resident check says no.
_IDENTITIES = [
    "unsloth/Muse-GGUF",
...
def test_clearing_the_box_for_a_quant_survives_the_next_load(override_store):
    # The carry-over copies a legacy bare entry's flags onto the first per-quant save and leaves the
    # bare entry standing, so clearing the box wrote an empty list, the entry was not stored, and
    # the next load fell through to the bare key with the flags still running.
...
def test_the_clear_leaves_the_legacy_row_for_the_quants_still_reading_it(override_store):
    # The bare row is the fallback for every quant with none of its own, so clearing Q4's box must
    # not take Q6's flags with it.
    settings.set_model_override(
        "unsloth/B-GGUF",
...
def test_the_clear_holds_when_another_quant_has_a_row_of_its_own(override_store):
    # The first fix stripped the bare row gated on no sibling row existing, which is the wrong
    # question: a sibling with its own row never reads the bare one.
    settings.set_model_override("unsloth/B-GGUF", llama_extra_args = ["--numa", "distribute"])
...
def test_clearing_one_quant_stops_the_legacy_repo_row_from_answering(monkeypatch):
    # The carry-over copies a legacy bare `repo` row's flags onto the first `repo:QUANT` save and
    # leaves the bare row in place, and a load reads the qualified key first, so an all-default save
    # fell back to a row no page can show and the cleared flags came back. The cleared box is kept
    # as an explicit empty list instead: a row that resolves and supplies no arguments.
    _mock_override_store(monkeypatch)
...
def test_an_empty_save_with_nothing_to_suppress_still_stores_nothing(monkeypatch):
    # The tombstone exists only to stop a fallback: with no row behind this one, "no launch flags"
    # and "nothing stored" are the same thing.
    _mock_override_store(monkeypatch)
    _put("unsloth/B-GGUF:Q4_K_M", llama_extra_args = [])
    assert settings.get_model_override("unsloth/B-GGUF:Q4_K_M") == {}
    assert settings.get_model_overrides() == {}
...
def test_a_fill_never_writes_the_tombstone(monkeypatch):
    # fill_absent_fields only adds what is missing, so an empty list in a fill is the absence of a
    # request, not a clear.
    _mock_override_store(monkeypatch)
    settings.set_model_override("unsloth/B-GGUF", llama_extra_args = ["--top-k", "40"])
...
def test_normalize_keeps_an_explicit_empty_list_only_when_asked(monkeypatch):
    # The two spellings of the same payload, kept apart by one flag: everywhere else an empty list
    # is a field nobody set.
    ...
    # The flag only decides what an EMPTY list means: a list with tokens is stored either way, since
    # the route runs validate_extra_args before this and the flag must not become a second filter.
...
def test_a_family_mlx_lm_remaps_is_listed_in_the_catalog(tmp_path, monkeypatch, model_type):
    # #8748 for Mistral, Devstral, Ministral, Magistral, Codestral and Kimi K2: the loader applies
    # MODEL_REMAPPING first, so predicting the import from model_type withheld every quant these
    # ship. The catalog advertises what is on disk.
    from types import SimpleNamespace
    from utils.hardware import hardware as hw
...
def test_a_gguf_only_endpoint_refuses_a_non_gguf_target_before_loading(monkeypatch):
    # /v1/completions, /v1/embeddings and the Anthropic routes read llama.cpp alone, and loading a
    # non-GGUF model unloads the resident GGUF, so an unguarded switch left them with a 503.
...
def test_a_custom_code_checkpoint_is_not_switchable(tmp_path):
    # An auto_map repo needs trust_remote_code plus the subject's approval fingerprint, and a switch
    # carries neither, so advertising it guarantees a load that fails after evicting the resident.
    from types import SimpleNamespace
...
def test_an_embedding_checkpoint_is_not_switchable(tmp_path):
    # Only the generative chat path consumes non-GGUF entries and /v1/embeddings is GGUF-only, so a
    # SentenceTransformer would be loaded as a language model and fail after the swap.
    from types import SimpleNamespace
...
def test_streaming_responses_refuses_a_non_gguf_swap(monkeypatch):
    ...
    for streaming in (True, False):
        ...
        with pytest.raises(RuntimeError):
            asyncio.run(inference_route.openai_responses(payload, object(), "tester"))
        # Only the streaming branch preflights here, and it must pin GGUF; the non-streaming one
        # delegates to the chat route, whose own switch is right NOT to pin. Asserting the key is
        # absent rather than falsy keeps that distinction.
        if streaming:
            assert captured["gguf_only"] is True
        else:
...
def test_an_audio_only_request_does_not_demand_vision_from_a_non_gguf_target(monkeypatch):
    # The chat route sets needs_vision for audio so a GGUF is asked for its projector; applying that
    # to a non-GGUF target refused the very audio checkpoints that can serve the request.
...
def test_the_advertised_alias_is_cleared_before_a_replacement_load(tmp_path):
    # load_model publishes active_model_name for the new weights, so an alias cleared afterwards
    # leaves a window where a request for the old model is answered by the new ones.
    import inspect
