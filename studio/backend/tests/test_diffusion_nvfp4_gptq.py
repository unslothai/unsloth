...
CPU and hermetic apart from the last test, which needs a Blackwell-class card because it scores the
correction through torchao's own NVFP4 quantiser. The grid maths itself runs anywhere: the two
quantisers are pure torch, so the correction, the step sampling, the Hessians and the activation
amax are all asserted without a GPU.
"""
...
def test_the_correction_lands_on_the_same_grid_as_round_to_nearest():
    """GPTQ's output must be exactly representable in NVFP4, or the packing re-rounds it and the
    correction is silently undone."""
    ...
    # Re-rounding an already-representable weight is a no-op, which is what makes it survive the
    # build's own quantise pass.
...
def test_the_damping_escalates_and_then_raises_rather_than_falling_back_to_rtn():
    """A degenerate Hessian is a failed calibration, not a reason to ship this one layer at
    round-to-nearest with nothing in the metadata saying so."""
...
def test_a_full_schedule_samples_the_named_steps_and_a_short_one_the_same_places():
    ...
    # z-image turbo's 8 steps have no step 37, so the schedule is sampled at the same PLACES.
    assert gq.sampled_steps(8, (0, 12, 25, 37)) == (0, 2, 4, 6)
    # flux schnell, 4 steps.
    assert gq.sampled_steps(4, (0, 12, 25, 37)) == (0, 1, 2, 3)
    # Degenerate schedules still name a step that exists.
...
def test_the_hessians_only_accumulate_on_the_armed_steps():
    ...
    try:
        ...
        # Normalised by that layer's own row count, since a trim gives layers different ones.
...
def test_only_the_layers_that_measure_better_take_their_correction():
    ...
    # The escape hatch takes the LEAST harmful regression first, so the number means something.
...
def test_the_baked_scale_is_the_running_max_over_every_forward():
    ...
    try:
        ...
        # A layer with no activation gets no entry at all rather than an infinite scale: the loader
        # refuses to convert an artifact with a missing scale, which is the right outcome.
...
def test_a_non_finite_forward_never_becomes_the_baked_scale():
    """The flux black-frame latch: one non-finite activation froze into the running scale and every
    later render was black. Baking it would make that permanent."""
    layers = _two_linears()
    acc = gq.ActivationAmaxAccumulator(layers).attach()
    try:
...
# ── CUDA: the correction scored through torchao's own quantiser ───────────────
...
def test_cuda_gptq_beats_rtn_on_a_512_square_layer_through_torchaos_quantiser():
    ...
    # The scorer re-rounds BOTH candidates through torchao, which is the quantiser that packs the
    # artifact: a correction torchao rounds away is not an improvement however good it looks here.
