...
def test_tool_call_carries_signed_thinking_for_private_continuation(monkeypatch):
    ...
    assert call["extra_content"]["anthropic"]["thinking_blocks"] == [
        {
            "type": "thinking",
    ...
    assert "opaque-signature" not in visible_text


def test_signed_thinking_blocks_precede_anthropic_tool_use(monkeypatch):
    captured: dict = {}
    ...
    assistant_blocks = captured["body"]["messages"][1]["content"]
    assert assistant_blocks[0] == thinking_block
