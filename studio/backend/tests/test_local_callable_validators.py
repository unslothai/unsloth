...
def test_the_wrapper_kills_oxlint_against_the_remaining_caller_budget():
    # Python's timeout SIGKILLs only the wrapper, so oxlint has to die inside validate.mjs, against
    ...
    # performance.now() is monotonic ms since process start, the same basis as the caller's timeout;
    # Date.now() would drift from it on a wall-clock step.
