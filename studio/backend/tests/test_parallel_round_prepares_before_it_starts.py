...
def _fast_sizing(monkeypatch):
    """The fake backend has no llama-server to count against, and every call's sizing would
    otherwise wait out an HTTP failure; a flat estimate keeps the starts together so the
    barriers below measure the round, not the harness.
    """
    monkeypatch.setattr(
