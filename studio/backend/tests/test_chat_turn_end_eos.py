...
def test_marker_in_vocab_but_not_in_template_is_ignored():
    # Base/coder model: <|im_end|> is in the vocab but the template does not use it, so it must not become a stop token.
...
def test_gemma4_turn_marker_from_template():
    # Gemma-4 ends turns with <turn|> while keeping a document eos, so <turn|> must be added as a
    # stop token.
    tok = _FakeTokenizer(
...
def test_resolve_using_reads_markers_from_template_but_ids_from_generation_tokenizer():
    # map_eos_token=True: the mapped template remaps <|im_end|> onto the doc-eos id while the
    # original keeps it atomic, so marker STRINGS come from the template and IDS from the original.
...
def test_starling_barred_end_of_turn_from_template():
    # OpenChat/Starling end turns with the BARRED <|end_of_turn|> (distinct from Gemma's
    # <end_of_turn>); eos synced to </s>=2, turn marker at 32000.
...
def test_dict_chat_template_scans_all_variants():
    # Hermes-3 style: chat_template is a {name: template} dict, so detection must scan every variant
    # rather than bail on a non-str container.
