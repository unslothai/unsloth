...
def test_force_float32_rope_demotes_every_component_on_mps():
    # Two components, several modules each: the connectors and the transformer both carry RoPE, so
    # demoting only the first one found would still crash inside the denoise loop.
...
def test_the_video_loader_demotes_rope():
    ...
    # leaves them green while LTX-2 goes back to raising on Metal.
    import ast
...
def test_decoder_sync_survives_a_torch_without_the_memory_reading(monkeypatch):
    ...
    decoder = _FakeDecoder()
    assert dd.install_decoder_sync(_pipe_with(decoder), _target("mps")) is True
    # No budget to compare against, so it must not silently decide the decode is fine: an unbounded
    # Wan decode is what grew past 148 GiB.
...
def test_decoder_sync_survives_a_synchronize_that_raises(monkeypatch):
    # The no-budget fallback synchronises every call, so a torch hiding recommended_max_memory would
    # then raise on every decoder call.
    torch = _mps_torch_without_recommended()
...
def test_physical_ids_are_translated_through_the_visibility_mask(monkeypatch):
    # CUDA_VISIBLE_DEVICES=4,5: physical 4 and 5 are the valid picks and torch sees 0 and 1, so
...
def test_several_cards_resolve_to_the_one_with_the_most_free_vram(monkeypatch):
    # The mixed box this exists for: ordinal 0 is the SMALL card, so taking the first id lands on
    # the GPU that cannot hold the checkpoint.
    torch = _make_torch(
        cuda_available = True,
        capability = (8, 0),
        device_count = 2,
...
def test_the_capability_probe_asks_about_the_selected_card(monkeypatch):
    ...
    # No selection probes with NO argument: a stub or older build that takes none would otherwise
    # raise into the fp16 fallback.
    seen.clear()
...
def test_the_device_scope_restores_the_previous_card(monkeypatch):
    # Route preflights run on a pooled executor, so a pin left set there is inherited by the NEXT
    # request on that thread, including an automatic one.
    calls: list = []
...
def test_the_rocm_bf16_probe_asks_about_the_selected_card(monkeypatch):
    # is_bf16_supported() takes no device argument, so asking about the selected card means making
    # it current; otherwise a bf16-capable pick behind an older default goes to fp32.
...
def test_the_placed_ordinal_records_the_card_an_automatic_load_used(monkeypatch):
    # /images/generate runs on a pooled worker, so a pinned load leaves that thread on its card and
    # a later automatic load has no ordinal to re-pin with; the landed card is recorded separately.
    current = [3]
...
def test_a_multi_card_pick_declines_to_rank_when_ranking_is_barred(monkeypatch):
    # The plan routes must not open a CUDA context while a trainer holds the cards; validating and
    # translating the ids costs none, so a bad pick is still refused at the plan.
