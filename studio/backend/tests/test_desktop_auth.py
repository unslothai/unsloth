...
def test_health_response_reports_desktop_capability_fields(monkeypatch):
    ...
    # DEVICE alongside the two it is set with: /api/health now waits on
    # ensure_hardware_detected(), which re-detects (and so recomputes CHAT_ONLY)
    # while DEVICE is None. CPU + "mlx_unavailable" is the state an Apple
    # Silicon host without a usable MLX stack actually ends detection in.
