...
def test_load_model_waits_for_old_worker_vram_before_spawn(monkeypatch):
    """A credited worker handoff must settle dead-process VRAM before allocation."""
    import types
    ...
    monkeypatch.setattr(o, "_ensure_subprocess_alive", lambda: True)
    monkeypatch.setattr(o, "_cancel_generation", lambda: events.append("cancel"))
    monkeypatch.setattr(o, "_shutdown_subprocess", lambda *a, **k: events.append("shutdown"))
    monkeypatch.setattr(
        o,
        "_wait_for_worker_vram_settle",
        lambda _since, **_kwargs: events.append("settle"),
    )
    monkeypatch.setattr(o, "_spawn_subprocess", lambda _cfg: events.append("spawn"))
    monkeypatch.setattr(
        o,
        "_wait_response",
        lambda t, timeout = 300.0: {"success": True, "model_info": {"identifier": "new"}},
    )
    monkeypatch.setattr(orch_mod.time, "sleep", lambda *_a, **_k: None)
    monkeypatch.setattr(orch_mod, "prepare_gpu_selection", lambda *a, **k: ([0], "sel"))
    ...
    ok = o.load_model(types.SimpleNamespace(identifier = "new", gguf_variant = None))

    assert ok is True
    assert events.index("shutdown") < events.index("settle") < events.index("spawn")


def test_load_model_waits_for_crashed_worker_vram_before_spawn(monkeypatch):
    """A dead process handle can still have driver reclaim in flight."""
    import types
    ...
    def clear_dead_worker(*_args, **_kwargs):
        events.append("shutdown")
        o._proc = None
        return True

    monkeypatch.setattr(o, "_shutdown_subprocess", clear_dead_worker)
    monkeypatch.setattr(
        o,
        "_wait_for_worker_vram_settle",
        lambda _since, **_kwargs: events.append("settle"),
...
def test_worker_vram_settle_uses_xpu_capable_live_telemetry(monkeypatch):
    samples = iter(
        [
            {"devices": [{"index": 0, "vram_total_gb": 32.0, "vram_used_gb": 24.0}]},
            {"devices": [{"index": 0, "vram_total_gb": 32.0, "vram_used_gb": 16.0}]},
            {"devices": [{"index": 0, "vram_total_gb": 32.0, "vram_used_gb": 16.0}]},
        ]
    )
    calls = []
    ...
    monkeypatch.setattr("utils.hardware.get_visible_gpu_utilization", utilization)
    monkeypatch.setattr(orch_mod.time, "sleep", lambda *_a, **_k: None)

    InferenceOrchestrator._wait_for_worker_vram_settle(
        time.monotonic(),
        max_wait = 1.0,
    )
...
def test_worker_vram_settle_does_not_accept_delayed_low_plateau(monkeypatch):
    samples = iter(
        [
            {"devices": [{"index": 0, "vram_total_gb": 24.0, "vram_used_gb": 22.0}]},
            {"devices": [{"index": 0, "vram_total_gb": 24.0, "vram_used_gb": 22.0}]},
            {"devices": [{"index": 0, "vram_total_gb": 24.0, "vram_used_gb": 12.0}]},
            {"devices": [{"index": 0, "vram_total_gb": 24.0, "vram_used_gb": 12.0}]},
        ]
    )
    calls = []
...
def test_worker_vram_settle_requires_full_expected_capacity(monkeypatch):
    """A stable partial reclaim cannot satisfy a credited handoff target."""
    samples = iter(
        [
    ...
    settled = InferenceOrchestrator._wait_for_worker_vram_settle(
        time.monotonic(),
        expected_free_gb = {0: 20.0},
        max_wait = 1.0,
    )
...
def test_load_model_refuses_handoff_when_expected_capacity_never_settles(monkeypatch):
    """The route's aggregate target is mandatory even when no worker handle remains."""
    import types
    ...
    monkeypatch.setattr(
        o,
        "_wait_for_worker_vram_settle",
        lambda _since, **kwargs: observed.append(kwargs["expected_free_gb"]) or False,
    )
    monkeypatch.setattr(
        o,
        "_spawn_subprocess",
        lambda _cfg: pytest.fail("must not spawn below the admitted post-handoff capacity"),
    )

    with pytest.raises(RuntimeError, match = "was not released"):
        o.load_model(
            types.SimpleNamespace(identifier = "new", gguf_variant = None),
            post_handoff_expected_free_gb = {0: 15.5},
        )

    assert observed == [{0: 15.5}]
