...
class TestSandboxEnvIsolation:
    ...
    def test_bash_userland_dirs_precede_system32(self, monkeypatch, tmp_path):
        # `bash -c` is non-login, so Git's usr\bin never joins PATH and must sort ahead of System32's FIND.EXE.
        from core.inference.tools import _build_safe_env
    ...
    def test_host_git_dir_appended_after_curated(self, monkeypatch, tmp_path):
        # #7317: Windows Git lives under Program Files, so bare `git` appends the host git dir, after the prefix.
        import core.inference.tools as tools_mod
        from core.inference.tools import _build_safe_env
    ...
    def test_scan_past_untrusted_git_shim(self, monkeypatch, tmp_path):
        ...
        (trusted_git / "git.EXE").write_text("")
        shim = tmp_path / "scoop" / "shims"
        shim.mkdir(parents = True)
    ...
    def test_bypass_env_installs_sitecustomize_path_shim(self, tmp_path):
        # Bypass mode installs the same /mnt/data remap shim, else those writes work only in normal mode.
...
class TestBashBlocklistPosition:
    ...
    def test_sed_exec_payload_blocked(self):
        # sed's `e COMMAND` hands COMMAND to the shell: a real command position inside the script argument.
    ...
    def test_sed_exec_payload_continues_past_backslash(self):
        # An `e` payload ending in a backslash carries onto the next line, and quoting hides it from raw text.
    ...
    def test_sed_under_find_exec_wrapper_blocked(self):
        # env/timeout/nice forward -exec to their target; reading the token after the flag hid the invocation.
    ...
    def test_sed_script_past_the_scan_window_fails_closed(self):
        # A flat argument cap padded the caller controls: 128 options pushed the real script out of view.
    ...
    def test_sed_sandbox_and_posix_modes_not_blocked(self):
        # --sandbox disables e/r/w and --posix drops the GNU extension `e` needs, so sed exits 1 (abbreviations too).
    ...
    def test_sed_scan_stops_at_the_find_exec_terminator(self):
        # `-exec CMD ... +` / `;` is a COMPLETE action, so running past it read the next `-e` as the program.
    ...
    def test_quoted_separator_operand_does_not_end_the_sed_scan(self):
        # shlex strips quoting, so a FILE operand spelled `';'` arrives as a separator does and hid the real `-e`.
    ...
    def test_redirection_is_not_the_sed_script(self):
        # The shell performs and REMOVES a redirection, but those words stayed in the token list and the
        # first was taken for the positional script. Verified on GNU sed 4.9: every form creates MARKER.
    ...
    def test_compound_operator_ends_the_sed_scan(self):
        # shlex's punctuation_chars emits a RUN of operator characters as one token, so bash's `
    ...
    def test_script_file_source_ends_a_continuation(self):
        # A source BOUNDARY closes a continuation open across it: `sed -e '1a\' -f /dev/null -e 'e touch MARKER'` fires.
    ...
    def test_xargs_supplied_sed_program_fails_closed(self):
        # xargs appends stdin to the command it builds (or substitutes -I), so the program need not be in the text.
    ...
    def test_only_a_real_assignment_rebinds_a_sed_program(self):
        # An assignment-shaped word that is no shell-state assignment leaves `$p` as it was; recording it hid a payload.
    ...
    def test_exec_flags_only_forward_from_a_command_word(self):
        # Any token spelled `fd` or `find` used to turn on exec-flag forwarding, so a later `-x` read as an exec flag.
    ...
    def test_redirection_standing_where_an_option_value_goes(self):
        # The shell removes a redirection wherever it sits, so an `-e` shaped like one takes the word BEHIND it.
    ...
    def test_late_program_flag_and_the_positional_are_alternatives(self):
        # Which of the two sed compiles depends on permutation, so joining them let `safe` swallow the payload.
        assert "rm" in self._find()("sed '1e rm -f victim' input -e safe")
        assert "rm" in self._find()("sed '1e rm -f victim' input -e p")

    def test_find_batches_only_at_a_real_plus_terminator(self):
        # find closes the batched form at `{} +` only, so a `+` elsewhere is an argument, while a quoted `';'` ends it.
    ...
    def test_glob_in_the_sed_program_position_fails_closed(self):
        # bash expands the word after this scan, so with a file named `1e rm -f victim` the program of `sed *` is it.
    ...
    def test_ansi_c_newline_still_ends_a_sed_comment(self):
        # ANSI-C decoding flattened the word's whitespace, and a sed comment ends at exactly that newline.
    ...
    def test_assignment_inside_a_function_body_does_not_persist(self):
        # bash has not run the function body and may never, so the assignment in it is cleared, not guessed.
        payload = "p='1e rm -f victim'"
    ...
    def test_exec_forwarding_survives_keywords_and_wrappers(self):
        # Scoping the exec-flag scan to a command word must not lose position at a keyword or a wrapper's operands.
    ...
    def test_live_expansion_matched_after_the_lexer_unescapes_it(self):
        # shlex removes the escaping as it splits, so an exact comparison read a generated program as already read.
        assert is_high_risk_tool_call(
    ...
    def test_quoted_redirection_operand_is_data(self):
        # The shell removes a redirection, but a QUOTED one is a word: `sed -f '>prog'` takes it as the script FILE.
    ...
    def test_ansi_c_apostrophe_keeps_the_program_intact(self):
        # An apostrophe in the decoded word sent it down the flattening path, destroying the comment's newline.
    ...
    def test_fd_attached_and_end_of_option_exec_flags(self):
        # fd takes the command attached to the short option, and only exact spellings opened an action.
    ...
    def test_fd_exec_flags_reach_the_child_command(self):
        # fd runs its -x / -X / --exec child as find runs -exec, but only find's spellings were scanned.
        assert "rm" in self._find()("fd -x rm -rf x")
    ...
    def test_exec_wrapper_chain_past_the_hop_budget_fails_closed(self):
        # Running out of wrapper-hop budget was reported as "no child", which reads safe, so the chain is blocked.
    ...
    def test_sed_behind_a_wrapper_option_with_an_operand(self):
        # A wrapper option whose value is a SEPARATE token consumes it; without that `env -u FOO sed` blamed FOO.
    ...
    def test_sed_program_held_in_a_variable(self):
        ...
        # The lexer splits `p=$(...)` at the `(`, and the leftover binding substituted a bare `$` for the
        # program, dressing an unread script as a plausible literal. The blocklist reports no name here.
        assert self._find()("p=$(printf '1e rm -f victim'); sed \"$p\" input") == set()

    def test_sed_program_uses_the_last_assignment_before_it(self):
        # bash expands `$p` to the most recent binding BEFORE the reference; a first-wins map kept the earliest.
        assert "rm" in self._find()("p='1,3p'; p='1e rm -f victim'; sed \"$p\" input")
    ...
    def test_sed_program_built_by_a_parameter_transformation(self):
        # `${p#x}` and its family are not modelled, so the program is UNREAD, not harmless (GNU sed 4.9 fires it).
