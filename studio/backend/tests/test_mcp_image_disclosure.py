...
def test_mapping_is_never_inferred_from_a_field_name():
    with pytest.raises(McpImageDisclosureError, match = "not discovered"):
        validate_image_input_mappings(
            [{"tool": "missing", "field": "imageBase64", "encoding": "base64"}],
            [_tool("other", "imageBase64")],
        )


def test_only_one_mapping_per_tool_is_allowed():
    with pytest.raises(McpImageDisclosureError, match = "one image input mapping"):
        validate_image_input_mappings(
            [
                {"tool": "inspect", "field": "first", "encoding": "base64"},
                {"tool": "inspect", "field": "second", "encoding": "data_url"},
            ],
            [
                {
                    "name": "inspect",
