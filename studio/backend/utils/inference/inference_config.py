...
def load_inference_config(
    ...
    """Load inference params for a model: model-specific YAML, then family defaults (inference_defaults.json), then default.yaml. An explicitly true/false ``thinking_mode`` lets a tier's optional ``sampling_modes`` block override that tier's flat values; ``None`` keeps the historical flat resolution. Returns temperature/top_p/top_k/min_p and so on."""
...
def _recommended_sampling(model_id: str, thinking_mode: Optional[bool] = None) -> Dict[str, Any]:
    """Per-model recommended sampling, resolved through :func:`load_inference_config` and cached by model id and reasoning mode. Only the fields the Chat UI adopts (:data:`_UI_RECOMMENDED_FIELDS`) are recommended, each validated finite and in range. With ``thinking_mode`` set the answer can differ from the ``.inference`` block of load/status, which is always resolved with no mode and keeps advertising the flat family row."""
    if not model_id:
        return {}
    try:
...
def resolve_effective_sampling(
    ...
    *,
    ...
    """Resolve the effective sampling params for a request. ``explicit`` maps each field in :data:`SAMPLING_FIELD_NAMES` to the client-sent value, or ``None`` when the client omitted it. Precedence, highest first: an operator ``UNSLOTH_SAMPLING_*`` pin, the client's explicit value, the per-model recommendation, then the static schema default. When ``fill_defaults`` is False a field with none of the first three is omitted rather than set to the static default, so a raw proxy body (``/v1/completions``) keeps llama-server's own default for that field. ``thinking_mode`` is three-valued: True/False select the per-mode recommendation, None preserves the historical flat preset."""
