...
dependency of the chat backend. That is a real cost on any environment that installs a
subset of Studio's requirements: it took the repo's own Source lint job red, where
``tests/studio/load_freeze/test_load_orchestrator.py`` imports the backend and PyYAML is
not installed.

``utils.models.gguf_metadata`` re-exports both names, so the existing callers are
