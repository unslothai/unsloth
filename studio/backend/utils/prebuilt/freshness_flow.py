...
def _fetch_newest_published_release_blocking(
    ...
    """Newest published (non-draft, non-prerelease) release object for `repo`, by ``published_at``. Resolves "latest" the way the installers do, NOT via GitHub's ``/releases/latest`` pointer, which sorts by commit date and can lag the build the installer installs, making detection and apply disagree (the downgrade / sticky-banner bug). None on any failure (offline, rate-limited), and skipped outright while the rate-limit lockout is in force."""
    import os
    import urllib.error
