...
def latest_cached_dataset_snapshot(
    ...
    newest_mtime = -1.0
    for entry in hf_cache_scan.iter_repo_cache_dirs("dataset", repo_id):
...
def cached_dataset_training_files(
    ...
    *,
    ...
    same_suffix = [path for path in candidates if path.suffix.lower() == suffix]
    subset_lower = subset.lower() if subset else ""
    split_lower = train_split.lower()
    split_matches = [
        path
        for path in same_suffix
        if _matches_label(snapshot, path, split_lower)
        and (not subset_lower or _matches_label(snapshot, path, subset_lower))
    ]
    if split_matches:
    ...
    subset_matches = [
        path
        for path in same_suffix
        if subset_lower and _matches_label(snapshot, path, subset_lower)
    ]
    if subset_matches:
        return [str(path) for path in subset_matches]
    return [str(path) for path in same_suffix]
