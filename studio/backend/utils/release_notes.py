...
"""Release notes for the update popup, taken from the newest GitHub release. Only the announcement is shown: the install instructions, the generated "What's Changed" and "New Contributors" lists, the "Full Changelog" line and the appended build provenance are stripped, wherever they were written. The fetch is lazy, cached, and skipped when update checks are off."""
...
# Unauthenticated callers get 60 requests an hour per IP, so a spent address backs off instead of retrying every 5 minutes. Used when the response carries no reset to wait for. The same window the freshness and changelog fetches use: one quota, one policy.
...
def _markdown_lines(text: str) -> list[str]:
    """``text`` split the way CommonMark ends lines. str.splitlines also breaks on U+2028, U+2029, NEL, vertical tab and form feed, none of which end a Markdown line: one in prose would otherwise turn the text after it into a heading and cut the announcement there."""
    return text.replace("\r\n", "\n").replace("\r", "\n").split("\n")
...
def scan_blocks(text: str):
    """Walk `text` as CommonMark blocks, yielding `Text` and `Heading` events. Only headings the renderer would show are `Heading`: one inside a fence, a comment, a raw HTML block or a list item arrives as ordinary text."""
...
def _is_platform(title: str) -> bool:
    """Whether `title` is one of the install block's per-platform headings. The platforms are separated by a slash as readily as by a comma, spaced or not, so the separators are read as one thing."""
...
def _http_error_source(
    ...
    if error.code in (403, 429):
        # One verdict for the local popup lockout and the shared one, from the same
        # helper the freshness and changelog fetches use: Retry-After, then a spent
        # primary quota, then GitHub's throttling text, else the plain back-off; and
        # None for a 403 that was a permission refusal, which is no lockout at all.
        # The helper already bounds the wait to one window, so a skewed header cannot
        # park the popup for as long as it liked.
...
def select_release(payload: Any) -> Release | None:
    """The newest published release in a GitHub releases response. Ordered by publication, never by tag: `v0.1.60-beta` was published after `v0.1.527-beta`, so sorting the tags numerically picks the wrong one."""
...
def _next_fence_state(open_fence: str | None, marker: str, rest: str) -> str | None:
    """Track the open fence marker. A closer must be the same character, at least as long, and carry nothing after it, so neither a ``` sample nor a ```` line with trailing text ends a ```` block early, while an opening fence may still have an info string. Only spaces and tabs count as nothing: other Unicode whitespace is content."""
...
def _hidden_structure(line: str, marker: str = "") -> str:
    """`line` as list tracking sees it once the renderer hides its text. The block renders nothing but still sits at its own column, so it closes the items to its left; only the indentation survives, since its contents are not Markdown. `marker` is the part of the line opening the item it is content of, which survives with it."""
    if marker:
...
def _interrupts_paragraph(line: str) -> bool:
    """Whether `line` starts a block that can break into an open paragraph. A quote marker always can. A list item only when it has content, and an ordered one only when it starts at 1; anything else is paragraph text."""
...
def _item_content(line: str, after_paragraph: bool) -> str:
    """`line` read from the content column of a list item that opens on it. A block written as an item's first content sits inside that item, so ``- ```` opens a fence. The padding is capped as `_open_lists` caps it, or ``-     ```` would read as a fence rather than the indented code it is. A swallowed marker opens no item, so its line is returned whole, as is one four columns past its container. Ported to the frontend as `itemContent` in markdown-list-columns.ts."""
...
def _may_be_lazy(line: str) -> bool:
    """Whether `line` can continue a paragraph it is indented out of. Only plain text can: a heading, fence, break or HTML block closes the item instead. An underline may never be lazy, so `===` left of an open item is more of the item's paragraph, and nor may a definition interrupt one. Three dashes still close the item, as `_INTERRUPTS` reads them as a break."""
    return (
...
def _continues_paragraph(line: str, column: int) -> bool:
    """Whether `line` reads as more of a paragraph open in its container. Measured from `column`, where the container's content starts: four columns past it the line is indented code, which may not interrupt a paragraph, so indentation alone never closes the one above it."""
...
def _close_dedented(
    ...
    """`columns` with every item `line` is written to the left of closed. Read inside the container, not from the margin: lazy text of the item's own paragraph leaves the item open rather than closing it."""
...
def _lazy_marker(line: str, state: _ListState, after_paragraph: bool, quoted: bool) -> bool:
    """Whether a marker-shaped `line` is really text of the paragraph above it. Only a marker inside the paragraph's own item interrupts it; one to the left closes that item. A quote owns its paragraph, so a marker written outside the quote opens a list of its own."""
    item = None if _THEMATIC_BREAK.match(line) else _LIST_ITEM.match(line)
...
def _open_lists(
    ...
    """The list items still open after `line`. A dedented line closes an item unless it is a lazy paragraph continuation. A new marker nests deeper and replaces a sibling. `quoted` marks a paragraph the quote above owns, so a marker outside the quote opens a list of its own."""
    columns = state.columns
    if not line.strip():
...
def _strip_raw_html(line: str, open_block: int | None) -> tuple[str, int | None]:
    """Drop the parts of a line inside a raw block, and return the open block. The state is the index of the open block in `_RAW_BLOCKS`, or None."""
