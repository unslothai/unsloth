...
# Whether the GGUF tensor table lists one exact tensor name, keyed by (file cache
# key, tensor name). None = unreadable, so callers can fail open. Backs the MTP
# drafter launchability check, which the memory-estimate route asks on a path the
# panel fires on every settings change.
