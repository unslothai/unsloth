...
Defaults mirror install.sh: on for macOS and Windows, opt-in on Linux via ``UNSLOTH_STUDIO_NATIVE_TLS=1`` (distro OpenSSL configurations vary), opt-out anywhere with ``0``. Explicit ``SSL_CERT_FILE``/``REQUESTS_CA_BUNDLE`` keep working but become additive rather than exclusive, since truststore keeps the OS anchors alongside them; ``0`` is the way back to a bundle being the only trust root.
...
# Children that cannot import this module (the `python -c` probes and prebuilt_core.py, each supplying os, sys and _TRUSTSTORE_VENDOR itself) carry the gate as source, generated from the same constants so it cannot drift from native_tls_enabled().
_INLINE_GATE = """\
...
def activate_native_tls() -> bool:
    ...
    if not native_tls_enabled():
        return False
    # uv's rustls ignores in-process injection (uv >= 0.11 reads UV_SYSTEM_CERTS, older reads UV_NATIVE_TLS). Mirror one value across both: uv takes either as an opt-in, so an opt-out in one spelling must carry to the other.
