...
# Limit concurrent inference. Registered immediately after LoggingMiddleware so
# Starlette wraps the gate INSIDE both SecurityHeadersMiddleware and
# MaxBodyMiddleware (add_middleware inserts at user_middleware[0]; later adds
# end up OUTER). With this placement:
#   - an oversized POST is 413'd by MaxBodyMiddleware before a concurrency
#     slot is ever acquired, so a slow uploader cannot hold the only slot
#     for the duration of their upload;
#   - a 429 generated by the gate on overflow passes back through
#     SecurityHeadersMiddleware on its way to the client, so it carries the
#     same CSP / X-Content-Type-Options / Referrer-Policy / Permissions-Policy
#     headers as every other response.
# LoggingMiddleware remains inner of the gate, so the request duration it
# records measures post-acquire processing time only -- matching the original
# logging contract.
