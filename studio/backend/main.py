...
# A master root pointed at the legacy path still owns runtimes beside it, so the
# equality alone would skip the export and leave unsloth_zoo on ~/.unsloth/llama.cpp
# while the install put llama.cpp under the root.
if _STUDIO_ROOT_RESOLVED != _LEGACY_STUDIO_ROOT or _MASTER_ROOT is not None:
    if not os.environ.get("UNSLOTH_STUDIO_HOME"):
        os.environ["UNSLOTH_STUDIO_HOME"] = str(_STUDIO_ROOT_RESOLVED)
    # Same derivation as run.py: the runtimes sit at the master root, beside
    # studio/, and marking the wrong directory managed would make the bundled
    # path look like an immutable user override.
