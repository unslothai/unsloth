...
# A request preference, deliberately NOT in VALID_TRANSPORTS: the on-disk .transport marker must keep naming the writer that produced a partial, or a resume picks the wrong strategy.
TRANSPORT_AUTO = "auto"
...
def incomplete_blob_hash(name: str) -> Optional[str]:
    """Return the logical HF blob hash represented by a partial filename. huggingface_hub historically wrote ``<etag>.incomplete``; version 1.18 changed the writer to a process-unique ``<etag>.<8 hex>.incomplete`` path before atomically moving it into place. Hub etags used as cache blob names are Git SHA-1 or LFS SHA-256 hex digests, which lets us remove the nonce without mis-parsing an arbitrary legacy filename containing a dot."""
...
def hf_partials_are_resumable(hub_cache: Optional[str] = None) -> bool:
    """Whether an interrupted download leaves bytes the next attempt can reuse. Up to 1.17 huggingface_hub appended to a shared ``<etag>.incomplete`` and restarted from its length over a Range request; 1.18 moved the writer to a process-unique ``<etag>.<nonce>.incomplete``, opened ``"wb"`` and unlinked in a ``finally`` (huggingface/huggingface_hub#4228), so an interrupted file is refetched from zero and whatever partial survives a hard kill can never be read again. An unreadable version answers True: not knowing which writer is installed is not grounds for deleting bytes that may still be resumable. On 1.18+ this also asks whether the download worker will put the 1.17 writer back (:mod:`hub.utils.resumable_partials`), since a restored resumer makes partials reusable again; that half turns on the filesystem the partial is on, so *hub_cache* names the root being asked about, because Unsloth remembers several and they need not lock alike, and without it a selected cache on a network mount would condemn a local cache's partials to the abandoned-partial sweep. Omitting it asks about the cache in force, which is where a new download lands. Deliberately not cached here: the verdict is a fact about a filesystem, not about a path, so a result keyed on the path alone survives a remount at the same name and outlives a momentary failure to probe. The expensive part, the lock probe, is cached in :mod:`hub.utils.resumable_partials` against the mounted device instead, and a probe that could not run raises rather than answering, so nothing remembers a bad moment."""
    try:
...
def partial_is_resumable(name: str, hub_cache: Optional[Path | str] = None) -> bool:
    """Whether any later attempt could append to this particular partial. Two conditions, and the layout half matters on its own: a nonce partial is private to the process that created it, so even a legacy writer will not reopen it, and that combination is reachable whenever caches are shared across environments, which this repo's own pins produce (Python 3.10+ takes hub >= 1.23, older takes 0.36.2, one cache between them). *hub_cache* is the root the partial lives under; callers walking more than one cache must pass it, since the answer is partly a property of that root's filesystem."""
...
def _safe_is_dir(path: Path, scan_errors: Optional[list] = None) -> bool:
    """``Path.is_dir()`` returning False instead of raising when the path or a parent is unreadable (e.g. a restricted ``~/.cache/huggingface/hub``), so cache enumeration skips that root rather than 500ing. ``scan_errors`` collects the swallowed error: a caller that only wants the dirs does not care, but "we could not even stat the root" and "the root is not there" are different answers to a hydrating job, and the first is not evidence the cache was deleted. os.stat, not Path.is_dir(): as of 3.14 is_dir() answers False for EVERY OSError instead of raising some and suppressing others, so the handler below could never see a permission or network-mount failure and the root was recorded as a measured absence."""
    try:
...
def blob_bytes_present(path: Path) -> int:
    """Sparse-aware on-disk size: XET/``hf_transfer`` ``.incomplete`` partials report a full ``st_size`` while only some blocks are allocated, so prefer ``st_blocks``, falling back to ``st_size`` where it is unreported (Windows, some network filesystems)."""
    st = path.stat()
...
def _holds_no_data(path: Path) -> bool:
    """Whether the file has no allocated extent anywhere, asked of the kernel rather than inferred. ``SEEK_DATA`` past the end of the last extent is ENXIO, so a file with nothing written raises on the very first seek; every other answer, an unsupported seek or an unreadable path, leaves the caller's size fallback in charge."""
...
def snapshot_selection_key(snapshot: Path) -> tuple[float, str]:
    """The one ordering every snapshot selector uses: mtime, then resolved path. mtime alone is not a total order, and each selector broke ties by its own iteration order (frozenset vs iterdir), so the inventory row and the variant picker could name different snapshots. The path breaks ties identically."""
    try:
...
def latest_snapshot_dir(repo_dir: Path) -> Optional[Path]:
    """Newest immediate child of ``repo_dir/snapshots``, or None. mtime is the signal huggingface_hub's from_pretrained resolves to; ties fall to ``snapshot_selection_key`` so every caller names the same directory."""
...
def snapshot_has_broken_symlinks(snapshot: Path) -> bool:
    """Whether ``snapshot`` links to a blob that is not finalized. Scoped to one snapshot on purpose: a ``.incomplete`` blob under ``blobs/`` belongs to whichever revision or scoped file set is fetching it, and the cache is shared, so its presence says nothing about the revision being validated, whereas a link this snapshot owns whose target is not there yet does. Windows hydrates the cache with copies rather than links, so there is nothing to dangle: a half-fetched file is simply absent, which the payload inventory catches instead."""
    try:
...
def scoped_delete_root(repo_type: str, repo_id: str, cache_path: Optional[str]) -> Optional[Path]:
    """Resolve the single cache root a delete of this repo may touch. Returns the active hub cache when *cache_path* is falsy, the owning cache root when *cache_path* points inside a known cache, or ``None`` when *cache_path* is set but not inside any known cache (caller should reject). This keeps a delete of one inventory row from removing copies in other, previously selected caches."""
    from utils.hf_cache_settings import get_hf_cache_paths
...
def resolve_delete_target_root(
    ...
    """Pick the single cache root a delete of this repo should target. An explicit *cache_path* wins (``None`` when it is not a known cache, so the caller can reject it). Otherwise prefer the active cache when it holds a copy, else the sole cache that does, so a model that lives only in a previously selected cache stays deletable while other caches are untouched."""
    if cache_path:
