...
The cache digest isolates state for the same repo across selectable Hub caches. The ``<key>`` mirrors HF's cache dir naming (models--<owner>--<name>, plus ``--variant--<variant>`` for a GGUF variant, datasets--<owner>--<name>) while keeping manifest, cancel-marker and atomic-write temp filenames inside common filesystem basename limits; very long repo IDs use a stable hash instead.

Path accessors are read-only unless a writer passes ``create=True``, and all return ``Optional[Path]``, yielding ``None`` when directory creation fails so callers fall through to existing on-disk-only behavior.
"""
...
def variant_is_hashed_fragment(variant: str) -> bool:
    """Whether *variant* is a digest fragment rather than a variant name. An unreadable state file falls back to its own filename, which for a variant :func:`_variant_fragment` had to hash is this digest; it names nothing, since it cannot be spelled back and re-keying would hash the digest again. Both tags count, the older one carrying no "@"."""
...
def _state_repo_key(
    ...
    *,
    ...
    """An injective write key, or the pre-migration key for reads. Repositories ending in ``variant`` otherwise alias a shorter repository's legacy double-hyphen variant, so both collision sides use hashes for new state."""
...
def variant_filename_prefix(
    ...
    *,
    ...
    """Lowercased prefix every variant-keyed state file for this repo shares. The single source the download_manifest enumerators match against, so the scheme in :func:`_entry_key` cannot drift from them silently."""
...
def _variant_fragment(
    ...
    *,
    ...
    # Double-hyphen variants can alias a repository component plus the --variant-- delimiter, so give them an injective hashed fragment.
...
def variant_key_fragments(variant: str) -> tuple[str, ...]:
    """Every ``--variant--`` fragment this variant can be stored under. A variant that cannot be spelled literally in a filename is stored hashed, so a reader recovering identity from the filename alone recovers the hash and not the variant; callers holding the variant string need these to match it."""
    fragments: list[str] = []
...
def legacy_cache_scope_name(hub_cache: str | Path) -> str:
    """The pre-``resolve`` scope name, for reads only. State written while ``resolve`` was unavailable (or by an older build) sits under this digest, and readers probe it after the canonical one so a digest change never orphans a manifest or cancel marker; writers only ever use :func:`cache_scope_name`. Must be given the caller's own spelling of the path: handed an already canonicalized one it returns the canonical digest and recovers nothing."""
    try:
...
def cache_scope_names(hub_cache: str | Path) -> tuple[str, ...]:
    """Every scope dir this cache's state can be filed under, canonical first. A single entry whenever the resolved and unresolved spellings agree, which is every path that resolves to itself, so on POSIX and on Windows for anything that is not a junction, a mapped drive, an 8.3 short name or a OneDrive redirect. The second entry is the read-side recovery for state filed under the unresolved spelling."""
