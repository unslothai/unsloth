...
def test_cached_models_scan_keeps_a_complete_pipeline_loadable(monkeypatch, tmp_path):
    row = _diffusion_scan(
        ...
        [
            ...
            # lone "-00001-of-00002" shard with no index, which declares two shards, ships one,
            # and is not loadable at all: with no index diffusers reads the component as
            # unsharded and asks for the plain "diffusion_pytorch_model.safetensors", never the
            # numbered file. The scan now judges that correctly and calls it partial, so the old
            # fixture was asserting that an unloadable pipeline reads as loadable.
