...
def _resolve_hub_token(header_token: HfTokenArg, query_token: Optional[str]) -> HfTokenArg:
    """Pick the credential for a route that still accepts the legacy ``?hf_token=``. Header first: it
    is the caller's real credential and a stale query parameter must not displace it. With neither
    explicit token present the sentinel is rebuilt rather than the header handed back: an ``or``
    chain ending on the query value would fall through ``False`` to ``None`` and restore the ambient
    token, while returning ``header_token`` itself would return an unresolved ``Depends`` object."""
...
def collect_local_models(
    ...
    *,
    ...
    """Scan ``models_root``, the HF caches, LM Studio dirs, and user scan folders, returning a deduplicated,
            hidden-filtered list. Shared by ``GET /models/local`` and the OpenAI-compatible catalog so the UI and
            the API never drift. ``models_root`` must already be validated by the caller."""
