...
async def _responses_non_streaming(
    ...
    try:
        # The impl's own api-monitor is suppressed here (skip_api_monitor is set
        # above), so the Responses monitor owns this turn; no subject to thread.
...
async def _read_json_body_limited(request: Request, *, max_bytes: int) -> Any:
    ...
    # Real Starlette requests always stream; the byte cap only applies there.
    # Non-streaming callers (test doubles that expose json() directly) bypass
    # the cap, which is moot without an attacker-controlled streamed body.
