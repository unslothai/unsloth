...
def _openai_model_objects() -> list[dict]:
    ...
    if llama_backend.is_loaded:
        ...
        _ctx = _positive_int_or_none(llama_backend.context_length)
        if _ctx is not None:
            model_obj["context_length"] = _ctx
        _max_ctx = _positive_int_or_none(llama_backend.max_context_length)
        if _max_ctx is not None:
            model_obj["max_context_length"] = _max_ctx
        _native_ctx = _positive_int_or_none(llama_backend.native_context_length)
        if _native_ctx is not None:
            model_obj["native_context_length"] = _native_ctx
