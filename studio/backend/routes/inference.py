...
_qwen_tts_voice_backend = None

# repo_id keyword match for routing /voice/load to the in-process Qwen3-TTS
# backend (e.g. Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice).
_QWEN_TTS_REPO_KEYWORDS = ("qwen3-tts", "qwen3_tts", "qwen-tts")


def get_qwen_tts_backend():
    global _qwen_tts_voice_backend
    if _qwen_tts_voice_backend is None:
        from core.inference.qwen_tts_backend import QwenTTSVoiceBackend
        _qwen_tts_voice_backend = QwenTTSVoiceBackend()
    return _qwen_tts_voice_backend


def _is_qwen_tts_identifier(model_identifier: str) -> bool:
    lower = model_identifier.lower()
    return any(kw in lower for kw in _QWEN_TTS_REPO_KEYWORDS)
...
async def voice_load_model(
    ...
    # Qwen3-TTS is a transformers speech-LLM (not GGUF) with its own in-process
    # backend; route it before the GGUF-only resolution below.
    if _is_qwen_tts_identifier(model_identifier):
        ...
        try:
            await asyncio.to_thread(qwen_tts_backend.load_model, model_identifier)
        except Exception as e:
            logger.error("Qwen3-TTS voice load error: %s", e, exc_info = True)
            raise HTTPException(
