...
def search_lexical(
    ...
    *,
    ...
    `newest_first` breaks TIES the other way round. FTS5 floors the IDF of a term the whole index
    shares, so every hit on a per-thread archive's own subject scores the same, and `ORDER BY s
    LIMIT k` then returns the k OLDEST rows: past k chunks on that subject the newest assignment is
    unreachable at any k. Ordering is by rowid, which is insertion order rather than exact
    conversation order, so this widens the candidate set and does not decide anything.
    """
