    ...
    try:
        ...
        for group, span, digest, chunks, seats, budget in pending:
            ...
            if stale is not None:
                # Same turn, vectors from an embedder the query side no longer asks for. The copy is replaced rather
                # than deduplicated, as ingestion does, since skipping it would leave the turn invisible to dense
                # search forever.
                # It keeps its POSITION and its TIMESTAMP: a re-embed rewrites HOW a turn is indexed, not WHEN it was
                # said, and renumbering or restamping would reorder the archive by the order its vectors were rebuilt.
                # NULL stays NULL, since numbering a pre-column row moves the oldest turn behind every numbered one
                # and the header would call it the conversation's last word.
...
def _conversation_order(row) -> tuple:
    """Sort key putting recalled turns in the order they were said. NULL ordinals sort FIRST, and
    that is not a fallback so much as a fact: they were written by a build that had no such
    column, so they genuinely predate every numbered turn in the same scope. Within a turn,
    `chunk_index` keeps a long message's pieces contiguous and in order, which relevance ordering
    gets wrong today. `created_at` breaks ties, because the ordinal is deliberately not UNIQUE:
    the write lock is best-effort, so two concurrent archive passes can compute the same MAX + 1
    and must tie-break rather than raise."""
    if row is None:
        return (2, 0, "", 0)
    ...
    *,
    ...
    if len(queries) > 1:
        ...
        if config.CONVERSATION_RECALL_ORDER == "chronological":
            # The same key `_conversation_order` uses on the single-query path: a turn with no ordinal predates every
            # numbered one, and `chunkIndex` keeps a long turn's pieces in writing order rather than in the order the
            # two queries returned them, which a stable sort would otherwise preserve.
            merged.sort(
                key = lambda source: (
