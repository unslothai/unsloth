...
# Ticks to wait for the pump to finalize an exited run's row before the queue
# marks it terminal itself; generous so a slow (but alive) finalize wins.
MAX_FINALIZE_WAITS = 6
...
class TrainingQueueManager:
    def __init__(self, backend = None):
        ...
        self._finalize_waits: Dict[str, int] = {}
        # Overridable for tests.
        self.poll_interval = POLL_INTERVAL
        self.settle_delay = SETTLE_DELAY
        self.max_pending = MAX_PENDING
        self.max_start_deferrals = MAX_START_DEFERRALS
        self.max_finalize_waits = MAX_FINALIZE_WAITS
    ...
    def _tick(self) -> None:
        ...
        if self._reconcile_running_items(backend):
            return
    ...
    def _reconcile_running_items(self, backend) -> bool:
        """Returns True while a prior run's row is still unreconciled, so the
        tick must not launch the next item against it."""
        blocked = False
        for item in studio_db.list_queue_items(statuses = ("starting", "running")):
            job_id = item.get("job_id")
            if not job_id:
                ...
                if item["status"] == "starting":
                    self._skip_interrupted_launch(item)
                continue
            if backend.is_training_active() and backend.current_job_id == job_id:
                self._finalize_waits.pop(job_id, None)
                continue
            run = studio_db.get_run(job_id)
            if run is None and item["status"] == "starting":
                # Claimed with a job id but the worker never created its run
                # row: the launch died before the spawn (process restart).
                self._skip_interrupted_launch(item)
                continue
            result_status = (run or {}).get("status") or "error"
            error_message = (run or {}).get("error_message")
            if result_status == "running":
                # Worker gone but the row still says running. Normally the pump
                # finalizes it within a tick, so hold queue advancement instead
                # of spawning against an unreconciled run. If the pump died with
                # the final write unwritten, no later write is coming: mark the
                # row terminal after a bounded wait rather than wedging forever.
                waits = self._finalize_waits.get(job_id, 0) + 1
                if waits <= self.max_finalize_waits:
                    self._finalize_waits[job_id] = waits
                    blocked = True
                    continue
                result_status = "error"
                error_message = "Run was never finalized after its worker exited"
                try:
                    ...
                    logger.warning("Failed to finalize stale run %s", job_id, exc_info = True)
            self._finalize_waits.pop(job_id, None)
            studio_db.update_queue_item_status(
                item["id"],
                "done",
    ...
    def _skip_interrupted_launch(self, item: dict) -> None:
        studio_db.update_queue_item_status(
            item["id"],
            "skipped",
        ...
        logger.warning("Queue item %s skipped: launch interrupted", item["id"])

    def _launch_item(self, backend, item: dict) -> None:
        if self._stop_runner.is_set():
            return
        # Claim with the job id already persisted: if the post-spawn "running"
        # update fails (e.g. a transient SQLite lock), reconcile can still tie
        # this row to its live run instead of skipping it as a dead launch.
        job_id = generate_job_id()
        if not studio_db.update_queue_item_status(
            item["id"], "starting", expected_status = "pending", job_id = job_id
        ):
            return
