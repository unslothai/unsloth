...
# Default LoRA target modules: the attention projections common to the SDXL U-Net and the DiT transformers (the diffusers/kohya convention). A family wanting a wider set overrides this in its own defaults.
DEFAULT_LORA_TARGETS: tuple[str, ...] = ("to_k", "to_q", "to_v", "to_out.0")

# diffusers SchedulerType names. piecewise_constant is excluded: it is the only scheduler needing a `step_rules` string the trainers never pass, so accepting it would pass normalized(), free the resident GPU workloads, then crash in the child.
# The other six run with only warmup/training steps.
...
# DiT families whose fp32 RoPE/embedder overflow fp16, so they train in bf16 only. Must stay in sync with the DiT trainer own specs (kept separate to avoid an import cycle).
...
# Architectures Studio can neither train nor load: not in the family registry but recognisable by name, so rejecting them by name turns a confusing mid-run crash into a clear error. Registry families are handled by the positive check in ``resolve_trainable_family``.
...
def resolve_trainable_family(base_model: str, model_family: Optional[str] = None) -> str:
    ...
    # GGUF weights (a ``.gguf`` file or ``*-GGUF`` repo) are inference-only: training needs the full diffusers pipeline, which a GGUF repo lacks. Reject by name even when the family is trainable.
    # Exempt a local diffusers checkout that merely has "gguf" in its path, identified by its ``model_index.json`` marker, not a bare ``is_dir()``.
...
# Per-family training defaults surfaced by the Train UI. Distilled/turbo bases and the big DiTs want different rank / learning rate / resolution; these are starting points, not hard limits. Families absent here fall back to the DiffusionLoraConfig defaults.
FAMILY_TRAIN_DEFAULTS: dict[str, dict[str, Any]] = {
    "sdxl": {"lora_rank": 16, "learning_rate": 1e-4, "resolution": 1024},
    # Warmup defaults: a short LR ramp keeps the first adapter updates from overshooting on the big flow-matching DiTs (whose logit-normal timestep draw concentrates loss mass mid-schedule); scaled for Studio short-run step budgets.
    ...
    # The Krea 2 authors recommended starting point (their DreamBooth script defaults): rank/alpha 32, lr 3e-4, 512px.
    "krea-2": {"lora_rank": 32, "learning_rate": 3e-4, "resolution": 512},
    # The upstream FLUX.2 DreamBooth references default to rank 16 / lr 1e-4; FLUX.2 uniform timestep draw benefits most from a warmup ramp.
    "flux.2-klein": {
        "lora_rank": 16,
        "learning_rate": 1e-4,
        "resolution": 512,
...
# The flow-matching DiT families (run by diffusion_dit_trainer). They expose the base_precision / compile levers and require bf16 compute on CUDA; SDXL is absent (it uses its own mixed_precision path). A set so the UI gate, the bf16 preflight and any dispatch stay in sync.
...
def dit_accelerator_missing_reason(resolved_family: str) -> Optional[str]:
    ...
    try:
        ...
        def probe(owner: Any) -> bool:
            # Each accelerator is probed on its own: one missing or throwing probe must not decide the other two. torch.mps.is_available() only exists from torch 2.5 while the supported floor is 2.4, so a shared try/except would swallow the AttributeError and wave a CPU-only host through.
            try:
...
def training_precision_preflight_error(resolved_family: str, base_precision: str) -> Optional[str]:
    ...
    if fam in _DIT_TRAIN_FAMILIES and mode in ("bf16", "int8", "fp8", "mxfp8"):
        # The DiT trainer dense precisions all require CUDA (_resolve_base_precision rejects them on device != "cuda"). bf16_unsupported_reason exempts a CPU-only host, so without this a dense request on a GPU-less host would pass the preflight, evict residents, then raise in the child.
        try:
            import torch
        ...
        # mxfp8 needs Blackwell (sm100+): its MX GEMM has no kernel below sm100 and raises at the first training step, AFTER a full dense load. Re-check here (mirroring _resolve_base_precision) so a stale/direct client fails fast before eviction.
        if mode == "mxfp8":
            try:
                import torch
...
def family_train_infos() -> list[dict[str, Any]]:
    ...
    for name in trainable_family_names():
        ...
        # base_precision applies to the DiT trainer only; SDXL keeps its mixed_precision lever, so the UI hides the precision selector for it. compile applies everywhere.
        is_dit = name in _DIT_TRAIN_FAMILIES
        # On a non-bf16 CUDA GPU the start preflight rejects EVERY DiT family (the DiT trainer requires bf16 on CUDA), so advertise no precision, else /info offers an nf4 DiT option that always 400s.
        # Otherwise drop any scheme this family DiT corrupts (fp8 on Qwen-Image; the inference path denies the same set), so the UI never offers a mode normalized() would reject.
        dit_block = (
        ...
        infos.append(
            {
                ...
                # compile is offered everywhere (SDXL regional U-Net + DiT), except a DiT family the GPU cannot train in bf16, where training is refused outright.
...
class DiffusionLoraConfig:
    ...
    data_dir: str
    output_dir: str
    # Dreambooth-style caption applied to any image without its own caption. Required if the dataset has no captions.jsonl / sidecar files.
    instance_prompt: Optional[str] = None
    resolution: int = 1024
    train_steps: int = 500
    # 0 = disabled (train for train_steps). Above 0 it overrides train_steps with num_epochs full passes over the dataset, in optimizer steps (see resolve_train_steps).
    num_epochs: int = 0
    ...
    # Precompute the VAE latents once (freeing the VAE for the run) instead of re-encoding every step. ``cache_variants`` crop/flip draws are frozen per image; the per-step VAE sampling noise itself is preserved.
    cache_latents: bool = True
    cache_variants: int = 4
    # Persistent on-disk conditioning cache directory (DiT trainer only). None (default) keeps the in-memory-only behavior;
    # a configured directory persists latent posterior stats and caption embeddings as safetensors keyed by content hash + family + resolution, so a fully warm cache skips loading the VAE and the multi-GB text encoders entirely.
    cond_cache_dir: Optional[str] = None
    # LoRA EMA decay (DiT trainer only). 0.0 (default) disables it; a positive value keeps an exponential moving average of the trainable LoRA params (warmup-ramped so short runs still absorb the trajectory) and exports it as a second adapter in the ema subdir.
    ema_decay: float = 0.0
    ...
    # TF32 matmuls + high fp32 matmul precision + cudnn autotuning for the run. Near-lossless; disable for strict bit-reproducibility A/Bs.
    enable_tf32: bool = True
    # DiT base transformer precision: "nf4" (bitsandbytes QLoRA, the memory floor and default), "bf16" (dense, fastest eager, compile-friendly), "int8" (torchao weight-only, half of bf16),
    # "fp8" (torchao float8 training on the frozen linears, Ada/Hopper/Blackwell + compile), or "auto" (by free VRAM + GPU class). Non-nf4 modes need a dense base repo. SDXL ignores it.
    base_precision: str = "nf4"
    # Training-time timestep shift applied to the flow-matching sigma draw. None resolves per family in normalized(): "auto" for qwen-image (reproducing the family inference sigma distribution), 1.0 (identity, the historical behavior) elsewhere.
    # A numeric value applies the standard linear shift s*u/(1+(s-1)*u). "auto" on a family without dynamic shifting falls back to identity.
    flow_shift: Optional[Any] = None  # float | "auto" | None
    # Per-sample probability of replacing the caption conditioning with the empty prompt (classifier-free-guidance dropout). 0.0 (default) disables it entirely.
    cfg_dropout: float = 0.0
    # Per-sample loss weighting over the drawn timestep: "none" (default, unweighted MSE) or "bell" (bsmntw-style Gaussian bell centered mid-schedule, normalized to mean 1).
    weighting_scheme: str = "none"
    # How often to emit a progress event (in optimizer steps).
    log_every: int = 1
    # Optional explicit family override ("sdxl" / "flux.1" / ...); None = detect from base_model. ``resolved_family`` is filled by normalized() with the trainer family that will run, so the process adapter can dispatch through the registry.
    ...
    def normalized(self) -> "DiffusionLoraConfig":
        ...
        # torch.manual_seed unpacks int64/uint64, so anything wider raises inside the trainer -- after the route already evicted the resident models. Catch it here, with the request.
        ...
        if resolved_family != "sdxl" and base_precision in ("bf16", "int8", "fp8", "mxfp8"):
            ...
            # Some DiT families are corrupted by fp8 activation range: outliers exceed even per-row fp8 range, so the frozen linears float8 compute learns against a garbage forward pass.
            # The inference path already denies these schemes; mirror it here so the run fails fast. int8 (per-token) is unaffected.
            from core.inference.diffusion_transformer_quant import _family_denied
        ...
        # flow_shift: None resolves to the family default ("auto" only for qwen-image, whose scheduler skips its static shift under use_dynamic_shifting and would otherwise train on unshifted uniform sigmas); an explicit value is validated and kept.
        ...
        if not isinstance(flow_shift, str):
            flow_shift = float(flow_shift)
            # isfinite as well as positive: JSON accepts 1e309, which floats to inf, and inf is not caught by a positivity-only guard (NaN fails every comparison), so it would reach the sigma table where s * u / (1 + (s - 1) * u) is NaN, poisoning every sampled sigma while progress looks normal.
...
class PermutationBatchSampler:
    ...
    def next_batch(self, k: int) -> list[int]:
        # k may exceed n (batch larger than the dataset): the permutation is refilled across as many cycles as needed so the caller always gets exactly k indices and the batch never shrinks.
        out: list[int] = []
        while len(out) < k:
    ...
    *,
    ...
    for meta_name in ("metadata.jsonl", "captions.jsonl"):
        ...
        if not meta_path.is_file():
            continue
        # Tolerate a bad upload (invalid UTF-8, or a line of non-object JSON): skip the record so the instance_prompt fallback still applies rather than crashing the trainer.
        try:
    ...
    for img in images:
        ...
        # 1. per-image sidecar caption file (the user explicit edit; wins over metadata). An EMPTY sidecar is a deliberate tombstone (written when a user clears a caption): it suppresses the metadata caption but leaves the image uncaptioned, so the instance_prompt fallback applies.
        for ext in _CAPTION_EXTS:
            ...
            if sidecar.is_file():
                ...
                try:
                    ...
                    # An unreadable sidecar reads as the empty tombstone, so the instance_prompt fallback applies instead of a 500 preflight.
                    caption = ""
                break
        # 2. metadata row keyed by file name (basename or relative path; as_posix so a Windows backslash path matches the jsonl forward-slash keys). A sidecar, even empty, wins.
        ...
        if caption:
            if verify_images:
                # Reject a corrupt/zero-byte/truncated image now via a cheap PIL header probe (verify() does not decode full pixels): otherwise it passes this filename-only discovery, the start route frees the resident GPU models, and the trainer only then crashes in Image.open.
                try:
...
# Host-memory budget for the AUTOMATIC latent cache. The cache holds two fp32 posterior tensors (mean/std, VAE scale folded in) per crop/flip variant per image, pinned on a CUDA host.
# At 1024px an SDXL variant is ~0.5 MiB and a 16-channel DiT variant several times that, so a few thousand images x cache_variants can exhaust host or pinned RAM; over budget the default falls back to per-step VAE encoding. A fixed constant keeps the gate dependency-free.
_LATENT_CACHE_BUDGET_BYTES = 4 * 1024**3  # 4 GiB

# Returned by the cache builders when the estimate exceeds budget: the caller keeps the VAE resident and encodes each step latents in-loop. A distinct sentinel from ``None`` (a stop requested mid-build).
...
def _apply_perf_flags(
    ...
    try:
        ...
        if cfg.enable_tf32:
            ...
            # The opt-out is a strict-fp32 A/B mode, so actively clear the flags rather than inherit ambient state (cudnn TF32 defaults to ON in torch).
            torch.backends.cuda.matmul.allow_tf32 = False
            torch.backends.cudnn.allow_tf32 = False
            torch.set_float32_matmul_precision("highest")
        if cudnn_benchmark:
            torch.backends.cudnn.benchmark = True
        # The cuDNN SDPA backend TRAINING graph is broken for the FLUX attention shapes on torch 2.10 + cu130 (B200): mha_graph.execute fails, then poisons the context into illegal memory accesses. Flash / mem-efficient SDPA are equivalent, so pin those for the run (restored on exit).
...
def _restore_perf_flags(snap: Optional[dict]) -> None:
    ...
    try:
        ...
        # Restore the exact pre-run cudnn SDPA state; None means the flag was unreadable (or absent) at apply time and was never touched.
        cuda_backends = getattr(torch.backends, "cuda", None)
        if (
...
# Official safetensors-only TRAINING bases trusted in addition to the inference allowlist (_TRUSTED_NON_GGUF_REPOS in core/inference/diffusion.py). The FLUX.2 bases are in that list too but kept here so the training gate stays independent of a future edit to it.
# Exact-match lowercased: extend deliberately; never add pickled weights or remote code.
...
def _assert_trusted_base_model(base_model: str) -> None:
    ...
    # An existing LOCAL base is loaded as a full pipeline by the trainer, which needs a model_index.json. Any existing path is "trusted" above, so reject a non-pipeline local dir here, before /diffusion/start frees the GPU models, rather than have the child fail after teardown.
...
def _publish_to_lora_catalog(lora_path: str, cfg: DiffusionLoraConfig) -> Optional[str]:
    ...
    try:
        ...
        # A retrain with the same adapter name must not clobber a prior mirror: pick the next free numeric suffix instead.
...
# Aliases from the generic Studio training payload onto DiffusionLoraConfig fields, so the diffusion trainer can also be driven by the shared training request shape.
_CONFIG_ALIASES = {
    "model_name": "base_model",
    "max_steps": "train_steps",
    # The generic payload num_epochs already matches the diffusion field name, but list it so the epochs override is threaded through the shared-payload path as explicitly as max_steps is.
...
def _config_from_dict(config: dict) -> DiffusionLoraConfig:
    ...
    for k, v in config.items():
        if k in valid:
            kwargs[k] = v
    # Epoch-mode payloads from the generic UI carry max_steps: 0 as the "use epochs" sentinel, which the alias copies as train_steps: 0. normalized() rejects train_steps below 1 before resolve_train_steps() applies num_epochs,
    # so drop a falsy train_steps when num_epochs is set and let the dataclass default stand in until epoch resolution.
    try:
