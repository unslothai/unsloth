...
"""Shared, family-agnostic building blocks for diffusion LoRA training: dataset discovery, request
config + validation, image loading, event emission, the stop protocol, adapter publishing, and
the family/trainer registry, so the SDXL U-Net trainer (``diffusion_lora_trainer``) and the
flow-matching DiT trainers share them. The pure helpers have no torch/diffusers import at call
time and are unit-tested without a GPU; this module only routes a request to the right training
loop."""
...
def repo_is_prequantized(base_model: str) -> bool:
    """Heuristic: a repo whose name marks a bitsandbytes 4-bit build already ships a quantized
    transformer, so it loads as-is for nf4 and cannot serve the dense (bf16/int8/fp8/mxfp8) base
    precisions."""
...
def get_trainer(family: str) -> Callable[..., str]:
    """Return the training entrypoint for ``family``. Imports the trainer module lazily so this
    shared module stays free of the heavy trainer imports (and any import cycle)."""
...
class DiffusionLoraConfig:
    """Everything a diffusion LoRA training run needs. Only ``base_model`` / ``data_dir`` /
    ``output_dir`` are required; the rest have sensible defaults."""
...
class PermutationBatchSampler:
    """Yields batch indices as consecutive slices of a reshuffled permutation of ``range(n)``, so
    every index is visited exactly once per cycle before any repeats -- an epoch-style full pass
    instead of the with-replacement draw that leaves part of a small dataset unseen at low step
    counts. When a cycle is exhausted the order is reshuffled from the run's own ``rng`` so the
    index stream stays seed-deterministic and each cycle differs. Both trainers share this so the
    SDXL ``_next_batch`` path and the DiT per-sample draw select indices the same way; step count
    and batch shapes are unchanged."""
    ...
    def next_batch(self, k: int) -> list[int]:
        # k may exceed n (batch larger than the dataset): the permutation refills across cycles so the caller always
        # gets exactly k indices.
        out: list[int] = []
        while len(out) < k:
    ...
    def state_dict(self) -> dict[str, Any]:
        """The position inside the current permutation cycle, for a resume checkpoint. The order
        itself is stored (not just the position): it was drawn from the run's rng, and restoring
        the rng alone would not reproduce a cycle already in progress."""
    ...
    def load_state_dict(self, state: Optional[dict[str, Any]]) -> bool:
        ...
        try:
            ...
            return False
        # A shortened or duplicate-carrying order is in range and plausible, so the cycle reshuffles early or serves
        # the same image twice with the RNG already past the original draw.
        ...
        try:
            ...
            return False
        # A position outside 0..len(order) is a damaged manifest, and clamping it re-serves the permutation from the
        # top or ends the cycle early behind an already-restored RNG.
...
def _latent_cache_forced() -> bool:
    """The user explicitly forced the latent cache on, bypassing the size gate. The explicit opt-in
    counterpart to ``UNSLOTH_DIFFUSION_NO_LATENT_CACHE``; only the automatic default is
    size-gated, so an explicit choice is honoured verbatim in either direction."""
...
def _restore_perf_flags(snap: Optional[dict]) -> None:
    """Undo ``_apply_perf_flags`` (the trainer subprocess is disposable, but in-process callers --
    tests, notebooks -- must not inherit mutated globals)."""
    if not snap:
        return
...
def _json_safe_progress(progress: Optional[dict[str, Any]]) -> dict[str, Any]:
    ...
    invalid strict JSON that a stricter reader rejects."""
    out: dict[str, Any] = {}
    ...
    *,
    ...
    if optimizer_state is not None:
        ...
        if not saved_optimizer:
            # An optimizer file with no class beside it is a hand-edited or half-written bundle, and foreign moments
            # load cleanly then die on the first step, after the preflight evicted the residents.
            raise ResumeError(
    ...
    if sampler is not None and not sampler.load_state_dict(ckpt.sampler_state):
        # Every image checkpoint carries sampler state, so a missing or malformed one leaves a fresh sampler behind an
        # RNG restored past the saved permutation: images silently skipped or repeated.
        raise ResumeError(
