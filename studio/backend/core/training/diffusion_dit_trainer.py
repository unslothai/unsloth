...
# carry added-kv projections; Z-Image is single-stream. Architecture-specific, so kept here
# rather than in the generic DEFAULT_LORA_TARGETS.
_FLUX_TARGETS = (
    "to_q",
    "to_k",
    "to_v",
...
# The Krea 2 authors' recommended default targets (their DreamBooth reference script): attention
# + SwiGLU + text-fusion projector + conditioning embedders. For long runs they suggest narrowing
# to the attention layers so prompt adherence doesn't drop.
_KREA2_TARGETS = (
    "img_in",
    "final_layer.linear",
    "to_q",
...
class _FamilySpec:
    ...
    force_bf16: bool
    # Approximate dense-bf16 transformer weight size, used by base_precision="auto" to decide which
    # mode fits the free VRAM (with headroom for activations + optimizer).
    dense_bf16_gb: float
    # Phased load so the multi-GB transformer never coexists with the text encoders + VAE:
    # ``load_conditioners`` builds the pipeline WITHOUT its transformer and returns (pipe, vae);
    # ``load_transformer`` then loads the transformer alone (trainable nf4 QLoRA when qlora=True)
    # once the conditioners are freed. Roughly halves peak VRAM for the big DiTs.
    ...
    # Encode a pixel tensor to (A, B) affine posterior params so a per-step sample is A + B * randn
    # (family normalisation folded in). B is None for a deterministic family. Used by the latent cache.
    encode_latent_stats: Callable[..., tuple]
    # Collate a list of per-caption embed tuples into one batched tuple on device. ``pad_to`` pins a
    # fixed text length for families with variable-length embeds (compile).
    collate: Callable[..., tuple]
    # One transformer forward: (transformer, noisy, timesteps, sigmas, embeds_batch, cfg, device,
    # weight_dtype) to model_pred aligned with target = noise - latents.
...
def _training_sigma_table(scheduler, flow_shift):
    ...
    if flow_shift == "auto":
        ...
        if getattr(sc, "shift_terminal", None):
            ...
            # (as it does at inference set_timesteps), never a batch.
...
def _bnb_4bit_config():
    ...
    return DiffusersBnb(
        ...
        # Double quantization compresses the per-block absmax scales with a second 8-bit pass: ~0.4
        # bits/param off the frozen base at no fidelity cost (the scales are metadata, not weights),
        # which matters most on the 20B+ DiTs where the nf4 base dominates VRAM.
...
def _load_dit_transformer(transformer_cls, cfg, device, base_precision):
    ...
    # _int8_quantize_base): quantizing first makes peft dispatch its TorchaoLoraLinear wrapper, whose
    # peft-0.18 constructor is incompatible with the torchao-0.16 config API.
    return transformer_cls.from_pretrained(
        cfg.base_model,
        subfolder = "transformer",
        torch_dtype = torch.bfloat16,
...
def _mx_module_filter(mod, fqn: str) -> bool:
    ...
    # base linear changes the output the LoRA regresses against. Keep biased linears in bf16.
...
def _resolve_base_precision(cfg, spec, device) -> str:
    ...
    if mode != "auto":
        ...
        # int8 has no runtime fallback (_int8_quantize_base imports torchao unconditionally), so an
        # explicit int8 against a missing torchao or the Windows-ROCm stub would leave the transformer
        # dense with compile disabled -- memory saving gone, likely OOM. The auto pick and /info already
        # gate on a FUNCTIONAL torchao; apply the same gate here. fp8 keeps its own fallback.
        ...
        # training step, AFTER a full dense load. /info advertises mxfp8 only on sm100+, so re-check here
        # to fail fast for a stale/direct client instead of crashing mid-run.
    ...
    # find_spec("torchao") is satisfied by the Windows-ROCm import stub whose quantize_ is a no-op.
    # has_functional_torchao imports the exact symbols _int8_quantize_base uses and rejects the stub.
...
def _flux_collate(
    ...
    # FLUX embeds are fixed-length (encode_prompt pads to max_sequence_length), so a plain cat
    # batches them; text_ids are shared position ids, identical across prompts.
...
# allocator churn. Cleared at run start.
...
def _flux_static_inputs(bsz, h, w, device):
    ...
    if hit is None:
        # Position ids drive RoPE and are indices, not activations, so keep them float32 (the dtype
        # diffusers' own pipeline builds) regardless of the bf16 training dtype.
...
def _qwen_encode_latents(vae, pixel_values):
    ...
    # AutoencoderKLQwenImage is a 3D (video) VAE: add a temporal dim, encode, drop it back into a
    # [B,16,1,H,W] latent normalised by the per-channel latents_mean / latents_std.
...
def _qwen_collate(
    ...
    # compile so the graph shape stays fixed) and batch the validity mask with them.
    ...
    # A single unpadded sample keeps the legacy None mask (identical math; avoids a behaviour delta
    # for existing single-image runs).
...
def _qwen_forward(transformer, noisy, timesteps, sigmas, embeds_batch, cfg, device, weight_dtype):
    ...
    # sample[1:] per entry (transformer_qwenimage.py), so a flat list breaks it.
...
def _zimage_encode_latent_stats(vae, pixel_values):
    # Z-Image trains from the posterior mode (deterministic), so the cached entry is the final latent
...
def _krea2_load_conditioners(cfg, device, weight_dtype):
    ...
    # conditioning pipeline is assembled per-component rather than from_pretrained(transformer=None);
    # see diffusion_krea2.py.
    import torch
...
def _krea2_encode_prompts(pipe, captions, device):
    ...
    with torch.no_grad():
        for cap in captions:
            ...
            # [1, 512, num_text_layers, 2560] with a [1, 512] validity mask (static shapes; padding sits
            # mid-template, BEFORE the assistant suffix, matching how the model was sampled at training).
            pe, mask = pipe.encode_prompt(
                prompt = cap,
                device = device,
                num_images_per_prompt = 1,
...
# Krea 2 conditions on the Qwen-Image VAE with the same per-channel latents_mean / latents_std
# normalisation, so latent encoding is shared.
...
def _krea2_collate(
    ...
    # Fixed-length embeds (see _krea2_encode_prompts), so collation is a plain concat with the mask
    # riding along; ``pad_to`` is moot because the shapes are static.
...
def _krea2_forward(transformer, noisy, timesteps, sigmas, embeds_batch, cfg, device, weight_dtype):
    ...
    pe, mask = embeds_batch
    # [B,16,1,H,W] to [B, (H/2)*(W/2), 64] 2x2 patches. Krea2Pipeline._pack_latents / _unpack_latents
    # are instance methods (they read self.patch_size), so the packing is inlined here like the
    # reference DreamBooth script (patch_size = 2).
...
# ── FLUX.2 (dev + Klein) ──────────────────────────────────────────────────────
# Both variants share Flux2Transformer2DModel and the packing/forward conventions of the upstream
# DreamBooth references; they differ only in the conditioning stack (dev: Mistral-3-Small via
# Flux2Pipeline; Klein: Qwen3 via Flux2KleinPipeline) and size. Latents train in the PATCHIFIED,
# batch-norm-normalised space the references use, from the posterior MODE (deterministic, so the
# latent cache stores the final latent and skips the per-step draw).
_FLUX2_TARGETS = (
    ...
    "to_v",
    "to_out.0",
    # Single-stream blocks: the fused qkv+mlp input projection carries the bulk of the capacity.
    # Their out proj is a PLAIN Linear named to_out whose suffix would also match the double-stream
    # ModuleList container (which peft cannot wrap), and the upstream scripts enumerate it per block
    # index, so it stays dense.
    "to_qkv_mlp_proj",
)
# The references train dev with its guidance-distillation vector at 3.5 (Klein applies it only
# when the variant's config carries guidance_embeds).
...
def _flux2_encode_prompts(pipe, captions, device):
    ...
    with torch.no_grad():
        for cap in captions:
            # encode_prompt pads to max_sequence_length, so the embeds are fixed-length and text_ids are
            # per-caption [1, txt_len, 4]. The per-class text_encoder_out_layers defaults differ (dev
            # (10,20,30) vs Klein (9,18,27)) and are left to the pipeline.
...
def _flux2_collate(
    ...
    # Fixed-length embeds give a plain concat; text_ids are PER-SAMPLE [1, txt_len, 4] (unlike
    # FLUX.1's shared grid), so they batch by concat too. ``pad_to`` is moot with static shapes.
    pe = torch.cat([e[0] for e in entries]).to(device = device, dtype = weight_dtype)
...
def _flux2_forward(transformer, noisy, timesteps, sigmas, embeds_batch, cfg, device, weight_dtype):
    ...
    # ``noisy`` is already in the patchified normalised space, so packing is the [B,C,H,W] to
    # [B, H*W, C] flatten the pipeline uses.
    ...
    # _unpack_latents_with_ids scatters per sample; diffusers 0.39 stacks the per-sample tensors
    # itself (its list annotation is stale), other builds may hand back the list. Same-resolution
    # batches align either way with target = noise - latents.
...
_SPECS: dict[str, _FamilySpec] = {
    ...
    "flux.2-dev": _FamilySpec(
        ...
        lora_targets = _FLUX2_TARGETS,
        force_bf16 = True,
        # 32B DiT; the Mistral conditioning stack (~46 GB bf16) is loaded, encoded and freed BEFORE this
        # lands on the device (the shared phased load).
        dense_bf16_gb = 64.5,
...
# HF repos gating access behind a license acceptance: training needs a token whose account
# accepted it. Checked by name (no network) so a missing token fails fast with an actionable
# message instead of a confusing 401 mid-load.
    ...
    for i, path in enumerate(image_paths):
        ...
        for u_left, u_top, flip in plan[i]:
            ...
            if not forced and not gated:
                # Size-gate the automatic cache off the first REAL encoded variant, before building the rest:
                # packed 16-channel DiT latents x variants x images of two fp32 tensors can exhaust host/pinned
                # RAM. Over budget we bail with the VAE resident so the loop encodes per step. ``b`` is None for
                # a deterministic-latent family, so only ``a`` contributes bytes there.
...
def _should_compile(
    ...
    if mode == "on":
        return True
    # auto: regional compile is the whole point of the dense modes (measured 2.6x on Z-Image bf16)
    # but fragile over bitsandbytes 4-bit modules (graph breaks in the dequant path), so it stays off
    # for QLoRA. fp8/mxfp8 are only competitive compiled (eager, their per-matmul dynamic casts run
    # 4-5x slower than bf16).
...
def _maybe_compile_transformer(
    ...
    try:
        ...
        # dynamic=True matches the inference speed layer's proven default: on torch 2.10 / B200 the
        # dynamic=False specialisation fused a gemm_and_bias epilogue that failed with
...
def run_dit_lora_training(
    ...
    *,
    ...
    # DiT families train in bf16 (Z-Image/Qwen require it; FLUX prefers it). An explicit fp16 request
    # on a bf16-only family is refused, not silently upgraded, so the choice is never misrepresented.
    # Validation runs before the heavy imports so a host without diffusers still sees the real error.
    ...
    # The flow-matching + 4-bit path is bf16 throughout (fp32 on a CPU-only box: unsupported for real
    # runs but keeps import/unit tests architecture-agnostic). Fail fast on pre-Ampere CUDA: bf16 is
    # required and the run would otherwise die deep in model load with an opaque dtype error. Gate on
    # NATIVE bf16 (capability major >= 8), since is_bf16_supported() counts pre-Ampere emulation.
    ...
    # every downstream read (scheduler length, loop range, progress totals) sees the same value.
    ...
    # disposable, but restoring keeps in-process callers clean).
...
def _train_dit(cfg, spec, pairs, rng, device, weight_dtype, on_event, _check_stop, _save_on_stop):
    ...
    # Phase 0: the persistent conditioning cache (opt-in via cond_cache_dir). When every planned
    # latent variant AND every caption embedding is already on disk, the run is "warm": the VAE and
    # the multi-GB text encoders are never loaded at all.
    ...
    # precomputed alongside the captions (the encoders are freed right after encoding).
    ...
    if getattr(cfg, "cond_cache_dir", None):
        try:
            # Namespace on the CHECKPOINT, not just the family: the keys below carry only caption/image
            # content and crop variant, so one cache dir reused for two checkpoints (or a new revision) would
            # let a warm run train on the other model's embeddings and latent stats.
    ...
    # The crop/flip variant plan is seed-deterministic, so the persistent keys are stable across runs
    # of the same config and the warm check can run before anything loads.
    plan = _plan_cache_variants(
        len(image_paths), cfg.cache_variants, cfg.center_crop, cfg.random_flip, cfg.seed
    )
    ...
    if caption_embeds is None:
        # Phase 1 (cold): conditioning only. The pipeline loads WITHOUT its transformer, so the text
        # encoders + VAE never share VRAM with the multi-GB denoiser. Caption embeddings are precomputed
        # once and the encoders freed: captions are constant and the encoders frozen, so this is exact.
        latent_cache = None
        ...
        # Phase 2: the VAE latent cache, then free the VAE too (the cache keeps the posterior affine
        # parameters, so per-step sampling noise is preserved).
        if use_cache:
            ...
            if latent_cache is LATENT_CACHE_OVER_BUDGET:
                # The estimated cache exceeded the host-memory budget; keep the VAE resident and fall through to
                # the in-loop encode path (latent_cache stays None).
                latent_cache = None
    ...
    if latent_cache is not None and vae is not None:
        ...
        if device == "cuda":
            torch.cuda.empty_cache()
    # Variant picks use their own stream so the training loop's index/noise draws stay
    # seed-deterministic whether or not the cache is enabled.
    variant_rng = random.Random(cfg.seed + 1)

    # Phase 3: only now load the transformer, in the resolved base precision (nf4 QLoRA by default;
    # bf16 / int8 / fp8 / mxfp8 are the dense speed modes; "auto" picks from free VRAM).
    ...
    if cfg.gradient_checkpointing:
        ...
        # memory access on the larger FLUX transformer, and non-reentrant is the recommended mode anyway
        # (it also handles a checkpointed segment whose inputs don't require grad).
        import functools
    ...
    # int8 / fp8 / mxfp8 convert the frozen base linears AFTER the LoRA attaches, so the adapter
    ...
    # LoRA EMA (opt-in via cfg.ema_decay above 0): shadows ONLY the trainable adapter params
    # (megabytes, not the base's gigabytes), initialised after the precision conversions so the
    # shadows track the final fp32 param objects. Exported as a second adapter under the ema subdir.
    ...
    # Compiled Qwen graphs need one fixed text length across steps: pin the pad bucket to the
    # dataset's longest caption (encode_prompt already caps it at 1024 tokens).
    qwen_pad_to = None
    ...
    # Timestep-shift + loss-weighting setup (see _training_sigma_table). getattr defaults keep an
    # un-normalized config (tests, direct callers) on the historical behavior.
    ...
    # The LR schedule advances once per optimizer update, so warmup/decay are counted in optimizer
    # steps; multiplying by the accumulation factor would stretch warmup past the run.
    ...
    # cycle before repeating, so a short run covers the whole dataset. Uses the loop's own rng to
    ...
    # bf16 autocast around the forward + loss, matching the diffusers dreambooth scripts: it
    # reconciles the fp32 LoRA params with the bnb 4-bit base matmuls in one compute dtype. Without
    # it the 4-bit backward on FLUX dies with an illegal-address / CUBLAS failure.
    autocast = (
    ...
    for opt_step in range(cfg.train_steps):
        ...
        for _ in range(cfg.gradient_accumulation_steps):
            ...
            if shift_active:
                ...
                # num_train_timesteps). Gather in fp32 from the table so bf16 rounding never skews it.
                timesteps = (
        ...
        if cfg.max_grad_norm and cfg.max_grad_norm > 0:
            ...
            # norm shows up even while the clip caps the update).
        ...
        now = time.time()
        if done == 1:
            # Step 1 pays the one-time costs (cudnn autotune, torch.compile warmup), so the reported rate
            # starts after it and reflects the steady state.
            t_steady = now
