...
from .os_sandbox import (
    SandboxCapability, SandboxBuildError, PreparedSandboxLaunch,
    _record, _OS_ISOLATION_SAFEGUARDS, scan_workdir_for_host_channels,
)


def capability_snapshot(*, force=False, execution_kind=None, selected_executable=None):
    ...
    identity = hashlib.sha256(repr((
        srt_adapter.installation_identity(), srt_probe.runtime_inputs(),
        execution_kind, selected_executable,
    )).encode()).hexdigest()
    diagnostic = reason.fields() if hasattr(reason, "fields") else {}
    return SandboxCapability(
        backend="srt", available=available, reason=str(reason), environment="win32",
        ...
        remediation=diagnostic.get("remediation", "Windows setup is optional in Auto. Use Set up Windows sandbox to install or repair isolation; this never retries a Python or Terminal call."),
        reason_code=diagnostic.get("reason_code"), diagnostic=diagnostic.get("diagnostic"),
    )
...
def prepare(plan, capability):
    ...
    record = _record(
        plan, capability, effective_mode="os_isolated", os_isolation=True,
        backend="srt", profile_id=capability.profile_id,
        safeguards=tuple(s for s in _OS_ISOLATION_SAFEGUARDS if s != "resource_limits"),
        limitations=capability.limitations,
    )
    prepared = PreparedSandboxLaunch(
        argv=plan.argv, workdir=plan.workdir, env=plan.env, preexec_fn=None,
        backend="srt", timeout_seconds=plan.timeout_seconds,
