...
class MLXInferenceBackend:
    ...
    def generate_chat_response(
        ...
        full_messages = self._conversation_to_render(messages, system_prompt, image)
        ...
        with self._generation_lock, _temporary_mlx_adapter_state(self._model, _adapter_state):
            ...
            try:
                ...
                if logits_processors is not None:
                    gen_kwargs["logits_processors"] = _row_logits_processors(logits_processors)
    ...
    def _generate_vlm(
        ...
        *,
        ...
        session = self._vlm_prompt_cache_session(_adapter_state, plan.images, prompt)
        if session is not None:
            plan.kwargs["prompt_cache"] = session.cache
            plan.kwargs["prompt_cache_state"] = session
            if session.media_block is not None:
                plan.kwargs.update(session.media_block.generate_kwargs())
            # Reused and unreused turns must prefill on the same grid to match.
            plan.kwargs["prefill_step_size"] = VLM_PROMPT_CACHE_PREFILL_STEP
        ...
        with (
            ...
            if plan.images and session is None:
