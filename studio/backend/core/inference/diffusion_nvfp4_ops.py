...
FlashInfer 0.6.6 registers no custom ops, so wrapping its entry points here is what makes
``fullgraph = True`` over a quantized block possible. The fake impls must reproduce FlashInfer's
allocation EXACTLY: a wrong meta shape is a silently mis-sized buffer, not an error. torch and
flashinfer are imported inside functions so the module imports on a torch-free host.
"""
...
def _swizzled_sf_numel(
    ...
    """Element count of a swizzled scale-factor buffer. Copied, not imported: the fake impls must not touch the FlashInfer JIT machinery."""
    return ((rows + row_size - 1) // row_size * row_size) * ((cols + 3) // 4 * 4)


def _device_guard(t: Any):
    """``torch.cuda.device`` for the tensor's own device. EVERY flashinfer call must sit inside one: FlashInfer installs no device guard, and a foreign current device bricks the card. ``_mm_impl`` spans the barrier AND the GEMM so the barrier fires on the device the GEMM reads."""
    import torch
...
def _barrier(device: Any):
    """The process-wide 1-element bf16 buffer for ``device``, UNCACHED while capturing: an allocation made inside a capture dies with the graph."""
    import torch
...
def _fire_barrier(device: Any):
    """Launch the ordering kernel between the quantiser and the GEMM. What protects the GEMM is a kernel EXISTING there, not one writing M x N bytes."""
    buf = _barrier(device)
    buf.zero_()
    return buf
...
def reset_barriers() -> None:
    """Drop every cached barrier: one allocated under a model's allocator state must not reach the next model's graph pool."""
    with _BARRIER_LOCK:
        _BARRIERS.clear()
...
# Plain functions as well as ``torch.ops`` entries, so a test can assert the call ORDER the barrier rests on.
...
def _mm_impl(xq: Any, wq: Any, x_sf: Any, w_sf: Any, alpha: Any, n: int, backend: str):
    """NVFP4 GEMM. Takes the weight row-major ``[N, K/2]`` and transposes inside, since a custom op's
    inputs are functionalised and a ``.T`` view is an alias Dynamo has to reason about.

    **A kernel MUST run between the activation quantiser and this GEMM**, or the GEMM reads operands
    the quantiser has not finished writing (cutlass is launched with PDL while the ``griddepcontrol``
    instructions that make PDL safe are compiled out of its build). What protects it is a kernel
    EXISTING, so a one-element fill is as good as the memset while ``torch.empty`` alone is NOT.
    Nothing ever reads the per-device barrier buffer, so a cached one cannot carry a stale NaN out.
    ``UNSLOTH_NVFP4_ZERO_BUFFER=1`` restores the full memset (slower, no safer).
    """
    import flashinfer
    import torch
...
def register_ops() -> None:
    """Register the two custom ops. Idempotent, thread safe, and deferred because this module has to import on a torch-free host."""
    global _REGISTERED
    if _REGISTERED:
        return
    with _REGISTER_LOCK:
...
# Deliberately NOT a registered buffer: one copy per layer is a byte that must not exist.
_LUT_CACHE: dict = {}


def e2m1_lut(device: Any):
    """The signed e2m1 decode table on ``device``, memoised so it is never allocated inside a CUDA-graph capture: a buffer created while recording is only valid while recording."""
    import torch
...
def dequantize_nvfp4_weight(
    ...
    *,
    ...
    """The packed NVFP4 operand as a dense ``[N, K]`` weight. TRANSIENT by contract: caching it is the
    second resident operand this lever exists to avoid. The arithmetic is torchao's, in torchao's
    order, so the protected step reads the SAME weight the unprotected step does."""
    import torch
...
def sf_matrix_shape(rows: int, cols: int) -> tuple[int, int]:
    """FlashInfer's 2D view of a swizzled scale buffer. The leading dim is NOT ``rows`` under non-trivial padding."""
...
def nvfp4_preflight(device: Any = None, *, refresh: bool = False) -> dict:
    """A tiny GUARDED 128x256 quantise + GEMM on ``device``, memoised per device index: only asking the JIT to build finds out whether FlashInfer runs here. Never raises."""
    import torch
    ...
    try:
        ...
        if _transient_preflight_failure(exc):
            # Not memoised: AUTO planning probes while the model about to be evicted still owns the card, so an allocation failure means "not now", not "not here".
            return dict(rec)
...
def select_nvfp4_backend(device: Any = None) -> str:
    """``"torchao"`` or ``"flashinfer"`` for ``device``. An explicit ``flashinfer`` request that fails import, capability or preflight falls back to torchao rather than to a bricked card."""
