...
class LlamaAdmissionQueue:
    ...
    async def acquire_parked_slot(
        ...
        *,
        ...
        to 0, a park resuming, which never gave its commitment back. It is held from
        arrivals until ``timeout_s`` lapses the claim; lapsing keeps the ticket's place in
        the slot order and still never admits it over budget, since llama.cpp answers an
        exhausted cache by clearing every slot it is decoding.

        ``deadline`` bounds a preempted holder's wait and is separate from the lapse above:
        None is the park default, where an approved prompt waits as long as it takes.
        ``progress`` makes that bound a STALL bound, so a resume queued behind a long answer
        waits through healthy draining.
        """
        want = max(0, int(tokens or 0))
        last_progress = progress() if progress is not None else None
        with self._lock:
            ...
            self._unpark_tokens[ticket] = want
        lapse_at = None if not timeout_s or timeout_s <= 0 else time.monotonic() + timeout_s
        try:
            while True:
                with self._lock:
                    if lapse_at is not None and time.monotonic() >= lapse_at:
                        self._unpark_lapsed.add(ticket)
                        lapse_at = None
                        self._grant_waiters_locked()
                    ahead, ahead_tokens = self._ticket_position_locked(ticket)
                    slot = self._take_slot_locked(ahead, want, ahead_tokens)
                    if slot is not None:
                        return slot
                    if cancel_event is not None and cancel_event.is_set():
                        return None
