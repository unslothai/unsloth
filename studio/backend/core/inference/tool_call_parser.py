...
def _strip_gemma_wrapperless_calls(text: str, enabled_tool_names: Optional[set] = None) -> str:
    ...
    arguments are removed whole). ``enabled_tool_names`` gates the strip like the parser gate: a
    disabled/example name stays visible; ``None`` strips every closed call. An unanchored
    (mid-sentence) call is prose too."""
...
def _first_sentinel(text: str, start: int) -> int:
    ...
    markup would put the full strip back on the per-token path for a plain prose answer. It is
    therefore confirmed against the arm it stands for: either the whole ``call:NAME{`` anchor is
    present, or the buffer ends inside a partial one that the next token could complete. Every
    append is checked, so a call that arrives a character at a time is caught while it is still a
    partial."""
    best = -1
...
def _signal_inside_leading_wrapperless_gemma(
    ...
    enabled wrapper-less Gemma call (sibling of the Mistral/bare-JSON leading guards). Markerless
    form, so gated on an enabled name (``None`` keeps the name-agnostic behaviour)."""
...
def _disabled_gemma_call_end_containing_signal(
    ...
    """End offset (exclusive) of the earliest DISABLED wrapper-less Gemma call whose balanced body
    contains the first foreign signal, else None. A disabled name is prose, so the quoted literal
    is data: the caller drops the span and recurses on the tail. An ENABLED call defers to the
    enabled-call guard."""
    ...
    *,
    ...
    while cursor < n:
        ...
        if not isinstance(name, str) or not name:
            break
        # Markerless JSON is ambiguous: treat it as a call only when the name is an enabled tool, else it is an
        # ordinary JSON answer
...
def _parse_gemma_tool_calls(
    ...
    *,
    ...
    not be stolen as a call. ``None`` keeps the name-agnostic behaviour."""
    out: list[dict] = []
...
def _top_level_bare_json_name(probe: str) -> Optional[str]:
    ...
    while i < n:
        ...
        if key == "function" and function_value is None and i < n and probe[i] == '"':
            ...
            i += consumed
            continue
        # Skip a non-name top-level value; a truncated one cannot prove a top-level name exists, so return None (keep
        # the text).
        if i < n and probe[i] == "{":
...
def strip_leading_bare_json_call(text: str, enabled_tool_names: Optional[set] = None) -> str:
    ...
    while True:
        ...
        # ``arguments``). An ordinary JSON answer like {"name":"web_search","result":"no call"} is content, so the
        # strip keeps it visible.
        try:
...
def _bare_json_call_shaped(obj) -> bool:
    ...
    if not isinstance(obj, dict):
        return False
    # The parser requires a TOP-LEVEL name; a nested one (e.g. in a "result" value of an ordinary JSON answer) is
    # data, and stripping it name-agnostically would delete content.
    name = obj.get("name") or obj.get("function") or ""
    if not isinstance(name, str) or not name:
        return False
    if "parameters" in obj:
