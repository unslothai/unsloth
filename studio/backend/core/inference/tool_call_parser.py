...
def strip_tool_markup(
    ...
    *,
    ...
    ``enabled_tool_names`` gates the bare reasoning-rehearsal form: a ``name[ARGS]{...}``
    whose ``name`` is not an enabled tool is prose and is kept, matching detection.
    """
    if final:
...
def parse_tool_calls_from_text(
    ...
    *,
    ...
    # ``name[ARGS]{json}`` use the shared tool_healing parser (the strict/Auto-Heal +
    # nested-marker + ``<|"|>`` handling GGUF relies on). ``enabled_tool_names`` gates the
    # ambiguous bare-rehearsal form so an inactive ``foo[ARGS]{..}`` stays prose.
