...
def apply_speed_optims(
    ...
    *,
    ...
    # TF32 (max) and cudnn.benchmark (any non-off CUDA load) are process-global; the caller snapshots/restores them so a later `off` load never inherits them.
    if mode == SPEED_OFF:
        return applied
    ...
    # Near-lossless: cuDNN autotunes the fixed-shape VAE convs (CUDA only). It may pick a different conv algorithm, so it is a "default"-tier (not bit-identical) win.
    if on_cuda:
        applied["cudnn_benchmark"] = _enable_cudnn_benchmark(logger)

    # Consumer-only: fp16 GEMMs accumulate in fp16 (~2x on GeForce-class parts; datacenter parts gain nothing). bf16 loads measured bit-identical with the flag on (36/36 same-seed cases), so on the neutral tiers it engages only when compute dtype is NOT fp16.
    # fp16 pipelines showed same-seed drift (mean 2-5%), so fp16 compute gets it only under ``max``. Guarded by _FP16_ACCUM_DENY and the UNSLOTH_DISABLE_FP16_ACCUM kill switch.
    if on_cuda:
    ...
    # default = LIGHT: GGUF compiles ONLY the dequant op chain (cheap, VRAM-free, resolution-invariant); dense has no dequant, so falls back to the regional block compile.
    # max = FULL: regional max-autotune compile of the repeated block (subsumes the dequant fusion). eager = no compile.
    if mode == SPEED_DEFAULT:
        if is_gguf and on_cuda and family_allows_compile:
            ...
            # A U-Net (SDXL) fuses QKV BEFORE its whole-module compile: 36.3 vs 39.3 ms/step (LPIPS 0.033). DiTs were neutral under the regional compile, so they keep the fuse on the max tier only.
    ...
    # A compiled U-Net family also compiles the VAE decode (a real share at SDXL step rate: 4.98 to 4.25 s over 4 images, LPIPS unchanged). DiT families skip it. dynamic=True keeps it resolution-robust; fullgraph=False tolerates offload hooks.
...
# U-Net denoisers ship no ``_repeated_blocks`` (heterogeneous block mix), so the regional compile cannot reach them; these classes get a WHOLE-module STATIC ``torch.compile`` instead.
# On SDXL (B200, 30 steps / 1024px): 26.9 ms/step vs the 45.9 ms/step bit-exact reference, 1.61x end-to-end at LPIPS 0.034, while dynamic=True compiles 5x slower for less win.
...
def _compile_repeated_blocks(
    ...
    *,
    ...
    if not dits and unet is None:
        return False
    # default: dynamic=True -- fast cold start, no recompile on resolution change. max: mode="max-autotune-no-cudagraphs" + dynamic=False -- Triton autotuning for a few % more, at a longer compile and a recompile per resolution.
    # CUDA-graph modes are NOT used: they crash on the regional block (its static output buffer is overwritten across steps).
    # fullgraph drops to False under a step cache OR offloading: both insert an ``@torch.compiler.disable``d function, i.e. a graph break fullgraph=True rejects. The break is cheap; the rest still compiles.
    kwargs: dict[str, Any] = {
    ...
    try:
        ...
        # Heterogeneous-block DiTs (e.g. Z-Image) compile ~one graph per distinct block shape, and Z-Image needs ~11, above dynamo default recompile_limit of 8. Past the limit a resident load hard-errors under fullgraph, so raise it to 64 (diffusers documented regional-compile fix).
        ...
        # Match eager intermediate rounding in inductor fused pointwise kernels: they keep chains in fp32 where eager materialises bf16 between ops, a per-forward delta a multi-step denoise amplifies.
        # Measured LPIPS vs eager: Qwen-Image 0.019 to 0.006, FLUX.1-dev 0.046 to 0.029 (+2% step), FLUX.2-klein 0.018 to 0.017, HunyuanVideo-1.5-720p 0.221 to 0.052, all at ~zero cost. Process-global, so snapshot_backend_flags restores it on unload.
        inductor_cfg = _inductor_config()
        if inductor_cfg is not None and hasattr(inductor_cfg, "emulate_precision_casts"):
        ...
        return False
    if unet is not None:
        # Whole-module static compile for the U-Net classes above. fullgraph mirrors the regional decision (in practice only offload lowers it); dynamic is ALWAYS False, so each new (height, width, batch) pays its own compile.
        # ``Module.compile`` keeps the module identity, so unload/status/LoRA see the same object.
    ...
    for transformer in dits:
        try:
            ...
            _warn(logger, "compile_repeated_blocks", exc)
            continue
        # A step cache engaged BEFORE this compile has already wrapped each block forward in a @torch.compiler.disable'd hook, so the compute branch would run eager and forfeit the regional compile.
        # Re-point the hooks inner forward at compiled wrappers (no-op without cache hooks); the toggle path is armed by apply_step_cache.
        try:
...
# Families the overflow harness found to produce non-finite activations / NEW black frames under fp16 accumulation. Empty by measurement: no overflow across all six families (bf16 bit-identical, fp16 finite; the fp16 same-seed drift is why fp16 compute is gated to ``max``).
