...
#
# A return signature alone is not enough to justify an entry here. It decides whether FBCache CAN
# hook the class, not whether the cache is correct for the loop above it: Qwen-Image-2.1 reuses a
# prefix KV cache, so the sequence shortens after step 0 and the stored residuals no longer line up.
# ``_reuses_prefix_kv`` refuses the cache for that, before this table is consulted. Read the loop.
