...
class VideoFamily:
    ...
    # Opt-in to the manual CUDA-graph capture of the denoiser forward (diffusion_cuda_graph.py); off for the video
    # families measured device-bound at 1.0x, on for MiniMax-H3, which runs ONE forward per step with no CFG and no
    # step cache, so a capture has a single stable input tree to replay.
...
_FAMILIES: tuple[VideoFamily, ...] = (
    VideoFamily(
        ...
        # The one video family whose denoiser forward is captured into a CUDA graph; see the field.
