...
_FAMILIES: tuple[VideoFamily, ...] = (
    ...
    VideoFamily(
        ...
        # MiniMaxH3TokenRefinerBlock) and the shape-stable unit is the whole transformer_blocks
        # stack: every block sees (1, S, 5376) plus an (S,) index tensor, and S is the PACKED
        # length -- 18,870 video rows + 207 audio rows + the caption's text rows at 960x544x124.
        # A caption is a fraction of a percent of that (S = 19,096 at 19 tokens vs 19,479 at 402,
        # a 2% swing), and S cannot change mid-denoise, so dynamic=True traces once and holds:
        # measured 1.298-1.342 s/step eager vs 1.000-1.040 compiled (1.30x), first forward 10.2 s,
        # zero recompiles across captions of 19/19/37/128/402 tokens. The loader only engages this
        # when the denoiser is RESIDENT; compiling a denoiser inside a full CPU-offload rotation
        # measured slower than eager, so that case stays on the no-compile speed tier.
