...
* Authenticated. Each launch mints a random credential that only travels to
  ...
  so another local user on the same machine cannot use the tunnel. SRT instead
  uses an explicitly validated per-launch Unix socket in an owned private
  directory; only that listener treats possession of its socket as authority.
...
class AllowlistProxy:
    ...
    def __init__(
        ...
        *,
        ...
        self._private_unix_authority = False
        self._thread: threading.Thread | None = None
        self._closed = threading.Event()
        self._cleanup_debt: str | None = None
    ...
    def serve_private_unix_listener(self, listener: socket.socket) -> None:
        """Authorize by a private per-launch Unix socket, never a shared TCP port.

        SRT's internal forwarder does not carry a proxy credential. Its socket
        must therefore be owned by this user in a private directory, with only
        this invocation receiving the bound path inside its namespace.
        """
        try:
            if os.name != "posix" or listener.family != socket.AF_UNIX:
                raise ValueError("private proxy authority requires a POSIX Unix socket")
            path = os.path.abspath(os.fsdecode(listener.getsockname()))
            if os.path.realpath(path) != path:
                raise ValueError("private proxy socket cannot use symlinks")
            ...
            raise
        self._private_unix_authority = True
        try:
            self.serve_listener(listener)
        except BaseException:
            self._private_unix_authority = False
    ...
    def close(self) -> None:
        ...
        with self._tunnel_lock:
            if self._closed.is_set():
                if self._cleanup_debt is not None:
                    raise RuntimeError(self._cleanup_debt)
                return
            self._closed.set()
        ...
        alive = int(thread is not None and thread is not current and thread.is_alive())
        ...
        for sock in pending:
            _close_quietly(sock)
        if alive:
            self._cleanup_debt = f"Tool network proxy cleanup left {alive} worker thread(s) running"
            raise RuntimeError(self._cleanup_debt)
    ...
    def _connect_upstream(self, host: str, port: int) -> socket.socket:
        ...
        for address in addresses:
            ...
            if remaining <= 0:
                ...
                break
            try:
                upstream = self._connect_address(address, port, deadline)
            except OSError as exc:
                if self._closed.is_set():
                    raise OSError("proxy closed during connect") from exc
                last_error = exc
                continue
    ...
    def _connect_address(self, address: str, port: int, deadline: float) -> socket.socket:
        """Register a nonblocking connection before it can create outbound traffic."""
        ...
        try:
            with self._tunnel_lock:
                if self._closed.is_set():
                    raise OSError("proxy is closed")
                self._tunnels.add(upstream)
                upstream.setblocking(False)
                result = upstream.connect_ex((address, port))
            pending = {0, errno.EINPROGRESS, errno.EWOULDBLOCK, errno.EALREADY, errno.EINTR}
