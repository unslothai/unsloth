...
class InferenceOrchestrator:
    ...
    def _spawn_subprocess(
        ...
        with (
            ...
            self._proc.start()
            # Held locally as well. _shutdown_subprocess can observe a not-yet-alive
            # child, clear self._proc and finish its sweep while start() is still
            # returning; re-reading the attribute here would then raise before this
            # child was ever adopted, leaving it outside both the teardown and the
            # lifetime record.
            _spawned_proc = self._proc
        ...
        if is_process_shutting_down() or self._proc is not _spawned_proc:
            ...
            # The local handle is the only one left, so reap it here.
            try:
