        ...
        try:
            ...
            for attempt in range(2):
                ...
                if resp.get("success"):
                    ...
                    # A load always spawns a fresh subprocess holding only this model, so mirror that. A lingering
                    # stale name would pass unload_model's "not in self.models" guard, and the worker's absent-name
                    # fallback would unload its *active* model, not the already-gone one.
                    self.models = {}
                    self.models[self.active_model_name] = _mirrored_model_entry(
                        model_info, model_name
                    )
                    # Lets the already-loaded shortcut tell a CPU request from the GPU model it would otherwise report
                    # as satisfied. Native audio only: marking anything else tells training a GPU model holds no VRAM.
