...
# Scanned/image-only pages get a transcription prompt instead of a summary so
# the page's actual text lands in the chat context rather than a description.
_OCR_PAGE_PROMPT = (
    "Transcribe everything on this page to Markdown. Preserve headings, "
    "lists, and tables. Output only the transcribed content with no "
...
_LOCAL_VLM_CAPTION_CONCURRENCY = max(
    1, int(os.environ.get("UNSLOTH_STUDIO_LOCAL_CAPTION_CONCURRENCY", "2"))
)
_DEFAULT_VLM_CAPTION_CONCURRENCY = max(
    1, int(os.environ.get("UNSLOTH_STUDIO_CAPTION_CONCURRENCY", "3"))
)
...
_PAGE_RENDER_DPI = max(36, int(os.environ.get("UNSLOTH_STUDIO_PAGE_RENDER_DPI", "120")))
_MAX_PAGE_RENDER_PIXELS = 4_000_000
# A PDF page is treated as scanned/image-only when its text layer is below this
# many non-whitespace characters and a single image covers most of the page.
# Born-digital pages already extract via pymupdf4llm, so only scanned pages are
# rendered for VLM OCR, keeping born-digital extraction render-free and fast.
_SCANNED_TEXT_MIN_CHARS = max(0, int(os.environ.get("UNSLOTH_STUDIO_SCANNED_TEXT_MIN_CHARS", "16")))
_SCANNED_IMAGE_AREA_FRAC = 0.5
_MIME_TO_SUFFIX = {
    "application/pdf": ".pdf",
...
def _page_is_scanned(page: Any) -> bool:
    """True when a PDF page has no usable text layer but is mostly a raster
    image (scanner output). Born-digital pages return False so they are never
    rendered for VLM OCR, keeping text-PDF extraction render-free."""
    try:
        text = page.get_text("text")
    except Exception:  # pragma: no cover - defensive against PyMuPDF variants
        return False
    if len("".join(text.split())) >= _SCANNED_TEXT_MIN_CHARS:
        return False
    try:
        rect = getattr(page, "rect", None)
        page_area = max(
            float(getattr(rect, "width", 0) or 0) * float(getattr(rect, "height", 0) or 0),
            1.0,
        )
        for info in page.get_image_info():
            ...
            if (width * height) / page_area >= _SCANNED_IMAGE_AREA_FRAC:
                return True
    except Exception:  # pragma: no cover - fall back to a coarse image check
        try:
...
def _extract_pdf(
    ...
    try:
        ...
        if max_figures > 0 and page_count > 0:
            # use_vlm_ocr renders every page (explicit scanned mode); the default
            # path renders only pages without a text layer, so born-digital PDFs
            # do no page rendering and issue no VLM calls.
            if use_vlm_ocr:
                pages_to_render = list(range(page_count))
            else:
                pages_to_render = [
                    index for index in range(page_count) if _page_is_scanned(doc[index])
                ]
            rendered_pages: set[int] = set()
            for position, page_index in enumerate(pages_to_render):
                if len(figures_out) >= max_figures:
                    truncated_count += len(pages_to_render) - position
                    break
            ...
            if not use_vlm_ocr:
                try:
                    ...
                    for page_index in range(page_count):
                        # A scanned page was already emitted whole as a kind="page"
                        # figure; skip it so its full-page raster is not re-added
                        # as a duplicate kind="figure".
                        if page_index in rendered_pages:
                            continue
                        page = doc[page_index]
                        try:
            ...
            async with sem:
                ...
                try:
                    ...
                    caption, error = await _describe_image_via_vlm(
                        ...
                        prompt = _OCR_PAGE_PROMPT if is_page else _DESCRIBE_PROMPT,
                        max_tokens = 1024 if is_page else 512,
                    )
