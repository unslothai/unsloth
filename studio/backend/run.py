...
def _repair_pinned_diffusers(silent: bool) -> None:
    """Repair before importing the app; exit if a peer still modifies packages at timeout."""
    echo = (lambda _line: None) if silent else (lambda line: print(line, flush = True))
    try:
        from utils.diffusers_repair import PeerInstallInProgress, repair_diffusers_before_imports
    ...
    except PeerInstallInProgress as exc:
