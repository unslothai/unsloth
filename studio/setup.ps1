...
if ($env:SKIP_STUDIO_BASE -ne "1" -and $env:STUDIO_LOCAL_INSTALL -ne "1") {
    ...
    # -I so the cwd cannot answer, mirroring setup.sh. Verdict-neutral otherwise:
    # installed_version_probe scans the venv's own purelib/platlib and never reads
    # sys.path, so dropping PYTHONPATH and user site cannot change what it finds.
    $InstalledVer = try {
        $_installedVersionOutput = & python -I -c "
import sys
