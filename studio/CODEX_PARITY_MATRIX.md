...
Feature source commit `3af2f62e6d72564b2fa0840ca491a48817f12c3d` is merged with fetched `upstream/main` at `93402937c321867ab7174017762534d1d8c692e1` through merge commit `4720558aa`. Final Deep Research project-context hardening is committed at `59995084d`, and the full CI matrix is committed at `b1e6809d1`. The implementation is ordinary source. The live PR head and remote checks must be verified after publication.
...
The connected GitHub flow prepares a bounded redacted request, freezes connector identity and request digest, and requires one-use confirmation. A connector error is reported as an unknown outcome so the user can check GitHub before retrying. Focused tests use a controlled connector. No live request is claimed.
