...
Assistant continuations are also left unchanged until the existing token-count API
can price continuation rendering. Later tool-result turns still run preflight.

## Tests

The existing backend tests cover MLX prompt counting and request-scoped worker IPC.
The compaction tests cover the shared counting contract, request policy,
rolling fitting, fallback, and per-iteration tool-loop fitting. The
