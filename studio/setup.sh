...
setup_frontend_node_get_installed_version() {
    NODE_OK=false
...
setup_frontend() {
    ...
    _NEED_FRONTEND_BUILD="${_NEED_FRONTEND_BUILD:-true}"
    ...
    if [[ "${bun_install_failed}" == "true" ]]; then
...
setup_unsloth_studio_path() {
    ...
    # shellcheck disable=SC2088 # We do not want ~/ to expand
...
install_transformers() {
    ...
    if [ "$_NEED_T5_INSTALL" = true ]; then
        ...
        run_quiet "install transformers 5.3.0" install_python_package --target "$VENV_T5_530_DIR" --no-deps "transformers==5.3.0"
        run_quiet "install huggingface_hub for t5_530" install_python_package --target "$VENV_T5_530_DIR" --no-deps "huggingface_hub==1.8.0"
        run_quiet "install hf_xet for t5_530" install_python_package --target "$VENV_T5_530_DIR" --no-deps "hf_xet==1.4.2"
        run_quiet "install tiktoken for t5_530" install_python_package --target "$VENV_T5_530_DIR" "tiktoken"
        ...
        run_quiet "install transformers 5.5.0" install_python_package --target "$VENV_T5_550_DIR" --no-deps "transformers==5.5.0"
        run_quiet "install huggingface_hub for t5_550" install_python_package --target "$VENV_T5_550_DIR" --no-deps "huggingface_hub==1.8.0"
        run_quiet "install hf_xet for t5_550" install_python_package --target "$VENV_T5_550_DIR" --no-deps "hf_xet==1.4.2"
        run_quiet "install tiktoken for t5_550" install_python_package --target "$VENV_T5_550_DIR" "tiktoken"
...
setup_llama_cpp_wsl() {
    ...
    for _pkg in "${_GGUF_DEPS[@]}"; do
        case "$_pkg" in
            build-essential) command -v gcc >/dev/null 2>&1 || _STILL_MISSING+=("$_pkg") ;;
            pciutils) command -v lspci >/dev/null 2>&1 || _STILL_MISSING+=("$_pkg") ;;
            libcurl4-openssl-dev) command -v curl-config >/dev/null 2>&1 || _STILL_MISSING+=("$_pkg") ;;
            *) command -v "$_pkg" >/dev/null 2>&1 || _STILL_MISSING+=("$_pkg") ;;
        esac
    done
...
build_llama_cpp_buildflags_macos() {
    # Metal takes precedence on Apple Silicon (CUDA/ROCm not functional on macOS)
    _BUILD_DESC="building (Metal)"
    CMAKE_ARGS+=(-DGGML_METAL=ON -DGGML_METAL_EMBED_LIBRARY=ON -DGGML_METAL_USE_BF16=ON -DCMAKE_INSTALL_RPATH=@loader_path -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON)
    CPU_FALLBACK_CMAKE_ARGS+=(-DGGML_METAL=OFF)
    _TRY_METAL_CPU_FALLBACK=true
}

build_llama_cpp_buildflags_nvidia_cuda() {
    ...
    CMAKE_ARGS+=(-DCMAKE_CUDA_FLAGS=--threads=0)
}

build_llama_cpp_buildflags_amd_rocm() {
...
build_llama_cpp() {
    ...
    for cmd in cmake git; do
        if ! command -v "${cmd}" &>/dev/null; then
            step "llama.cpp" "skipped (${cmd} not found)" "$C_WARN"
            [ -f "$LLAMA_SERVER_BIN" ] || _LLAMA_CPP_DEGRADED=true
        fi
    done
    ...
    if command -v nvcc &>/dev/null; then
        ...
        local NVCC_EXPORT_PATH
        NVCC_EXPORT_PATH=$(dirname "${NVCC_PATH}")
        export PATH="${NVCC_EXPORT_PATH}:$PATH"
        GPU_BACKEND="cuda"
    fi
    ...
    if [ -z "$GPU_BACKEND" ]; then
        if command -v hipcc &>/dev/null; then
            ...
            local ROCM_HIPCC_EXPORT_PATH
            ROCM_HIPCC_EXPORT_PATH="$(dirname "${ROCM_HIPCC}")"
            export PATH="${ROCM_HIPCC_EXPORT_PATH}:$PATH"
            GPU_BACKEND="rocm"
        fi
    fi
