...
# Carve-outs, same reasoning as the mirrors: a recorded size binds a path only
# when every claim on it carries one and the file sits below the smallest --
...
# collision, not damage; shared non-runtime roots
...
if [ "$_SKIP_VERSION_CHECK" != true ] && [ "${SKIP_STUDIO_BASE:-0}" != "1" ] && [ "${STUDIO_LOCAL_INSTALL:-0}" != "1" ]; then
    ...
    # -I so the cwd cannot answer: setup cd's into $SCRIPT_DIR, and without it a
    # stray re.py/json.py beside this script shadows the stdlib install_manifest
    # imports. Verdict-neutral otherwise -- installed_version_probe scans the
    # venv's own purelib/platlib via _metadata_scan_paths and never reads sys.path,
    # so dropping PYTHONPATH and user site cannot change which records it finds.
    if INSTALLED_VER=$("$VENV_DIR/bin/python" -I -c "
import sys
...
" "$INSTALLED_VER" "$UNSLOTH_DESKTOP_BACKEND_VERSION" 2>/dev/null; then
        ...
        # match the one that took the fast path: INSTALLED_VER above answers for the
        # newest readable record, and an interrupted upgrade can leave a second
        # dist-info beside it -- the probe re-picks a healthy tied sibling. A crashed
        # or silent probe says nothing about the venv and leaves the fast path alone.
        # LAST of the interpreter escapes on purpose: it is the only bounded-but-slow
        # one, and the "= true" guard means an arm above that already cleared the flag
        # skips it outright. Still deliberately BEFORE the XPU escapes:
        # tests/sh/test_setup_xpu_fastpath_escape.sh extracts that chain from the
        # _setup_pin= assignment down and runs it directly, and it must stay free of
        # anything that launches an interpreter (a wedged Intel driver would hang one).
