...
EOF
    ...
    # Resolved ONCE, ahead of the report: it shells out to lspci, and it now gates four arms
    # rather than the one it was written for. Empty means "not a generation Unsloth skips".
    _setup_unsup_gfx=$(_setup_unsupported_gfx_any "$_setup_mkt") || _setup_unsup_gfx=""
    # A visible AMD GPU does not override an explicit CPU, CUDA, or XPU selection. Name that
    # choice separately instead of diagnosing the expected unavailable CUDA/HIP runtime as a
    # kernel or group-membership failure.
    ...
    if [ -z "$_setup_non_rocm_backend" ]; then
        ...
        case "$_setup_backend_leaf" in
            ...
            cu[0-9]*)
                case "${_setup_backend_leaf#cu}" in
                    *[!0-9]*) ;;
                    *) _setup_non_rocm_backend=cuda ;;
                esac
                ;;
        esac
    fi
    ...
    # Every torch-cannot-use-it arm below is guarded on _setup_unsup_gfx being empty, so the
    # "no ROCm PyTorch wheels" arm further down outranks all three. On an RDNA 1 or Polaris
    # host torch is CPU-only by construction, so torch.cuda.is_available() is False and the
    # last arm would otherwise win and send the user after the amdgpu driver and the render
    # and video groups -- the advice unslothai#8577 removed, because no group membership and
    # no driver reinstall produces a ROCm wheel for a generation Unsloth does not ship one
    # for. The same reasoning covers the first two arms: changing the backend pin or the
