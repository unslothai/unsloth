...
def recorded_no_torch(root: Optional[Path] = None) -> Optional[bool]:
    ...
    The manifest key and marker are affirmative evidence. None means neither
    recorded the mode, so callers must fall back to platform detection rather
    than infer intent from the current package contents.
    """
