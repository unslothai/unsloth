...
class TestGlobSkippedWhenNotBothConfigs(unittest.TestCase):
    ...
    def _run_both_exist_block(self, is_model, is_peft, supports_llama32, model_name, is_local_dir=False):
        ...
        glob_mock = MagicMock(return_value=[
    ...
    def test_glob_skipped_when_is_model_false(self):
        both_exist, glob_called = self._run_both_exist_block(
            is_model=False, is_peft=True, supports_llama32=True,
    ...
    def test_glob_skipped_when_is_peft_false(self):
        both_exist, glob_called = self._run_both_exist_block(
            is_model=True, is_peft=False, supports_llama32=True,
    ...
    def test_glob_skipped_when_both_false(self):
        both_exist, glob_called = self._run_both_exist_block(
            is_model=False, is_peft=False, supports_llama32=True,
    ...
    def test_glob_skipped_when_supports_llama32_false(self):
        both_exist, glob_called = self._run_both_exist_block(
            is_model=True, is_peft=True, supports_llama32=False,
            model_name="org/some-model",
        )
    ...
    def test_glob_called_when_both_true_and_supports_llama32(self):
        both_exist, glob_called = self._run_both_exist_block(
            is_model=True, is_peft=True, supports_llama32=True,
            model_name="org/mixed-repo",
        )
        self.assertTrue(glob_called, "glob should be called when is_model and is_peft are both True")
        self.assertTrue(both_exist)

    def test_local_dir_skips_glob(self):
        both_exist, glob_called = self._run_both_exist_block(
            is_model=True, is_peft=True, supports_llama32=True,
            model_name="/local/path/to/model", is_local_dir=True,
        )
