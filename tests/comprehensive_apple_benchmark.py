...
class ComprehensiveBenchmark:
    ...
    def run_mlp_benchmark(self, batch_size = 1, seq_len = 1, dim = 4096, hidden_dim = 11008):
        ...
        def torch_mlp(x):
            ...
            u = (
                torch.nn.functional.linear(x, upW) + (x @ A.T) @ B.T * S
            )  # Reuse A,B for simplicity
            act = torch.nn.functional.silu(g) * u
            return (
                torch.nn.functional.linear(act, downW) + (act @ B.T.T) @ B.T * S
            )  # Reuse for dummy
        ...
        if HAS_MLX:
            ...
            with torch.no_grad():
                y_mlx = apply_lora_mlp_swiglu(
                    X, gateW, None, A, B, S, upW, None, A, B, S, downW, None, A, B, S
                )
    ...
    def run_qkv_benchmark(self, batch_size = 1, seq_len = 1, dim = 4096, hidden_dim = 4096):
        ...
        # LoRA Dummy
        QA = torch.randn(8, dim, device = self.device, dtype = self.dtype)
        QB = torch.randn(dim, 8, device = self.device, dtype = self.dtype)
        QS = 1.0
