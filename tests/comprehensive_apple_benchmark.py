...
try:
    ...
    from unsloth.kernels.mlx.bridge import torch_to_mlx, mlx_to_torch, synchronize_mps, mlx_context
...
class ComprehensiveBenchmark:
    def __init__(self, device="mps", dtype=torch.float16):
        if not torch.backends.mps.is_available() and device == "mps":
            print(f"{colors.RED}Warning: MPS not available, switching to CPU.{colors.ENDC}")
    ...
    def run_mlp_benchmark(self, batch_size=1, seq_len=1, dim=4096, hidden_dim=11008):
        log_header(f"MLP Benchmark: B={batch_size}, S={seq_len}, D={dim}, H={hidden_dim}")
        ...
        if HAS_MLX:
            ...
            log_result("MLX 4-Bit (Cached)", mlx_4_latency, error_q, extra=f"{colors.GREEN}[Quantized]{colors.ENDC}")
    ...
    def diagnose(self):
        ...
        if HAS_MLX:
            ...
            try:
                ...
                status = f"{colors.GREEN}OK{colors.ENDC}" if diff < 1e-4 else f"{colors.RED}FAIL (Diff: {diff}){colors.ENDC}"
            ...
            try:
                ...
                print(f" Custom Metal GEMV Kernel: {colors.RED}FAILED/UNAVAILABLE: {e}{colors.ENDC}")
