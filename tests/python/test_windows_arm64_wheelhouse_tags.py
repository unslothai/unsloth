...
class TestTheSkipGateAuditsTheArm64FilteredFile:
    ...
    def test_pip_install_and_the_audits_share_it(self, ips):
        """Four call sites: both installs, the closure record, and the on-disk skip check.

        The count is the invariant's teeth, not the invariant itself: every path that installs a
        requirements file, or reasons about one, has to filter it the same way, or a caller decides
        a pin is satisfied against a file different from the one that was installed. ``pip_install``
        and ``pip_install_try`` are now both such paths, so a fourth shared call site is the
        invariant holding rather than loosening. A caller that grew its own filtering would still
        show up here, as a count that moved without a matching call site.
        """
        assert (
            STACK_SRC.count("_effective_requirements(req)") == 4
