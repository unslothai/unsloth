...
def test_splade_last_uses_a_distinct_head_per_eos_slot(splade, checkpoint):
    ...
    wrong = torch.tensor([math.log1p(v) if v > 0.0 else 0.0 for v in at_last], dtype = torch.float32)
...
def test_two_optimizer_steps_change_the_head_weights(splade, checkpoint):
    ...
    assert any(not torch.equal(old, new.detach()) for old, new in zip(before, head.sparse_lm_heads))
