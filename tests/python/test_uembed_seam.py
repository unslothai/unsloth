...
def _load_selection_function():
    ...
    function = next(
        (
            ...
            if isinstance(node, ast.FunctionDef) and node.name == "_ensure_uembed_offset_pooling"
        ),
        None,
    )
...
def test_positive_num_eos_replaces_stock_modules_json_pooling(ensure_offset_pooling, stock_mode):
