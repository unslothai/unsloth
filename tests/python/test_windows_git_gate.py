...
def test_an_unbuilt_local_llama_dir_still_requires_git(tmp_path, trigger):
    ...
    # the normal install, so the source build still runs and still needs git. Suppressing
    # the requirement here let a no-git host silently degrade to a prebuilt instead.
    env = {
        "UNSLOTH_LOCAL_LLAMA_CPP_DIR": str(tmp_path),
        trigger: "1",
    }
