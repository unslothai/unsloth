...
def test_baseline_module_assembly_still_builds_stock_pooling_with_the_passed_mode():
    ...
    assert (
        isinstance(mode_argument, ast.Name) and mode_argument.id == "pooling_mode"
...
def test_offset_pooling_is_wired_as_the_opt_in_branch_of_module_assembly():
    ...
    guard = guards[0]
    assert not any(
        _calls_to(statement, "Pooling") for statement in guard.body
    ), "stock Pooling must not run on the offset branch"
    assert any(
        _calls_to(statement, "Pooling") for statement in guard.orelse
