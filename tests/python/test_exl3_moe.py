...
def _make_experts(num_experts=4, hidden=8, inter=6, cache=0):
...
def _reference(exp, hidden_states, top_k_index, top_k_weights, act_fn):
    ...
    for ei in hit:
        ...
        gu = torch.cat([exp._gate_up[e].gate.get_weight_tensor(),
                        exp._gate_up[e].up.get_weight_tensor()], dim=1).t().to(cur.dtype)
...
class TestExl3QuantizedExperts(unittest.TestCase):
    def _inputs(self, tokens=5, hidden=8, num_experts=4, topk=2):
        torch.manual_seed(0)
