...
eval_fastpath() {
    ...
    (
        ...
        substep() { :; }
        # The slice also carries the payload probe's fast-path arm now. Model a
        # healthy managed install -- probe version equal to the metadata version
        # that got here -- so the block runs for real. Left undefined it is a
        # command-not-found whose empty answer no-ops all three arms, which is
        # indistinguishable from the escape working.
        _bounded_pkg_probe() { printf '%s' "$INSTALLED_VER"; }
