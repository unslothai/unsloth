...
#
# The two sentinels outside the venv carry the venv they belong to, and are written here
# exactly as their writers emit them: bare `: >` placeholders would only prove that any file
# of that name flips the layout, which is the defect the negative case below covers.
for _sent in owner conf shim; do
    ...
    case "$_sent" in
        ...
        conf)  mkdir -p "$H/flat/share"
               printf "UNSLOTH_EXE='%s/unsloth_studio/bin/unsloth'\n" "$F" \
                   > "$H/flat/share/studio.conf" ;;
        shim)  mkdir -p "$H/flat/bin"
               {
                   printf '%s\n' '#!/bin/sh'
                   printf '%s\n' '# Generated by install.sh --portable. Keeps every Unsloth path inside'
                   printf "exec '%s/unsloth_studio/bin/unsloth' \"\$@\"\n" "$F"
               } > "$H/flat/bin/unsloth"
               chmod +x "$H/flat/bin/unsloth" ;;
    esac
    out="$(env -i HOME="$H" PATH="$PATH" USER="${USER:-tester}" \
...
# ...and a file of the right NAME that is not ours flips nothing. `unsloth` is an ordinary
# word, so a reused root can hold the user's own bin/unsloth helper beside their own
# unsloth_studio virtualenv; reading that as a flat install of ours moved their environment
# aside and deleted the rollback copy once the install succeeded.
H="$(new_home)"
mkdir -p "$H/flat/unsloth_studio/lib" "$H/flat/bin" "$H/flat/share"
: > "$H/flat/unsloth_studio/pyvenv.cfg"
printf '#!/bin/sh\necho my own helper\n' > "$H/flat/bin/unsloth"
printf 'written by something else\n' > "$H/flat/share/studio.conf"
F="$(CDPATH= cd -P -- "$H/flat" && pwd -P)"
out="$(env -i HOME="$H" PATH="$PATH" USER="${USER:-tester}" \
    UNSLOTH_PORTABLE=1 UNSLOTH_HOME="$F" bash -c "$SNIP" _)"
check "unrelated files of the right name do not flatten" "$F/studio" "$(field "$out" 2)"
