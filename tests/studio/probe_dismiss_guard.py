...
The guard must swallow too little nowhere:
  ...
  held_enter     press and hold on the control, press ENTER while still held, release. The press
                 focuses the control, so Enter activates it from the keyboard and the browser
                 generates a click (`detail === 0`, no `pointerId`) BEFORE the release click. One
                 arm, two clicks: whichever arrives first spends it. Distinct from
                 `held_modifier`, where the key produces no click at all and the only question is
                 whether the guard disarms on keydown.
...
# Watch the Delete button ITSELF, not just the message count.
#
# "Did a message disappear" is the outcome, but it cannot separate "the click was swallowed" from
# "the click landed and the handler happened not to delete". A listener on the control answers the
# question the guard is actually about: did a click reach it. The document-level recorder is added
# BEFORE the gesture, so it is ahead of the capture listener `arm()` installs and sees every click
# whether or not that one stops propagation -- `swallowClick` calls `stopPropagation` but not
# `stopImmediatePropagation`, so a sibling capture listener on `document` still runs.
#
# `detail` and `pointerId` are what separate the two clicks in this case. A click the browser
# synthesises from a pointer release carries `detail >= 1` and the gesture's `pointerId`; a click
# generated by activating a focused control from the KEYBOARD carries `detail === 0` and no
# pointer. Recording both means the verdict does not rest on ordering alone.
WATCH_DELETE_JS = """
() => {
  const del = window.__heavyThread.actionButton("Delete message");
  if (!del) return null;
  const shape = (e) => ({
    detail: e.detail,
    pointerId: e.pointerId === undefined ? null : e.pointerId,
    isTrusted: e.isTrusted,
    target: e.target ? e.target.tagName : null,
    targetText: e.target && e.target.textContent
      ? e.target.textContent.trim().slice(0, 24) : null,
  });
  window.__dismissProbe = { deleteClicks: [], docClicks: [], activeAfterPress: null };
  del.addEventListener("click", (e) => {
    window.__dismissProbe.deleteClicks.push(shape(e));
  });
  document.addEventListener("click", (e) => {
    window.__dismissProbe.docClicks.push(shape(e));
  }, true);
...
# Which control the press actually focused. The claim under test depends on the dismissing
# pointerdown leaving focus ON the Delete button; if the menu's own close returns focus to its
# trigger instead, Enter activates the TRIGGER and the premise never forms. That is a real
# outcome, not a probe failure, so it is recorded rather than asserted.
ACTIVE_JS = """
() => {
  ...
  return {
    ...
    isDelete: a === window.__heavyThread.actionButton("Delete message"),
    isTrigger: a === window.__heavyThread.actionButton("More"),
  };
}
"""
...
async def one_case(page, case: str) -> dict:
    ...
    if case == "quick":
        ...
        # Press and HOLD on the Delete button, press Enter while STILL HOLDING, then release.
        #
        # `held_modifier` covers the keys that generate no click. Enter is the one that does: the
        # dismissing `pointerdown` focuses the control it landed on, and Enter on a focused button
        # is an activation, so the browser generates a click from the KEYBOARD before the pointer
        # is anywhere near released. The guard has exactly one arm and `swallowClick` spends it on
        # whichever click arrives first, so if that is the keyboard one, the release click that
        # the arm exists for arrives with the guard already gone.
        #
        # Recorded rather than assumed at every step, because two things have to hold for the
        # sequence to mean anything and neither is guaranteed: the press has to leave focus on
        # Delete (Radix could return it to the trigger on close), and Enter has to produce a click
        # while a mouse button is down.
        spot = await page.evaluate(WATCH_DELETE_JS)
        if not spot:
            return {"case": case, "error": "no Delete button to watch"}
        x, y = spot["x"], spot["y"]
        await page.mouse.move(x, y)
        await page.mouse.down()
        await page.wait_for_timeout(120)
        active = await page.evaluate(ACTIVE_JS)
        await page.keyboard.press("Enter")
        await page.wait_for_timeout(120)
        mid = await page.evaluate("() => window.__dismissProbe.deleteClicks.length")
        await page.mouse.up()
        await page.wait_for_timeout(1500)
        after = await page.evaluate(FACTS_JS)
        probe = await page.evaluate("() => window.__dismissProbe")
        return {
            "case": case,
            "activeAfterPress": active,
            "deleteClicksBeforeRelease": mid,
            # Every click the document saw, in order, with the two fields that say which is which.
            "docClicks": probe["docClicks"],
            # The only ones that matter: clicks that actually reached the control.
            "deleteClicks": probe["deleteClicks"],
            "reachedDelete": len(probe["deleteClicks"]) > 0,
...
def main() -> int:
    ...
    # A click that REACHED the destructive control is the failure whether or not the handler
    # behind it happened to remove a message on this run. Asking only "did the count drop" makes
    # the verdict depend on what Delete does rather than on whether the guard held.
    reached = [c["case"] for c in result["cases"] if c.get("reachedDelete")]
    if reached:
        print(
            f"[probe] FAIL: a click reached the Delete control via {reached}",
            flush = True,
        )
        deleted = deleted + reached
