...
def test_one_repetition_that_never_opened_the_menu_poisons_the_median() -> None:
    ...
    summary = HARNESS.summarise(
        {
            "menu": [
                menu_repetition(80.0)["menu"],
                menu_repetition(None)["menu"],
                menu_repetition(120.0)["menu"],
            ]
        }
    )
...
def clean_cell() -> dict:
    ...
    # #9062 seeds one page per action, so the cell records how far apart those seeds landed.
    # 0 is the clean case: every page seeded the same number of characters.
    ...
    # Borrowed from the sibling module rather than rebuilt here. #9062's verdict requires
    # per-action proof fields the original fixture predates -- the scroll's pointer placement,
    # the jump's start position, and the fixture size each repetition began from -- and keeping
    # a second copy of those in step with the first is exactly how the two would drift apart.
    ...
    return {
        ...
        # #9062 seeds one page per action, so a cell records how far apart those seven seeds
        # landed. 0 is the clean case: every page seeded the same number of characters, which
        # is what makes the per-action columns comparable with each other.
        "seed_chars_spread": 0,
        "pages_seeded": 7,
        # #9062 publishes TWO tables per cell, and `as_table` picks one by swapping what the
        # `actions` key holds. The verdict runs over every table named here, so listing both is
        # what makes these assertions cover the isolated and the carry-over driver rather than
        # only whichever one happened to be first.
        "tables": ["isolated", "sequenced"],
        # One mapping behind all three keys: `as_table` swaps what `actions` holds,
        # and both tables must be present or the verdict silently checks only one.
        "isolated_actions": actions,
        "sequenced_actions": actions,
        "actions": actions,
    }
...
class StubPage:
    """Records the order of the calls sequenced_repetition() makes, and nothing else.
    ...
    def evaluate(
        ...
        if "__hvTokens = undefined" in script:
            return None
        # #9062 gives the scroll arm a real pointer over message content, so the stub has to
        # answer the two evaluates that precondition makes: the viewport rectangle it aims at,
        # and the hit test that proves where the cursor landed. Answering "on a message" is the
        # HEALTHY case, so the ordering assertions below still describe a good run.
        if "getBoundingClientRect" in script and "viewport()" in script:
            return {"x": 0.0, "y": 0.0, "w": 1000.0, "h": 800.0}
        if "elementFromPoint" in script:
            return {"on_message": True, "tag": "p"}
    ...
    def mouse(self):
        if not hasattr(self, "_mouse"):
            self._mouse = StubPage._Mouse(self.log)
        return self._mouse
...
def repetition_log() -> list:
    page = StubPage()
    HARNESS.sequenced_repetition(page, None)
    return page.log


def isolated_log(action: str) -> list:
    """The ISOLATED runner's call order for one action.

    The two restore assertions below moved here when #9016 merged into #9062. #9016 had a single
    runner that restored the deleted message inside the sequence. #9062 has two, and they differ
    ON PURPOSE: the carry-over runner resets NOTHING between actions, because the residue is the
    thing that table measures, and its `reopen` deliberately runs on a thread one message shorter.
    Asserting a restore there would contradict the contract that table is published under.

    The isolated runner is where the property still holds, and where it matters more, since that
    is the headline table. Nothing is weakened: the same order is asserted, against the runner
    that claims it.
    """
    page = StubPage()
    HARNESS.isolated_repetitions(page, None, action)
    return page.log
...
def test_the_deleted_message_is_restored_before_anything_else_is_measured() -> None:
    ...
    log = isolated_log("delete")
    assert ("evaluate", "restore") in log, log
    # Per repetition, so the SECOND repetition starts from the restored fixture rather than from
    # a thread the first one shortened.
    assert log.index(("action", "DELETE_JS")) < log.index(("evaluate", "restore")), log
    second = log.index(("action", "DELETE_JS"), log.index(("evaluate", "restore")))
    assert log.index(("evaluate", "restore")) < second, log
...
def test_the_restored_thread_is_re_expanded_and_re_highlighted_before_re_open() -> None:
    ...
    nxt = log.index(("action", "DELETE_JS"), restore)
    assert any(entry == ("wait", "highlighting") for entry in log[restore:nxt]), log
    assert any(entry == ("evaluate", "expandTools") for entry in log[restore:nxt]), log
...
def test_warnings_and_errors_are_captured_into_separate_lists() -> None:
    ...
    assert "self.console_errors: list[str] = []" in HARNESS_SOURCE
    # `self.` because #9062 moved capture onto a per-page object: each of the seven pages
    # keeps its own lists and merge_seeds sums the errors across them.
    assert 'page.on("pageerror", lambda e: self.console_errors.append' in HARNESS_SOURCE
