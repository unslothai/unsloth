...
# structlog is a hard studio.txt requirement imported lazily, so a bare setdefault
# used to park an empty placeholder before anything imported the real package and
# then shadow it for the rest of the session: every later module calling
# structlog.get_logger at import time raised AttributeError, but only when this file
# was collected first. Stub only a genuinely absent package, and give it that
# attribute. sys.modules is checked first because find_spec() raises ValueError on a
# module whose __spec__ is None, which another test's bare stub would leave behind.
