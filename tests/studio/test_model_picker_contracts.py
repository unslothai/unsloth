...
def test_a_pinned_cached_row_loads_from_the_id_the_backend_pinned():
    ...
    variant_request = re.search(
        r"listGgufVariants\(repoId, hfToken, \{(.*?)\n\s*\}\)", picker, re.S
    )
    assert variant_request
    assert "...(localSource ? { localPath: localSource } : {})" in variant_request.group(1)
    assert "preferLocalCache" not in variant_request.group(1)
...
def test_chat_autoload_scopes_variant_lookup_to_cached_repo_path():
    ...
    assert 'params.set("prefer_local_cache", "true")' in request
    assert 'params.set("local_path", localPath)' in request
