...
# What this action is allowed to cache, and the argument for each.
# Its key hashes the same inputs studio/setup.sh checks before rebuilding, so a hit means the build inputs are
# byte-identical rather than merely similar.
# tests/studio/test_frontend_dist_cache.py holds that agreement together and is where the reasoning lives.
...
def test_cold_install_lanes_never_adopt_this_action(name: str) -> None:
    ...
    # Named separately because the frontend dist cache can now be adopted WITHOUT this action
    # that is the whole point of splitting it out for the Windows jobs, which call install.ps1 from a hand-written step.
