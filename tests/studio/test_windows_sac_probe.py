...
def test_an_empty_bootstrap_file_means_rotated(tmp_path, monkeypatch):
    """Studio truncates .bootstrap_password on Windows when it cannot delete it, so an existence check picked the empty string over the operator's password and every login failed on exactly the locked-down machines."""
    s = _load_scenario()
    (tmp_path / "auth").mkdir()
...
def test_a_model_already_resident_is_evicted_first_and_never_counts_as_loaded(
    ...
    """/load answers already_loaded for a resident model and starts nothing, so no PE is loaded inside the evidence window and the absence of events would read as an allow."""
    s = _load_scenario()
...
def test_a_padded_load_reply_is_read_for_its_deferred_error():
    """/api/inference/load commits a 200 after 15 seconds and reports a later failure only in the body, so a download followed by the CodeIntegrity refusal this probe exists to catch was recorded as a successful load."""
    s = _load_scenario()
...
def test_the_signature_audit_fails_on_any_missing_bundle():
    """A native exit code is not terminating in pwsh and a later download replaces $LASTEXITCODE, so a family whose asset was missing was audited as absent and passed under enforce."""
    body = WORKFLOW.read_text(encoding = "utf-8")
...
def test_the_powershell_probe_fails_closed_on_the_log_and_finishes_the_policy_refresh():
    ...
    # revert refreshes the policy set in every branch, absent file included, and clears AuditPolicyApplied only after the refresh succeeded.
    block = ps1[
...
def test_the_venv_inventory_comes_from_the_running_interpreter_and_cannot_be_empty():
    ...
    # The event scoping in collect must resolve the venv the same way the inventory does, or a custom-home venv is counted as somebody else's - and it reads run's recorded answer rather than resolving it again, since the override may only ever have been set in the shell that ran run.
    collect = ps1[ps1.index("function Invoke-Collect") : ps1.index("function Invoke-Revert")]
...
def test_the_app_control_audit_keeps_its_positive_control():
    ...
    # Applying a policy is not the same as it being active, and the job must refuse rather than assume.
...
def test_the_inventory_root_is_the_selected_runtime_not_the_binary_directory():
    """<root>\\\\build\\\\bin\\\\Release is a supported layout (llama_cpp_path_settings.llama_server_candidates), so taking the parent of resolved_binary inventoried Release\\\\ alone and scoped every sibling PE under the selected root out into 'other'."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_event_scoping_matches_path_tails_literally():
    """-like reads [ and ] as pattern syntax, and a directory called [llama] is legal and supported (tests/test_installer_system32_guard.py), so an unescaped tail matched none of its events and dropped its 3076/3077 records out of the Unsloth headline."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    collect = ps1[ps1.index("function Invoke-Collect") : ps1.index("function Invoke-Revert")]
...
def test_installer_trees_under_a_custom_studio_home_are_recorded_for_revert():
    """setup.ps1 resolves $NodeParent from UNSLOTH_STUDIO_HOME/STUDIO_HOME and $UnslothHome from the parent of the managed llama.cpp dir, so hard coding %USERPROFILE%\\\\.unsloth left the custom-home trees administrator-owned with no ACL repair."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    init = ps1[ps1.index("function Initialize-Studio") : ps1.index("function Save-Baseline")]
...
def test_trees_the_installer_created_are_recorded_even_when_the_install_fails():
    """A run that created node\\\\ or .cache\\\\ and then died before the managed interpreter existed still left administrator-owned trees"""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    init = ps1[ps1.index("function Initialize-Studio") : ps1.index("function Save-Baseline")]
...
def test_prepare_fails_when_a_running_studio_cannot_be_restarted():
    """The existing process loaded its venv native modules before the policy and the window existed, and run deliberately does not restart a responsive Studio, so a silent return let prepare report completion for a cell whose startup loads can never be observed."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    init = ps1[ps1.index("function Initialize-Studio") : ps1.index("function Save-Baseline")]
...
def test_each_defender_preference_is_applied_on_its_own_and_deviations_recorded():
    """One shared try meant the first policy-controlled setting threw and every later Set-MpPreference was never called, so the cell ran without the documented cloud-block and PUA baseline with nothing in the evidence."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    raise_block = ps1[
        ps1.index("Write-Section 'Raise security settings'") : ps1.index(
            "Write-Section 'CodeIntegrity log'"
    ...
    # Written when a setting did not take, and cleared on the success path, so a retry of the label cannot leave a marker contradicting its own evidence.
...
def test_an_efi_dismount_failure_is_never_reported_as_a_completed_stage():
    """The next stage's Mount-Efi finds the EFI tree already there, returns $false and so never retries the unmount, leaving S"""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_revert_exits_nonzero_when_a_studio_tree_acl_repair_failed():
    """The ACL loop sat outside the aggregate failure accounting, so revert printed 'revert complete' and exited zero while the user's own Studio tree was still unreadable."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    revert = ps1[ps1.index("function Invoke-Revert") :]
...
def test_a_partial_inventory_says_so_in_the_evidence():
    """Get-ChildItem discarded the error for a subtree it could not read and the caller only rejects a completely empty result, so a partial inventory was described as every PE under the tree."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_a_partial_collection_is_marked_inside_the_zip():
    """A console warning does not travel with the archive, so a zip missing the raw evtx or a core artifact read as a complete evidence package."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    collect = ps1[ps1.index("function Invoke-Collect") : ps1.index("function Invoke-Revert")]
    # evtx export, staging, redaction failure, no-interpreter, an unverified empty window, one that no policy could have filled, one Smart App Control was never shown to be refusing code in, a scenario that loaded nothing, and audit events raised by a policy other than the one installed
...
def test_the_app_control_verdict_cannot_pass_on_an_unread_channel():
    """SilentlyContinue turned a failed query into an empty array and the job reported that no runtime binary would be refused."""
    workflow = WORKFLOW.read_text(encoding = "utf-8")
...
def test_the_audit_channel_resize_is_verified_not_assumed():
    """A channel that was already enabled reads back enabled even when the resize failed, and the 1 MB default can wrap between the positive control and the verdict."""
    workflow = WORKFLOW.read_text(encoding = "utf-8")
...
def test_acl_repair_reaches_a_custom_home_outside_the_user_profile():
    """An UNSLOTH_STUDIO_HOME on another volume (D:\\\\Unsloth) is exactly where the elevated installer leaves administrator-owned trees, and anchoring containment on %USERPROFILE% discarded every one of them."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    revert = ps1[ps1.index("function Invoke-Revert") :]
...
def test_prepare_proves_the_audit_policy_actually_evaluates_loads():
    ...
    # An enforcing machine refuses the control outright
    assert (
...
def test_a_completed_revert_spends_its_baseline():
    """revert leaves baseline.json behind, so a later prepare on the same label reused the first run's snapshot and a second revert would write those stale Defender values back over whatever the machine legitimately carries now."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_the_app_control_verdict_polls_before_reporting_an_allow():
    """3076 delivery is asynchronous - the positive control polls for exactly that reason - and the exercise step ends the moment llama-server exits, so one query could run before the record landed and report a clean allow."""
    workflow = WORKFLOW.read_text(encoding = "utf-8")
    verdict = workflow[workflow.index("      - name: Verdict\n        if: always()") :]
    verdict = verdict[: verdict.index("      - name: Export the CodeIntegrity events")]
...
def test_the_scenario_password_is_cleared_even_when_the_run_is_interrupted():
    """Ctrl+C stops the pipeline without entering the normal path or the catch, and the script runs in the operator's own session, so the secret stayed in that console and was inherited by everything started from it afterwards."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    run = ps1[ps1.index("function Invoke-Run") : ps1.index("function Invoke-Collect")]
...
def test_a_custom_studio_home_is_normalized_the_way_studio_normalizes_it(monkeypatch):
    """UNSLOTH_STUDIO_HOME=~\\\\my-studio is supported (storage_roots.studio_root calls expanduser().resolve()), but the raw string is a cwd-relative directory named '~', so auth/.bootstrap_password was read from the wrong place and a never-opened Studio logged in with the wrong password."""
    s = _load_scenario()
...
def test_a_prefix_colliding_sibling_is_not_scoped_as_the_selected_runtime():
    """The channel is machine-wide and the matrix runs two builds side by side, so ...\\\\llama.cpp-b10830 sitting beside the selected ...\\\\llama.cpp had its 3076/3077 counted as a verdict on the build the scenario actually drove."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_an_empty_window_with_no_observed_load_never_reads_as_an_allow():
    """A scenario that stopped at login loaded no PE under the policy, so the positive control alone must not earn the clean-allow line - and the caveat used to be worked out after the zip was written, so the archive said nothing about it."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    collect = ps1[ps1.index("function Invoke-Collect") : ps1.index("function Invoke-Revert")]
...
def test_a_studio_that_never_answered_fails_prepare():
    """Start-Studio's result was discarded, so prepare printed 'prepare complete' over a Studio that never started and whose venv modules therefore never loaded inside the window."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    init = ps1[ps1.index("function Initialize-Studio") : ps1.index("function Save-Baseline")]
...
def test_only_studio_is_treated_as_studio_on_the_port():
    """prepare force-stops whatever owns the port, and 8888 is a busy default, so a status-only check could kill an unrelated server."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_a_numeric_defender_readback_is_compared_not_waved_through():
    """Get-MpPreference returns CIM numbers on some builds, so MAPSReporting 0 against a requested Advanced has to compare unequal"""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    fn = ps1[
...
def test_revert_reads_the_defender_values_back_before_spending_the_baseline():
    """An ignored restore leaves $failed at zero, so revert reported success and stamped RevertCompletedAt over a machine still carrying the raised settings."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    revert = ps1[ps1.index("function Invoke-Revert") :]
...
def test_install_roots_survive_a_prepare_retry():
    """A retry finds the first attempt's trees already present, so replacing the recorded list dropped them and revert left them administrator-owned."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    init = ps1[ps1.index("function Initialize-Studio") : ps1.index("function Save-Baseline")]
...
def test_an_installer_that_produced_no_interpreter_fails_prepare():
    """prepare printed 'prepare complete' with no Studio to start, and the operator only found out when run rejected the empty venv."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    init = ps1[ps1.index("function Initialize-Studio") : ps1.index("function Save-Baseline")]
...
def test_the_ci_verdict_needs_a_runtime_that_was_actually_extracted():
    """The verdict runs on always(), so a download or expand failure left a clean signature-only verdict in the job summary for a runtime that was never loaded."""
    workflow = WORKFLOW.read_text(encoding = "utf-8")
    verdict = workflow[workflow.index("      - name: Verdict\n        if: always()") :]
    verdict = verdict[: verdict.index("      - name: Export the CodeIntegrity events")]
...
def test_the_scenario_will_not_hand_its_password_to_an_unidentified_server():
    """discover_port picks the port the operator's Studio password is posted to, and these defaults are shared (8888 is Jupyter's), so a catch-all 200 used to be enough to receive the credential."""
    s = _load_scenario()
    seen: list[str] = []
...
def test_an_efi_mount_this_probe_owns_is_retried_by_the_next_stage():
    """$script:EfiStillMounted dies with the process, so the next stage found an EFI-backed S:, called it pre-existing and never retried the unmount while still reporting success."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_a_lost_preexisting_policy_backup_is_a_revert_failure():
    """Falling through to the removal branch deleted an administrator's policy and then reported the rollback complete."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    revert = ps1[ps1.index("function Invoke-Revert") :]
    assert (
...
def test_prepare_requires_the_log_capacity_it_asked_for():
    """A channel that was already enabled reads back enabled even when the resize was refused, and the 1 MB default wraps while every venv load is audited"""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    prepare = ps1[
        ps1.index("function Invoke-Prepare") : ps1.index("function Get-SignatureInventory")
    ]
...
def test_a_failed_dismount_is_reclaimed_by_the_next_revert():
    """The policy block clears AuditPolicyApplied even when the dismount that follows the refresh failed, so the next revert skipped the block, never reached Mount-Efi, and stamped the rollback complete with S"""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_a_recorded_tree_revert_refuses_to_touch_is_a_rollback_failure():
    """A UNSLOTH_STUDIO_HOME set for the prepare shell only is gone after a reboot, so the trees prepare recorded fall outside the roots revert derives."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    revert = ps1[ps1.index("function Invoke-Revert") :]
...
def test_the_readme_pins_the_runtime_before_the_window_opens():
    """prepare opens the event window and then installs or restarts Studio, so installing the pinned release afterwards put loads from two releases in one window, over the one managed directory collect scopes events by."""
    body = (PROBE_DIR / "README.md").read_text(encoding = "utf-8")
    pin = body[
...
def test_a_spent_baseline_is_never_written_back_over_the_machine():
    """revert reloaded a baseline carrying RevertCompletedAt and reapplied every Defender and channel value in it, silently undoing anything changed since the revert that spent it - which for Defender means lowering protections nobody asked to lower."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    revert = ps1[ps1.index("function Invoke-Revert") :]
...
def test_only_the_verified_loopback_listener_is_ever_stopped():
    """Test-StudioResponding verifies http://127.0.0.1:$port, but -LocalPort on its own returns every listener holding that port on any local address, and this function force-stops each owner and its children."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    stop = ps1[ps1.index("function Stop-Studio") : ps1.index("function Initialize-Studio")]
...
def test_the_managed_venv_is_recorded_when_the_studio_home_already_exists():
    """The elevated installer creates <home>\\\\unsloth_studio, but a Studio home that already exists is not in $absentBefore and the venv was in no candidate list at all."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    init = ps1[ps1.index("function Initialize-Studio") : ps1.index("function Save-Baseline")]
    cands = init[init.index("$candidates = @(") : init.index("$absentBefore = @(")]
    assert "(Join-Path (Get-StudioHome) 'unsloth_studio')" in cands
    # And revert must accept it
    revert = ps1[ps1.index("function Invoke-Revert") :]
...
def test_run_refuses_a_label_whose_revert_already_completed():
    """revert leaves window-start.txt behind, so the window still on disk is the one the reverted run measured."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    run = ps1[ps1.index("function Invoke-Run") : ps1.index("function Invoke-Collect")]
...
def test_collect_scopes_the_venv_run_measured_not_the_one_this_shell_resolves():
    """Both resolutions go through UNSLOTH_STUDIO_HOME, and a reboot between the stages is supported, so a collect from a shell without that override scoped the window against the legacy default while run had inventoried a custom venv."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_the_prepare_timestamp_is_read_back_culture_invariantly():
    """ConvertFrom-Json turns CapturedAt into a DateTime, PowerShell stringifies that with the invariant culture (MM/dd/yyyy) to bind the string overload, and [datetime]::Parse reads the CURRENT one."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    run = ps1[ps1.index("function Invoke-Run") : ps1.index("function Invoke-Collect")]
...
def test_collect_reads_the_studio_logs_of_the_install_run_measured():
    """Both the log root and the redactor interpreter went through Get-StudioHome, so a collect from a shell without a custom UNSLOTH_STUDIO_HOME looked under the legacy default"""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
...
def test_run_refuses_an_event_window_older_than_its_own_baseline():
    """prepare captures a fresh baseline for a reverted label and only reopens the window at the very end, so a throw in between - a CodeIntegrity channel it cannot raise, a missing -AuditPolicy, a policy that will not load - leaves an unspent baseline beside the previous run's window-start.txt."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    run = ps1[ps1.index("function Invoke-Run") : ps1.index("function Invoke-Collect")]
    ...
    # collect must NOT gain the same guard
    collect = ps1[ps1.index("function Invoke-Collect") : ps1.index("function Invoke-Revert")]
...
def test_revert_stops_only_the_elevated_studio_this_probe_started(tmp_path):
    """Every stage asserts elevation, so the Studio that prepare/run starts (and each terminal/Python tool it spawns) holds an administrator token."""
    import shutil
...
def test_run_refuses_a_label_that_was_never_prepared():
    """Get-RunDir creates the directory, so a new or mistyped -Label reached run empty and skipped every baseline-keyed guard"""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    run = ps1[ps1.index("function Invoke-Run") : ps1.index("function Invoke-Collect")]
...
def test_a_reused_label_never_archives_an_earlier_upgrade_transcript():
    """collect stages the whole run directory, and the stale sweep runs after the winget block, so a label reused without -UpgradePackages archived the previous run's irreversible upgrades as if this run had made them."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    prepare = ps1[
        ps1.index("function Invoke-Prepare") : ps1.index("function Get-SignatureInventory")
    ]
...
def test_events_are_scoped_by_the_evaluated_file_not_the_requesting_process():
    """A 3076/3077 message names both the requesting process and the file ("a process (%4) attempted to load %2")."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    collect = ps1[ps1.index("function Invoke-Collect") : ps1.index("function Invoke-Revert")]
...
def test_the_app_control_verdict_scopes_by_the_evaluated_file(tmp_path):
    """A 3076 message names the requesting process and the file it tried to load, so matching the whole message counted an in-runtime llama-server loading a file outside the runtime as a runtime file blocked."""
    import shutil
...
def test_a_studio_whose_record_cannot_be_written_is_stopped_and_fails(tmp_path):
    """An elevated Studio with no probe-studio.json is one revert can neither stop nor report, so a failed record write must stop the process tree and fail the stage instead of warning and carrying on."""
    import shutil
...
def test_the_audit_policy_control_needs_an_event_from_the_audit_policy_itself(tmp_path):
    """On a Smart App Control enforcing machine the unsigned control is refused by Smart App Control's own policy with a 3077 whether or not the installed NoISG audit policy evaluates anything."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    prepare = ps1[
        ps1.index("function Invoke-Prepare") : ps1.index("function Get-SignatureInventory")
    ]
...
def test_collect_waits_for_code_integrity_delivery_to_settle(tmp_path):
    """The channel is written asynchronously, so a single snapshot taken soon after run could grade zero events and print a clean allow."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    collect = ps1[ps1.index("function Invoke-Collect") : ps1.index("function Invoke-Revert")]
...
def test_the_ci_verdict_needs_a_completed_exercise_before_an_allow():
    """A --version that exited non-zero for a reason that raises no 3076 (a missing dependency, a loader failure before the DLL graph is mapped) used to reach the clean signature-only verdict."""
    workflow = WORKFLOW.read_text(encoding = "utf-8")
...
def test_a_baseline_write_that_dies_keeps_the_previous_snapshot(tmp_path):
    """revert restores Defender, the event log and the audit policy from baseline.json, so a rewrite that fails part way (a full volume, an interrupt) must leave the last good snapshot, not a truncated file."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    helper = ps1[
...
def test_collect_reads_the_studio_home_run_recorded(tmp_path):
    """With a custom UNSLOTH_STUDIO_HOME that has no interpreter of its own, the selected venv is the legacy one while the backend writes its logs under the configured home, so the home is recorded by run rather than inferred from the venv."""
    ps1 = (PROBE_DIR / "sac-probe.ps1").read_text(encoding = "utf-8")
    run = ps1[ps1.index("function Invoke-Run") : ps1.index("function Invoke-Collect")]
...
def test_the_tool_error_prefixes_mirror_the_backend_set():
    """The loop reports a failed tool with any of the backend's error prefixes ("Search failed", "Execution error", "Blocked:", ...), not only "Error:"."""
    import ast
...
def test_a_transient_refresh_failure_keeps_the_refresh_token(monkeypatch):
    """A transport error or a 5xx from /api/auth/refresh may never have reached Studio's store, so the single-use token is kept for the next 401."""
    s = _load_scenario()
    answers = [
        (0, "connection reset"),
        (503, "busy"),
