...
# the caller's UV_OVERRIDE lines, which can carry authenticated direct URLs, so it must never
# outlive the run: install.sh removes its twin from the EXIT/signal traps and install.ps1 must do
# the same from the outer finally. Text/AST assertions plus one behavioural cleanup check.
...
# the overrides-guarded call and the no-torch-installed fallback.
...
# install.sh empties _UNSLOTH_TORCH_OVERRIDES before arming its traps so an inherited value can
# never be rm'd; under `irm | iex` the script scope is the caller's session, so the same reset
# must precede the outer try.
...
# Behavioural: run the real finally body with a live temp file holding a credential-bearing
# inherited override line, exactly as an interrupted install would leave it.
...
# Strip the `finally { ... }` wrapper and run the statements themselves: Invoke-Expression evaluates
# in this scope, so the block's $script: writes land where the installer's would.
...
# PowerShell's -notmatch is case-insensitive unless written -cnotmatch, so a caller override
# spelled `Torch<2.11` is dropped and the generated exact pin wins.
$filterPattern = $null
