...
class TestClassificationIsAFaithfulTranslation:
    ...
    def test_rocm_hip_marker(self, ver, hip, cuda):
        # The Linux ROCm classification reads out of _probe_rocm_torch, which is where
        # the repair paths get it from. It used to sit inline in _ensure_rocm_torch; the
        # rename is what this guard is meant to catch, so the equivalence was re-checked
        # against the same reference implementation rather than assumed across the move.
        env = {
            "re": re,
            "version": ver,
            "hip": hip,
        ...
        _run_assignments("_probe_rocm_torch", {"installed_version", "marker"}, env)
        assert env["marker"] == _old_rocm_marker(ver, hip, cuda)
        assert env["installed_version"] == ver.lower()
