...
class TestSetupShKfdOnlyHost:
    ...
    def test_the_report_site_uses_the_lspci_aware_lookup(self):
        """Resolved once into _setup_unsup_gfx, then read by every arm that needs it.

        It gates four arms now, not the one it was written for, and it shells out to
        lspci, so the call moved ahead of the chain. Both halves are asserted: the
        single resolve, and the report arm reading that same variable.
        """
        src = _normalised(_SETUP_SH)
        assert (
