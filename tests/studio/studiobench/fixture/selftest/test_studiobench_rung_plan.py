...
def test_stream_duration_is_rung_independent():
    """The property the film depends on. Every rung's stream must drain in the same ballpark."""
...
def test_every_rung_lands_close_to_the_size_it_claims():
    for rung, plan in _plans().items():
        total = plan.seeded_chars + plan.streamed_chars
...
def test_the_ladder_is_strictly_increasing_in_seeded_mass():
    ...
    masses = [plans[r].seeded_chars + plans[r].streamed_chars for r in order]
