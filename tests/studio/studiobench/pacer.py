...
class StreamStats:
    ...
    def as_dict(self) -> dict:
        ...
        for k in ("first_chunk_ms", "last_chunk_ms", "write_block_ms", "duration_ms",
                  "achieved_gap_ms"):
...
def _chunk_frame(request_id: str, model: str, delta: dict,
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:                                       # noqa: N802
        ...
        if path in ("/healthz", "/_pacer/healthz"):
            ...
            self._json(200, {"object": "list",
                             "data": [{"id": m, "object": "model", "owned_by": "studiobench"}
                                      for m in ids]})
        elif path == "/_pacer/stats":
            with self.state.lock:
                self._json(200, {"requests_seen": self.state.requests_seen,
    ...
    def _stream(self, body: dict) -> None:
        ...
        stats = StreamStats(request_id = request_id, tag = script.tag,
                            started_at = time.monotonic(), requested_gap_ms = script.gap_ms)
        ...
        try:
            ...
            while sent < len(chunks):
                ...
                for _ in range(deficit):
                    ...
                    if (script.hold_after_chars is not None
...
class Pacer:
    ...
    def load(self, reasoning: str, content: str, *, cadence: str = "field",
             tag: str = "", hold_after_chars: Optional[int] = None,
             model: str = "studiobench-pacer",
             chunk_chars: Optional[int] = None, gap_ms: Optional[int] = None) -> None:
        ...
        self.state.set_script(Script(
            reasoning = reasoning, content = content, chunk_chars = chunk_chars, gap_ms = gap_ms,
            model = model, tag = tag, hold_after_chars = hold_after_chars,
        ))
    ...
    def expected_duration_ms(self, reasoning: str, content: str, cadence: str = "field",
                             chunk_chars: Optional[int] = None,
                             gap_ms: Optional[int] = None) -> float:
...
def _selftest() -> int:
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            data = json.dumps({"model": "studiobench-pacer", "stream": True,
                               "messages": [{"role": "user", "content": "go"}],
                               "stream_options": {"include_usage": True}}).encode(),
            headers = {"Content-Type": "application/json",
                       "Authorization": "Bearer sb-local"},
        )
        started = time.monotonic()
        ...
        print(f"pacer selftest OK: {stats['chunks_sent']} chunks, {stats['chars_sent']} chars, "
        ...
        payload = json.dumps({"model": "studiobench-pacer", "stream": True,
        ...
        print(f"pacer backpressure OK: {s2['bursts']} bursts, max deficit {s2['max_deficit']}, "
