...
class Surface:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {"id": self.id, "group": self.group, "title": self.title,
                "reach": [list(s) for s in self.reach],
                "restore": [list(s) for s in self.restore],
                "root": list(self.root), "settle": self.settle,
                "conditional": self.conditional, "also_in_film": self.also_in_film,
                "volatile": self.volatile, "notes": self.notes}
...
ESCAPE_OUT: tuple[Step, ...] = (("press", "Escape"), ("wait", 200), ("press", "Escape"),
                                ("wait", 300))
...
GREETING_IS_RANDOM = ("the empty chat's greeting is chosen by pickRandom "
...
HUB_IS_REMOTE = ("the catalogue is fetched from Hugging Face, so its content and its arrival time "
...
UNEXPLAINED_DRIFT = ("measured to differ between sweeps of the SAME build while a back-to-back "
...
def _route(surface_id: str, title: str, path: str, settle: dict,
           conditional: Optional[str] = None, notes: str = "",
           volatile: Optional[str] = None) -> Surface:
    return Surface(
        id = surface_id, group = "route", title = title,
        reach = (("goto", path), ("wait", 600)),
        settle = settle, restore = HOME, root = ROOT_ROUTE,
        conditional = conditional, notes = notes, volatile = volatile)
...
def _settings_tab(tab_id: str, title: str) -> Surface:
    ...
    return Surface(
        id = f"settings:{tab_id}", group = "settings", title = title,
        reach = (("goto", "/settings"), ("wait", 500),
                 ("click", f'[data-testid="settings-tab-{tab_id}"]'), ("wait", 400)),
        ...
        settle = {"js": f'!!document.querySelector(\'[data-testid="settings-tab-{tab_id}"]\')'
...
_SURFACES: list[Surface] = [
    ...
    # ── routes ──────────────────────────────────────────────────────
    _route("route:chat", "Chat, empty state", "/chat",
           {"visible": COMPOSER}, volatile = GREETING_IS_RANDOM),
    _route("route:projects", "Projects", "/projects",
           {"js": 'location.pathname === "/projects"'}),
    _route("route:hub", "Hub / model catalogue", "/hub",
    ...
    _route("route:train", "Train (Studio)", "/studio",
    ...
    _route("route:export", "Export", "/export",
           {"js": 'location.pathname === "/export"'}),
    _route("route:images", "Images", "/images",
           {"js": 'location.pathname === "/images"'}),
    _route("route:video", "Video", "/video",
           {"js": 'location.pathname === "/video"'},
           conditional = "a host without video support renders the gate instead of the page"),
    _route("route:audio", "Audio", "/audio",
           {"js": 'location.pathname === "/audio"'}),
    _route("route:data-recipes", "Data recipes", "/data-recipes",
           {"js": 'location.pathname === "/data-recipes"'}),
    _route("route:api-monitor", "API monitor", "/api-monitor",
    ...
    _route("route:not-found", "Not found", "/studiobench-no-such-route",
    ...
    dataclasses.replace(_settings_tab("agents", "Settings: Agents"),
                        volatile = UNEXPLAINED_DRIFT),
    dataclasses.replace(
    ...
    Surface(id = "settings:search", group = "settings", title = "Settings: search results",
            ...
            reach = (("goto", "/settings"), ("wait", 500),
                     ...
                     ("wait", 500)),
            settle = {"js": '(document.querySelector(".settings-surface main").innerText || "")'
                            '.trim().length > 0'},
            restore = ESCAPE_OUT + HOME,
            root = (".settings-surface", '[data-slot="dialog-content"]')),

    # ── the sidebar ─────────────────────────────────────────────────
    Surface(id = "sidebar:expanded", group = "sidebar", title = "Sidebar, expanded",
            reach = (("wait", 200),),
            settle = {"visible": '[data-slot="sidebar-container"]'},
            restore = (), root = ROOT_SIDEBAR),
    Surface(id = "sidebar:collapsed", group = "sidebar", title = "Sidebar, collapsed to the rail",
            ...
            root = ROOT_SIDEBAR),
    Surface(id = "sidebar:account-menu", group = "sidebar", title = "Account menu",
            reach = (("click", 'button[aria-label^="Unsloth account menu"]'), ("wait", 400)),
            settle = {"visible": '.app-user-menu'},
            restore = ESCAPE_OUT, root = (".app-user-menu",)),
    Surface(id = "sidebar:thread-menu", group = "sidebar", title = "Chat row options menu",
            reach = (("hover", '[class*="group/recent-item"]'), ("wait", 300),
                     ("click", 'button[aria-label="Chat options"]'), ("wait", 400)),
            settle = {"count_at_least": ['[role="menuitem"]', 3]},
            restore = ESCAPE_OUT,
            root = ('[role="menu"]',),
            conditional = "needs at least one chat in the sidebar"),
    Surface(id = "sidebar:organize-menu", group = "sidebar", title = "Organize chats menu",
            reach = (("hover",
                      '[data-slot="sidebar-group-label"]:has(button[aria-label="Organize chats"])'),
                     ("wait", 300),
                     ("click", 'button[aria-label="Organize chats"]'), ("wait", 400)),
            settle = {"count_at_least": ['[role="menuitemradio"]', 2]},
            restore = ESCAPE_OUT, root = ('[role="menu"]',)),
    Surface(id = "sidebar:new-project", group = "sidebar", title = "New project dialog",
            reach = (("hover", '[class*="group/projects-item"]'), ("wait", 300),
                     ("click", 'button[aria-label="New project"]'), ("wait", 500)),
            settle = {"visible": '[data-slot="dialog-content"]'},
            restore = ESCAPE_OUT + HOME, root = ('[data-slot="dialog-content"]',)),
    Surface(id = "sidebar:search", group = "sidebar", title = "Chat search command dialog",
            reach = (("click", 'button[aria-label="Search"]'), ("wait", 500)),
            settle = {"visible": ".chat-search-surface"},
            restore = ESCAPE_OUT, root = (".chat-search-surface",)),
    Surface(id = "sidebar:workflows", group = "sidebar", title = "Images workflow sub-rows",
            reach = (("hover", '[class*="group/images-item"]'), ("wait", 300),
                     ("click", 'button[aria-label="Show workflows"]'), ("wait", 400)),
    ...
    Surface(id = "chat:model-picker", group = "chat", title = "Model picker",
            ...
            restore = ESCAPE_OUT, root = (".unsloth-model-selector-menu",),
            also_in_film = True),
    Surface(id = "chat:model-picker-ondevice", group = "chat",
            ...
            restore = ESCAPE_OUT, root = (".unsloth-model-selector-menu",)),
    Surface(id = "chat:plus-menu", group = "chat", title = "Tools and attachments menu",
            reach = (("click", 'button[aria-label="Tools and attachments"]'), ("wait", 400)),
            settle = {"count_at_least": ['[role="menuitem"]', 1]},
            restore = ESCAPE_OUT, root = ('[role="menu"]',), also_in_film = True),
    Surface(id = "chat:run-settings", group = "chat", title = "Run settings sheet",
            reach = (("click", 'button[aria-label="Open run settings"]'), ("wait", 600)),
            settle = {"visible": 'button[aria-label="Close run settings"]'},
            restore = (("click_if", 'button[aria-label="Close run settings"]'),
                       ("wait", 400)) + HOME,
    ...
    Surface(id = "chat:permission-menu", group = "chat", title = "Tool permission menu",
            reach = (("click", 'button[aria-label="Permission level for tool calls"]'),
                     ("wait", 400)),
            settle = {"count_at_least": ['[role="menuitem"], [role="option"]', 2]},
            restore = ESCAPE_OUT, root = ('[role="menu"]', '[role="listbox"]')),
    Surface(id = "chat:system-prompt", group = "chat", title = "System prompt editor",
            ...
            reach = (("click", 'button[aria-label="Open run settings"]'), ("wait", 800),
                     ("click", 'button[aria-label="Edit system prompt"]'), ("wait", 800)),
            settle = {"visible": '[data-slot="dialog-content"]'},
            restore = ESCAPE_OUT + HOME, root = ('[data-slot="dialog-content"]',)),
    Surface(id = "chat:temporary", group = "chat", title = "Temporary chat, on",
            reach = (("click", 'button[aria-label="Turn on temporary chat"]'), ("wait", 500)),
            settle = {"visible": 'button[aria-label="Turn off temporary chat"]'},
            restore = (("click_if", 'button[aria-label="Turn off temporary chat"]'),
                       ("wait", 300)) + HOME,
            root = ROOT_ROUTE),
    Surface(id = "chat:composer-filled", group = "chat", title = "Composer holding text",
            ...
            settle = {"js": '(document.querySelector('
                            '\'textarea[aria-label="Message input"]\') || {}).value ? true : false'},
            restore = (("fill", COMPOSER, ""), ("wait", 200)) + HOME,
            root = ROOT_ROUTE, also_in_film = True, volatile = GREETING_IS_RANDOM),

    # ── the hub ─────────────────────────────────────────────────────
    Surface(id = "hub:datasets", group = "hub", title = "Hub, datasets tab",
            reach = (("goto", "/hub"), ("wait", 800),
                     ("click", 'button[aria-label="Datasets"]'), ("wait", 800)),
            ...
            settle = {"js": 'location.search.includes("kind=datasets") && '
                            '(document.body.innerText || "").includes("Downloads")'},
            restore = HOME, root = ROOT_ROUTE,
    ...
    Surface(id = "hub:format-filter", group = "hub", title = "Hub, format filter menu",
            reach = (("goto", "/hub"), ("wait", 800),
                     ("click", 'button[aria-label="Format filter"]'), ("wait", 400)),
            settle = {"count_at_least": ['[role="option"]', 2]},
            restore = ESCAPE_OUT + HOME, root = ('[role="listbox"]',),
            conditional = "needs the catalogue, which needs network"),
    Surface(id = "hub:sort-menu", group = "hub", title = "Hub, sort menu",
            reach = (("goto", "/hub"), ("wait", 800),
                     ("click", 'button[aria-label="Sort models"]'), ("wait", 400)),
            settle = {"count_at_least": ['[role="option"]', 2]},
            restore = ESCAPE_OUT + HOME, root = ('[role="listbox"]',),
            conditional = "needs the catalogue, which needs network"),
    Surface(id = "hub:hf-token", group = "hub", title = "Hub, Hugging Face token entry",
            reach = (("goto", "/hub"), ("wait", 800),
                     ("click", 'button[aria-label="Set Hugging Face token"]'), ("wait", 800)),
    ...
    Surface(id = "hub:compact-layout", group = "hub", title = "Hub, compact layout",
            reach = (("goto", "/hub"), ("wait", 800),
                     ("click", 'button[aria-label="Compact"]'), ("wait", 600)),
    ...
    # ── the media pages ─────────────────────────────────────────────
    Surface(id = "images:presets", group = "media", title = "Images, presets menu",
            reach = (("goto", "/images"), ("wait", 800),
                     ("click", 'button[aria-label="Manage image generation presets"]'),
                     ("wait", 500)),
            settle = {"js": 'document.querySelectorAll(\'[role="menu"], '
                            '[data-radix-popper-content-wrapper]\').length > 0'},
            restore = ESCAPE_OUT + HOME,
            root = ('[role="menu"]', "[data-radix-popper-content-wrapper]")),
    Surface(id = "images:model-picker", group = "media", title = "Images, model picker",
            reach = (("goto", "/images"), ("wait", 800),
                     ("click", ".unsloth-model-selector-trigger"), ("wait", 600)),
            settle = {"visible": ".unsloth-model-selector-menu"},
            restore = ESCAPE_OUT + HOME, root = (".unsloth-model-selector-menu",)),
    Surface(id = "video:presets", group = "media", title = "Video, presets menu",
            reach = (("goto", "/video"), ("wait", 800),
    ...
    # ── train ───────────────────────────────────────────────────────
    Surface(id = "train:model-picker", group = "train", title = "Train, model picker",
            reach = (("goto", "/studio"), ("wait", 1200),
                     ("click", 'button[aria-label="Model: Select model"]'), ("wait", 600)),
            settle = {"js": 'document.querySelectorAll(\'.unsloth-model-selector-menu, '
    ...
    Surface(id = "train:method", group = "train", title = "Train, method selector",
            reach = (("goto", "/studio"), ("wait", 1200),
                     ("click", 'button[aria-label^="Method:"]'), ("wait", 500)),
            settle = {"js": 'document.querySelectorAll(\'[role="menu"], [role="listbox"], '
                            '[data-radix-popper-content-wrapper]\').length > 0'},
            restore = ESCAPE_OUT + HOME,
            root = ('[role="menu"]', '[role="listbox"]', "[data-radix-popper-content-wrapper]"),
            conditional = "a chat-only host redirects /studio to /chat"),
    Surface(id = "train:dataset", group = "train", title = "Train, dataset picker",
            reach = (("goto", "/studio"), ("wait", 1200),
                     ("click", 'button[aria-label^="Dataset:"]'), ("wait", 600)),
            settle = {"js": 'document.querySelectorAll(\'[role="menu"], [role="listbox"], '
                            '[data-radix-popper-content-wrapper]\').length > 0'},
            restore = ESCAPE_OUT + HOME,
            root = ('[role="menu"]', '[role="listbox"]', "[data-radix-popper-content-wrapper]"),
            conditional = "a chat-only host redirects /studio to /chat"),
    Surface(id = "train:image-training", group = "train", title = "Image LoRA training",
            reach = (("goto", "/studio"), ("wait", 1200),
                     ("click", 'button[aria-label="Image training"]'), ("wait", 1200)),
            ...
            settle = {"js": 'location.pathname === "/images" && '
    ...
    # ── the api monitor ─────────────────────────────────────────────
    Surface(id = "api:status-filter", group = "api", title = "API monitor, status filter",
            reach = (("goto", "/api-monitor"), ("wait", 800),
                     ("click", 'button[aria-label="Filter by status"]'), ("wait", 400)),
            settle = {"js": 'document.querySelectorAll(\'[role="menu"], [role="listbox"], '
                            '[data-radix-popper-content-wrapper]\').length > 0'},
            restore = ESCAPE_OUT + HOME,
            root = ('[role="menu"]', '[role="listbox"]', "[data-radix-popper-content-wrapper]")),
...
KNOWN_UNCOVERED: tuple[dict, ...] = (
    {"id": "route:login", "title": "Login page",
    ...
    {"id": "route:change-password", "title": "Change password page",
    ...
    {"id": "route:data-recipe-editor", "title": "Data recipe editor (/data-recipes/$recipeId)",
    ...
    {"id": "dialog:destructive-confirms", "title": "Delete / archive / shutdown confirmations",
    ...
    {"id": "dialog:promise-driven", "title": "HF token warning, remote code consent, "
    ...
    {"id": "picker:connected-tab", "title": "Model picker, Connected tab",
    ...
    {"id": "picker:model-config-page", "title": "Model picker, inference settings page",
    ...
    {"id": "chat:message-surfaces", "title": "Message action bar, reasoning pane, tool blocks, "
    ...
    {"id": "recipe-studio:*", "title": "Recipe studio block sheet and its four dialogs",
     "reason": "reached from inside a recipe being edited, which needs a saved recipe"},
    {"id": "tour:guided", "title": "Guided tour overlay",
    ...
    {"id": "monitor:floating", "title": "Floating resource monitor and API monitor overlays",
...
__all__ = ["Surface", "surfaces", "surface_ids", "get_surface", "groups", "validate_registry",
           "RegistryError", "KNOWN_UNCOVERED", "KNOWN_STATE_PATH", "VERBS"]
