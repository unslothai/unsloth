...
def dump(page: Page, vite: subprocess.Popen[str] | None) -> None:
    ...
    if vite is not None:
        info("vite tail:")
        # Snapshot it. The drain thread is still appending to that deque, and `info`
        # writes to stdout, so iterating it live raises "deque mutated during
        # iteration" and drops the tail in precisely the noisy failure it exists for.
        # `list()` of a deque runs in C with no bytecode in between, so it is atomic.
