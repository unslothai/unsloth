...
def test_research_mode_is_single_chat_and_detaches_without_cancel() -> None:
    ...
    # runSignal, not abortSignal: each run gets its own controller, forwarded from the
    # thread signal, so one chat's Stop cannot abort a sibling streaming in the background.
