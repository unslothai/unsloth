...
# Aggressive CUDA spoof for the consolidated CPU-only CI job.
# Extends tests/conftest.py's harness with deeper patches that unblock more patch_* / unsloth_zoo init paths on a
...
def apply() -> None:
    ...
    # Settle bitsandbytes against the real torch first.
    # Its __init__ does `if torch.cuda.is_available(): from .backends.cuda import ops`, and that module reads
    # torch._C._cuda_getCurrentRawStream at import.
    # On a CPU-only wheel that attribute is absent, so a bitsandbytes imported AFTER this spoof raises AttributeError
    # (or OSError hunting libhipblas for the ROCm spoof) rather than ImportError, which slips past the `except
    # ImportError` guards its importers use.
    # Importing it here, while is_available() is still False, caches the CPU path in sys.modules for everything that
    try:
    ...
    # pin_memory drop: pin_memory=True raises on a CPU-only build;
    for _name in (
        "empty",
        "zeros",
        "ones",
