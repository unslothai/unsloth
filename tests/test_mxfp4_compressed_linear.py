...
def _mlp_lora_pair(hidden, inter):
    ...
    packed = [_filled(inter, hidden, seed = 1), _filled(inter, hidden, seed = 2), _filled(hidden, inter, seed = 3)]
    for module in packed:
        module.weight_scale.data.clamp_(118, 126)
    dense = []
    for module in packed:
        linear = nn.Linear(module.in_features, module.out_features, bias = False, dtype = torch.bfloat16)
    ...
    for layers in (packed, dense):
        model = peft.get_peft_model(
            MLP(*layers),
            peft.LoraConfig(r = 8, lora_alpha = 16, target_modules = ["gate_proj", "up_proj", "down_proj"]),
        ).cuda()
...
def test_fused_lora_mlp_on_a_packed_base_matches_the_dense_base(rows):
    ...
    x = torch.randn(1, rows, 64, device = "cuda", generator = torch.Generator("cuda").manual_seed(0)).to(torch.bfloat16)
    results = []
    for mlp in (packed, dense):
        xa = x.clone().requires_grad_()
        with torch.autocast("cuda", dtype = torch.bfloat16):
            y = apply_lora_mlp_swiglu(mlp, xa, inplace = False)
            y = y + apply_lora_o(type("O", (), {"o_proj": mlp.down_proj})(), y.new_ones(1, rows, 128) * 0.01)
...
def test_packed_linear_forward_backward_close_to_the_dense_decode_on_cuda(rows, bias):
    ...
    x = torch.randn(rows, 160, device = "cuda", generator = torch.Generator("cuda").manual_seed(1)).to(torch.bfloat16)
    xa, xb = x.clone().requires_grad_(), x.clone().requires_grad_()
    ya = module(xa)
    yb = torch.nn.functional.linear(xb, dense, None if module.bias is None else module.bias.to(torch.bfloat16))
