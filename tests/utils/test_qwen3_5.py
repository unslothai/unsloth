...
def _import_qwen3_5_module():
    """
    Import unsloth.models.qwen3_5 with heavy GPU-only symbols mocked out.
    Returns the module object.
    """
    import importlib
    import sys

    # Provide a minimal stub for anything pulled in via `from .llama import *`
    # that would fail without CUDA / full unsloth environment.
    fake_unsloth_fused_ce_loss = _make_fake_unsloth_fused_ce_loss()
    fake_empty_logits = torch.zeros(1)  # sentinel used in fused-CE early-return path

    # Patch at the module level so the `from .llama import *` star-import picks them up
    with patch.dict("sys.modules", {}):
        import unsloth.models.qwen3_5 as mod

        # Inject mocks for symbols that come from `from .llama import *`
        mod.unsloth_fused_ce_loss = fake_unsloth_fused_ce_loss
        mod.EMPTY_LOGITS = fake_empty_logits

    return mod, fake_unsloth_fused_ce_loss, fake_empty_logits
...
class TestComputeLossOrLogits(unittest.TestCase):
    ...
    def test_training_path_calls_fused_ce_returns_early(self):
        """labels present + UNSLOTH_RETURN_LOGITS unset → fused CE, early_return=True."""
        helper = self.mod._qwen3_5_compute_loss_or_logits
        self_ = _make_self()
        hidden = torch.randn(2, 8, HIDDEN_DIM)
    ...
    def test_no_labels_returns_full_logits_no_loss(self):
        """No labels → full logits computed, loss=None, early_return=False."""
        helper = self.mod._qwen3_5_compute_loss_or_logits
        self_ = _make_self()
        hidden = torch.randn(2, 8, HIDDEN_DIM)
