...
from unsloth.kernels.gelu import gelu_backward_triton, gelu_forward_triton
...
def test_relu_kernel_fwd(test_matrix):
    ...
    triton_output = gelu_forward_triton(test_matrix)
...
def test_gelu_backward_kernel(test_matrix):
    ...
    triton_output = gelu_backward_triton(test_matrix, grad_input)

    # Compute PyTorch's GeLU gradient for comparison
    torch_gelu = torch.nn.GELU()
    torch_output = torch.autograd.grad(torch_gelu.sum(), test_matrix, grad_outputs=grad_input)[0]
