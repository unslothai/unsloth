...
def test_reference_band_accepts_drift_and_rejects_a_real_move(tmp_path, observed, expected_status):
    ...
    verdict = check_reference([{"step": 1, "loss": observed}], ref, rel_tol = 0.10, abs_floor = 0.05)
...
def test_reference_band_absolute_floor_tolerates_near_zero_losses():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        verdict = check_reference([{"step": 10, "loss": 0.002}], ref, rel_tol = 0.10, abs_floor = 0.05)
...
def test_reference_absent_is_not_a_failure():
    ...
    verdict = check_reference([{"step": 1, "loss": 1.0}], Path("/nonexistent/ref.json"), 0.1, 0.05)
    ...
    if not floored:
        ...
        for index in range(len(metrics)):
            for field in ("loss", "grad_norm"):
                ...
                assert (
                    check_reference(moved, ref, 0.10, 0.05)["status"]
...
def test_band_failure_reaches_the_failure_list(tmp_path):
    ...
    ref = tmp_path / "ref.json"
    ref.write_text(json.dumps({"metrics": [{"step": 1, "loss": 10.0}, {"step": 2, "loss": 1.0}]}))
    verdict = check_reference(
        [{"step": 1, "loss": 10.0}, {"step": 2, "loss": 4.0}], ref, 0.10, 0.05
    )
...
def test_a_length_mismatch_is_a_failure_too(tmp_path):
    ...
    ref = tmp_path / "ref.json"
    ref.write_text(json.dumps({"metrics": [{"step": 1, "loss": 1.0}]}))
    verdict = check_reference([{"step": 1, "loss": 1.0}, {"step": 2, "loss": 1.0}], ref, 0.10, 0.05)
...
def test_matching_nan_grad_norms_are_within_band(tmp_path):
    ...
    nan = float("nan")
    ref = tmp_path / "ref.json"
    ref.write_text(json.dumps({"metrics": [{"step": 1, "loss": 10.0, "grad_norm": nan}]}))
    verdict = check_reference([{"step": 1, "loss": 10.0, "grad_norm": nan}], ref, 0.10, 0.05)
    assert verdict["status"] == "ok"
...
def test_a_moved_scaler_skip_pattern_is_out_of_band(tmp_path, swap):
    ...
    ref = tmp_path / "ref.json"
    ref.write_text(json.dumps({"metrics": [{"step": 1, "loss": 10.0, "grad_norm": ref_value}]}))
    verdict = check_reference([{"step": 1, "loss": 10.0, "grad_norm": obs_value}], ref, 0.10, 0.05)
    assert verdict["status"] == "out_of_band"
...
def test_a_field_that_stopped_being_logged_is_out_of_band(tmp_path):
    ...
    ref = tmp_path / "ref.json"
    ref.write_text(json.dumps({"metrics": [{"step": 1, "loss": 1.0, "grad_norm": 3.0}]}))
