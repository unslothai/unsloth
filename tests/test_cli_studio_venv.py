...
def test_reexec_cli_in_studio_venv_execs_on_unix(monkeypatch):
    ...
    monkeypatch.setattr(
        studio,
        "_studio_venv_python",
        lambda: Path("/tmp/unsloth-studio/unsloth_studio/bin/python"),
    )
    ...
    assert captured["executable"] == "/tmp/unsloth-studio/unsloth_studio/bin/python"
    assert captured["args"] == [
        "/tmp/unsloth-studio/unsloth_studio/bin/python",
        "-m",
        "unsloth_cli",
        "train",
        "--help",
...
def test_reexec_cli_in_studio_venv_noops_inside_venv(monkeypatch):
    monkeypatch.setattr(studio, "STUDIO_HOME", Path("/tmp/unsloth-studio"))
    monkeypatch.setattr(sys, "prefix", "/tmp/unsloth-studio/unsloth_studio")

    called = {"value": False}

    def fake_studio_python():
        called["value"] = True
        return Path("/tmp/unsloth-studio/unsloth_studio/bin/python")
