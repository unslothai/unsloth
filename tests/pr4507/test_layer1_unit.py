...
_CANDIDATES = [
    os.path.join(os.path.dirname(__file__), "..", "..", "studio", "backend"),         # inside repo
    os.path.join(os.path.dirname(__file__), "..", "..", "unsloth", "studio", "backend"),  # outside repo
]
...
def _import_hardware_funcs():
    ...
    from utils.hardware.hardware import safe_num_proc, safe_thread_num_proc, dataset_map_num_proc
...
class TestPythonPathPropagation(unittest.TestCase):
    ...
    def test_L1_11_relative_path_becomes_absolute(self):
        ...
        self.assertTrue(os.path.isabs(cache), f"Cache path should be absolute, got: {cache}")
    ...
    def test_L1_12_no_duplicate_if_already_present(self):
        ...
        self.assertEqual(count, 1, f"Cache path appears {count} times in PYTHONPATH: {pp}")
    ...
    def test_L1_14_skipped_on_linux(self):
        ...
        self.assertEqual(cache, "unsloth_compiled_cache", "Cache path should be unchanged on linux")
...
class TestDatasetUtilsIntegerBypass(unittest.TestCase):
    ...
    def test_L1_17_fork_num_proc_4_preserved(self):
        ...
        self.assertEqual(result, 4,
            "num_proc=4 on fork platform must be preserved as-is")
    ...
    def test_L1_18_spawn_num_proc_none_returns_none(self):
        ...
        self.assertIsNone(result,
            "num_proc=None on spawn platform must remain None")
    ...
    def test_L1_20_actual_dataset_utils_block1_spawn_integer(self):
        ...
        dataset_utils_path = os.path.join(
            ...
            "lib", "python3.13", "site-packages", "unsloth_zoo", "dataset_utils.py"
        )
