...
def _make_backend():
    ...
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "unsloth_studio_src"))
...
def test_reasoning_tokens_stream_immediately():
    ...
    content_events = [e for e in events if e["type"] == "content"]
    assert len(content_events) >= 3, f"Expected at least 3 content events (2 reasoning + 1 content), got: {content_events}"

    # First content events should contain <think> tag
    assert "<think>" in content_events[0]["text"], "First content should have <think> tag"
