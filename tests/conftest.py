...
def pytest_configure(config):
    """Ensure mocks are set up before any test collection."""
...
def _setup_mocks():
    ...
    zoo_rl.execute_with_time_limit = (
        lambda timeout, fn, *args, **kwargs: fn(*args, **kwargs)
    )
    ...
    zoo_patching.patch_model_and_tokenizer = lambda model, tokenizer: (
        model,
        tokenizer,
    )
    ...
    zoo_gc.unsloth_offloaded_gradient_checkpoint = (
        lambda module, *args, **kwargs: module.forward
    )
