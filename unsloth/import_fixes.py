...
if os.environ.get("UNSLOTH_ENABLE_LOGGING", "0") != "1":
    ...
    warnings.filterwarnings(
        "ignore", message = r"Importing.*from torchao\.dtypes.*is deprecated",
        category = DeprecationWarning
    )
    warnings.filterwarnings(
        "ignore", message = r"Importing BlockSparseLayout from torchao\.dtypes is deprecated",
        category = DeprecationWarning
    )

    # SWIG builtin type warnings (from bitsandbytes/triton SWIG bindings)
    warnings.filterwarnings(
        "ignore", message = r"builtin type Swig.*has no __module__ attribute",
        category = DeprecationWarning
    )

    # Triton autotuner deprecation (https://github.com/triton-lang/triton/pull/4496)
    warnings.filterwarnings(
        "ignore", message = r"warmup, rep, and use_cuda_graph parameters are deprecated",
        category = DeprecationWarning
    )

    # Python 3.12+ multiprocessing fork warning in multi-threaded processes
    warnings.filterwarnings(
        "ignore", message = r".*multi-threaded.*use of fork\(\) may lead to deadlocks",
        category = DeprecationWarning
    )

    # Resource warnings from internal socket/file operations
    warnings.filterwarnings(
        "ignore", message = r"unclosed.*socket",
        category = ResourceWarning
    )
    warnings.filterwarnings(
        "ignore", message = r"unclosed file.*dev/null",
        category = ResourceWarning
    )
