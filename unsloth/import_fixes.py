...
def fix_torchao_safe_int_mm_repr_probe():
    """Stop torchao's int8 GEMM from syncing the device to ask a question.

    ``torchao/kernel/intmm.py::safe_int_mm`` branches on
    ``dynamo_is_compiling() or "FakeTensor" in input.__repr__()``. The second
    half is a string test on a repr, and building the repr of a real CUDA
    tensor formats its element values, which calls ``.item()``. So every eager
    int8 linear pays a device sync, and inside ``torch.cuda.graph`` the same
    call raises ``cudaErrorStreamCaptureUnsupported``, which is what stops
    Studio's int8 diffusion transformer from being captured. Asking
    ``torch._subclasses.fake_tensor.is_fake`` instead answers the same question
    without touching the data: verified bit-identical on the GEMM, on a
    compiled Linear and on a whole compiled transformer.

    Version window: the same body ships from torchao 0.10.0 through v0.18.0,
    and torchao main carries it unchanged under
    ``torchao.quantization.quantize_.workflows.int8.kernels`` after the int8
    workflow move, so there is no upper bound to gate on and both module names
    are covered. The gate is structural instead, and deliberately
    narrow: every landmark of the body that was verified must still be in the
    installed source, and the module must still carry the ``out_dtype`` and
    ``dynamo_is_compiling`` this replacement borrows. Anything else is left
    alone, because the replacement is a full copy of a body it must not
    misrepresent.

    Installed through a meta path finder rather than by importing torchao, so
    ``import unsloth`` pays nothing here and a torchao imported later is still
    covered -- which the int8 prequant path needs, since it only ``torch.load``s
    torchao subclasses and never calls ``quantize_``.

    Set ``UNSLOTH_TORCHAO_INT_MM_FIX=0`` to keep upstream's behaviour.
