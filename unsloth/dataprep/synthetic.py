...
class SyntheticDataKit:
    ...
    def chunk_data(self, filename = None):
        ...
        lock = get_lock(filename, timeout = 20)
        try:
            with lock:
            ...
            if os.environ.get("UNSLOTH_LOGGING_ENABLED", "0") == "1":
                logger.error(f"Unsloth: Failed to chunk data because {str(e)}")
            pass
        pass
    pass
    ...
    def prepare_qa_generation(
        ...
        lock = get_lock("synthetic_data_kit_config.yaml")
        try:
            with lock:
                with open("synthetic_data_kit_config.yaml", "w", encoding = "utf-8") as f: f.write(config)
        except Exception as e:
            if os.environ.get("UNSLOTH_LOGGING_ENABLED", "0") == "1":
                logger.error(f"Unsloth: Failed to write synthetic_data_kit_config.yaml because {str(e)}")
            pass
        pass
