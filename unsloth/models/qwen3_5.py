...
def _qwen3_5_compute_loss_or_logits(
    ...
    Returns:
        ...
        early_return (bool) — True when the fused-CE path returns directly,
                              avoiding the loss_function call below.
    """
    ...
    # Eval / inference path
    logits = self.lm_head(hidden_states.to(lm_head_weight.dtype))
    loss = None
...
def Qwen3_5ForConditionalGeneration_fast_forward(
    ...
    **kwargs,
    ...
    # Normalise both generation knobs — unsloth_fast_generate injects num_logits_to_keep=1
    ...
    loss, logits, early_return = _qwen3_5_compute_loss_or_logits(
        self,
        outputs[0],
        labels,
        logits_to_keep,
    ...
    if not return_dict:
        # outputs is a tuple — use index slicing to stay compatible
...
def Qwen3_5ForCausalLM_fast_forward(
    ...
    **kwargs,
    ...
    loss, logits, _ = _qwen3_5_compute_loss_or_logits(
        self,
        outputs[0],
        labels,
        logits_to_keep,
...
class FastQwen3_5Model(FastQwen3Model):
    """
    ...
    @staticmethod
    def pre_patch():
        # Patch both the VLM wrapper (used by all Qwen3.5 text models, including
        # base variants) and the pure-text class for completeness.
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # which hardcodes model_patcher=FastQwen3Model and would incorrectly apply Qwen3
