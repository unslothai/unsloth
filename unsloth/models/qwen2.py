...
class FastQwen2Model(FastLlamaModel):
    ...
    def from_pretrained(
        ...
        if model_patcher is None: model_patcher = FastQwen2Model
        # Qwen2 does NOT support RoPE Scaling!
        if rope_scaling is not None:
            logger.warning_once("Unsloth: Qwen2 models do not support RoPE scaling.")
        pass
        ...
        # Qwen2 does NOT support RoPE Scaling sadly so we have to error out.
        if max_seq_length > model_max_seq_length:
            raise RuntimeError(
                "Unsloth: Unfortunately Qwen2 type models do not support RoPE scaling!\n" \
