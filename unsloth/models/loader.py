...
import os, contextlib, sys, warnings
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if SUPPORTS_LLAMA32:
            ...
            if os.path.isdir(model_name_for_config):
                ...
                try:
                    ...
                    # If Hub lookup fails, we assume only one config exists or it's handled by AutoConfig later
                    both_exist = False
        ...
        if DEVICE_TYPE == "mps" and load_in_4bit:
            ...
            # 3. Fallback to on-the-fly quantization if no MLX weights found
            if not mlx_loaded:
                fast_quantize(model)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if DEVICE_TYPE == "mps" and load_in_4bit:
            ...
            # 3. Fallback to on-the-fly quantization if no MLX weights found
            if not mlx_loaded:
                print("Unsloth: No MLX weights found. Falling back to on-the-fly quantization...")
                fast_quantize(model)
