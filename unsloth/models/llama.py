...
def _fast_prepare_inputs_for_generation(
    ...
    **kwargs,
    ...
    # Process cache and attention mask when cache exists
    if past_key_values is not None:
        # Get shape from input_ids or inputs_embeds
    ...
    # Build result - use inputs_embeds on first step, input_ids on cached steps
    result = {
    ...
    if use_inputs_embeds:
        ...
        # Explicitly set input_ids to None to prevent forward() from seeing
        # both inputs_embeds and a dummy input_ids tensor (shape [batch, 0])
