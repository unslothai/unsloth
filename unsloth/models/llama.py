...
def LlamaAttention_fast_forward_inference(
    ...
    if do_prefill:
        ...
        self.attention = torch.empty(
            (bsz, n_heads, 1, KV_CACHE_INCREMENT + seq_len), dtype=dtype, device="cuda"
        )
        ...
        if lora_dropout == 0 and bias == "none":
            for idx, layer in enumerate(model.model.model.layers):
                ...
                if (
                    ...
                    and (getattr(gate_proj, "lora_magnitude_vector", None) is None)
                    and (getattr(up_proj, "lora_magnitude_vector", None) is None)
                    and (getattr(down_proj, "lora_magnitude_vector", None) is None)
                ):
                ...
                if (
                    ...
                    and (getattr(q_proj, "lora_magnitude_vector", None) is None)
                    and (getattr(k_proj, "lora_magnitude_vector", None) is None)
                    and (getattr(v_proj, "lora_magnitude_vector", None) is None)
                ):
                ...
                if (
                    ...
                    and (getattr(o_proj, "lora_magnitude_vector", None) is None)
                ):
