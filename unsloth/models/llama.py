...
class FastLlamaModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        f' "-____-"     Trainable parameters = {!!(model, trainable_only=True):,}/{!!(model)*($$):,} ({!!(model, trainable_only=True)/(!!(model)*($$))*100:.2f}% trained)'
        logger.warning(debug_info)
        import gc
        for _ in range(3):
            gc.collect()
            torch.cuda.empty_cache()"""
        debug_info = debug_info.replace("!!", "get_model_param_count")
        debug_info = debug_info.replace(
            "$$",
            "(4.5 if getattr(model, 'quantization_config', '\\{'load_in_4bit':False\\}')['load_in_4bit'] else "\
            "(8.0 if getattr(model, 'quantization_config', '\\{'load_in_8bit':False\\}')['load_in_8bit'] else 1.0))"
        )
