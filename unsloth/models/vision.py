...
# Hybrid Mamba/SSM model families whose checkpoint layouts do not resolve through
# transformers' bnb quantizer's strict module lookup — the load dies with an opaque
# AttributeError deep in get_submodule(). #4037. Matched against config.model_type and
# architectures, never by repo name.
_HYBRID_MAMBA_BNB_BLOCKLIST = (
    "jamba",
    "zamba2",
)


def _hybrid_mamba_bnb_preflight(model_config, load_in_4bit, load_in_8bit):
    """Raise a clear error before downloading shards for known-broken bnb loads.

    Hybrid Mamba/SSM checkpoints carry parameter names that transformers' bnb quantizer
    cannot resolve (strict get_submodule per weight), so the crash surfaces only after
    every shard is downloaded — with no hint of the cause. Fail fast instead. #4037
    """
    if not (load_in_4bit or load_in_8bit):
        return
    # Exact token match (word boundaries), not substring: "jamba" must not match
    # "megajamba" or "jamba2" unless that exact token appears in model_type or
    # architectures. HF model_types are single tokens, so token-set membership is
    # the right predicate.
    model_type = str(getattr(model_config, "model_type", "") or "").lower()
    architectures = [str(a).lower() for a in (getattr(model_config, "architectures", None) or [])]
    tokens = set((model_type + " " + " ".join(architectures)).split())
    hit = next((m for m in _HYBRID_MAMBA_BNB_BLOCKLIST if m in tokens), None)
    if hit is not None:
        raise ValueError(
            f"Unsloth: 4-bit/8-bit (bitsandbytes) loading of hybrid Mamba/SSM models "
            f"(family '{hit}') is not supported yet.\n\n"
            "The checkpoint's parameter names do not resolve through transformers' bnb "
            "quantizer, which fails with an opaque error like:\n"
            "    AttributeError: JambaAttention has no attribute `feed_forward`\n\n"
            "Workarounds:\n"
            "  1. Load in 16-bit instead: load_in_4bit = False (needs more VRAM).\n"
            "  2. Check for a newer transformers version matching your checkpoint layout.\n"
            "  3. Track https://github.com/unslothai/unsloth/issues/4037 for support."
        )


def _raise_quantizer_error(error, model_name):
    """Turn a bnb-quantizer AttributeError into a diagnosis, else re-raise unchanged.

    A `... has no attribute ...` AttributeError escaping the model load means a
    checkpoint parameter name did not resolve in the constructed module tree — almost
    always a layout mismatch (hybrid models without a patch, or a checkpoint converted
    with another transformers version). Re-raise with that diagnosis; other
    AttributeErrors pass through untouched so real bugs are never masked. #4037
    """
    if "has no attribute" not in str(error):
        raise error
    raise ValueError(
        f"Unsloth: Loading '{model_name}' failed inside the bitsandbytes quantizer:\n"
        f"    {error}\n\n"
        "This means a checkpoint parameter name does not match the model architecture "
        "transformers constructed. Common causes:\n"
        "  - hybrid Mamba/SSM models (Jamba, Zamba2, Nemotron-H) without an Unsloth patch,\n"
        "  - a checkpoint converted with a different transformers version than installed.\n\n"
        "Try load_in_4bit = False (16-bit), or upgrade/downgrade transformers to match the "
        "checkpoint. If that fails, open an issue with model name + versions."
    ) from error
...
class FastBaseModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Fail fast on known-broken bnb loads (hybrid Mamba/SSM families) before any
        # shard or device-map planning. #4037
        _hybrid_mamba_bnb_preflight(auto_config, load_in_4bit, load_in_8bit)
        ...
        try:
            ...
            if not fast_inference:
                ...
                except AttributeError as _bnb_err:
                    # The bnb quantizer's strict per-weight module lookup dies with a raw
                    # AttributeError that looks like a corrupt checkpoint; translate it.
                    # Only when bnb is actually active — a 16-bit load raising an
                    # AttributeError is a real bug and must pass through untouched. #4037
                    if load_in_4bit or load_in_8bit:
                        _raise_quantizer_error(_bnb_err, model_name)
                    else:
                        raise
