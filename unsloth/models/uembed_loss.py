...
class UEmbedUnifiedLoss(nn.Module):
    ...
    def __init__(
        ...
        is_integer = isinstance(alpha_warmup_steps, int) and not isinstance(
            alpha_warmup_steps, bool
        )
    ...
    def _columns(
        ...
        dense = [self._embedding(c, SENTENCE_EMBEDDING_KEY, i) for i, c in enumerate(forwarded)]
        sparse = [self._embedding(c, SPARSE_EMBEDDING_KEY, i) for i, c in enumerate(forwarded)]
    ...
    def forward(
        self,
        sentence_features: Iterable[Any],
