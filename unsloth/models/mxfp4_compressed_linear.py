...
def _packed_linear(x, packed, scale, bias, dtype):
    return mxfp4_packed_matmul(x.to(dtype), packed, scale, bias = None if bias is None else bias.to(dtype))
...
class Mxfp4PackedLinear(nn.Linear):
    ...
    def mxfp4_quant_state(self):
        ...
        if state is None or state.packed is not packed or state.scale is not scale or state.dtype != self.compute_dtype:
            state = self.__dict__["_unsloth_packed_state"] = Mxfp4PackedState(packed, scale, self.compute_dtype)
        return state
...
class Mxfp4PackedState:
    ...
    def dequantize(self, W = None, dtype = None):
