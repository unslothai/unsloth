...
def patch_sft_trainer_tokenizer():
    ...
    for path_to_trainer in (
        ...
        check_text = (
            ...
            "        raise RuntimeError('Unsloth: No GPU detected. AMD ROCm users: install ROCm-enabled PyTorch from https://rocm.docs.amd.com')\n"
            "    try:\n"
            "        a = np.array([torch.cuda.memory_reserved(i)/1024**3 for i in range(torch.cuda.device_count())])\n"
            "    except Exception:\n"
            "        pass\n"
