...
# behind TORCH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL, which it reads once while
# loading its C++ extension - before any model code runs. With the gate shut
# every sub-quadratic backend declines, SDPA falls through to MATH, and MATH
...
# opened it - only install.sh's WSL drop-in did. Open it here, before anything
# below can import torch, honouring a pre-existing value so an explicit "0"
# stays a working opt-out. Harmless on non-ROCm hosts, which never read it.
