...
def seeded_dropout(
    x: torch.Tensor, p: float, seed: int, scale: bool = True
) -> torch.Tensor:
    if p <= 0.0 or not (
        x.requires_grad or x.training if hasattr(x, "training") else True
    ):
        return x
    device = x.device
    dtype = x.dtype
...
class DeterministicDropout(torch.nn.Module):
    def __init__(self, p: float, seed: int = 3407):
        ...
        self.register_buffer(
            "_counter", torch.zeros((), dtype = torch.int64), persistent = False
        )
