...
def _packed_base(base_layer):
    ...
    cls = type(base_layer)
    if not getattr(cls, "_unsloth_mxfp4_packed_linear", False) or getattr(cls, "mxfp4_quant_state", None) is None:
        return None
...
def _is_packed_state(quant_state):
    return getattr(quant_state, "_unsloth_packed_weight_state", False)
...
_fast_dequantize_unpacked = fast_dequantize


def fast_dequantize(W, quant_state = None, out = None, use_global_buffer = False):
    # A packed MXFP4 base hands in weight_packed (or its .t()) with a decoder as the quant state.
    if _is_packed_state(quant_state):
        return quant_state.dequantize(W)
    return _fast_dequantize_unpacked(W, quant_state, out = out, use_global_buffer = use_global_buffer)
