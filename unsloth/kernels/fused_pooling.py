...
class FusedLayerNormMeanPool(torch.autograd.Function):
    ...
    def backward(ctx, grad_pooled):
        ...
        need_dbias = ctx.needs_input_grad[2]
        dW = None
        d_bias = None
        if need_dW or need_dbias:
            # Build per-token grad only when affine parameters are trainable.
            ...
            if need_dW:
                normed = (X_flat.float() - mu_buf.unsqueeze(1)) * inv_var.unsqueeze(1)
