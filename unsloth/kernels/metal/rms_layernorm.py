...
RMS_BACKWARD_BODY = """
    ...
    // We use scalar loop for maximum robustness during debug, 
    // performance can be tuned with float4 later once correctness is 100%
    ...
    threadgroup float tg_dots[32]; // Accommodate up to 1024 threads
...
def mlx_rms_layernorm_forward(X_mlx, W_mlx, eps, gemma = False):
    ...
    This reaches peak performance (~0.6ms).
    """
    if not gemma:
        # Standard RMSNorm is natively supported and highly optimized
        # We need to compute r (inverse variance) for the backward pass
        # r = 1 / sqrt(mean(X^2) + eps)
        # MLX doesn't return r, so we compute it separately in a fast way
