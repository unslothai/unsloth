...
class MPSCrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits: torch.Tensor, labels: torch.Tensor, 
                logit_softcapping: float = 0, logit_scaling: float = 0):
        # Save input shape
        ...
        logsumexp = logits_max.squeeze(-1) + torch.log(torch.exp(logits_shifted).sum(dim=-1))
    ...
    def backward(ctx, dloss: torch.Tensor):
        ...
        dlogits = probs
        dlogits.scatter_add_(1, safe_labels.unsqueeze(1), 
...
def mps_cross_entropy_loss(logits: torch.Tensor, labels: torch.Tensor, 
                           logit_softcapping: float = 0, logit_scaling: float = 0):
