...
def dispatch_lora_mlp_swiglu(X, gateW, gateW_quant, gateA, gateB, gateS,
                             upW, upW_quant, upA, upB, upS,
                             downW, downW_quant, downA, downB, downS):
    if DEVICE_TYPE == "mps" and USE_MPS_FALLBACK:
        from .fast_lora import mps_apply_lora_mlp_swiglu
        fn = _get_compiled_fn(mps_apply_lora_mlp_swiglu)
        return fn(X, gateW, gateW_quant, gateA, gateB, gateS,
                     upW, upW_quant, upA, upB, upS,
                     downW, downW_quant, downA, downB, downS)
    else:
        from ..fast_lora import LoRA_MLP, swiglu_fg_kernel, swiglu_DWf_DW_dfg_kernel
        return LoRA_MLP.apply(X, gateW, gateW_quant, gateA, gateB, gateS,
                             upW, upW_quant, upA, upB, upS,
                             downW, downW_quant, downA, downB, downS,
                             swiglu_fg_kernel, swiglu_DWf_DW_dfg_kernel)

def dispatch_lora_qkv(X, QW, QW_quant, QA, QB, QS,
                      KW, KW_quant, KA, KB, KS,
                      VW, VW_quant, VA, VB, VS, inplace=True):
    if DEVICE_TYPE == "mps" and USE_MPS_FALLBACK:
        from .fast_lora import mps_apply_lora_qkv
        fn = _get_compiled_fn(mps_apply_lora_qkv)
        return fn(X, QW, QW_quant, QA, QB, QS,
                     KW, KW_quant, KA, KB, KS,
                     VW, VW_quant, VA, VB, VS)
    else:
        from ..fast_lora import LoRA_QKV
        return LoRA_QKV.apply(X, QW, QW_quant, QA, QB, QS,
                             KW, KW_quant, KA, KB, KS,
                             VW, VW_quant, VA, VB, VS, inplace)
