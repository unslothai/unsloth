...
class MPSLoRA_MLP(torch.autograd.Function):
    ...
    def forward(ctx, X, gateW, gateW_quant, gateA, gateB, gateS,
        ...
        ctx.save_for_backward(X, gateW, gateA, gateB, upW, upA, upB, downW, downA, downB, e, g, h)
        ctx.gateS, ctx.upS, ctx.downS = gateS, upS, downS
        return i

    @staticmethod
    def backward(ctx, dY):
        X, gateW, gateA, gateB, upW, upA, upB, downW, downA, downB, e, g, h = ctx.saved_tensors
        ...
        raise NotImplementedError("Manual backward for MPSLoRA_MLP not yet implemented. Use autograd for now.")
