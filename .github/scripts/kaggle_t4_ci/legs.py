...
UNWIRED: dict[str, str] = {
    ...
    "multi_gpu": (
        "MEASURED AND HELD, and the note this replaces was WRONG about why. "
        "The leg itself is fine: unsloth-probe-multigpu-r2-a280e2 is passed "
        "true, failures [], with the DEVICE_COUNT > 1 bindings live for the "
        "first time in this CI.\n"
        "WITHDRAWN, the second count. That note said the Default leg failed 3 "
        "of 3 whenever this leg was present, with\n"
        "    PicklingError: Can't pickle <class 'MonthDayNano'>\n"
        "and blamed a sixth concurrent install perturbing another leg's "
        "overlay resolution. That was two defects in THIS DRIVER, not an "
        "interaction with this leg. `uv venv` ran without --python, so uv's "
        "`managed` python-preference built the leg on CPython 3.13 while the "
        "image is 3.12; --system-site-packages then inherited nothing, the "
        "overlay resolved 44 packages instead of 3, and pyarrow landed in a "
        "--target directory that dill pickles BY VALUE because its path is "
        "neither under a sys prefix nor named site-packages. Both are fixed. "
        "unsloth-probe-venvfix-r1-b50886 and -r2-8799a3 run this exact six-leg "
        "set: Default passes 2 of 2, every leg reports python 3.12.13, and "
        "there is no PicklingError anywhere in either session.\n"
        "UNDER RE-MEASUREMENT, the first count. The old A/B read\n"
        "    A (5 legs)  1936.4  1980.3  2301.8\n"
        "    B (+ this)  2125.1  2342.0  2482.1\n"
        "mean +243.6s, B slower 3 of 3. Every one of those B runs was also "
        "building a 3.13 virtualenv and downloading torch 2.12 and 41 other "
        "packages that arm A got from the image, which is exactly the wall "
        "clock the +243.6s was attributed to this leg. The sign may well "
        "survive -- a session has 4 vCPUs and a leg is mostly install -- but "
        "the number cannot be quoted as it stands.\n"
        "WHAT WOULD UNBLOCK IT: the ab3 A/B, four sessions on one commit with "
        "both fixes in, decides it. If B is still slower the leg belongs on "
        "the nightly, where it installs alone; the `legs` input and the "
        "schedule trigger both exist already."
    ),
    "latest_compile": (
        ...
        "So this moves into KERNELS the day #1103 merges, and not before: "
        "wiring it against zoo main today would put a load-time traceback in "
        "front of every PR for a bug that is already fixed and waiting."
    ),
    "grpo": (
