...
UNWIRED: dict[str, str] = {
    ...
    "grpo": (
        ...
        "The cause was reward_length saturating at 200 characters while the "
        "model emits 2534-3396, so every completion scored 1.0 and every group "
        "tied. It is now len/(len+200), which cannot saturate. Two independent "
        "sessions on two accounts then recorded non-zero reward_std on every "
        "step -- reward3-045dba and crashax-b-27d9e3 -- with "
        "frac_reward_zero_std 0.0 throughout. The reward blocker is closed.\n"
        "WHAT KEEPS THIS UNWIRED IS THE CRASH, AND IT IS NOW MEASURED RATHER "
        "THAN GUESSED. Nine sessions on the current stack with the flashinfer "
        "uninstall in place: shipped-d15695, rep2-b03be8, rep3-bc3828, "
        "reward3-045dba and crashax-b clean; reward-158b39, crashax-a, "
        "crashax-c2 and crashax-d2 crashed. FOUR IN NINE, 44%. The previous "
        "note here said one in four, from a four-session window that opened on "
        "three clean runs; four more sessions moved the rate the wrong way. A "
        "44% red in front of every PR, on a fault no reader can act on, is not "
        "something to wire into the per-PR kernel -- a NIGHTLY kernel is where "
        "this belongs, and that scheduling does not exist yet.\n"
        "Peak was 13.39-13.40GB of 14.56 in all nine, crashed and clean alike, "
        "so memory does not distinguish them; and it vanishes under "
        "--cuda-launch-blocking. That is a race, and it survives BOTH the "
        "flashinfer uninstall and UNSLOTH_VLLM_NO_FLASHINFER=1. Worth filing "
        "upstream: nine sessions at a 44% reproduction rate is a better report "
        "than most.\n"
