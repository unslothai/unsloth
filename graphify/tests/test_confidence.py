...
def test_confidence_score_round_trip():
    ...
    for link in links:
        ...
        assert isinstance(
            score, float
        ), f"confidence_score should be float, got {type(score)}"
...
def test_report_shows_avg_confidence_for_inferred():
    ...
    assert (
        "avg confidence" in report
    ), "Report should show avg confidence for INFERRED edges"
...
def test_report_inferred_tag_with_score():
    ...
    report = generate(
        G, communities, cohesion, labels, gods, [surprise], detection, tokens, "."
    )
    assert (
        "INFERRED 0.82" in report
