...
def _make_graph() -> nx.Graph:
    ...
    G.add_node(
        "n2",
        label = "cluster",
        source_file = "cluster.py",
        source_location = "L5",
        community = 0,
    )
    G.add_node(
        "n3", label = "build", source_file = "build.py", source_location = "L1", community = 1
    )
    G.add_node(
        "n4", label = "report", source_file = "report.py", source_location = "L1", community = 1
    )
    G.add_node(
        "n5",
        label = "isolated",
...
def test_subgraph_to_text_truncates():
    ...
    text = _subgraph_to_text(
        G, {"n1", "n2", "n3", "n4"}, [("n1", "n2")], token_budget = 1
    )
