...
```
`--portable` defaults the root to `~/.unsloth`; `--root DIR` (or `UNSLOTH_HOME=DIR`) puts Studio, llama.cpp, whisper.cpp, the managed Node runtime and every download cache under `DIR`. Everything Unsloth itself owns lives there: no shell rc line, no `~/.local/bin` shim and no desktop entry, so launch it by absolute path with `DIR/bin/unsloth studio -p 8888` and remove it with `rm -rf DIR`. What portable mode does not contain is system packages, which belong to the distro and which `rm -rf DIR` cannot unwind. On Debian/Ubuntu (WSL included) the installer still reaches for `apt-get` when a prerequisite is missing: `curl`, if neither `curl` nor `wget` is installed, and `cmake`, `git`, `build-essential` and `libcurl4-openssl-dev` for the optional source builds. A ROCm-on-WSL install also writes `/etc/profile.d/unsloth-rocm-wsl.sh`. As a normal user nothing is installed without sudo and the installer asks before escalating; as root it happens unattended, so install those packages yourself first if the machine has to stay untouched. The install is self-contained on the machine that created it; it is not relocatable to another machine.

Two things to know about living with a portable install. Updating the default `~/.unsloth` root needs no flags: a plain `curl -fsSL https://unsloth.ai/install.sh | sh` finds the marker the first run left and keeps the layout, so the caches stay where you put them. A root you named yourself is not found that way, because portable mode deliberately writes nothing outside it, so name it again on the update: `curl -fsSL https://unsloth.ai/install.sh | UNSLOTH_HOME=DIR sh`, or run `DIR/bin/unsloth studio update`, or update from a shell that already carries `UNSLOTH_HOME` (both `DIR/share/studio.conf` and the `DIR/bin/unsloth` wrapper export it). A flagless run there installs a fresh normal Studio under `~/.unsloth` instead, and downloads the whole environment and model cache again. To go back to a normal install, ask for it with `UNSLOTH_PORTABLE=0`, which works from a shell opened inside the portable install and converts that install rather than starting a fresh one elsewhere. Passing `--portable` or `--root` at the same time is refused, since one asks for portable and the other asks for normal. And portable mode moves the Hugging Face model cache under the root, so if you already have models in `~/.cache/huggingface` they stay on disk but stop being read and download again; the installer says so when it sees one. Export `HF_HUB_CACHE` to keep sharing the old cache, at the cost of that part no longer living under the root. Any cache location you set yourself is left alone in portable mode, and the installer lists the ones it kept.

The uninstaller follows the same rule. `rm -rf DIR` on a root you named removes it in one piece, and so does `UNSLOTH_HOME=DIR sh scripts/uninstall.sh`. The one exception is the default `~/.unsloth`, which `--portable` picks for you rather than you naming it: there the uninstaller removes what Unsloth put in it and leaves anything else you keep there, exactly as it always has.
