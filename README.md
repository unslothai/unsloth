...
A reinstall or repair keeps the previous environment beside the new one until the new one works, so the drive needs room for both. Delete it first instead (a failed install then has nothing to restore):
```bash
