...
# bitsandbytes on AMD ROCm: continuous-release_main wheel (ROCm 4-bit GEMV fix, bnb PR #1887 post-0.49.2; bnb <= 0.49.2 NaNs at decode shape on every AMD GPU), PyPI >=0.49.1 fallback; drop once bnb 0.50+ ships on PyPI.
_install_bnb_rocm() {
    _label="$1"
    _venv_py="$2"
    case "$_ARCH" in
