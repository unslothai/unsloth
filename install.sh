...
if [ -x "$VENV_DIR/bin/python" ] || _dir_has_entries "$VENV_DIR"; then
    # Matching guard to the .venv branch below: in env-mode $STUDIO_HOME is a user-chosen workspace, so refuse to nuke an existing $STUDIO_HOME/unsloth_studio that lacks Unsloth sentinels. Accept the in-VENV ownership marker so partial-install retries are not blocked. Sentinels must be regular files: -f follows symlinks to files (the legitimate ln -s shim shape) but rejects directories and broken or dir-targeted symlinks.
