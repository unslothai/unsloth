...
create_studio_shortcuts() {
    ...
    # The launcher is Bash (not POSIX sh) since it uses arrays, [[ ]], etc.
...
PID_FILE="${XDG_RUNTIME_DIR:-/tmp}/unsloth-studio-launcher.pid"
...
_acquire_lock() {
    if [ -f "$PID_FILE" ]; then
        ...
        if [ -n "$_old_pid" ] && kill -0 "$_old_pid" 2>/dev/null; then
            # Another launcher is running; wait for it
...
_launch_cmd="\"$UNSLOTH_EXE\" studio -H 0.0.0.0 -p $BASE_PORT"
