...
def parse_args():
    p = argparse.ArgumentParser(
        description = "Benchmark Qwen3-MoE SparseMoeBlock (TF 4.56.2)"
    )
    p.add_argument("--batch-sizes", type = int, nargs = "+", default = [1, 4])
    p.add_argument("--seq-lens", type = int, nargs = "+", default = [128, 512])
    p.add_argument(
        "--dtypes", choices = DTYPE_MAP.keys(), nargs = "+", default = ["fp16", "bf16"]
    )
    p.add_argument(
...
def main():
    ...
    for dtype_name in args.dtypes:
        ...
        try:
            target = instantiate_target(
                args.target_class, config, device, dtype, train = False
            )
        except Exception as exc:
        ...
        for batch in args.batch_sizes:
            for seq in args.seq_lens:
                ...
                inputs = torch.randn(
                    batch, seq, config.hidden_size, dtype = dtype, device = device
                )

                tgt_out, tgt_grad, tgt_logits, _, _, tgt_stable = run_once(
                    target, inputs
                )
                ...
                router_diff = (
                    tensor_max_diff(tgt_logits, ref_logits)
                ...
                tgt_fwd, tgt_bwd, tgt_stable2 = average_timing(
                    target, inputs, args.num_iters, timer_tgt
                )
                ref_fwd, ref_bwd, ref_stable2 = average_timing(
                    ref, inputs, args.num_iters, timer_ref
                )
                print(
