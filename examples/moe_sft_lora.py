...
def main() -> None:
    ...
    if has_native:
        ...
        print(f"[WARNING] No native grouped-GEMM kernel for {DEVICE_TYPE_TORCH.upper()}; "
    ...
    if reasons:
        print(f"[FAIL] MoE SFT LoRA on {device} did not meet the success criteria "
              f"({kernel}):")
        for reason in reasons:
            print(f"    - {reason}")
    else:
        print(f"[PASS] MoE SFT LoRA on {device} ({len(steps)} steps, "
