...
def run_probes(*, world, host=None, db_path=None, limit=None, on_chunk=None) -> list[dict]:
    coro = _run_probes(
        world=world, host=host, db_path=db_path, limit=limit, on_chunk=on_chunk
    )
    try:
...
async def _run_probes(*, world, host=None, db_path=None, limit=None, on_chunk=None) -> list[dict]:
...
async def _execute_probe(*, world: Path, command: str, host, on_chunk) -> bool:
    ...
    try:
        ...
        result = host.run_action(
            sim_id, "terminal", {"command": command}, on_chunk=on_chunk
        )
