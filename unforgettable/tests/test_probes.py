...
def _insert_probe(db_path, title: str, body: str, *, status: str = "active"):
...
def _instant_ok(session_id, name, arguments, timeout=None, on_chunk=None) -> str:
    return "ok\n"
...
def test_cli_run_does_not_pass_on_chunk(tmp_path: Path, db_path: Path, monkeypatch):
    ...
    def fake_run_probes(*, world, host=None, db_path=None, limit=None, on_chunk=None):
        seen["host"] = host
