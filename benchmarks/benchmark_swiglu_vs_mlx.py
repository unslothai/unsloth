...
# Check custom kernel
try:
    from unsloth.kernels.metal.swiglu import (
    ...
    print(
        f"Custom Metal Kernel: {'✅ Available' if HAS_CUSTOM_KERNEL else '❌ Not Available'}"
    )
except ImportError:
...
def benchmark_fn(fn: Callable, warmup: int = 10, iterations: int = 50) -> float:
    ...
    # Force sync for MLX
    if HAS_MLX:
        mx.eval(result) if hasattr(result, "__mlx_array__") or isinstance(
            result, mx.array
        ) else None
    ...
    for _ in range(iterations):
        ...
        # Sync appropriately
        if HAS_MLX and (
            hasattr(result, "__mlx_array__") or isinstance(result, mx.array)
        ):
            mx.eval(result)
...
def run_benchmark():
    ...
    for batch, seq, dim, desc in configs:
        ...
        if HAS_MLX and hasattr(mx, "fast") and hasattr(mx.fast, "metal_kernel"):
            try:
                ...
                print(
                    f"   Fused Metal Kernel:       {t_fused:7.3f} ms | {tp_fused:7.2f} GB/s | {speedup_vs_mlx:.2f}x vs MLX"
                )
    ...
    if results:
        ...
        if avg_speedup > 1.0:
            print(
                f"✅ Your fused kernel is {avg_speedup:.2f}x FASTER than MLX composed!"
            )
            print(f"   This is because fusing eliminates {2} extra memory passes.")
        else:
            print(
                f"⚠️  Fused kernel is {1/avg_speedup:.2f}x slower - may need optimization"
            )

        print()
        print("Theory: Fused kernel should be ~1.5-3x faster due to:")
        print(
            "  - MLX composed: 3 separate ops = 3 kernel launches + 2 intermediate buffers"
        )
