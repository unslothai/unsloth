...
def run_performance_benchmark():
    ...
    for cfg in configs:
        ...
        print(f"   Unsloth Metal Forward (Fused): {t_metal_fwd:7.3f} ms | {tp_metal_fwd:7.2f} GB/s ({speedup_fwd:.2f}x vs PyTorch)")
        ...
        def unsloth_metal_backward():
            return metal_module.metal_rms_layernorm_backward(dY_torch, X_torch, W_torch, r_torch, eps, gemma = False)
        ...
        print(f"   Unsloth Metal Backward (Fused): {t_metal_bwd:7.3f} ms | {tp_metal_bwd:7.2f} GB/s")
        print()
...
def run_correctness_tests():
    ...
    if platform.system() == "Darwin":
        ...
        out_metal, r_metal = metal_module.metal_rms_layernorm(X_mps, W_mps, eps, gemma = False)
        diff = np.abs(out_metal.cpu().float().numpy() - ref.numpy())
        print(f"  Forward Parity (fp16): max_diff={diff.max():.2e} mean={diff.mean():.2e} {'✅' if diff.max() < 1e-2 else '❌'}")

        # 2. Backward Correctness
        dY_mps = torch.randn_like(out_metal)
        dX_metal, dW_metal = metal_module.metal_rms_layernorm_backward(dY_mps, X_mps, W_mps, r_metal, eps, gemma = False)
        ...
        print(f"  Backward dX Parity:    max_diff={diff_dX.max():.2e} {'✅' if diff_dX.max() < 1e-2 else '❌'}")
        print(f"  Backward dW Parity:    max_diff={diff_dW.max():.2e} {'✅' if diff_dW.max() < 5e-2 else '❌'}")
        ...
        print(f"  Unsloth Gemma Forward:  max_diff={diff_gemma.max():.2e} {'✅' if diff_gemma.max() < 1e-2 else '❌'}")
    else:
