...
if _HAS_TRITON:
    ...
    def _nvfp4_gemm_lowrank(
        ...
        RANK: tl.constexpr,
        FOLD_ALPHA: tl.constexpr,
        A_SWIZZLED: tl.constexpr,
        B_SWIZZLED: tl.constexpr,
        ...
        for k0 in range(0, tl.cdiv(K, BLOCK_K)):
            ...
            asf = tl.load(
                ASF
                + _sf_offsets(
                    offs_m[:, None], kblk[None, :], a_k_pad_blocks, A_SWIZZLED, stride_asf
                ),
                mask = mask_m[:, None],
                other = 0,
            )
            bsf = tl.load(
                BSF
                + _sf_offsets(
                    offs_n[:, None], kblk[None, :], b_k_pad_blocks, B_SWIZZLED, stride_bsf
                ),
                mask = mask_n[:, None],
                other = 0,
            )
            acc = tl.dot_scaled(
                a,
                asf.to(tl.float8e4nv, bitcast = True),
                "e2m1",
                b,
                bsf.to(tl.float8e4nv, bitcast = True),
                "e2m1",
                acc,
            )
            a_ptrs += BLOCK_K // 2
        ...
        if RANK > 0:
            ...
            t = tl.load(
                T + offs_m[:, None] * stride_tm + offs_r[None, :], mask = mask_m[:, None], other = 0.0
            )
            l2t = tl.load(
                L2T + offs_r[:, None] * stride_l2r + offs_n[None, :],
                mask = mask_n[None, :],
                other = 0.0,
            )
            if FOLD_ALPHA:
    ...
    def _quant_down(
        ...
        RANK: tl.constexpr,
        HAS_LR: tl.constexpr,
        EMIT_T_COLS: tl.constexpr,
        KPAD_COLS: tl.constexpr,
        T_REPS: tl.constexpr,
        BLOCK_M: tl.constexpr,
        BLOCK_K: tl.constexpr,
    ):
        ...
        for k0 in range(0, tl.cdiv(K, BLOCK_K)):
            ...
            kb = k0 * NBLK + offs_kb
            tl.store(
                XSF + _sf_swizzled_offset(offs_m[:, None], kb[None, :], k_pad_blocks),
        ...
        if HAS_LR:
            tl.store(
                T + offs_m[:, None] * stride_tm + offs_r[None, :],
                tacc.to(T.dtype.element_ty),
                mask = mask_m[:, None],
            )
        if EMIT_T_COLS:
    ...
    def _emit_t_cols(
        ...
        tl.store(
            XQ + offs_m[:, None] * stride_qm + (K // 2 + tl.arange(0, KPAD_COLS // 2))[None, :],
            tl.zeros((BLOCK_M, KPAD_COLS // 2), dtype = tl.uint8),
            mask = mask_m[:, None],
        )
        tl.store(
            XSF
            + _sf_swizzled_offset(
                offs_m[:, None], (K // 16 + tl.arange(0, KPAD_COLS // 16))[None, :], k_pad_blocks
            ),
            tl.zeros((BLOCK_M, KPAD_COLS // 16), dtype = tl.uint8),
            mask = mask_m[:, None],
        )
        ...
        tl.store(
            XQ + offs_m[:, None] * stride_qm + (K // 2 + tl.arange(0, RANK // 2))[None, :],
            (tlo | (thi << 4)).to(tl.uint8),
            mask = mask_m[:, None],
        )
        tl.store(
            XSF
            + _sf_swizzled_offset(
                offs_m[:, None], (K // 16 + tl.arange(0, RANK // 16))[None, :], k_pad_blocks
            ),
            tsf.to(tl.uint8, bitcast = True),
            mask = mask_m[:, None],
        )
        if T_REPS >= 2:
        ...
        if T_REPS >= 4:
            ...
            tl.store(
                ...
                + _sf_swizzled_offset(
                    offs_m[:, None],
                    ((K + 2 * RANK) // 16 + tl.arange(0, RANK // 16))[None, :],
                    k_pad_blocks,
                ),
                tsf.to(tl.uint8, bitcast = True),
                mask = mask_m[:, None],
            ...
            tl.store(
                ...
                + _sf_swizzled_offset(
                    offs_m[:, None],
                    ((K + 3 * RANK) // 16 + tl.arange(0, RANK // 16))[None, :],
                    k_pad_blocks,
                ),
                rsf.to(tl.uint8, bitcast = True),
                mask = mask_m[:, None],
    ...
    def _assemble(
        ...
        mask_k = offs_k2 < (K // 2)
        v = tl.load(
            XQ_SRC + offs_m[:, None] * stride_src + offs_k2[None, :],
            mask = mask_m[:, None] & mask_k[None, :],
            other = 0,
        )
        tl.store(
            XQ_DST + offs_m[:, None] * stride_dst + offs_k2[None, :],
            v,
            mask = mask_m[:, None] & mask_k[None, :],
        )
...
def preflight(device = None) -> dict:
    ...
    for d in devs:
        ...
        rec = {
            "name": torch.cuda.get_device_name(dd),
...
def torch_nvfp4_quantize(x, gsf):
    ...
    ratio = torch.where(
        step > 0, xb / torch.where(step > 0, step, torch.ones_like(step)), torch.zeros_like(xb)
    )
    a = ratio.abs()
    code = (
        (a > 0.25).int()
...
def triton_quant_down(
    ...
    *,
    ...
    M, K = x.shape
    cfg = dict(dict(BLOCK_M = 32, BLOCK_K = 256, num_warps = 4, num_stages = 3) if cfg is None else cfg)
    ...
    with dev_guard(x):
        _quant_down[grid](
            ...
            xq,
            x_sf,
            t if t is not None else xq,
            l1t if l1t is not None else xq,
            gsf,
            M,
            K,
            k_pad,
            x.stride(0),
            xq.stride(0),
            t.stride(0) if t is not None else 0,
            l1t.stride(0) if l1t is not None else 0,
            RANK = max(rank, 16),
            HAS_LR = rank > 0,
            EMIT_T_COLS = kaug_cols > 0,
            KPAD_COLS = kaug_cols,
            T_REPS = t_reps,
            BLOCK_M = cfg["BLOCK_M"],
            BLOCK_K = cfg["BLOCK_K"],
    ...
    *,
    ...
    with dev_guard(xq):
        _assemble[grid](
            xq,
            xq_aug,
            x_sf.view(torch.uint8).reshape(-1),
            sf_aug,
            t,
            gsf,
            M,
...
def triton_mm_lowrank(
    ...
    *,
    ...
    K = K2 * 2
    cfg = dict(
        dict(BLOCK_M = 128, BLOCK_N = 128, BLOCK_K = 256, GROUP_M = 8, num_warps = 4, num_stages = 3)
        if cfg is None
        else cfg
    )
    ...
    with dev_guard(aq):
        _nvfp4_gemm_lowrank[grid](
            ...
            w_sf.view(torch.uint8).reshape(-1),
            bias if bias is not None else d,
            out,
            alpha,
            t if t is not None else d,
            l2t if l2t is not None else d,
            M,
            N,
            K,
            aq.stride(0),
            ...
            out.stride(0),
            t.stride(0) if t is not None else 0,
            l2t.stride(0) if l2t is not None else 0,
            0 if a_swizzled else K // 16,
            0 if b_swizzled else K // 16,
            pad_blocks(a_swizzled),
            pad_blocks(b_swizzled),
...
def balance_windowed_percol(
    ...
    if lo > hi:
        info["reason"] = (
            "empty c window: amax(L2)*amax(t) > amax(w)*amax(x), so no scalar keeps "
...
def dequant_nvfp4(q, sf_swz, gsf, rows: int, k: int):
    ...
    lut = torch.tensor(
        list(E2M1_GRID) + [-v for v in E2M1_GRID], device = q.device, dtype = torch.float32
    )
    kb = k // 16
...
def timed(
    ...
    try:
        ...
        out.update(
            {
                "graph_min_ms": gv[0],
                "graph_p50_ms": gv[len(gv) // 2],
                "min_ms": gv[0],
                "p50_ms": gv[len(gv) // 2],
                "p90_ms": gv[int(len(gv) * 0.9)],
                "timed_by": "cuda_graph",
                "eager_over_graph": (
                    out["eager_p50_ms"] / gv[len(gv) // 2] if gv[len(gv) // 2] > 0 else None
                ),
            }
        )
    except Exception as exc:  # noqa: BLE001
...
def _dry_run_jobs():
    ...
    gemm_sig = {
        ...
        "HAS_BIAS": "constexpr",
        "RANK": "constexpr",
        "FOLD_ALPHA": "constexpr",
        "A_SWIZZLED": "constexpr",
        "B_SWIZZLED": "constexpr",
        "BLOCK_M": "constexpr",
        "BLOCK_N": "constexpr",
        "BLOCK_K": "constexpr",
        "GROUP_M": "constexpr",
...
def bench_shape(label, M, K, N, rank, args, caps):
    ...
    try:
        ...
        res["times"]["fp8_w8a8_gemm"] = timed(
            lambda: torch._scaled_mm(
                x8, w8.t(), scale_a = xs, scale_b = wrow.t(), bias = bias, out_dtype = torch.bfloat16
            ),
            warm,
            iters,
        )
    ...
    build_q = (
        (lambda t, gs: fi.nvfp4_quantize(t, gs, do_shuffle = False))
        if fi is not None
    ...
    t_raw = x.float() @ l1.t()
    l1p, l2p, s_bal, s_ok = balance_factors(
        l1.to(torch.bfloat16), l2.to(torch.bfloat16), amax_x, float(t_raw.abs().amax()), amax_w
    )
    ...
    if fi is None:
        ...
        try:
            ...
            res["times"]["unfused_down_only"] = timed(lambda: torch.mm(x, l1t, out = td), warm, iters)
    ...
    if fi is not None and rank <= pad and _HAS_TRITON and s_ok:
        try:
            ...
            res["times"]["kaug_assemble_only"] = timed(
                lambda: triton_assemble_kaug(q0f, s0f, td2, ag, M, K, kaug_cols = pad), warm, iters
            )
            oc3 = torch.zeros(M, N, device = dev, dtype = torch.bfloat16)
            y_k3 = fi.mm_fp4(
                aq3, wq2.T, asf3, wsf2.T, alpha2, torch.bfloat16, out = oc3, backend = args.fi_backend
            ).clone()
    ...
    if not _HAS_TRITON:
        ...
        try:
            ...
            y_f = triton_mm_lowrank(
                aq, asf_u, wq, wl, alpha, t = t_bf, l2t = l2t, out = torch.zeros_like(ob)
            )
            res["times"]["triton_fused"] = timed(
                lambda: triton_mm_lowrank(aq, asf_u, wq, wl, alpha, t = t_bf, l2t = l2t, out = ob),
                warm,
                iters,
            )
    ...
    if _HAS_TRITON and rank <= pad and (K + pad) % 64 == 0 and s_ok:
        try:
            ...
            kcfg = dict(BLOCK_M = 128, BLOCK_N = 128, BLOCK_K = 64, GROUP_M = 8, num_warps = 4, num_stages = 3)
            y_kt = triton_mm_lowrank(
                aq2t, asf2t, wq2t, wl2, alpha2t, out = torch.zeros_like(ob), cfg = kcfg
            )
            ...
            if tb:
                res["kaug_triton_branch_cost_pct_of_triton_base"] = (
                    res["times"]["kaug_triton_gemm_only"]["p50_ms"] / tb - 1.0
                ) * 100.0
        except Exception as exc:  # noqa: BLE001
    ...
    if base:
        ...
        if mf_p:
            for k in (
                "kaug_gemm_only",
                "kaug_full_assemble",
                "kaug_full_fusedquant",
                "unfused_branch_full",
                "triton_fused",
            ):
                v = _p(k)
                if v is not None and v < mf_p * 0.98:
                    viol.append({"variant": k, "ms": v, "mm_fp4_ms": mf_p, "ratio": v / mf_p})
        if viol:
            res["CONSISTENCY_VIOLATION"] = {
                "rows": viol,
                "meaning": (
                    "a variant that contains a full mm_fp4 came in below mm_fp4 on the "
                    "same shape. The timing of at least one component is not the timing "
                    "of this row's workload; do not quote a ratio from it."
                ),
            }
            print(
                f"      CONSISTENCY VIOLATION on {label} r{rank}: "
                + ", ".join(
                    f"{v['variant']}={v['ms']:.4f} < mm_fp4={v['mm_fp4_ms']:.4f}" for v in viol
                ),
                flush = True,
            )

        asm, kg, uu, mf = (
            _p("kaug_assemble_only"),
            _p("kaug_gemm_only"),
            _p("unfused_up_only"),
            base,
        )
...
def window_check(args) -> dict:
    ...
    for label, M, K, N in shapes:
        ...
        x = (
            (torch.randn(min(M, 4096), K, device = dev, generator = g) * 0.05 * chan.reshape(1, -1))
            .to(torch.bfloat16)
            .contiguous()
        )
        ...
        with dev_guard(w):
            ...
            wdq = dequant_nvfp4(
                wq.view(torch.uint8), wsf.view(torch.uint8).reshape(-1), wg, N, K
            ).to(torch.bfloat16)
        ...
        for form, hd in forms:
            ...
            if form == "formA":
                ...
                facs = dict(facs)
                facs["gauge"] = (
                    l1c.float().div(1e6).to(torch.float32),
            ...
            for rank, (l1, l2) in facs.items():
                ...
                try:
                    ...
                    l1p, l2p, sc, okb, winfo = balance_windowed_percol(l1, l2, amax_x, at, amax_w)
                except Exception as exc:  # noqa: BLE001
                    out["rows"].append(
                        {
                            "label": label,
                ...
                if okb and sc >= 1.0:
                    ...
                    npad = (N + 127) // 128 * 128
                    a = (
                        wsf.view(torch.uint8)
                        .reshape(-1)[: npad * kpad]
                        .reshape(npad // 128, kpad // 4, 512)
                    )
    ...
    base = {
        r["label"]: r for r in out["rows"] if r.get("rank") == ranks[0] and r.get("form") == "formA"
    }
    out["guard_controls"] = {
        "forced_empty": {
            "n_cases": len(ctrl),
            ...
            "reading": (
                "L2 * 1e6 with L1 untouched has genuinely oversized weight-side factors "
        ...
        "gauge_invariance": {
            ...
            "reading": (
                "L1 / 1e6 with L2 * 1e6 is the same function; the per-column window must "
    ...
    for form in ("formA", "formA_H"):
        rs = [
            r
            for r in out["rows"]
            if r.get("form") == form and r.get("rank") not in ("forced_empty", "gauge")
        ]
    ...
    out["summary"] = {
        ...
        "all_bit_identical": all(
            r.get("w_nibbles_identical")
            and r.get("w_scales_identical")
            and r.get("w_gsf_identical")
            for r in built
        ),
        "reading": (
            "a refusal is the CORRECT outcome for a closed window, not a failure. What "
        ...
        "caveat": (
            "the weights here are synthetic Gaussian, whose quantisation residual is far "
...
def selftest(args) -> dict:
    ...
    def rec(
        ...
        checks.append(
            {
                "check": name,
                "ok": (None if skipped else bool(ok)),
    ...
    if not _HAS_TRITON:
        rec(
            "kernel_source_parity",
            None,
            "triton not importable, embedded kernels not defined",
            skipped = True,
        )
    else:
    ...
    try:
        ...
        rec(
            "c_window_gauge_invariance",
            moved < 1e-5 and feas0 == feas1,
        ...
        rec(
            "c_window_refuses_oversized_l2",
            not feas2,
            f"lo {lo2:.6g} > hi {hi2:.6g} -> refused"
            if not feas2
    ...
    if not has_cuda:
        ...
        x = (torch.randn(M, K, device = dev, dtype = torch.bfloat16, generator = gen) * 0.05).contiguous()
        w = (torch.randn(N, K, device = dev, dtype = torch.bfloat16, generator = gen) * 0.02).contiguous()
        ag, wg = global_scale(x), global_scale(w)
        ...
        if fi is not None:
            ...
            try:
                ...
                for t_, gs_ in ((x, ag), (w, wg)):
                    ...
                    fdq = dequant_nvfp4(
                        fq.view(torch.uint8), fsf.view(torch.uint8).reshape(-1), gs_, rows, cols
                    )
                    tdq = dequant_nvfp4(
                        tq.view(torch.uint8), tsf.view(torch.uint8).reshape(-1), gs_, rows, cols
                    )
                    ...
                    det.append(
                        f"{tuple(t_.shape)}: scales_exact={sfe} nibble_disagree="
                        f"{nd}/{tot} ({100 * rate:.4f}%) max_code_step={dcode} "
                        f"sign_flips={flips} rms_ratio={ratio:.6f}"
                    )
                rec(
                    "torch_quantiser_faithful_to_flashinfer",
                    ok_all,
            ...
            try:
                ...
                rec(
                    "e2m1_tie_rule_is_round_half_to_even",
                    got_f == rne and got_t == rne,
        ...
        if not _HAS_TRITON:
            ...
            if fi is None:
                rec(
                    "triton_quantiser_vs_flashinfer",
                    None,
                ...
                try:
                    ...
                    sfe = bool(
                        torch.equal(
                            unswizzle_sf(sf_t.view(torch.uint8).reshape(-1), M, K).view(
                                torch.uint8
                            ),
                            unswizzle_sf(sf_f.view(torch.uint8).reshape(-1), M, K).view(
                                torch.uint8
                            ),
                        )
                    )
            ...
            try:
                ...
                t = (
                    torch.randn(M, rank, device = dev, dtype = torch.bfloat16, generator = gen) * 0.01
                ).contiguous()
                aug_q, aug_sf = triton_assemble_kaug(
                    base_q.view(torch.uint8), base_sf, t, ag, M, K, kaug_cols = pad
                )
                nib = bool(
                    torch.equal(
                        aug_q[:, : K // 2].contiguous().view(torch.uint8),
                ...
                sfe = bool(
                    torch.equal(
                        sf_aug[:, : K // 16].contiguous().view(torch.uint8),
                        sf_base.view(torch.uint8),
                    )
                )
                rec(
                    "assemble_first_K_bit_identical",
                    nib and sfe,
            ...
            if fi is None:
                ...
                try:
                    ...
                    rec("triton_gemm_vs_mm_fp4", False, f"{type(exc).__name__}: {str(exc)[:220]}")
...
def main() -> int:
    ...
    if any(
        isinstance(v, dict) and v.get("mm_fp4", "").startswith(("RuntimeError", "Error"))
        for v in pf.values()
    ):
        print(
            "preflight reported a device where mm_fp4 does not work; those columns will be "
            "skipped with a reason rather than silently replaced",
            flush = True,
        )
    ...
    for label, M, K, N in shapes:
        for rank in ranks:
            ...
            print(
                f"{label:24s} r{rank:<3d} mm_fp4 {g('mm_fp4')}  unfused "
                f"{g('unfused_branch_full')}  kaug {g('kaug_gemm_only')}  triton_fused "
                f"{g('triton_fused')}  tk {g('triton_kernels_nvfp4')}",
                flush = True,
            )
